"""Deterministic HIT_REACTION_FRONT adapter over the approved R4 cutout rig.

Six non-looping source-only frames. Motion tracks are sampled before the first
pixel is rendered. Recoil, recovery, support and weapon gates are evaluated from
targets or rendered source-only layers, never from scalar claims alone.
"""
...
PHASES = (
    "H0-impact-onset",
    "H1-recoil-acceleration",
    "H2-recoil-peak",
    "H3-stagger-settle",
    "H4-recovery",
    "H5-recovery-complete",
)
...
CONTACT_WINDOWS = tuple((index, index + 1, side) for index in range(5) for side in ("left", "right"))
RECOIL_PEAK_FRAME = 2
RECOVERY_COMPLETE_FRAME = 5
EXPECTED_MARKERS = (
    ("impact_onset", 0),
    ("recoil_peak", 2),
    ("recovery_start", 3),
...
def _base_target(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "orientation": "front",
        "generator": {
            "kind": "deterministic-skeleton-only-hit-reaction",
...
def load_context(spec: Mapping[str, Any], root: Path) -> dict[str, Any]:
    ...
    context["hit_contract"] = read_json(root / str(params["contract_ref"]))
    return context
...
def _hit_plan() -> dict[str, Any]:
    phase_plans = {
        phase: {
            ...
            "depth_role": {"left_leg": "planted_support", "right_leg": "planted_support", "left_arm": "reactive_recoil", "right_arm": "reactive_recoil"},
    ...
    value = {
        ...
        "plan_id": "animation-hit-front-source-only-v1",
...
def prepare(spec: Mapping[str, Any], context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"hit_required_motion_tracks_missing:{','.join(missing)}")
...
def _plan_and_structural(context: Mapping[str, Any], target: Mapping[str, Any], details: Mapping[str, Any], phase: str, plan: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    pair["hard_gates"]["explicit_hit_allowed_pair_rules"] = bool(plan.get("allowed_expected_occlusion_pairs"))
...
def _foot_record(context: Mapping[str, Any], target: Mapping[str, Any], details: Mapping[str, Any], spec: Mapping[str, Any], ground_reference_y: float | None) -> dict[str, Any]:
    ...
    return {
        ...
        "hard_gates": gates,
        "status": "HIT_FOOT_GROUND_QA_PASSED" if all(gates.values()) and both_planted else "HIT_FOOT_GROUND_GAP",
    }
...
def _contact_foot_qa(targets: list[Mapping[str, Any]], spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"contact_windows": windows, "threshold_px": limit, "hard_gates": {"all_contact_windows": all(item["gate"] for item in windows)}, "status": "HIT_CONTACT_FOOT_QA_PASSED" if all(item["gate"] for item in windows) else "HIT_CONTACT_FOOT_GAP"}
...
def _weapon_qa(spec: Mapping[str, Any], context: Mapping[str, Any], targets: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "hard_gates": gates,
        "status": "HIT_WEAPON_QA_PASSED" if all(gates.values()) else "HIT_WEAPON_QA_GAP",
    }
...
def _temporal_qa(spec: Mapping[str, Any], context: Mapping[str, Any], prepared: Mapping[str, Any], records: list[Mapping[str, Any]], outputs: list[Image.Image]) -> dict[str, Any]:
    ...
    peak_index = max(range(len(displacements)), key=lambda index: displacements[index])
    unique_peak = peak_index == RECOIL_PEAK_FRAME and displacements.count(max(displacements)) == 1
    ...
    same_sign_x = (root_x_values[0] * root_x_values[RECOIL_PEAK_FRAME]) > 0.0
    same_sign_y = (root_y_values[0] * root_y_values[RECOIL_PEAK_FRAME]) > 0.0
    monotonic_into_peak = abs(root_x_values[0]) < abs(root_x_values[1]) < abs(root_x_values[RECOIL_PEAK_FRAME])
    contact = _contact_foot_qa(targets, spec)
    foot_frame_gate = all(record.get("feet", {}).get("status") == "HIT_FOOT_GROUND_QA_PASSED" for record in records)
    ...
    heads = [_xy(target["joints"]["nose"]) for target in targets]
    death_like = max(abs(value) for value in torso_values) > float(thresholds["death_torso_rotation_max_deg"]) or max(root_y_values) > float(thresholds["death_root_drop_max_px"]) or any(head[1] >= pelvis_point[1] for head, pelvis_point in zip(heads, pelvis))
    recovery_ratio = displacements[RECOVERY_COMPLETE_FRAME] / max(displacements[RECOIL_PEAK_FRAME], 1e-6)
    gates = {
        "frame_count_six_non_loop": int(spec["frame_count"]) == 6 and spec["loop"] is False,
        "all_target_hashes_distinct": len({target_digest(target) for target in targets}) == len(targets),
        "impact_causality": same_sign_x and same_sign_y and monotonic_into_peak and abs(root_x_values[0]) >= float(thresholds["impact_onset_min_root_x_px"]),
        "recoil_magnitude": displacements[RECOIL_PEAK_FRAME] >= float(thresholds["recoil_peak_displacement_min_px"]) and _path(pelvis) >= float(thresholds["root_path_min_px"]),
        "unique_recoil_peak": unique_peak,
        "recovery_convergence": recovery_ratio <= float(thresholds["recovery_to_peak_ratio_max"]) and displacements[RECOVERY_COMPLETE_FRAME] < displacements[RECOIL_PEAK_FRAME] and target_digest(targets[RECOVERY_COMPLETE_FRAME]) != target_digest(targets[0]),
        "dual_foot_support": all(record.get("feet", {}).get("support_side") == "both" for record in records if isinstance(record.get("feet"), Mapping)),
        "foot_contact_windows": contact["status"] == "HIT_CONTACT_FOOT_QA_PASSED",
        "foot_ground_all_frames": foot_frame_gate,
        "weapon_wrist_continuity": weapon["status"] == "HIT_WEAPON_QA_PASSED",
        "not_death_like_collapse": not death_like,
    ...
    return {
        ...
        "recoil": {
            ...
            "recovery_to_peak_ratio": round(recovery_ratio, 6),
        ...
        "metrics": {
            ...
            "peak_displacement_px": round(displacements[RECOIL_PEAK_FRAME], 6),
            "recovery_displacement_px": round(displacements[RECOVERY_COMPLETE_FRAME], 6),
            "recovery_to_peak_ratio": round(recovery_ratio, 6),
            ...
            "foreground_height_variation": round(height_variation, 6),
        },
        "contact": contact,
        "weapon": weapon,
        "hard_gates": gates,
        "status": "HIT_TEMPORAL_QA_PASSED" if all(gates.values()) else "HIT_TEMPORAL_QA_GAP",
    }
...
def qa(spec: Mapping[str, Any], context: Mapping[str, Any], manifest: Mapping[str, Any], root: Path) -> dict[str, Any]:
    ...
    for index, item in enumerate(manifest["frames"]):
        ...
        gates = {
            ...
            "foot_ground": foot["status"] == "HIT_FOOT_GROUND_QA_PASSED",
    ...
    top_gates = {
        "frame_count_exact": len(records) == int(spec["frame_count"]) == 6,
        "non_loop": spec["loop"] is False,
        "all_frames": bool(records) and all(record["passed"] for record in records),
        "temporal_quality": temporal["status"] == "HIT_TEMPORAL_QA_PASSED",
    ...
    return {
        ...
        "status": "CUTOUT_ANIMATION_RUNTIME_V1_HIT_REACTION_FRONT_TECHNICALLY_QUALIFIED" if qualified else "HIT_REACTION_FRONT_STRUCTURAL_OR_TEMPORAL_GAP",
        "frames": records,
        "temporal": temporal,
        "body_mechanics": {
            "metrics": temporal["metrics"],
            "recoil": temporal["recoil"],
            "hard_gates": {key: value for key, value in temporal["hard_gates"].items() if key in {"impact_causality", "recoil_magnitude", "unique_recoil_peak", "recovery_convergence", "not_death_like_collapse"}},
            "status": "HIT_BODY_MECHANICS_QA_PASSED" if all(temporal["hard_gates"][key] for key in ("impact_causality", "recoil_magnitude", "unique_recoil_peak", "recovery_convergence", "not_death_like_collapse")) else "HIT_BODY_MECHANICS_GAP",
        },
        "foot_ground": {"frames": [record["feet"] for record in records], "contact": temporal["contact"], "ground_reference_y": None if ground_reference is None else round(float(ground_reference), 6), "status": "HIT_FOOT_GROUND_QA_PASSED" if temporal["hard_gates"]["foot_ground_all_frames"] and temporal["hard_gates"]["foot_contact_windows"] and temporal["hard_gates"]["dual_foot_support"] else "HIT_FOOT_GROUND_GAP"},
