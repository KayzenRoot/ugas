...
DECLARED_BODY_THRESHOLDS = (
    "body_root_path_min_px", "torso_rotation_range_min_deg", "left_wrist_counter_path_min_px",
    "head_counter_motion_max_deg", "root_horizontal_excursion_min_px", "root_horizontal_excursion_max_px",
    "root_vertical_excursion_min_px", "root_vertical_excursion_max_px",
)
ATTACK_V1_BASELINE_PATH = "profiles/animation/attack-front-v1.json"
ATTACK_V1_BASELINE_COMMIT = "c11196e5e854a0fbc6ec62e959de5ecc28d492ce"
ATTACK_V1_BASELINE_SHA256 = "8634852cba11a545668587251f775c248f712174a483b9a05cfd8d2117bf6f0d"
...
def _declared_thresholds(spec: Mapping[str, Any]) -> dict[str, float]:
    raw = spec.get("qa_profile", {}).get("thresholds", {})
    values: dict[str, float] = {}
    for name in DECLARED_BODY_THRESHOLDS:
        value = raw.get(name)
        if isinstance(value, bool) or value is None:
            raise ValueError(f"qa_threshold_missing_or_invalid:{name}")
        number = float(value)
        if not math.isfinite(number):
            raise ValueError(f"qa_threshold_nonfinite:{name}")
        values[name] = number
    return values


def _attack_v1_reference(context: Mapping[str, Any] | None) -> dict[str, Any]:
    if context is None or not context.get("root"):
        return {"available": False, "reason": "attack_v1_context_missing", "path": ATTACK_V1_BASELINE_PATH}
    root = Path(context["root"])
    path_value = root / ATTACK_V1_BASELINE_PATH
    result: dict[str, Any] = {
        "available": False,
        "path": ATTACK_V1_BASELINE_PATH,
        "authority_commit": ATTACK_V1_BASELINE_COMMIT,
        "expected_sha256": ATTACK_V1_BASELINE_SHA256,
    }
    try:
        ...
        if digest != ATTACK_V1_BASELINE_SHA256:
            result["reason"] = "attack_v1_baseline_hash_mismatch"
            return result
        reference = json.loads(canonical_raw.decode("utf-8"))
        if not isinstance(reference, Mapping) or reference.get("animation_id") != "attack-front-v1" or int(reference.get("frame_count", -1)) != 10:
            result["reason"] = "attack_v1_baseline_schema_mismatch"
            return result
        result["available"] = True
        result["reason"] = "verified_known_path_immutable_hash"
        result["spec"] = reference
        return result
    except (OSError, UnicodeError, ValueError, TypeError, json.JSONDecodeError) as error:
        result["reason"] = f"attack_v1_baseline_unreadable:{type(error).__name__}"
        return result
...
def _body_mechanics(spec: Mapping[str, Any], targets: list[Mapping[str, Any]], samples: list[Mapping[str, Any]], context: Mapping[str, Any] | None = None) -> dict[str, Any]:
    thresholds = _declared_thresholds(spec)
    pelvis = [_xy(target["joints"]["pelvis"]) for target in targets]
    ...
    v1_reference = _attack_v1_reference(context)
    v1_path = None
    if v1_reference["available"]:
        historical_targets = v1.prepare(v1_reference["spec"], context)["targets"]
    ...
    metrics = {
        ...
        "attack_v1_baseline": {key: value for key, value in v1_reference.items() if key != "spec"},
    ...
    gates = {
        "body_root_path_min_px": metrics["root_path_length_px"] >= thresholds["body_root_path_min_px"],
        "torso_rotation_range_min_deg": metrics["torso_rotation_range_deg"] >= thresholds["torso_rotation_range_min_deg"],
        "right_shoulder_to_wrist_path_gt_zero": metrics["right_shoulder_to_wrist_path_length_px"] > 0.0,
        "right_shoulder_to_wrist_path_gt_attack_v1": bool(v1_reference["available"]) and v1_path is not None and metrics["right_shoulder_to_wrist_path_length_px"] > v1_path,
        "left_wrist_counter_path_min_px": metrics["left_wrist_counter_path_length_px"] > thresholds["left_wrist_counter_path_min_px"],
        "head_counter_motion_max_deg": metrics["head_counter_motion_range_deg"] <= thresholds["head_counter_motion_max_deg"],
        ...
        "root_horizontal_excursion_in_declared_bounds": thresholds["root_horizontal_excursion_min_px"] <= ranges["root_horizontal_excursion_px"] <= thresholds["root_horizontal_excursion_max_px"],
        "root_vertical_excursion_in_declared_bounds": thresholds["root_vertical_excursion_min_px"] <= ranges["root_vertical_excursion_px"] <= thresholds["root_vertical_excursion_max_px"],
    }
...
def _unwrap_angles(angles: list[float]) -> list[float]:
    if not angles or any(not math.isfinite(float(value)) for value in angles):
        return []
    result = [float(angles[0])]
    for raw in angles[1:]:
        current = float(raw)
        previous_raw = float(angles[len(result) - 1])
        delta = (current - previous_raw + 180.0) % 360.0 - 180.0
        result.append(result[-1] + delta)
    return result


def _weapon_relational_metrics(
    spec: Mapping[str, Any],
    tips: list[tuple[float, float]],
    angles: list[float],
) -> dict[str, Any]:
    threshold = spec.get("qa_profile", {}).get("thresholds", {})
    try:
        active = sorted({int(value) for value in threshold["active_window_frames"]})
        hit_frame = int(threshold["hit_event_frame"])
    except (KeyError, TypeError, ValueError):
        return {"hard_gates": {"declared_weapon_timeline": False}, "status": "ATTACK_V2_WEAPON_ARC_GAP", "reason": "declared_weapon_timeline_missing_or_invalid"}
    unwrapped = _unwrap_angles(angles)
    if len(unwrapped) != len(tips) or len(unwrapped) < 3 or hit_frame <= 0 or hit_frame >= len(unwrapped) - 1 or not active:
        return {"hard_gates": {"trajectory_available": False}, "status": "ATTACK_V2_WEAPON_ARC_GAP", "reason": "weapon_trajectory_missing_or_out_of_range"}
    velocities = [
        {"from_frame": index - 1, "to_frame": index, "tip_motion_px": math.dist(tips[index - 1], tips[index]), "angular_velocity_degrees_per_frame": unwrapped[index] - unwrapped[index - 1]}
        for index in range(1, len(tips))
    ]
    active_velocities = [item for item in velocities if item["to_frame"] in active]
    pre_active_velocities = [item for item in velocities if item["to_frame"] < min(active)]
    pre_hit_velocities = [item for item in velocities if min(active) <= item["to_frame"] <= hit_frame]
    strike_velocity = velocities[hit_frame - 1]["angular_velocity_degrees_per_frame"]
    strike_sign = 1.0 if strike_velocity > 0.0 else -1.0 if strike_velocity < 0.0 else 0.0
    signed_pre_hit = [strike_sign * item["angular_velocity_degrees_per_frame"] for item in pre_hit_velocities] if strike_sign else []
    acceleration_steps = [signed_pre_hit[index] - signed_pre_hit[index - 1] for index in range(1, len(signed_pre_hit))]
    signs = [1 if value > 1e-12 else -1 if value < -1e-12 else 0 for value in acceleration_steps]
    sign_changes = [(previous, current) for previous, current in zip(signs, signs[1:]) if previous and current and previous != current]
    coherent_acceleration = bool(signed_pre_hit) and all(value > 0.0 for value in signed_pre_hit) and any(value > signed_pre_hit[0] for value in signed_pre_hit[1:]) and all(not (previous < 0 and current > 0) for previous, current in sign_changes) and signed_pre_hit[-1] >= signed_pre_hit[0]
    active_peak = max((abs(item["angular_velocity_degrees_per_frame"]) for item in active_velocities), default=0.0)
    pre_active_peak = max((abs(item["angular_velocity_degrees_per_frame"]) for item in pre_active_velocities), default=0.0)
    immediate = velocities[hit_frame]
    post_hit_path = sum(item["tip_motion_px"] for item in velocities if item["from_frame"] >= hit_frame)
    immediate_post_hit_path = sum(item["tip_motion_px"] for item in velocities if hit_frame <= item["from_frame"] < min(hit_frame + 2, len(tips) - 1))
    total_path = sum(item["tip_motion_px"] for item in velocities)
    angular_accelerations = [velocities[index]["angular_velocity_degrees_per_frame"] - velocities[index - 1]["angular_velocity_degrees_per_frame"] for index in range(1, len(velocities))]
    angular_jerk = [angular_accelerations[index] - angular_accelerations[index - 1] for index in range(1, len(angular_accelerations))]
    hard_gates = {
        "active_peak_exceeds_pre_active_peak": bool(active_velocities) and active_peak > pre_active_peak,
        "pre_hit_acceleration_coherent_toward_strike": coherent_acceleration,
        "post_hit_path_nonzero": post_hit_path > 0.0,
        "immediate_post_hit_path_nonzero": immediate_post_hit_path > 0.0,
        "immediate_post_hit_same_strike_sign": strike_sign != 0.0 and immediate["angular_velocity_degrees_per_frame"] * strike_sign > 0.0,
    }
    return {
        "metrics": {
            "active_peak_abs_angular_speed": round(active_peak, 6),
            "pre_active_peak_abs_angular_speed": round(pre_active_peak, 6),
            "strike_sign": int(strike_sign),
            "pre_hit_signed_angular_speeds": [round(value, 6) for value in signed_pre_hit],
            "post_hit_path_px": round(post_hit_path, 6),
            "immediate_post_hit_path_px": round(immediate_post_hit_path, 6),
            "total_path_px": round(total_path, 6),
            "max_abs_angular_acceleration": round(max((abs(value) for value in angular_accelerations), default=0.0), 6),
            "max_abs_angular_jerk": round(max((abs(value) for value in angular_jerk), default=0.0), 6),
        },
        ...
        "unwrapped_angles": [round(value, 6) for value in unwrapped],
        "accelerations": [{"from_frame": index, "to_frame": index + 1, "angular_acceleration_degrees_per_frame2": round(value, 6)} for index, value in enumerate(angular_accelerations)],
        "active_window_frames": active,
        "hit_event_frame": hit_frame,
        "hard_gates": hard_gates,
        "status": "ATTACK_V2_WEAPON_ARC_QA_PASSED" if all(hard_gates.values()) else "ATTACK_V2_WEAPON_ARC_GAP",
        "proof_source": "src/ugas/animation_profiles/attack_front_v2.py:unwrapped_relational_weapon_trajectory",
    }


def weapon_relational_pre_render_qa(spec: Mapping[str, Any], targets: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    result = _weapon_relational_metrics(spec, tips, angles)
    ...
    result["render_allowed"] = result["status"] == "ATTACK_V2_WEAPON_ARC_QA_PASSED"
    return result
...
def prepare(spec: Mapping[str, Any], context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    temporal_gates = {
        ...
        "root_horizontal_excursion_in_declared_bounds": float(threshold["root_horizontal_excursion_min_px"]) <= series["root_horizontal_excursion_px"] <= float(threshold["root_horizontal_excursion_max_px"]),
        "root_vertical_excursion_in_declared_bounds": float(threshold["root_vertical_excursion_min_px"]) <= series["root_vertical_excursion_px"] <= float(threshold["root_vertical_excursion_max_px"]),
    ...
    pre_render_weapon = weapon_relational_pre_render_qa(spec, targets)
    if not pre_render_weapon["render_allowed"]:
        raise ValueError("ATTACK_V2_WEAPON_ARC_GAP")
    return {"tracks": tracks, "samples": samples, "targets": targets, "plan": _attack_plan(), "presentation": spec["presentation_transform"], "phases": list(PHASES), "series": series, "temporal_gates": temporal_gates, "proxies": proxies, "weapon_relational_pre_render": pre_render_weapon}
...
def _weapon_arc_qa_v0111_rejected(spec: Mapping[str, Any], frame_records: list[Mapping[str, Any]], pre_render: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """Retained name for historical traceability; rejected v0.11.1 lane is inactive."""
    return {"status": "REJECTED_HISTORICAL_V0111_LANE", "active": False, "reason": "v0.11.2_uses_unwrapped_relational_weapon_trajectory"}
...
def _weapon_arc_qa(spec: Mapping[str, Any], frame_records: list[Mapping[str, Any]], pre_render: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    tips = [tuple(item["weapon"]["tip_presented"]) for item in frame_records]
    angles = [float(item["weapon"]["signed_sword_angle_degrees"]) for item in frame_records]
    result = _weapon_relational_metrics(spec, tips, angles)
    velocities = result.get("velocities", [])
    path = lambda first, last: round(sum(float(item["tip_motion_px"]) for item in velocities if first <= int(item["from_frame"]) and int(item["to_frame"]) <= last), 6)
    ...
    peak = max(velocities, key=lambda item: abs(float(item["angular_velocity_degrees_per_frame"])), default={"from_frame": -1, "to_frame": -1, "tip_motion_px": 0.0, "angular_velocity_degrees_per_frame": 0.0})
    gates = {
        **dict(result.get("hard_gates", {})),
        "sword_pivot_attached_all_frames": all(bool(item["weapon"].get("pivot_attached")) for item in frame_records),
        "active_window_frames_exact": [int(item["index"]) for item in frame_records if int(item["index"]) in active] == sorted(active),
        "hit_event_frame_exact": hit_frame == HIT_EVENT_FRAME,
        "weapon_tip_path_nonzero": path(0, len(frame_records) - 1) > 0.0,
    ...
    if pre_render is not None:
        gates["pre_render_relational_gate_replay"] = dict(pre_render.get("hard_gates", {})) == dict(result.get("hard_gates", {}))
    result["active_window_frames"] = sorted(active)
    result["hit_event_frame"] = hit_frame
    result["signed_sword_angle_degrees_by_frame"] = [{"frame": index, "degrees": round(value, 6)} for index, value in enumerate(result.get("unwrapped_angles", []))]
    result["tip_xy_presented"] = [{"frame": item["index"], "x": item["weapon"]["tip_presented"][0], "y": item["weapon"]["tip_presented"][1]} for item in frame_records]
    result["path_lengths_px"] = {"total": path(0, len(frame_records) - 1), "pre_hit": path(0, hit_frame), "active": path(min(active), max(active)), "recovery": path(hit_frame + 1, len(frame_records) - 1), "post_hit_follow_through": path(hit_frame, min(len(frame_records) - 1, hit_frame + 2))}
    result["peak_speed"] = peak
    result["sword_head_collision_pixels"] = critical_head
    result["sword_torso_forbidden_penetration_pixels"] = forbidden_torso
    result["hard_gates"] = gates
    result["status"] = "ATTACK_V2_WEAPON_ARC_QA_PASSED" if all(gates.values()) else "ATTACK_V2_WEAPON_ARC_GAP"
    result["proof_source"] = "src/ugas/animation_profiles/attack_front_v2.py:unwrapped_relational_weapon_trajectory"
    return result
...
def _temporal_qa(spec: Mapping[str, Any], prepared: Mapping[str, Any], frame_records: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    threshold = spec["qa_profile"]["thresholds"]
    weapon = _weapon_arc_qa(spec, frame_records, prepared.get("weapon_relational_pre_render"))
