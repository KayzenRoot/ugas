"""Fail-closed, provider-neutral orchestration runtime for UGAS v0.24.7.

Forward-only correction of the v0.24.7 TEST_ONLY control plane.  This module
...
TEST_ONLY_ROUTE_EXECUTOR_IDENTITY = "test-only-fake-executor-v0247"
CIRCUIT_STATES = frozenset({"CLOSED", "OPEN", "HALF_OPEN"})
CIRCUIT_CHECKPOINT_FIELDS = frozenset({"state", "failures", "probe_in_flight", "route_executor_identity", "project_id", "failure_threshold"})
NODE_DEADLINE_FIELDS = frozenset({"started_seconds", "deadline_seconds", "parent_deadline_seconds", "elapsed_seconds", "status", "error_class"})
ATTEMPT_REQUIRED_STATES = frozenset({"RUNNING", "SUCCEEDED", "FAILED"})
ATTEMPT_FORBIDDEN_STATES = frozenset({"PLANNED", "VALIDATED", "READY", "BLOCKED", "CANCELLED"})
GOVERNED_APPROVED_FAMILIES = ("maps_minimap", "ui_asset_family", "vfx_asset_family")
APPROVED_AUTHORITY_REGISTRY: dict[str, dict[str, str]] = {
    "maps_minimap": {
        "revision": "v0.21.3",
        "path": "docs/evidence/maps-minimap-runtime-v0213/map-contract-v0213.json",
        "approved_commit": "1fb298885c56ccbf3df7dfdcb1be37fe2dc23af3",
        "status": "MERGED_CLOSED",
        "content_hash": "c9c52304774afa3f7ac1d997f709bae7ec57a3c35f5a9b1a0c56fefa10eca035",
        "semantic_hash": "da3448da66edaa7f254a060ae67988df62da4a2ccceebfb1cf032af7e8b4c3dd",
    },
    "ui_asset_family": {
        "revision": "v0.22.3",
        "path": "docs/evidence/ui-asset-family-runtime-v0223/ui-style-authority-v0223.json",
        "approved_commit": "b08b9c3df74ef6a23046be396289e2fd72dc336b",
        "status": "MERGED_CLOSED",
        "content_hash": "50fef443249098c703a14f2027ec7e67538a749110de51b318ee604652a44dbd",
        "semantic_hash": "993337cf2b0ab3c5ea37c1a7f371acf93242b5b21227950d8c3529a8fc0a37a8",
    },
    "vfx_asset_family": {
        "revision": "v0.23.4",
        "path": "docs/evidence/vfx-asset-family-runtime-v0234/vfx-family-manifest-v0234.json",
        "approved_commit": "dee98f8cd89ebd83a36ead7a22a184700d6e916f",
        "status": "MERGED_CLOSED",
        "content_hash": "4d9e6fe3b056b0987b6c7978f826813ece340701d481fdc22436e450f7b2119c",
        "semantic_hash": "ebc9bdfb045e2a240f93d60f92a0b402f45262dca5f438be74b36916ad6ffefc",
    },
    "items_props": {
        "revision": "v0.19.1",
        "path": "docs/evidence/maps-minimap-runtime-v0212/items-props-authority-bindings-v0212.json",
        "approved_commit": "1fb298885c56ccbf3df7dfdcb1be37fe2dc23af3",
        "status": "APPROVED_FOUNDATION",
        "content_hash": "01b0b467ae5eb6b60d561552ee21766f1edab31fb8835b4570a2886b39a07b58",
        "semantic_hash": "59cfa25b1dbeff5cb7628c0f18b770788f2477fd5dc3ba750f0f632becb581b9",
    },
}
GIT_COMMIT_WITNESS = "GIT_COMMIT_WITNESS"
FROZEN_REGISTRY_WITNESS = "FROZEN_REGISTRY_WITNESS"
...
def _assert_git_commit(value: Any, label: str) -> None:
    if not isinstance(value, str) or len(value) != 40 or any(char not in "0123456789abcdef" for char in value):
        _reject("ORCH_GIT_COMMIT_INVALID", f"{label} must be a lowercase git commit hex digest")
...
def validate_dependency_ref(reference: Mapping[str, Any], *, expected_project_id: str | None = None) -> dict[str, Any]:
    ...
    if "approved_commit" in value:
        _assert_git_commit(value["approved_commit"], "dependency_ref.approved_commit")
    return value
...
def validate_dag(dag: Mapping[str, Any], request: Mapping[str, Any] | None = None, *, authority_payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if authority_payload is not None:
        for item in nodes:
            expected = node_authority_input_contract_hash(item, authority_payload)
            if item["input_contract_hash"] != expected:
                _reject("ORCH_AUTHORITY_INPUT_CONTRACT_MISMATCH", f"input_contract_hash is stale for {item['node_id']}")
    for item in nodes:
...
class CircuitBreaker:
    ...
    def admit_half_open_probe(self) -> bool:
        if self.state != "HALF_OPEN":
            _reject("ORCH_CIRCUIT_HALF_OPEN_PROBE_INVALID", "half-open probe admission requires HALF_OPEN state")
        return self.allow()


def admit_half_open_probe(breaker: CircuitBreaker) -> bool:
    return breaker.admit_half_open_probe()
...
class CircuitRegistry:
    ...
    def for_identity(self, project_id: str, route_executor_identity: str) -> CircuitBreaker:
        key = (project_id, route_executor_identity)
        return self._breakers.setdefault(key, CircuitBreaker())

    def get(self, project_id: str, route_executor_identity: str) -> CircuitBreaker | None:
        return self._breakers.get((project_id, route_executor_identity))
...
class OrchestrationRuntime:
    ...
    _route_executor_identity: str = field(default="", init=False)
    _breaker: CircuitBreaker = field(init=False)

    def __post_init__(self) -> None:
        if self.repo_root is not None and self.request.get("dependency_refs"):
            binding = bind_governed_orchestration(
                self.request,
                self.dag,
                Path(self.repo_root),
            ...
            self.dag = binding["dag"]
            self.authority_binding_hash = binding["authority_binding_hash"]
        else:
        ...
        self._route_executor_identity = str(self.execution_policy.get("route_executor_identity") or TEST_ONLY_ROUTE_EXECUTOR_IDENTITY)
        if not self._route_executor_identity.strip():
            _reject("ORCH_ROUTE_EXECUTOR_IDENTITY_INVALID", "route_executor_identity is required")
        self._execution_policy_identity = sha256_value({"max_concurrency": self.max_concurrency, "family_limits": self.family_limits, "execution_policy": self.execution_policy, "authority_binding_hash": self.authority_binding_hash or "", "route_executor_identity": self._route_executor_identity})
        ...
        self._breaker = self.circuit_registry.for_identity(self.request["project_id"], self._route_executor_identity)
    ...
    @property
    def route_executor_identity(self) -> str:
        return self._route_executor_identity

    def circuit_checkpoint(self) -> dict[str, Any]:
        return {"state": self._breaker.state, "failures": self._breaker.failures, "probe_in_flight": self._breaker._probe_in_flight, "route_executor_identity": self._route_executor_identity, "project_id": self.request["project_id"], "failure_threshold": self._breaker.failure_threshold}
        ...
        try:
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    if fault == "persistent_retryable" or (fault == "retryable" and attempt == 1):
...
def _policy_identity(max_concurrency: int, family_limits: Mapping[str, int], execution_policy: Mapping[str, Any], authority_binding_hash: str, route_executor_identity: str = TEST_ONLY_ROUTE_EXECUTOR_IDENTITY) -> str:
    return sha256_value({"max_concurrency": max_concurrency, "family_limits": dict(family_limits), "execution_policy": dict(execution_policy), "authority_binding_hash": authority_binding_hash, "route_executor_identity": route_executor_identity})
...
def _finite_non_negative_number(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)) or value < 0 or value != value or value == float("inf"):
        _reject("ORCH_CHECKPOINT_DEADLINE_INVALID", f"{label} must be a bounded non-negative finite number")
    return float(value)


def _validate_checkpoint_attempts(attempts: Any, states: Mapping[str, str], nodes_by_id: Mapping[str, Mapping[str, Any]]) -> None:
    if not isinstance(attempts, Mapping):
        _reject("ORCH_CHECKPOINT_ATTEMPTS_INVALID", "checkpoint attempts must be an object")
    for node_id, state in states.items():
        has_attempt = node_id in attempts
        if state in ATTEMPT_FORBIDDEN_STATES and has_attempt:
            _reject("ORCH_CHECKPOINT_ATTEMPTS_INVALID", f"never-dispatched node {node_id} may not carry attempt count")
        if state in ATTEMPT_REQUIRED_STATES and not has_attempt:
            _reject("ORCH_CHECKPOINT_ATTEMPTS_INVALID", f"dispatched node {node_id} must carry exactly one attempt count")
    for node_id, count in attempts.items():
        if node_id not in nodes_by_id:
            _reject("ORCH_CHECKPOINT_ATTEMPTS_UNKNOWN_NODE", f"attempt references unknown node {node_id}")
        if isinstance(count, bool) or not isinstance(count, int) or count < 1 or count > nodes_by_id[node_id]["retry_policy"]["max_attempts"]:
            _reject("ORCH_CHECKPOINT_ATTEMPTS_INVALID", f"attempt count for {node_id} is out of bounds")


def _nodes_entered_running(events: Iterable[Mapping[str, Any]]) -> set[str]:
    return {str(event["node_id"]) for event in events if event.get("to") == "RUNNING"}


def _validate_checkpoint_node_deadlines(node_deadlines: Any, nodes_by_id: Mapping[str, Mapping[str, Any]], events: Iterable[Mapping[str, Any]], parent_deadline_seconds: float) -> None:
    if not isinstance(node_deadlines, Mapping):
        _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", "checkpoint node_deadlines must be an object")
    running_nodes = _nodes_entered_running(events)
    for node_id in running_nodes:
        if node_id not in node_deadlines:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline record is required for node {node_id} that entered RUNNING")
    for node_id, record in node_deadlines.items():
        if node_id not in nodes_by_id:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_UNKNOWN_NODE", f"deadline references unknown node {node_id}")
        if not isinstance(record, Mapping):
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline record for {node_id} must be an object")
        _assert_no_unknown(record, NODE_DEADLINE_FIELDS, f"node_deadlines.{node_id}")
        started = _finite_non_negative_number(record.get("started_seconds"), f"node_deadlines.{node_id}.started_seconds")
        deadline = _finite_non_negative_number(record.get("deadline_seconds"), f"node_deadlines.{node_id}.deadline_seconds")
        if deadline < started:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline_seconds must be >= started_seconds for {node_id}")
        node_timeout = float(nodes_by_id[node_id]["timeout_seconds"])
        if deadline > started + node_timeout:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline_seconds exceeds node timeout bound for {node_id}")
        parent_bound = _finite_non_negative_number(record.get("parent_deadline_seconds", parent_deadline_seconds), f"node_deadlines.{node_id}.parent_deadline_seconds")
        if deadline > parent_bound:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline_seconds exceeds parent deadline for {node_id}")
        if "elapsed_seconds" in record:
            _finite_non_negative_number(record["elapsed_seconds"], f"node_deadlines.{node_id}.elapsed_seconds")
        status = record.get("status")
        if status not in {"SUCCEEDED", "TIMEOUT", "RETRYABLE"}:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline status for {node_id} is invalid")
        if status == "SUCCEEDED" and "error_class" in record:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"SUCCEEDED deadline for {node_id} may not carry error_class")
        if status == "TIMEOUT" and "error_class" not in record:
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"TIMEOUT deadline for {node_id} requires error_class")
        if "error_class" in record and not isinstance(record["error_class"], str):
            _reject("ORCH_CHECKPOINT_NODE_DEADLINES_INVALID", f"deadline error_class for {node_id} must be a string")


def _validate_checkpoint_circuit(circuit: Any, *, project_id: str, route_executor_identity: str) -> dict[str, Any]:
    if not isinstance(circuit, Mapping):
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "checkpoint circuit must be an object")
    _assert_no_unknown(circuit, CIRCUIT_CHECKPOINT_FIELDS, "checkpoint circuit")
    state = circuit.get("state")
    if state not in CIRCUIT_STATES:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "checkpoint circuit state is invalid")
    ...
    if isinstance(failure_threshold, bool) or not isinstance(failure_threshold, int) or failure_threshold < 1:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "checkpoint circuit failure_threshold must be a positive integer")
    failures = circuit.get("failures")
    if isinstance(failures, bool) or not isinstance(failures, int) or failures < 0 or failures > MAX_RETRIES:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "checkpoint circuit failures must be a bounded non-negative integer")
    probe_in_flight = circuit.get("probe_in_flight")
    if type(probe_in_flight) is not bool:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "checkpoint circuit probe_in_flight must be a literal bool")
    if circuit.get("route_executor_identity") != route_executor_identity:
        _reject("ORCH_CHECKPOINT_CIRCUIT_IDENTITY_MISMATCH", "checkpoint circuit route identity is stale")
    if circuit.get("project_id") != project_id:
        _reject("ORCH_CHECKPOINT_CIRCUIT_IDENTITY_MISMATCH", "checkpoint circuit project identity is stale")
    if state == "OPEN" and probe_in_flight is True:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "OPEN circuit may not carry an in-flight probe")
    if state == "OPEN" and failures < failure_threshold:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "OPEN circuit requires failures >= failure_threshold")
    if state == "CLOSED":
        if probe_in_flight is True:
            _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "CLOSED circuit may not carry an in-flight probe")
        if failures >= failure_threshold:
            _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "CLOSED circuit requires failures < failure_threshold")
    if state == "HALF_OPEN" and failures < failure_threshold:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "HALF_OPEN circuit requires failures >= failure_threshold")
    return {"state": state, "failures": failures, "probe_in_flight": probe_in_flight, "route_executor_identity": route_executor_identity, "project_id": project_id, "failure_threshold": failure_threshold}


def _breaker_from_checkpoint(circuit: Mapping[str, Any]) -> CircuitBreaker:
    validated = _validate_checkpoint_circuit(circuit, project_id=str(circuit["project_id"]), route_executor_identity=str(circuit["route_executor_identity"]))
    breaker = CircuitBreaker(failure_threshold=int(validated["failure_threshold"]))
    breaker.state = str(validated["state"])
    breaker.failures = int(validated["failures"])
    breaker._probe_in_flight = bool(validated["probe_in_flight"])
    return breaker
...
def validate_checkpoint(checkpoint: Mapping[str, Any], request: Mapping[str, Any], dag: Mapping[str, Any], completed_results: Mapping[str, Mapping[str, Any]] | None = None, *, execution_policy: Mapping[str, Any] | None = None, max_concurrency: int | None = None, family_limits: Mapping[str, int] | None = None, authority_binding_hash: str | None = None) -> dict[str, Any]:
    ...
    if value.get("event_hash") != final_event_hash:
        _reject("ORCH_CHECKPOINT_EVENT_HASH_MISMATCH", "top-level event_hash does not match the verified event chain")
    nodes_by_id = {node["node_id"]: node for node in dag_value["nodes"]}
    _validate_checkpoint_attempts(value.get("attempts", {}), states, nodes_by_id)
    _validate_checkpoint_node_deadlines(value.get("node_deadlines", {}), nodes_by_id, events, float(request_value["timeout_seconds"]))
    route_executor_identity = str((execution_policy or value.get("execution_policy", {})).get("route_executor_identity") or TEST_ONLY_ROUTE_EXECUTOR_IDENTITY)
    circuit = _validate_checkpoint_circuit(value.get("circuit", {}), project_id=request_value["project_id"], route_executor_identity=route_executor_identity)
    ...
    if max_concurrency is not None or family_limits is not None or execution_policy is not None or authority_binding_hash is not None:
        ...
        route_executor_identity = str(policy.get("route_executor_identity") or TEST_ONLY_ROUTE_EXECUTOR_IDENTITY)
        ...
        _validate_checkpoint_circuit(value.get("circuit", {}), project_id=request_value["project_id"], route_executor_identity=route_executor_identity)
    return {"status": "PASS", "checkpoint_status": expected_status, "request_hash": request_value["request_hash"], "dag_hash": dag_value["dag_hash"], "completed": len(completed), "event_sequence": len(events), "identity_hash": stored_identity.get("identity_hash"), "final_event_hash": final_event_hash, "circuit": circuit}
...
def resume_from_checkpoint(request: Mapping[str, Any], dag: Mapping[str, Any], checkpoint: Mapping[str, Any], **kwargs: Any) -> OrchestrationRuntime:
    ...
    circuit_registry = kwargs.pop("circuit_registry", None)
    validation = validate_checkpoint(checkpoint, request, dag, execution_policy=execution_policy, max_concurrency=max_concurrency, family_limits=family_limits, authority_binding_hash=authority_binding_hash)
    runtime = OrchestrationRuntime(request, dag, max_concurrency=max_concurrency, family_limits=dict(family_limits), executor=kwargs.pop("executor", None) or FakeExecutor(), clock=DeterministicClock(float(checkpoint.get("clock_elapsed_seconds", 0.0))), execution_policy=execution_policy, authority_binding_hash=authority_binding_hash, circuit_registry=circuit_registry)
    restored_breaker = _breaker_from_checkpoint(checkpoint.get("circuit", {}))
    if circuit_registry is not None:
        existing = circuit_registry.get(runtime.request["project_id"], runtime.route_executor_identity)
        if existing is not None and (existing.state != restored_breaker.state or existing.failures != restored_breaker.failures or existing._probe_in_flight != restored_breaker._probe_in_flight or existing.failure_threshold != restored_breaker.failure_threshold):
            _reject("ORCH_CHECKPOINT_CIRCUIT_REGISTRY_CONFLICT", "circuit registry conflicts with checkpoint breaker state")
    runtime._breaker = restored_breaker
    if circuit_registry is not None:
        circuit_registry._breakers[(runtime.request["project_id"], runtime.route_executor_identity)] = restored_breaker
    ...
    runtime._last_event_hash = str(validation["final_event_hash"])
...
def _canonical_repo_bytes(raw: bytes, path: str) -> bytes:
...
def git_object_witness_available(repo_root: Path) -> bool:
    """Deterministic Git vs official no-git snapshot probe. Never request-selected.

    Presence of `.git` selects GIT_COMMIT_WITNESS. Official snapshots have no `.git`
    and use FROZEN_REGISTRY_WITNESS. A failed `git show` never selects no-git.
    """

    return (Path(repo_root) / ".git").exists()


def resolve_authority_verification_mode(repo_root: Path) -> str:
    if git_object_witness_available(repo_root):
        return GIT_COMMIT_WITNESS
    return FROZEN_REGISTRY_WITNESS


def _validate_approved_authority_tuple(family: str, revision: str, path: str, status: str | None = None, approved_commit: str | None = None) -> dict[str, str]:
    registry = APPROVED_AUTHORITY_REGISTRY.get(family)
    if registry is None:
        _reject("ORCH_DEPENDENCY_AUTHORITY_UNAPPROVED", f"family {family} is not in the approved authority registry")
    if not registry.get("content_hash") or not registry.get("semantic_hash"):
        _reject("ORCH_DEPENDENCY_AUTHORITY_REGISTRY_HASH_MISSING", f"canonical registry hashes are required for {family}")
    if registry["revision"] != revision or registry["path"] != path:
        _reject("ORCH_DEPENDENCY_AUTHORITY_TUPLE_MISMATCH", f"authority tuple does not match approved registry for {family}")
    if approved_commit is not None and approved_commit != registry["approved_commit"]:
        _reject("ORCH_DEPENDENCY_APPROVED_COMMIT_REGISTRY_MISMATCH", f"approved_commit does not match registry for {family}")
    if status is not None and status != registry["status"]:
        _reject("ORCH_DEPENDENCY_AUTHORITY_STATUS_MISMATCH", f"status {status} does not match canonical registry status {registry['status']} for {family}")
    return registry


def build_approved_dependency_ref(repo_root: Path, *, project_id: str, family: str) -> dict[str, Any]:
    registry = APPROVED_AUTHORITY_REGISTRY[family]
    return build_dependency_ref(repo_root, project_id=project_id, family=family, revision=registry["revision"], path=registry["path"], status=registry["status"], approved_commit=registry["approved_commit"])


def build_governed_dependency_refs(repo_root: Path, *, project_id: str, families: Iterable[str] = GOVERNED_APPROVED_FAMILIES) -> list[dict[str, Any]]:
    return [build_approved_dependency_ref(repo_root, project_id=project_id, family=family) for family in families]


def node_authority_input_contract_hash(node: Mapping[str, Any], authority_payload: Mapping[str, Any]) -> str:
    return sha256_value({
        "node_id": node["node_id"],
        "operation": node["operation"],
        "asset_family": node["asset_family"],
        "scheduling_key": node["scheduling_key"],
        "resource_class": node["resource_class"],
        "authority_binding_hash": authority_payload.get("authority_binding_hash"),
        "resolved_authority_hashes": authority_payload.get("resolved_authority_hashes"),
        "authorities": [{key: item.get(key) for key in ("family", "revision", "path", "resolved_content_hash", "resolved_semantic_hash", "approved_commit") if key in item} for item in authority_payload.get("authorities", [])],
    })


def build_dependency_ref(repo_root: Path, *, project_id: str, family: str, revision: str, path: str, authority_id: str | None = None, status: str | None = None, approved_commit: str | None = None) -> dict[str, Any]:
    registry = _validate_approved_authority_tuple(family, revision, path, status)
    if approved_commit is not None and approved_commit != registry["approved_commit"]:
        _reject("ORCH_DEPENDENCY_APPROVED_COMMIT_REGISTRY_MISMATCH", f"approved_commit does not match registry for {family}")
    ...
    raw = _canonical_repo_bytes(target.read_bytes(), safe_path)
    commit = approved_commit or registry["approved_commit"]
    value: dict[str, Any] = {"project_id": project_id, "family": family, "revision": revision, "path": safe_path, "content_hash": sha256_bytes(raw), "semantic_hash": _semantic_hash(raw), "read_only": True, "status": registry["status"], "approved_commit": commit}
...
def resolve_dependency_ref(reference: Mapping[str, Any], repo_root: Path, *, expected_project_id: str | None = None, require_approved_commit: bool = False) -> dict[str, Any]:
    value = validate_dependency_ref(reference, expected_project_id=expected_project_id)
    registry = _validate_approved_authority_tuple(value["family"], value["revision"], value["path"], value.get("status"), value.get("approved_commit"))
    if require_approved_commit or value["family"] in GOVERNED_APPROVED_FAMILIES:
        if "approved_commit" not in value:
            _reject("ORCH_DEPENDENCY_APPROVED_COMMIT_REQUIRED", f"approved_commit is required for governed family {value['family']}")
        if value["approved_commit"] != registry["approved_commit"]:
            _reject("ORCH_DEPENDENCY_APPROVED_COMMIT_REGISTRY_MISMATCH", f"approved_commit does not match registry for {value['family']}")
    root = Path(repo_root).resolve()
    ...
    if observed_content_hash != registry["content_hash"] or observed_semantic_hash != registry["semantic_hash"]:
        _reject("ORCH_DEPENDENCY_AUTHORITY_REGISTRY_HASH_MISMATCH", f"current authority bytes do not match frozen canonical hashes for {safe_path}")
    mode = resolve_authority_verification_mode(root)
    if mode == GIT_COMMIT_WITNESS:
        if "approved_commit" not in value:
            _reject("ORCH_DEPENDENCY_APPROVED_COMMIT_REQUIRED", f"approved_commit is required for git-backed family {value['family']}")
        approved_raw = _canonical_repo_bytes(_git_blob_at(root, value["approved_commit"], safe_path), safe_path)
        approved_content_hash = sha256_bytes(approved_raw)
        approved_semantic_hash = _semantic_hash(approved_raw)
        if approved_content_hash != registry["content_hash"] or approved_semantic_hash != registry["semantic_hash"]:
            _reject("ORCH_DEPENDENCY_AUTHORITY_REGISTRY_HASH_MISMATCH", f"git-backed authority bytes do not match frozen canonical hashes for {safe_path}")
        if approved_raw != raw:
            _reject("ORCH_DEPENDENCY_AUTHORITY_BYTES_CHANGED", f"authority bytes differ from approved commit for {safe_path}")
    return {**value, "status": registry["status"], "resolved_content_hash": observed_content_hash, "resolved_semantic_hash": observed_semantic_hash, "resolved_bytes": len(raw), "resolved_from": "repository", "verification_mode": mode}


def resolve_dependency_refs(refs: Iterable[Mapping[str, Any]], repo_root: Path, *, expected_project_id: str | None = None, require_approved_commit: bool = False) -> dict[str, Any]:
    resolved = [resolve_dependency_ref(reference, repo_root, expected_project_id=expected_project_id, require_approved_commit=require_approved_commit) for reference in refs]
    if not resolved:
        _reject("ORCH_DEPENDENCY_AUTHORITY_MISSING", "at least one approved authority is required")
    identity_payload = [{key: item[key] for key in ("project_id", "family", "revision", "path", "resolved_content_hash", "resolved_semantic_hash", "read_only", "status", "approved_commit") if key in item} for item in resolved]
    modes = {item.get("verification_mode") for item in resolved}
    return {"status": "PASS", "authorities": resolved, "authority_binding_hash": sha256_value(identity_payload), "resolved_authority_hashes": {item["family"]: item["resolved_content_hash"] for item in resolved}, "verification_mode": next(iter(modes)) if len(modes) == 1 else sorted(modes)}


def bind_governed_orchestration(
    ...
    *,
    ...
    validated_request = validate_request(request)
    if not validated_request.get("dependency_refs"):
        _reject("ORCH_DEPENDENCY_AUTHORITY_MISSING", "at least one approved authority is required")
    resolved = resolve_dependency_refs(
        validated_request["dependency_refs"],
        repo_root,
        expected_project_id=validated_request["project_id"],
        require_approved_commit=True,
    )
    computed_hash = resolved["authority_binding_hash"]
    if authority_binding_hash is not None and authority_binding_hash != computed_hash:
        _reject("ORCH_AUTHORITY_BINDING_MISMATCH", "authority binding does not match resolved repository bytes")
    validated_dag = validate_dag(dag, validated_request, authority_payload=resolved)
    return {
        "request": validated_request,
        "dag": validated_dag,
        "authority_payload": resolved,
        "authority_binding_hash": computed_hash,
    }


def _git_checked(repo_root: Path, args: list[str], *, rejection_class: str, detail: str) -> bytes:
    try:
        completed = subprocess.run(["git", "-C", str(repo_root), *args], capture_output=True, check=True)
    except (OSError, subprocess.CalledProcessError) as exc:
        _reject(rejection_class, f"{detail}: {exc}")
    return completed.stdout


def _git_ls_tree_blob(repo_root: Path, ref: str, path: str) -> str | None:
    try:
        completed = subprocess.run(["git", "-C", str(repo_root), "ls-tree", ref, "--", path], capture_output=True, check=True, text=True)
...
def _parse_ls_tree_z(payload: bytes) -> list[dict[str, str]]:
    ...
    for record in payload.split(b"\0"):
        ...
        try:
            ...
            mode, typ, object_id = meta.decode("ascii").split(" ", 2)
        except (ValueError, UnicodeDecodeError):
            _reject("HISTORICAL_TREE_MALFORMED", "malformed git ls-tree record")
        if typ not in {"blob", "tree", "commit"} or not mode or not object_id:
            _reject("HISTORICAL_TREE_MALFORMED", f"malformed git ls-tree identity {meta!r}")
        try:
            path = validate_safe_relative_path(path_bytes.decode("utf-8"))
        except OrchestrationContractError:
            raise
        except Exception as exc:
            _reject("HISTORICAL_TREE_MALFORMED", f"malformed git ls-tree path: {exc}")
        entries.append({"mode": mode, "type": typ, "object_id": object_id, "path": path})
    return entries


def _resolve_git_commit(repo_root: Path, ref: str) -> str:
    raw = _git_checked(repo_root, ["rev-parse", "--verify", f"{ref}^{{commit}}"], rejection_class="HISTORICAL_REF_UNRESOLVED", detail=f"unresolved historical ref {ref}")
    commit = raw.decode("ascii", errors="replace").strip()
    if not commit:
        _reject("HISTORICAL_REF_UNRESOLVED", f"unresolved historical ref {ref}")
    return commit


def normalized_git_entry(mode: str, typ: str, object_id: str, path: str) -> dict[str, str]:
    return {"mode": mode, "type": typ, "object_id": object_id, "path": validate_safe_relative_path(path)}


def fingerprint_historical_entries(entries: Iterable[Mapping[str, str]]) -> str:
    ordered = [normalized_git_entry(item["mode"], item["type"], item["object_id"], item["path"]) for item in entries]
    ordered.sort(key=lambda item: item["path"])
    return sha256_value(ordered)


def diff_historical_entry_sets(authority_entries: Iterable[Mapping[str, str]], observed_entries: Iterable[Mapping[str, str]]) -> list[dict[str, Any]]:
    authority_map = {item["path"]: normalized_git_entry(item["mode"], item["type"], item["object_id"], item["path"]) for item in authority_entries}
    observed_map = {item["path"]: normalized_git_entry(item["mode"], item["type"], item["object_id"], item["path"]) for item in observed_entries}
    differences: list[dict[str, Any]] = []
    for path in sorted(set(authority_map) - set(observed_map)):
        differences.append({"path": path, "reason": "deleted", "authority_entry": authority_map[path]})
    for path in sorted(set(observed_map) - set(authority_map)):
        differences.append({"path": path, "reason": "extra", "observed_entry": observed_map[path]})
    for path in sorted(set(authority_map) & set(observed_map)):
        authority_entry = authority_map[path]
        observed_entry = observed_map[path]
        if authority_entry == observed_entry:
            continue
        if authority_entry["mode"] != observed_entry["mode"] and authority_entry["object_id"] == observed_entry["object_id"] and authority_entry["type"] == observed_entry["type"]:
            reason = "mode_mismatch"
        elif authority_entry["type"] != observed_entry["type"]:
            reason = "type_mismatch"
        elif authority_entry["object_id"] != observed_entry["object_id"]:
            reason = "object_mismatch"
        else:
            reason = "entry_mismatch"
        differences.append({"path": path, "reason": reason, "authority_entry": authority_entry, "observed_entry": observed_entry})
    return differences


def enumerate_git_tree_entries(repo_root: Path, ref: str, roots: Iterable[str]) -> dict[str, Any]:
    commit = _resolve_git_commit(repo_root, ref)
    requested_roots = [validate_safe_relative_path(root) for root in roots]
    entries_by_path: dict[str, dict[str, str]] = {}
    root_objects: list[dict[str, Any]] = []
    for root in requested_roots:
        listing = _git_checked(repo_root, ["ls-tree", "-z", commit, "--", root], rejection_class="HISTORICAL_REF_UNRESOLVED", detail=f"git ls-tree failed for root {root} at {ref}")
        root_entries = _parse_ls_tree_z(listing)
        if not root_entries:
            _reject("HISTORICAL_ROOT_EMPTY", f"requested historical root has zero git entries: {root}")
        root_entry = root_entries[0]
        root_objects.append({"requested_root": root, "root_object": {"mode": root_entry["mode"], "type": root_entry["type"], "object_id": root_entry["object_id"], "path": root_entry["path"]}})
        recursive = _git_checked(repo_root, ["ls-tree", "-r", "-z", commit, "--", root], rejection_class="HISTORICAL_REF_UNRESOLVED", detail=f"git ls-tree -r failed for root {root} at {ref}")
        recursive_entries = _parse_ls_tree_z(recursive)
        if not recursive_entries:
            _reject("HISTORICAL_ROOT_EMPTY", f"requested historical root has zero recursive entries: {root}")
        for entry in recursive_entries:
            previous = entries_by_path.get(entry["path"])
            if previous is not None and previous != entry:
                _reject("HISTORICAL_TREE_MALFORMED", f"conflicting git entry identity for {entry['path']}")
            entries_by_path[entry["path"]] = entry
    ordered = [entries_by_path[path] for path in sorted(entries_by_path)]
    return {
        "ref": ref,
        ...
        "fingerprint": fingerprint_historical_entries(ordered),
    }


def enumerate_git_tree_paths(repo_root: Path, ref: str, roots: Iterable[str]) -> list[str]:
    return [entry["path"] for entry in enumerate_git_tree_entries(repo_root, ref, roots)["entries"]]


def compare_historical_evidence_tree(repo_root: Path, authority_ref: str, roots: Iterable[str], *, label: str) -> dict[str, Any]:
    observed_ref = "HEAD"
    requested_roots = [validate_safe_relative_path(root) for root in roots]
    authority = enumerate_git_tree_entries(repo_root, authority_ref, requested_roots)
    observed = enumerate_git_tree_entries(repo_root, observed_ref, requested_roots)
    differences = diff_historical_entry_sets(authority["entries"], observed["entries"])
    exact_entry_set = not differences
    if differences:
        _reject("HISTORICAL_TREE_MISMATCH", f"{label} historical evidence tree mismatch: {len(differences)} difference(s)")
    return {
        "status": "PASS" if exact_entry_set else "FAIL",
        "label": label,
        "authority_ref": authority_ref,
        "observed_ref": observed_ref,
        "requested_roots": requested_roots,
        "authority_entries_count": authority["entries_count"],
        "observed_entries_count": observed["entries_count"],
        "exact_entry_set": exact_entry_set,
        "authority_fingerprint": authority["fingerprint"],
        "observed_fingerprint": observed["fingerprint"],
        "equality": exact_entry_set and authority["fingerprint"] == observed["fingerprint"],
        "authority_root_objects": authority["root_objects"],
        "observed_root_objects": observed["root_objects"],
        "set_equal": exact_entry_set,
        "differences": differences,
        "paths": [entry["path"] for entry in authority["entries"]],
    }


def compare_git_authority_tree(repo_root: Path, authority_ref: str, paths: Iterable[str], *, label: str) -> dict[str, Any]:
    observed_ref = "HEAD"
    path_list = sorted({validate_safe_relative_path(path) for path in paths})
    authority_blobs: dict[str, str | None] = {}
    observed_blobs: dict[str, str | None] = {}
    differences: list[dict[str, Any]] = []
    for path in path_list:
        authority_blob = _git_ls_tree_blob(repo_root, authority_ref, path)
        observed_blob = _git_ls_tree_blob(repo_root, observed_ref, path)
        authority_blobs[path] = authority_blob
        observed_blobs[path] = observed_blob
        if authority_blob is None or observed_blob is None:
            differences.append({"path": path, "authority_blob": authority_blob, "observed_blob": observed_blob, "reason": "missing"})
        elif authority_blob != observed_blob:
            differences.append({"path": path, "authority_blob": authority_blob, "observed_blob": observed_blob, "reason": "blob_mismatch"})
    set_equal = not differences
    fingerprint = sha256_value({"label": label, "authority_ref": authority_ref, "observed_ref": observed_ref, "authority_blobs": authority_blobs, "observed_blobs": observed_blobs})
    return {
        "label": label,
        "authority_ref": authority_ref,
        "observed_ref": observed_ref,
        "authority_file_count": sum(1 for blob in authority_blobs.values() if blob is not None),
        "observed_file_count": sum(1 for blob in observed_blobs.values() if blob is not None),
...
def validate_historical_immutability(repo_root: Path, *, label: str = "governed-authority") -> dict[str, Any]:
    proofs = [compare_git_authority_tree(repo_root, entry["approved_commit"], [entry["path"]], label=f"{label}:{family}") for family, entry in APPROVED_AUTHORITY_REGISTRY.items()]
    set_equal = all(proof["set_equal"] for proof in proofs)
    return {
        "status": "PASS" if set_equal else "FAIL",
        "label": label,
        "proof_count": len(proofs),
        "set_equal": set_equal,
        "fingerprint": sha256_value(proofs),
        "proofs": proofs,
    }


def prove_half_open_second_admission_blocked(registry: CircuitRegistry, project_id: str, route_id: str) -> dict[str, Any]:
    breaker = registry.get(project_id, route_id)
    if breaker is None:
        breaker = CircuitBreaker(failure_threshold=2)
        registry._breakers[(project_id, route_id)] = breaker
    if breaker.state != "HALF_OPEN":
        breaker.failures = max(breaker.failures, breaker.failure_threshold)
        breaker.state = "OPEN"
        breaker.half_open()
    breaker._probe_in_flight = False
    first_allow = admit_half_open_probe(breaker)
    second_allow = breaker.allow()
    return {
        "status": "PASS" if first_allow and not second_allow else "FAIL",
        "project_id": project_id,
        "route_executor_identity": route_id,
        "first_probe_admitted": first_allow,
        "second_probe_blocked": not second_allow,
        "probe_in_flight_after_first": breaker._probe_in_flight,
    }


def probe_deadline_boundary_truth() -> dict[str, Any]:
    """Deterministic deadline probe: duration == allowed is canonical success."""

    def _node(node_id: str, timeout: int = 10) -> dict[str, Any]:
        return {"node_id": node_id, "dependencies": [], "operation": "EXECUTE", "input_contract_hash": sha256_value({"node": node_id, "probe": "deadline"}), "dependency_hashes": {}, "timeout_seconds": timeout, "retry_policy": {"max_attempts": 1, "retryable_errors": []}, "resource_class": "cpu-test", "scheduling_key": "deadline-probe", "priority": 10, "asset_family": "maps_minimap"}

    before_request = build_request(request_id="deadline-before", project_id="deadline-probe", request_kind="ASSET_OPERATION", asset_family="maps_minimap", operation="EXECUTE", inputs={}, dependency_refs=[], outputs={"project_id": "deadline-probe", "path": "out.json"}, quality_profile="qa", budget_profile="bounded", priority=50, timeout_seconds=5, retry_policy={"max_attempts": 1, "retryable_errors": []}, idempotency_key="deadline-before", production_intent=False, test_only=True)
    before_dag = build_dag(before_request, [_node("before")])
    before_snapshot = OrchestrationRuntime(before_request, before_dag, executor=FakeExecutor(durations={"before": 4.0})).run()
    exact_request = build_request(request_id="deadline-exact", project_id="deadline-probe", request_kind="ASSET_OPERATION", asset_family="maps_minimap", operation="EXECUTE", inputs={}, dependency_refs=[], outputs={"project_id": "deadline-probe", "path": "out.json"}, quality_profile="qa", budget_profile="bounded", priority=50, timeout_seconds=5, retry_policy={"max_attempts": 1, "retryable_errors": []}, idempotency_key="deadline-exact", production_intent=False, test_only=True)
    exact_dag = build_dag(exact_request, [_node("exact")])
    exact_snapshot = OrchestrationRuntime(exact_request, exact_dag, executor=FakeExecutor(durations={"exact": 5.0})).run()
    after_request = build_request(request_id="deadline-after", project_id="deadline-probe", request_kind="ASSET_OPERATION", asset_family="maps_minimap", operation="EXECUTE", inputs={}, dependency_refs=[], outputs={"project_id": "deadline-probe", "path": "out.json"}, quality_profile="qa", budget_profile="bounded", priority=50, timeout_seconds=5, retry_policy={"max_attempts": 1, "retryable_errors": []}, idempotency_key="deadline-after", production_intent=False, test_only=True)
    after_dag = build_dag(after_request, [_node("after")])
    after_snapshot = OrchestrationRuntime(after_request, after_dag, executor=FakeExecutor(durations={"after": 6.0})).run()
    node_exact_request = build_request(request_id="node-deadline-exact", project_id="deadline-probe", request_kind="ASSET_OPERATION", asset_family="maps_minimap", operation="EXECUTE", inputs={}, dependency_refs=[], outputs={"project_id": "deadline-probe", "path": "out.json"}, quality_profile="qa", budget_profile="bounded", priority=50, timeout_seconds=20, retry_policy={"max_attempts": 1, "retryable_errors": []}, idempotency_key="node-deadline-exact", production_intent=False, test_only=True)
    node_exact_dag = build_dag(node_exact_request, [_node("node-exact", timeout=3)])
    node_exact_snapshot = OrchestrationRuntime(node_exact_request, node_exact_dag, executor=FakeExecutor(durations={"node-exact": 3.0})).run()
    return {
        "before_boundary": before_snapshot["states"]["before"] == "SUCCEEDED" and before_snapshot["clock_elapsed_seconds"] < before_request["timeout_seconds"],
        "exact_boundary": exact_snapshot["states"]["exact"] == "SUCCEEDED" and exact_snapshot["clock_elapsed_seconds"] == exact_request["timeout_seconds"],
        "after_boundary": after_snapshot["states"]["after"] == "FAILED" and after_snapshot["node_deadlines"]["after"].get("error_class") == "REQUEST_DEADLINE_EXCEEDED",
        "node_exact_boundary": node_exact_snapshot["states"]["node-exact"] == "SUCCEEDED" and node_exact_snapshot["clock_elapsed_seconds"] == 3.0,
        "canonical_rule": "duration == allowed remaining budget succeeds; duration > allowed fails",
        "proof_source": "deterministic fake clock probe",
    }
...
def assert_provider_boundary(source: str | Path | None = None) -> dict[str, Any]:
    if source is None:
        ...
        source_name = "ugas.orchestration_runtime_v0247.OrchestrationRuntime"
...
def validate_no_repo_local_uads(repo_root: Path) -> None:
    ...
    if not (root / ".git").exists():
        return
    try:
        completed = subprocess.run(["git", "-C", str(root), "ls-files", "-z", "--", ".uads"], capture_output=True, check=True)
    except (OSError, subprocess.CalledProcessError):
        return
    tracked = [item.decode("utf-8", errors="replace") for item in completed.stdout.split(b"\0") if item]
    if tracked:
        _reject("ORCH_REPO_LOCAL_UADS_FOOTPRINT", f"UADS runtime material is tracked in the repository: {tracked[0]}")


UADS_HANDOFF_FIELDS = frozenset({
    "work_order_id", "run_or_dispatch_id", "route_status", "selected_profile_id",
    "selected_profile_digest", "selected_profile_digest_unavailable_reason",
    "dispatch_status", "execution_mode", "project_footprint",
})
UADS_SAFE_ID = re.compile(r"^[A-Za-z][A-Za-z0-9._-]{2,63}$")
UADS_SHA256 = re.compile(r"^[a-f0-9]{64}$")
UADS_SUCCESSFUL_DISPATCH = frozenset({"DISPATCHED"})
CANONICAL_CORRECTION_HISTORY_FIELDS = ("status", "rejected_reviewed_head", "findings", "historical_evidence_unchanged")


def _uads_field_unsafe(value: str) -> bool:
    return ntpath.isabs(value) or ("\\" in value and ":" in value) or value.startswith("~") or ".uads" in value.casefold()


def sanitize_uads_handoff(payload: Mapping[str, Any]) -> dict[str, Any]:
    _validate_no_secret_like(payload)
    _assert_no_unknown(payload, UADS_HANDOFF_FIELDS, "uads handoff")
    required = {"work_order_id", "run_or_dispatch_id", "route_status", "selected_profile_id", "dispatch_status", "execution_mode", "project_footprint"}
    missing = required - set(payload)
    if missing:
        _reject("ORCH_UADS_HANDOFF_INVALID", f"missing sanitized UADS handoff fields: {sorted(missing)}")
    value = {key: payload[key] for key in UADS_HANDOFF_FIELDS if key in payload}
    for key, child in list(value.items()):
        if not isinstance(child, str) or not child:
            _reject("ORCH_UADS_HANDOFF_INVALID", f"UADS handoff field {key} must be a non-empty string")
        if _uads_field_unsafe(child):
            _reject("ORCH_UADS_HANDOFF_PATH_REJECTED", f"UADS handoff field {key} contains unsafe runtime material")
    if value.get("execution_mode") != "GLOBAL_FIRST":
        _reject("ORCH_UADS_EXECUTION_MODE_INVALID", "UADS execution_mode must be GLOBAL_FIRST")
    if value.get("project_footprint") != "ZERO":
        _reject("ORCH_UADS_PROJECT_FOOTPRINT_INVALID", "UADS project_footprint must be ZERO")
    if value.get("route_status") != "SELECTED":
        _reject("ORCH_UADS_ROUTE_STATUS_REJECTED", "UADS route_status must be SELECTED")
    if value.get("dispatch_status") not in UADS_SUCCESSFUL_DISPATCH:
        _reject("ORCH_UADS_DISPATCH_STATUS_REJECTED", "UADS dispatch_status must be a successful dispatch state")
    for key in ("work_order_id", "run_or_dispatch_id"):
        child = value.get(key)
        if not isinstance(child, str) or not UADS_SAFE_ID.fullmatch(child):
            _reject("ORCH_UADS_IDENTIFIER_INVALID", f"UADS {key} is not a bounded safe identifier")
    profile = value.get("selected_profile_id")
    if not isinstance(profile, str) or not profile or not UADS_SAFE_ID.fullmatch(profile):
        _reject("ORCH_UADS_PROFILE_INVALID", "selected_profile_id must be present and safe")
    digest = value.get("selected_profile_digest")
    reason = value.get("selected_profile_digest_unavailable_reason")
    if digest is not None:
        if not isinstance(digest, str) or not UADS_SHA256.fullmatch(digest):
            _reject("ORCH_UADS_PROFILE_DIGEST_INVALID", "selected_profile_digest must be lowercase SHA-256")
    elif not isinstance(reason, str) or not reason.strip():
        _reject("ORCH_UADS_PROFILE_DIGEST_UNAVAILABLE", "selected_profile_digest is unavailable and no truthful reason was recorded")
    return value


def validate_canonical_correction_history(manifest_history: Mapping[str, Any], canonical: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(canonical, Mapping) or not canonical:
        _reject("ORCH_CORRECTION_HISTORY_MISSING", "canonical correction-history file is missing or malformed")
    if not isinstance(manifest_history, Mapping):
        _reject("ORCH_CORRECTION_HISTORY_MISMATCH", "manifest correction_history is missing or malformed")
    failures: list[str] = []
    for field_name in CANONICAL_CORRECTION_HISTORY_FIELDS:
        if manifest_history.get(field_name) != canonical.get(field_name):
            failures.append(field_name)
    if failures:
        _reject("ORCH_CORRECTION_HISTORY_MISMATCH", f"manifest correction_history does not match canonical file: {failures}")
    return {"status": "PASS", "correction_history": copy.deepcopy(dict(canonical))}
...
HARD_GATE_IDS = (
    ...
    "authority_git_commit_witness", "authority_frozen_registry_witness", "authority_binding_equal_across_modes",
    "uads_global_first_mode", "uads_zero_project_footprint", "uads_route_selected",
    "uads_dispatch_successful", "uads_selected_profile_present", "uads_repo_local_absent",
    "artifact_inventory_consistent", "artifact_single_self_attestation",
)
...
__all__ = [
    "ALLOWED_TRANSITIONS", "APPROVED_AUTHORITY_REGISTRY", "ATTEMPT_FORBIDDEN_STATES", "ATTEMPT_REQUIRED_STATES",
    "BoundedExecutionStore", "CIRCUIT_CHECKPOINT_FIELDS", "CircuitBreaker", "CircuitRegistry", "DEPENDENCY_REF_FIELDS",
    "DeterministicAdmissionScheduler", "DeterministicBatchScheduler", "DeterministicClock", "ExecutionCoordinator",
    "FakeClock", "FakeExecutor", "GOVERNED_APPROVED_FAMILIES", "HARD_GATE_IDS", "IdempotencyStore", "MAX_DEPTH", "MAX_FANOUT", "MAX_NODES",
    "NODE_DEADLINE_FIELDS", "NodeDeadlineExceeded", "NonRetryableExecutionError", "OrchestrationContractError", "OrchestrationRuntime",
    "RequestDeadlineExceeded", "RetryableExecutionError", "SUPPORTED_ASSET_FAMILIES", "TEST_ONLY_ROUTE_EXECUTOR_IDENTITY", "VERSION",
    "GIT_COMMIT_WITNESS", "FROZEN_REGISTRY_WITNESS", "git_object_witness_available", "resolve_authority_verification_mode",
    "adapt_legacy_generation_request", "admit_half_open_probe", "assert_provider_boundary", "atomic_write_checkpoint",
    "bind_governed_orchestration", "build_approved_dependency_ref", "build_dag", "build_dependency_ref", "build_governed_dependency_refs",
    "build_request",     "canonical_json", "compare_git_authority_tree", "compare_historical_evidence_tree", "deterministic_schedule", "diff_historical_entry_sets", "enumerate_git_tree_entries", "enumerate_git_tree_paths", "evaluate_hard_gates",
    "fingerprint_historical_entries", "node_authority_input_contract_hash", "node_contract_hash", "normalized_git_entry", "probe_deadline_boundary_truth", "prove_half_open_second_admission_blocked",
    "provider_spy_targets", "resolve_dependency_ref", "resolve_dependency_refs", "resume_from_checkpoint", "sanitize_telemetry", "sanitize_uads_handoff",
    "sha256_bytes", "sha256_value", "strict_boolean_observation", "strict_gate", "validate_authority_binding", "validate_canonical_correction_history", "validate_checkpoint",
    "validate_circuit_access", "validate_concurrency_limits", "validate_concurrency_observation", "validate_dependency_ref",
    "validate_dag", "validate_historical_immutability", "validate_idempotent_replay", "validate_no_dynamic_nodes",
    "validate_no_repo_local_uads", "validate_output_identity", "validate_production_boundary", "validate_provider_spy_counts",
