"""Fail-closed governance checks for the v0.12.1 observability correction."""
...
NEXT_ACTION = "external_review_observability_dashboard_v0121"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, review_text: str) -> dict[str, Any]:
    ...
    if state.get("schema_version") != CURRENT_VERSION or state.get("version") != CURRENT_VERSION: failures.append("version_must_be_0.12.1")
    ...
    if state.get("allowed_next_actions") != [NEXT_ACTION]: failures.append("next_action_must_wait_for_external_dashboard_review_v0121")
    previous = state.get("previous_release") if isinstance(state.get("previous_release"), Mapping) else {}
    if previous.get("version") != "0.12.0": failures.append("previous_release_must_be_0.12.0")
    ...
    checks = {"status": CURRENT_GATE, "production_routing": "BLOCKED", "production_approved": False, "dashboard_read_only": True, "local_only": True, "telemetry_upload": False, "new_generation": 0, "historical_v0120_preserved": True}
    ...
    for literal in ("0.12.1", "0.12.0", "0.11.2", "APPROVED_PILOT", "production_routing=BLOCKED", "production_approved=false", "local-only", "read-only", NEXT_ACTION):
