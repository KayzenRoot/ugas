"""Fail-closed active-state contract for the v0.19.0 items/props slice."""
...
CURRENT_GATE = "ITEMS_PROPS_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED"
NEXT_ACTION = "external_review_items_props_v0190"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str = "", review_text: str = "", roadmap_text: str = "") -> dict[str, Any]:
    ...
    expected = {
        ...
        "creatures_monsters": "APPROVED_FOUNDATION", "items_props_runtime": "TECHNICALLY_QUALIFIED_FOUNDATION",
        "real_item_prop_asset_coverage": "NONE", "synthetic_item_prop_fixture": "TEST_ONLY", "production_routing": "BLOCKED",
        "production_approved": False, "new_generation": 0, "always_on_dashboard_policy": "ENABLED", "runtime_mode": "DOCKER_ALWAYS_ON_LOCAL",
    }
    ...
    if previous.get("version") != "0.18.2" or previous.get("merge_commit") != BASELINE_HEAD: failures.append("previous_release_must_bind_v0182_merge")
    external = _mapping(state.get("external_visual_review"))
    if external.get("items_props_runtime") != "REQUIRED" or external.get("creatures_monsters_runtime") != "APPROVED_FOUNDATION": failures.append("external_review_boundary_invalid")
    ...
    if review.get("pr_number") not in {0, 9} or review.get("real_pr_checks_green") not in {True, False}: failures.append("review:pr_binding")
    ...
    for literal in ("0.19.0", CURRENT_PHASE, CURRENT_GATE, NEXT_ACTION, "creatures_monsters=APPROVED_FOUNDATION", "real_item_prop_asset_coverage=NONE", "synthetic_item_prop_fixture=TEST_ONLY", "production_approved=false", "production_routing=BLOCKED", "new_generation=0", "environment_tilesets"):
