...
from .providers import comfyui_healthcheck, detect_render_capability
...
def main(argv: list[str] | None = None) -> int:
    ...
    install.add_argument("--profile", default="generic-2d")
    ...
    route.add_argument("--comfyui", choices=["available", "unavailable"], default="available")
    route.add_argument("--render-node", choices=["available", "unavailable"], default="available")
    ...
    if args.command == "install":
        ...
        print(json.dumps(detect_render_capability(dry_run=args.dry_run), indent=2, ensure_ascii=False))
    return 0
