"""UGAS v0.4.0 machine-readable CLI."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="ugas", description="Universal Game Asset Studio 0.4.0")
    ...
    capability = sub.add_parser("capability"); capability.add_argument("--url", "--endpoint", default="http://127.0.0.1:8188"); capability.add_argument("--model", default="flux2-klein-4b-nvfp4"); capability.add_argument("--workflow", default="flux2-klein-4b-text-to-image"); capability.add_argument("--dry-run", action="store_true"); _json_flag(capability)
    ...
    image = generate_sub.add_parser("image"); image.add_argument("prompt"); image.add_argument("--model", default="flux2-klein-4b-nvfp4"); image.add_argument("--workflow", default="flux2-klein-4b-text-to-image"); image.add_argument("--seed", type=int, default=1); image.add_argument("--width", type=int, default=256); image.add_argument("--height", type=int, default=256); image.add_argument("--requires-transparency", action="store_true"); _common_generation(image)
    master = generate_sub.add_parser("master-sprite"); master.add_argument("prompt"); master.add_argument("--candidates", type=int, default=4); master.add_argument("--seed", type=int, default=1); master.add_argument("--width", type=int, default=384); master.add_argument("--height", type=int, default=384); master.add_argument("--transparent", action="store_true"); _common_generation(master)
    ...
    pilot = generate_sub.add_parser("sprite-pilot"); pilot.add_argument("prompt"); pilot.add_argument("--model", default="flux2-klein-4b-nvfp4"); pilot.add_argument("--workflow", default="flux2-klein-4b-text-to-image"); pilot.add_argument("--seed", type=int, default=1); pilot.add_argument("--width", type=int, default=256); pilot.add_argument("--height", type=int, default=256); pilot.add_argument("--columns", type=int, default=1); pilot.add_argument("--rows", type=int, default=1); pilot.add_argument("--requires-transparency", action="store_true"); _common_generation(pilot)
    ...
    try:
        ...
        if args.command == "generate":
            if args.generation_action == "master-sprite":
                _json(generate_master_sprite(root, endpoint=args.url, prompt=args.prompt, profile=args.profile, candidates=args.candidates, seed=args.seed, width=args.width, height=args.height, output_dir=args.output_dir, transparent=args.transparent)); return 0
