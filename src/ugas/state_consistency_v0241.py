"""Fail-closed active-state validator for UGAS orchestration v0.24.1."""
...
CURRENT_GATE = "ORCHESTRATION_RUNTIME_HARDENING_F31_F36_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED"
STOP_REASON = "ORCHESTRATION_RUNTIME_HARDENING_F31_F36_EXTERNAL_REVIEW_REQUIRED"
NEXT_ACTION = "external_review_orchestration_runtime_v0241"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, roadmap_text: str, matrix: Mapping[str, Any], binding: Mapping[str, Any] | None = None, evidence: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if v0240.get("status") != "CORRECTION_REQUIRED" or v0240.get("rejected_reviewed_head") != REJECTED_REVIEWED_HEAD or v0240.get("historical_evidence_unchanged") is not True:
    ...
    if binding_value:
        if binding_value.get("base_main_sha") != BASELINE_MAIN_SHA or binding_value.get("reviewed_head") != REJECTED_REVIEWED_HEAD or binding_value.get("status") != "CORRECTION_REQUIRED":
            failures.append("v0240_binding_invalid")
    evidence_value = evidence or {}
    if evidence_value and evidence_value.get("orchestration_root") != "docs/evidence/orchestration-runtime-v0241/":
        failures.append("evidence_root_invalid")
    active = "\n".join((checkpoint_text, roadmap_text))
    for literal in (VERSION, CURRENT_GATE, STOP_REASON, NEXT_ACTION, "baseline_main_sha", "production_routing=BLOCKED", "new_generation=0", "F31", "F32", "F33", "F34", "F35", "F36", "external review"):
