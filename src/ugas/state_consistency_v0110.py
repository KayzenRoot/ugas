"""Consistency checks for the active v0.11.0 motion-quality runtime slice."""
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, review_text: str) -> dict[str, Any]:
    ...
    if state.get("schema_version") != CURRENT_SCHEMA_VERSION: failures.append("state_schema_version_must_be_0.11.0")
    if state.get("version") != CURRENT_SCHEMA_VERSION: failures.append("state_version_must_be_0.11.0")
    if state.get("phase") != CURRENT_PHASE: failures.append("state_phase_invalid_for_v0110")
    ...
    if previous.get("version") != "0.10.0": failures.append("previous_release_must_be_0.10.0")
    if state.get("allowed_next_actions") != ["external_review_attack_front_v2"]: failures.append("allowed_next_actions_must_stop_at_attack_v2_external_review")
    ...
    if nested.get("new_generation_started") is not False or nested.get("new_generation_jobs") != 0 or nested.get("sam2_runs") != 0 or nested.get("comfyui_generation_jobs") != 0 or nested.get("diffusion_runs") != 0: failures.append("v0110_must_record_no_new_generation")
    ...
    required_texts = ("0.11.0", "0.10.0", CURRENT_PHASE, "attack-front-v2", "attack-front-v1", "motion_tracks", "APPROVED_PILOT", "SDXL_OPENPOSE_CONTROL_GAP_CONFIRMED_AT_MODEL_CARD_SETTINGS", "LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED", "REVIEW_INDEX_VERIFIED", CURRENT_GATE, "external_review_attack_front_v2", "production_routing=BLOCKED", "sam2_runs=0", "comfyui_generation_jobs=0", "diffusion_runs=0", "new_generation=0", "historical replay", "decision=QUALIFIED")
