...
CURRENT_GATES = {
    "CUTOUT_RIG_FRONT_WALK_FRAME_GAP", "CUTOUT_RIG_FRONT_WALK_TEMPORAL_GAP", "CUTOUT_RIG_FRONT_WALK_LOOP_GAP",
    "CUTOUT_RIG_FRONT_WALK_PACKAGING_GAP", "CUTOUT_RIG_FRONT_WALK_8FRAME_TECHNICALLY_QUALIFIED",
}
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, review_text: str) -> dict[str, Any]:
    ...
    if not isinstance(state.get("allowed_next_actions"), list) or state.get("allowed_next_actions") != ["external_review_front_walk_cycle"]: failures.append("allowed_next_actions_must_stop_at_external_review")
    ...
    if nested.get("status") != state.get("current_gate"):
        failures.extend(("nested_status_must_equal_current_gate", "nested_status_must_equal_current_gate_or_stop_reason"))
    if nested.get("contradictory_walk_or_anchor_promotion") is not False:
        failures.extend(("contradictory_walk_or_anchor_promotion_must_be_false", "active_documents_promote_blocked_walk_or_anchor_result"))
...
def assert_state_consistency(state: Mapping[str, Any], checkpoint_text: str, review_text: str) -> dict[str, Any]:
    ...
    if result["status"] != "STATE_CONSISTENCY_PASSED": raise StateConsistencyError("; ".join(result["failures"]))
    return result
