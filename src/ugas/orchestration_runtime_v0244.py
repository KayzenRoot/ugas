"""Fail-closed, provider-neutral orchestration runtime for UGAS v0.24.4.

Forward-only correction of the v0.24.4 TEST_ONLY control plane.  This module
...
TEST_ONLY_ROUTE_EXECUTOR_IDENTITY = "test-only-fake-executor-v0244"
...
def _validate_approved_authority_tuple(family: str, revision: str, path: str, status: str | None = None, approved_commit: str | None = None) -> dict[str, str]:
    ...
    if status is not None and status not in {registry["status"], "APPROVED", "APPROVED_FOUNDATION", "MERGED_CLOSED"}:
        _reject("ORCH_DEPENDENCY_REF_STATUS_REJECTED", "dependency authority is not approved")
    return registry
...
def build_dependency_ref(repo_root: Path, *, project_id: str, family: str, revision: str, path: str, authority_id: str | None = None, status: str = "APPROVED_FOUNDATION", approved_commit: str | None = None) -> dict[str, Any]:
    ...
    value: dict[str, Any] = {"project_id": project_id, "family": family, "revision": revision, "path": safe_path, "content_hash": sha256_bytes(raw), "semantic_hash": _semantic_hash(raw), "read_only": True, "status": status or registry["status"], "approved_commit": commit}
...
def resolve_dependency_ref(reference: Mapping[str, Any], repo_root: Path, *, expected_project_id: str | None = None, require_approved_commit: bool = False) -> dict[str, Any]:
    ...
    return {**value, "resolved_content_hash": observed_content_hash, "resolved_semantic_hash": observed_semantic_hash, "resolved_bytes": len(raw), "resolved_from": "repository", "verification_mode": mode}
...
def enumerate_git_tree_paths(repo_root: Path, ref: str, roots: Iterable[str]) -> list[str]:
    ...
    for root in roots:
        try:
            completed = subprocess.run(["git", "-C", str(repo_root), "ls-tree", "-r", "--name-only", ref, "--", root], capture_output=True, check=True, text=True)
        except (OSError, subprocess.CalledProcessError):
            continue
        for line in completed.stdout.splitlines():
            path = line.strip()
            if path:
                collected.add(validate_safe_relative_path(path))
    return sorted(collected)


def compare_historical_evidence_tree(repo_root: Path, authority_ref: str, roots: Iterable[str], *, label: str) -> dict[str, Any]:
    observed_ref = "HEAD"
    authority_paths = enumerate_git_tree_paths(repo_root, authority_ref, roots)
    observed_paths = enumerate_git_tree_paths(repo_root, observed_ref, roots)
    authority_set = set(authority_paths)
    observed_set = set(observed_paths)
    differences: list[dict[str, Any]] = []
    for path in sorted(authority_set - observed_set):
        differences.append({"path": path, "reason": "deleted"})
    for path in sorted(observed_set - authority_set):
        differences.append({"path": path, "reason": "extra"})
    for path in sorted(authority_set & observed_set):
        ...
        if authority_blob is None or observed_blob is None:
            differences.append({"path": path, "reason": "missing_blob", "authority_blob": authority_blob, "observed_blob": observed_blob})
        elif authority_blob != observed_blob:
            differences.append({"path": path, "reason": "blob_mismatch", "authority_blob": authority_blob, "observed_blob": observed_blob})
    ...
    fingerprint = sha256_value({"label": label, "authority_ref": authority_ref, "observed_ref": observed_ref, "authority_paths": authority_paths, "observed_paths": observed_paths, "differences": differences})
    return {
        "status": "PASS" if set_equal else "FAIL",
        "label": label,
        "authority_ref": authority_ref,
        "observed_ref": observed_ref,
        "authority_tree": authority_ref,
        "observed_tree": observed_ref,
        "authority_file_count": len(authority_paths),
        "observed_file_count": len(observed_paths),
        "exact_file_set": set_equal,
        "set_equal": set_equal,
...
def assert_provider_boundary(source: str | Path | None = None) -> dict[str, Any]:
    if source is None:
        ...
        source_name = "ugas.orchestration_runtime_v0244.OrchestrationRuntime"
...
__all__ = [
    ...
    "build_request", "canonical_json", "compare_git_authority_tree", "compare_historical_evidence_tree", "deterministic_schedule", "enumerate_git_tree_paths", "evaluate_hard_gates",
    "node_authority_input_contract_hash", "node_contract_hash", "probe_deadline_boundary_truth", "prove_half_open_second_admission_blocked",
    "provider_spy_targets", "resolve_dependency_ref", "resolve_dependency_refs", "resume_from_checkpoint", "sanitize_telemetry",
