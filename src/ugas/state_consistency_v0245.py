"""Fail-closed active-state validator for UGAS orchestration v0.24.5."""
...
CURRENT_GATE = "ORCHESTRATION_RUNTIME_HARDENING_F41_F42_F43_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED"
STOP_REASON = "ORCHESTRATION_RUNTIME_HARDENING_F41_F42_F43_EXTERNAL_REVIEW_REQUIRED"
NEXT_ACTION = "external_review_orchestration_runtime_v0245"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, roadmap_text: str, matrix: Mapping[str, Any], binding: Mapping[str, Any] | None = None, evidence: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if v0244.get("status") != "CORRECTION_REQUIRED" or v0244.get("rejected_reviewed_head") != REJECTED_REVIEWED_HEAD or v0244.get("historical_evidence_unchanged") is not True:
    ...
    if binding_value:
        if binding_value.get("base_main_sha") != BASELINE_MAIN_SHA or binding_value.get("reviewed_head") != REJECTED_REVIEWED_HEAD or binding_value.get("status") != "CORRECTION_REQUIRED":
            failures.append("v0244_binding_invalid")
    evidence_value = evidence or {}
    if evidence_value and evidence_value.get("orchestration_root") != "docs/evidence/orchestration-runtime-v0245/":
        failures.append("evidence_root_invalid")
    active = "\n".join((checkpoint_text, roadmap_text))
    for literal in (VERSION, CURRENT_GATE, STOP_REASON, NEXT_ACTION, "baseline_main_sha", "production_routing=BLOCKED", "new_generation=0", "F-41", "F-42", "F-43", "F-38", "F-37R", "F-39", "F-40", "F-31R", "F-32RR", "F-33", "F-34", "F-35RR", "F-35RC", "F-36", "external review"):
...
__all__ = ["BASELINE_MAIN_SHA", "BRANCH", "CURRENT_GATE", "CURRENT_PHASE", "NEXT_ACTION", "NEXT_CANDIDATE", "REJECTED_REVIEWED_HEAD", "STOP_REASON", "V0241_REJECTED_HEAD", "V0242_REJECTED_HEAD", "V0243_REJECTED_HEAD", "V0244_SEMANTIC_HEAD", "VERSION", "validate_state_consistency"]
