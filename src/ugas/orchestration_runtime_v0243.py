"""Fail-closed, provider-neutral orchestration runtime for UGAS v0.24.3.

Forward-only correction of the v0.24.3 TEST_ONLY control plane.  This module
...
TEST_ONLY_ROUTE_EXECUTOR_IDENTITY = "test-only-fake-executor-v0243"
...
def _validate_approved_authority_tuple(family: str, revision: str, path: str, status: str | None = None) -> dict[str, str]:
...
def resolve_dependency_ref(reference: Mapping[str, Any], repo_root: Path, *, expected_project_id: str | None = None, require_approved_commit: bool = False) -> dict[str, Any]:
    ...
    registry = _validate_approved_authority_tuple(value["family"], value["revision"], value["path"], value.get("status"))
    if require_approved_commit or value["family"] in GOVERNED_APPROVED_FAMILIES:
        ...
        if value["family"] in GOVERNED_APPROVED_FAMILIES and value["approved_commit"] != registry["approved_commit"]:
...
def assert_provider_boundary(source: str | Path | None = None) -> dict[str, Any]:
    if source is None:
        ...
        source_name = "ugas.orchestration_runtime_v0243.OrchestrationRuntime"
