"""Fail-closed VFX runtime correction for F-26S, F-29 and F-30.

This module is deliberately forward-only.  The v0.23.2 runtime and evidence
remain historical; this layer reuses its fixture authority and replaces only
the fallback proof, output validator and determinism contract.
"""
...
REJECTED_REVIEWED_HEAD = "d7501177a9f4480921226b9832ffc237544a6433"
...
def _authority_identity() -> dict[str, str]:
    payload = {
        ...
        "path": "src/ugas/vfx_asset_family_runtime_v0233.py",
        "revision": VERSION,
    }
...
def build_effect_fixture(effect_class: str, output_root: Path, index: int = 0) -> dict[str, Any]:
    ...
    semantic["effect_id"] = semantic["effect_id"].replace("v0232", "v0233")
    semantic["semantic_revision"] = VERSION
    semantic["semantics"]["fixture_seed"] = 23300 + index
    ...
    record["provenance"] = {
        **record["provenance"],
        "generator": "deterministic-test-fixture-renderer-v0233",
        "seed": 23300 + index,
        "input_hash": "",
...
def validate_provenance(record: Mapping[str, Any], root: Path) -> None:
    ...
    _require(
        isinstance(value, Mapping)
        and value.get("generator") == "deterministic-test-fixture-renderer-v0233"
