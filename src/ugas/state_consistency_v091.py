"""Consistency checks for the active v0.9.1 generic-runtime correction."""
...
CURRENT_GATES = {"ANIMATION_RUNTIME_V1_GENERIC_STATUS_CONTRACT_GAP", "ANIMATION_RUNTIME_V1_TIMING_SCHEMA_GAP", "IDLE_FRONT_DUAL_FOOT_DRIFT_GAP", "IDLE_FRONT_LAYER_BBOX_QA_GAP", "IDLE_FRONT_OCCLUSION_QA_GAP", "IDLE_FRONT_CANONICAL_REPLAY_DRIFT", CURRENT_GATE}
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, review_text: str) -> dict[str, Any]:
    ...
    if state.get("schema_version") != CURRENT_SCHEMA_VERSION: failures.append("state_schema_version_must_be_0.9.1")
    if state.get("version") != CURRENT_SCHEMA_VERSION: failures.append("state_version_must_be_0.9.1")
    if state.get("phase") != CURRENT_PHASE: failures.append("state_phase_invalid_for_v091")
    ...
    if previous.get("version") != "0.9.0": failures.append("previous_release_must_be_0.9.0")
    ...
    if nested.get("new_generation_started") is not False or nested.get("new_generation_jobs") != 0 or nested.get("sam2_runs") != 0 or nested.get("comfyui_generation_jobs") != 0: failures.append("v091_must_record_no_new_generation")
    ...
    required_texts = ("0.9.1", "0.9.0", CURRENT_PHASE, "0.8.1", "0.7.3", "SDXL_OPENPOSE_CONTROL_GAP_CONFIRMED_AT_MODEL_CARD_SETTINGS", "LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED", "REVIEW_ARCHIVE_VERIFIED", CURRENT_GATE, "external_review_idle_front", "production_routing=BLOCKED", "sam2_runs=0", "comfyui_generation_jobs=0", "not-claimed", "decision=QUALIFIED", "dual-foot", "head_bbox", "torso_bbox")
