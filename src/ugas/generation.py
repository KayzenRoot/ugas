"""Real v0.4.1 2D generation, lane qualification and revision orchestration."""
...
def generate_master_sprite(repo_root: Path, *, endpoint: str, prompt: str, profile: str = "generic-2d", candidates: int = 4,
    ...
    for round_index in range(max_rounds + 1):
        ...
        if round_index: retry_log.append({"round": round_index, "reason": "previous round produced no eligible candidate", "prompt_sha256": prompt_sha256(round_prompt)})
        for index in range(spec.candidate_count):
            ...
            try:
                ...
                output = Path(result["output"]); elapsed_ms = round((time.perf_counter() - started) * 1000); qa = validate_output(output, width=width, height=height); output_hash = qa.get("technical", {}).get("sha256"); metrics = candidate_metrics(output, width=width, height=height, requires_transparency=False, occupancy_target=spec.subject_occupancy_target, duplicate=output_hash in seen_hashes)
                duplicate = output_hash in seen_hashes or metrics.get("perceptual_hash") in seen_phashes; metrics = candidate_metrics(output, width=width, height=height, requires_transparency=False, occupancy_target=spec.subject_occupancy_target, duplicate=duplicate); seen_hashes.add(output_hash); seen_phashes.add(metrics.get("perceptual_hash"))
    ...
    revision = _revision(spec.asset_id, 1, best_output, best["qa"], extra={"candidate_id": best["candidate_id"], "candidate_metrics": best["metrics"], "model_id": selected_model, "workflow_id": selected_workflow}); asset["revisions"].append(revision); asset["current_revision"] = revision; asset_path = _persist_asset(asset, asset_dir)
    result: dict[str, Any] = {"status": "VISUAL_REVIEW_REQUIRED", "asset_id": spec.asset_id, "asset_path": str(asset_path), "spec_path": str(spec_path), "candidate_set": str(candidate_path), "contact_sheet": str(asset_dir / "candidates-contact-sheet.png"), "master_before_background_removal": str(best_output), "compiled_prompt": compiled, "candidates": entries, "lane": lane, "model_id": selected_model, "workflow_id": selected_workflow, "fallback_reason": fallback_reason, "visual_approval": "pending"}
    ...
    for lane_name, model_id, workflow_id in (("FAST", FAST_MODEL, FAST_TEXT), ("QUALITY", QUALITY_MODEL, QUALITY_TEXT)):
        for offset in range(3):
            ...
            try:
                ...
                output = Path(result["output"]); qa = validate_output(output, width=width, height=height); metrics = candidate_metrics(output, width=width, height=height, occupancy_target=spec.subject_occupancy_target); entries.append({"lane": lane_name, "model_id": model_id, "workflow_id": workflow_id, "seed": candidate_seed, "path": str(output), "sha256": qa.get("technical", {}).get("sha256"), "steps": 4 if lane_name == "FAST" else 50, "guidance": 1.0 if lane_name == "FAST" else 4.0, "runtime_ms": round((time.perf_counter() - started) * 1000), "dimensions": {"width": width, "height": height}, "hard_gates": metrics.get("hard_gates", {}), "eligible": metrics.get("eligible", False), "qa": qa})
            except GenerationError as exc:
...
def background_remove(repo_root: Path, image_or_asset_id: str, *, endpoint: str, output_dir: Path | None = None) -> dict[str, Any]:
    asset_path: Path | None = None; asset: dict | None = None
    try: asset_path, asset = load_asset(repo_root, image_or_asset_id); source = Path(asset["current_revision"]["output_path"])
    except MasterAssetError:
        source = Path(image_or_asset_id)
        if not source.is_file(): raise GenerationError(f"image or asset not found: {image_or_asset_id}")
    if not source.is_file(): raise GenerationError(f"source image not found: {source}")
    client = ComfyUIClient(endpoint, timeout=30.0); evidence = probe_comfy_capability(repo_root, client, "birefnet", "birefnet-background-removal", capability="background-removal")
    if evidence["state"] not in {"ready", "verified"}: raise GenerationError(f"BiRefNet capability gap: {evidence.get('failure_reason')}")
    ...
    workflow_record = load_workflow(repo_root, "birefnet-background-removal"); model = load_model(repo_root, "birefnet"); workflow = bind_workflow(workflow_record["api"], prompt="", model_names=_model_names(model), image_filename=filename); graph_check = validate_api_workflow(workflow, node_info=client.node_info())
    if not graph_check["live_valid"]: raise GenerationError(f"BiRefNet native node validation failed: {graph_check['missing_nodes']}")
    source_info = inspect_png(source); destination_dir = _output_dir(repo_root, output_dir, "background-removal"); started = time.perf_counter(); job, outputs = _run_job(repo_root, client, workflow, output_dir=destination_dir, filename="birefnet-output.png", profile=(asset or {}).get("profile", "generic-2d"), capability="background-removal", workflow_id="birefnet-background-removal", model_id="birefnet", prompt="native BiRefNet background removal", seed=0, width=source_info["width"], height=source_info["height"], input_hashes={"reference_sha256": sha256(source)})
    elapsed_ms = round((time.perf_counter() - started) * 1000); output_info = [(Path(item["path"]), inspect_png(Path(item["path"]))) for item in outputs]; transparent_source = next((path for path, info in output_info if info["has_transparent_pixels"]), None)
    if transparent_source is None: raise GenerationError("BiRefNet native output did not contain transparent pixels")
    mask_source = next((path for path, _ in output_info if path != transparent_source), None); transparent_output = destination_dir / "master-transparent.png"
    if transparent_source != transparent_output: transparent_output.write_bytes(transparent_source.read_bytes())
    mask_output = destination_dir / "master-mask.png" if mask_source else None
    if mask_source and mask_output and mask_source != mask_output: mask_output.write_bytes(mask_source.read_bytes())
    qa = validate_output(transparent_output, width=source_info["width"], height=source_info["height"], requires_transparency=True); halo = detect_halo(transparent_output)
    if qa["status"] != "TECHNICAL_VALID": raise GenerationError("BiRefNet output failed real transparency QA")
    structural = None
    if asset is not None and asset_path is not None:
        asset["requires_transparency"] = True; current = asset["current_revision"]; previous_revisions = asset.get("revisions", [])
        revision = _revision(asset["asset_id"], len(previous_revisions) + 1, transparent_output, qa, derived_from={"revision_id": current["revision_id"], "output_sha256": current["output_sha256"]}, transparency_status="TRANSPARENCY_VALID", extra={"background_removal": {"workflow_id": "birefnet-background-removal", "model_id": "birefnet", "source_sha256": sha256(source), "mask_path": str(mask_output) if mask_output else None, "halo": halo}})
        if current.get("workflow_id") == QUALITY_EDIT or current.get("workflow_id") == FAST_EDIT:
            source_revision_id = (current.get("derived_from") or {}).get("revision_id"); source_revision = next((item for item in previous_revisions if item.get("revision_id") == source_revision_id), None)
            if source_revision:
                structural = reference_edit_structural_qa(Path(source_revision["output_path"]), transparent_output, thresholds=(json.loads(Path(asset["master_asset_spec"]).read_text(encoding="utf-8")).get("generation_policy", {}).get("reference_edit_qa", {})))
                revision["reference_edit_qa"] = structural; write_json(repo_root / "docs" / "evidence" / "reference-edit-qa.json", structural)
        asset.setdefault("revisions", []).append(revision); asset["current_revision"] = revision; asset["updated_at"] = _now(); save_asset(asset_path, asset)
    evidence = {**evidence, "state": "verified", "asset_capabilities_qualified": ["background-removal", "transparent-sprite-master"], "smoke_test": {"status": "passed", "source_sha256": sha256(source), "output": inspect_png(transparent_output), "mask": inspect_png(mask_output) if mask_output else None, "alpha_metrics": inspect_png(transparent_output).get("alpha_metrics"), "halo": halo, "runtime_ms": elapsed_ms}}
    write_json(repo_root / "docs" / "evidence" / "birefnet.json", evidence); checker = destination_dir / "master-checkerboard.png"; checkerboard_preview(transparent_output, checker); append_event(repo_root / "tmp" / "provenance.jsonl", {"event": "background-removed", "workflow": "birefnet-background-removal", "model": "birefnet", "source_sha256": sha256(source), "output_sha256": qa["technical"]["sha256"], "mask_path": str(mask_output) if mask_output else None})
    return {"status": "TRANSPARENCY_VALID", "output": str(transparent_output), "mask": str(mask_output) if mask_output else None, "checkerboard": str(checker), "qa": qa, "halo": halo, "reference_edit_qa": structural, "capability_evidence": evidence}
...
def refine_master(repo_root: Path, asset_id: str, *, instruction: str, endpoint: str) -> dict[str, Any]:
    ...
    revision_dir = asset_path.parent / "revisions"; started = time.perf_counter(); job, outputs = _run_job(repo_root, client, workflow, output_dir=revision_dir, filename=f"reference-edit-{len(asset.get('revisions', [])) + 1}.png", profile=asset.get("profile", "generic-2d"), capability="reference-edit", workflow_id=workflow_id, model_id=model_id, prompt=prompt, seed=seed, width=dimensions["width"], height=dimensions["height"], input_hashes={"reference_sha256": sha256(source)})
    elapsed_ms = round((time.perf_counter() - started) * 1000); output = Path(outputs[0]["path"]); qa = validate_output(output, width=dimensions["width"], height=dimensions["height"])
    if qa["status"] != "TECHNICAL_VALID": raise GenerationError("reference-edit output failed technical QA")
    reference_evidence = {**evidence, "state": "verified", "asset_capabilities_qualified": ["reference-edit"], "smoke_test": {"status": "passed", "source_sha256": sha256(source), "output": inspect_png(output), "instruction": compiled_instruction, "instruction_sha256": prompt_sha256(compiled_instruction), "runtime_ms": elapsed_ms}}; write_json(repo_root / "docs" / "evidence" / "reference-edit.json", reference_evidence)
...
def sprite_pilot(repo_root: Path, **kwargs) -> dict:
    ...
    if columns != 1 or rows != 1: raise GenerationError("sprite-grid workflow not qualified in v0.4.1")
