"""Fail-closed active-state checks for the v0.17.1 equipment QA correction."""
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str = "", review_text: str = "", roadmap_text: str = "") -> dict[str, Any]:
    ...
    expected = {
        ...
        "current_gate": CURRENT_GATE, "multi_direction_animation_runtime": "APPROVED_FOUNDATION",
        "real_directional_character_asset_coverage": "SOUTH_ONLY", "real_equipment_asset_coverage": "NONE_OR_EXPLICITLY_APPROVED_ONLY",
        "synthetic_equipment_fixture": "TEST_ONLY", "production_routing": "BLOCKED",
    ...
    if not isinstance(forbidden, list) or not {"direct_main_push", "enable_production_routing", "new_generation", "creatures_monsters"}.issubset(forbidden): failures.append("forbidden_boundaries_incomplete")
    previous = _mapping(state.get("previous_release"))
    if previous.get("version") != "0.16.2" or previous.get("review") != "REVIEW-v0.16.2.md": failures.append("previous_release_must_bind_v0162")
