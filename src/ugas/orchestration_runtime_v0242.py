"""Fail-closed, provider-neutral orchestration runtime for UGAS v0.24.2.
...
TEST_ONLY_ROUTE_EXECUTOR_IDENTITY = "test-only-fake-executor-v0242"
...
class OrchestrationRuntime:
    ...
    def circuit_checkpoint(self) -> dict[str, Any]:
        return {"state": self._breaker.state, "failures": self._breaker.failures, "probe_in_flight": self._breaker._probe_in_flight, "route_executor_identity": self._route_executor_identity, "project_id": self.request["project_id"]}
...
def _validate_checkpoint_attempts(attempts: Any, states: Mapping[str, str], nodes_by_id: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    for node_id, count in attempts.items():
        ...
        if state in {"PLANNED", "VALIDATED", "READY"}:
            _reject("ORCH_CHECKPOINT_ATTEMPTS_INVALID", f"never-dispatched node {node_id} may not carry attempt count")


def _validate_checkpoint_node_deadlines(node_deadlines: Any, nodes_by_id: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    for node_id, record in node_deadlines.items():
        ...
        _finite_non_negative_number(record.get("started_seconds"), f"node_deadlines.{node_id}.started_seconds")
        _finite_non_negative_number(record.get("deadline_seconds"), f"node_deadlines.{node_id}.deadline_seconds")
        if "parent_deadline_seconds" in record:
            _finite_non_negative_number(record["parent_deadline_seconds"], f"node_deadlines.{node_id}.parent_deadline_seconds")
...
def _validate_checkpoint_circuit(circuit: Any, *, project_id: str, route_executor_identity: str) -> dict[str, Any]:
    ...
    if state == "CLOSED" and failures != 0 and probe_in_flight is True:
        _reject("ORCH_CHECKPOINT_CIRCUIT_INVALID", "CLOSED circuit probe/failure invariants are invalid")
    return {"state": state, "failures": failures, "probe_in_flight": probe_in_flight, "route_executor_identity": route_executor_identity, "project_id": project_id}


def _breaker_from_checkpoint(circuit: Mapping[str, Any], *, failure_threshold: int = 2) -> CircuitBreaker:
...
def validate_checkpoint(checkpoint: Mapping[str, Any], request: Mapping[str, Any], dag: Mapping[str, Any], completed_results: Mapping[str, Mapping[str, Any]] | None = None, *, execution_policy: Mapping[str, Any] | None = None, max_concurrency: int | None = None, family_limits: Mapping[str, int] | None = None, authority_binding_hash: str | None = None) -> dict[str, Any]:
    ...
    _validate_checkpoint_node_deadlines(value.get("node_deadlines", {}), nodes_by_id)
...
def resume_from_checkpoint(request: Mapping[str, Any], dag: Mapping[str, Any], checkpoint: Mapping[str, Any], **kwargs: Any) -> OrchestrationRuntime:
    ...
    if circuit_registry is not None:
        ...
        if existing is not None and (existing.state != restored_breaker.state or existing.failures != restored_breaker.failures or existing._probe_in_flight != restored_breaker._probe_in_flight):
...
def resolve_dependency_ref(reference: Mapping[str, Any], repo_root: Path, *, expected_project_id: str | None = None, require_approved_commit: bool = False) -> dict[str, Any]:
    ...
    _validate_approved_authority_tuple(value["family"], value["revision"], value["path"], value.get("status"))
...
def assert_provider_boundary(source: str | Path | None = None) -> dict[str, Any]:
    if source is None:
        ...
        source_name = "ugas.orchestration_runtime_v0242.OrchestrationRuntime"
...
__all__ = [
    "ALLOWED_TRANSITIONS", "APPROVED_AUTHORITY_REGISTRY", "BoundedExecutionStore", "CircuitBreaker", "CircuitRegistry", "DEPENDENCY_REF_FIELDS",
    ...
    "adapt_legacy_generation_request", "assert_provider_boundary", "atomic_write_checkpoint", "build_approved_dependency_ref", "build_dag",
    "build_dependency_ref", "build_governed_dependency_refs", "build_request", "canonical_json", "deterministic_schedule", "evaluate_hard_gates",
    "node_authority_input_contract_hash", "node_contract_hash", "probe_deadline_boundary_truth", "provider_spy_targets", "resolve_dependency_ref", "resolve_dependency_refs",
