"""Fail-closed active-state validator for UGAS orchestration v0.24.2."""
...
CURRENT_GATE = "ORCHESTRATION_RUNTIME_HARDENING_F31R_F32R_F34R_F35R_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED"
STOP_REASON = "ORCHESTRATION_RUNTIME_HARDENING_F31R_F32R_F34R_F35R_EXTERNAL_REVIEW_REQUIRED"
NEXT_ACTION = "external_review_orchestration_runtime_v0242"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, roadmap_text: str, matrix: Mapping[str, Any], binding: Mapping[str, Any] | None = None, evidence: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if v0241.get("status") != "CORRECTION_REQUIRED" or v0241.get("rejected_reviewed_head") != REJECTED_REVIEWED_HEAD or v0241.get("historical_evidence_unchanged") is not True:
    ...
    if binding_value:
        if binding_value.get("base_main_sha") != BASELINE_MAIN_SHA or binding_value.get("reviewed_head") != REJECTED_REVIEWED_HEAD or binding_value.get("status") != "CORRECTION_REQUIRED":
            failures.append("v0241_binding_invalid")
    evidence_value = evidence or {}
    if evidence_value and evidence_value.get("orchestration_root") != "docs/evidence/orchestration-runtime-v0242/":
        failures.append("evidence_root_invalid")
    active = "\n".join((checkpoint_text, roadmap_text))
    for literal in (VERSION, CURRENT_GATE, STOP_REASON, NEXT_ACTION, "baseline_main_sha", "production_routing=BLOCKED", "new_generation=0", "F-31R", "F-32R", "F-34R", "F-35R", "F-33", "F-36", "external review"):
        if literal.casefold() not in active.casefold():
            failures.append(f"documents_missing:{literal}")
    return {"status": CURRENT_GATE if not failures else "ORCHESTRATION_STATE_FAILED", "version": VERSION, "failures": failures, "checked": {"baseline_main_sha": state.get("baseline_main_sha"), "phase": state.get("phase"), "current_gate": state.get("current_gate"), "allowed_next_actions": state.get("allowed_next_actions"), "production_routing": state.get("production_routing"), "new_generation": state.get("new_generation"), "rejected_reviewed_head": v0240.get("rejected_reviewed_head")}}


__all__ = ["BASELINE_MAIN_SHA", "BRANCH", "CURRENT_GATE", "CURRENT_PHASE", "NEXT_ACTION", "NEXT_CANDIDATE", "REJECTED_REVIEWED_HEAD", "STOP_REASON", "VERSION", "validate_state_consistency"]
