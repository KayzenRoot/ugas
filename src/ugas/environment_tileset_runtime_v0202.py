"""Fail-closed environment/tileset runtime and QA integrity for UGAS v0.20.2.
...
def validate_metrics(metrics: Mapping[str, Any]) -> None:
    ...
    _require(metrics.get("origin") in {"TOP_LEFT", "CENTER"}, "ORIGIN_CONVENTION_UNSUPPORTED", "unsupported origin")
...
def tile_to_world(tile_x: int, tile_y: int, metrics: Mapping[str, Any]) -> tuple[float, float]:
    ...
    offset = 0.5 if metrics["origin"] == "CENTER" else 0.0
    return ((float(tile_x) + offset) * unit, (float(tile_y) + offset) * unit * direction)
...
def world_to_tile(world_x: float, world_y: float, metrics: Mapping[str, Any]) -> tuple[int, int]:
    ...
    direction = 1.0 if metrics["grid_orientation"] == "Y_DOWN" else -1.0
    offset = 0.5 if metrics["origin"] == "CENTER" else 0.0
    return (floor(float(world_x) / unit - offset), floor((float(world_y) * direction) / unit - offset))
...
def validate_effective_tile_variant(effective_tile: Mapping[str, Any], variant: Mapping[str, Any], tileset_context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    effective_binding = variant.get("effective_binding")
    override_binding = variant.get("overrides", {}).get("atlas_binding")
    resolved_binding = resolved.get("binding")
    _require(isinstance(effective_binding, Mapping) and isinstance(override_binding, Mapping), "VARIANT_BINDING_AUTHORITY_MISMATCH", str(variant.get("variant_id")))
    _require(effective_binding == override_binding, "VARIANT_BINDING_AUTHORITY_MISMATCH", str(variant.get("variant_id")))
    _require(effective_binding == resolved_binding, "VARIANT_VISUAL_IDENTITY_MISMATCH", str(variant.get("variant_id")))
    _require(variant.get("content_hash") == effective_binding.get("decoded_pixel_hash"), "VARIANT_CONTENT_HASH_MISMATCH", str(variant.get("variant_id")))
    _require(variant.get("atlas_revision") == effective_binding.get("atlas_revision"), "VARIANT_ATLAS_REVISION_MISMATCH", str(variant.get("variant_id")))
    ...
    if variant.get("visual_bytes_changed"):
        _require(binding == resolved_binding, "VARIANT_VISUAL_IDENTITY_MISMATCH", str(variant.get("variant_id")))
        _require(effective_binding == resolved_binding, "VARIANT_VISUAL_IDENTITY_MISMATCH", str(variant.get("variant_id")))
...
def validate_tileset_manifest(root: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for family_id, family in family_records.items():
        ...
        _require(isinstance(supported_masks, list) and all(isinstance(mask, int) and not isinstance(mask, bool) for mask in supported_masks), "INVALID_SUPPORTED_MASK", family_id)
        _require(len(supported_masks) == len(set(supported_masks)), "INVALID_SUPPORTED_MASK", family_id)
        valid_masks = set(CARDINAL_SUPPORTED_MASKS if policy == CARDINAL_ONLY else EIGHT_SUPPORTED_MASKS)
        _require(set(supported_masks).issubset(valid_masks), "INVALID_SUPPORTED_MASK", family_id)
        expected_pairs = {(class_id, mask) for class_id in TILE_CLASSES for mask in supported_masks}
        seen: set[tuple[str, int]] = set()
        for item in family.get("autotile_variants", []):
            mask = item.get("adjacency_mask")
            pair = (item.get("target_class_id"), mask)
            _require(mask in set(supported_masks), "AUTOTILE_MASK_NOT_DECLARED", str(item))
            _require(pair not in seen, "DUPLICATE_AUTOTILE_MASK", str(item))
            ...
            _require(item.get("target_class_id") in TILE_CLASSES, "TILE_CLASS_INVALID", str(item))
            _require(item.get("target_layer_role") == CLASS_TO_LAYER[item["target_class_id"]], "TILE_CLASS_LAYER_CONTRADICTION", str(item))
            resolved = tiles_by_id[variant["resolved_tile_id"]]
            _require(item.get("autotile_tile_id") == resolved["tile_id"], "AUTOTILE_TILE_BINDING_MISMATCH", str(item))
            _require(item.get("file_sha256") == resolved["binding"]["file_sha256"] and item.get("decoded_pixel_hash") == resolved["binding"]["decoded_pixel_hash"] and item.get("atlas_revision") == resolved["binding"]["atlas_revision"], "AUTOTILE_BYTE_BINDING_MISMATCH", str(item))
            seen.add(pair)
        _require(seen == expected_pairs, "REQUIRED_AUTOTILE_MASK_MISSING", family_id)
...
def _validate_production_candidate(manifest: Mapping[str, Any], root: Path | None) -> None:
    _require(root is not None, "MANIFEST_VALIDATION_REQUIRED", "production registration requires a filesystem-backed manifest root")
    _require(manifest.get("schema_version") == SCHEMA_VERSION, "SCHEMA_VERSION_INVALID", str(manifest.get("schema_version")))
    ...
    _require(manifest.get("tileset_id") and manifest.get("tileset_revision"), "SCHEMA_REQUIRED_FIELD_MISSING", "tileset identity")
    _require(manifest.get("test_only") is False and manifest.get("production_safe") is True, "TEST_FIXTURE_IN_PRODUCTION_REGISTRY", "production candidate cannot be TEST_ONLY")
    ...
    _require(isinstance(manifest.get("tiles"), list) and isinstance(manifest.get("terrain_families"), list), "MANIFEST_VALIDATION_REQUIRED", "semantic production manifest fields are required")
    _require(Path(root).is_dir(), "MANIFEST_VALIDATION_REQUIRED", str(root))
...
class EnvironmentTileRegistry:
    ...
    def register(self, manifest: Mapping[str, Any]) -> None:
        if self.production:
            if manifest.get("production_routing") == "ENABLED" and self.production_policy.production_routing != "ENABLED":
                _require(False, "PRODUCTION_ROUTING_BLOCKED", "candidate routing is ENABLED while system policy is BLOCKED")
            if manifest.get("test_only") is True and manifest.get("production_approved") is True:
                _require(False, "PRODUCTION_ROUTING_BLOCKED", "system production routing is BLOCKED")
            if manifest.get("test_only") is True:
                _require(False, "TEST_FIXTURE_IN_PRODUCTION_REGISTRY", "TEST_ONLY environment fixture rejected")
            _require(self.production_policy.production_routing == "ENABLED", "PRODUCTION_ROUTING_BLOCKED", "system production routing is BLOCKED")
            _validate_production_candidate(manifest, self.root)
...
def generate_fixture_pack(output_dir: Path) -> dict[str, Any]:
    ...
    for family_index, (family_id, policy, masks) in enumerate(family_specs):
        ...
        atlas_revision = f"atlas-{family_id}-v0202-r1"
        ...
        for tile in tile_records:
            ...
            item = {"terrain_family_id": family_id, "variant_id": variant_id, "adjacency_mask": mask, "target_class_id": tile["class_id"], "target_layer_role": tile["primary_layer"], "autotile_tile_id": tile["tile_id"], "file_sha256": tile["binding"]["file_sha256"], "decoded_pixel_hash": tile["binding"]["decoded_pixel_hash"], "atlas_rect": tile["binding"]["atlas_rect"], "atlas_revision": tile["binding"]["atlas_revision"], "identity": f"{family_id}:{tile['class_id']}:{policy}:{mask:03d}", "visual_transition_role": _transition_role(mask, policy), "shared_visual_identity": False}
    ...
    manifest: dict[str, Any] = {"schema_version": SCHEMA_VERSION, "manifest_type": "environment-tileset-runtime-qa-integrity", "tileset_id": "ugas-test-environment-v0202", "tileset_revision": "tileset-v0202-r1", "classes": list(TILE_CLASSES), "metrics": metrics, "layer_order": list(LAYER_ORDER), "layers": [{"layer_role": role, "order": index, "acyclic": True} for index, role in enumerate(LAYER_ORDER)], "terrain_families": families, "tiles": all_tiles, "variants": all_variants, "seam_pairs": seam_pairs, "production_approved": False, "production_routing": "BLOCKED", "production_registry_empty": True, "test_only": True, "production_safe": False, "prop_sockets": [{"socket_id": "world-prop-socket", "socket_type": "typed_prop_socket", "prop_class": "environmental_prop", "prop_asset_path": None}], "prop_asset_paths": [], "provenance": {}, "fixture_label": "TEST_ONLY_TILE_QA_BOARD_V0202"}
    manifest["provenance"]["manifest_hash"] = _record_hash(manifest, drop=("provenance",))
    write_json(output_dir / "tileset-manifest-v0202.json", manifest)
    write_json(output_dir / "tileset-identity-v0202.json", {"schema_version": SCHEMA_VERSION, "tileset_id": manifest["tileset_id"], "tileset_revision": manifest["tileset_revision"], "manifest_hash": manifest["provenance"]["manifest_hash"], "tile_ids": [tile["tile_id"] for tile in all_tiles], "family_policies": {family["terrain_family_id"]: family["adjacency_policy"] for family in families}})
    write_json(output_dir / "edge-signatures-v0202.json", {"schema_version": SCHEMA_VERSION, "records": [{"tile_id": tile["tile_id"], "edges": tile["edge_signatures"]} for tile in all_tiles]})
    write_json(output_dir / "layer-collision-navigation-v0202.json", {"schema_version": SCHEMA_VERSION, "layers": manifest["layers"], "tiles": [{"tile_id": tile["tile_id"], "class_id": tile["class_id"], "primary_layer": tile["primary_layer"], "collision_navigation": tile["collision_navigation"]} for tile in all_tiles]})
    write_json(output_dir / "autotile-class-layer-contract-v0202.json", {"schema_version": SCHEMA_VERSION, "classes": [{"class_id": class_id, "primary_layer": CLASS_TO_LAYER[class_id]} for class_id in TILE_CLASSES], "variant_targets": [{"variant_id": variant["variant_id"], "target_class_id": variant["target_class_id"], "target_layer_role": variant["target_layer_role"]} for variant in all_variants]})
    write_json(output_dir / "autotile-byte-binding-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PASS", "records": binding_records})
    write_json(output_dir / "autotile-resolution-matrix-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PASS", "records": resolution_records})
    write_json(output_dir / "atlas-byte-integrity-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PASS", "records": [{"tile_id": tile["tile_id"], "file_sha256": tile["binding"]["file_sha256"], "decoded_pixel_hash": tile["binding"]["decoded_pixel_hash"], "atlas_rect": tile["binding"]["atlas_rect"], "atlas_revision": tile["binding"]["atlas_revision"]} for tile in all_tiles]})
    write_json(output_dir / "seam-qa-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PASS", "actual_transition_variants": True, "records": seam_pairs})
    write_json(output_dir / "effective-variant-validation-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PASS", "variant_count": len(all_variants), "materialized_before_resolution": True})
    write_json(output_dir / "world-metric-roundtrip-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PASS", "world_units_per_tile": metrics["world_units_per_tile"], "pixel_resolutions": [16, 32, 64], "positions_invariant": True, "grid_orientation": metrics["grid_orientation"], "origins": ["TOP_LEFT", "CENTER"]})
    write_json(output_dir / "cache-identity-v0202.json", {"schema_version": SCHEMA_VERSION, "fields": ["requested_class_id", "resolved_class_id", "requested_layer", "resolved_layer", "adjacency_mask", "variant_id", "atlas_revision", "content_hash"], "status": "PASS"})
    write_json(output_dir / "production-registry-v0202.json", {"schema_version": SCHEMA_VERSION, "status": "PRODUCTION_REGISTRY_EMPTY", "entries": [], "production_approved": False, "production_routing": "BLOCKED", "new_generation": 0})
    write_json(output_dir / "test-only-qa-board-v0202.json", {"schema_version": SCHEMA_VERSION, "label": "TEST_ONLY_TILE_QA_BOARD_V0202", "production_registry": [], "classes": list(TILE_CLASSES), "families": [family["terrain_family_id"] for family in families], "mask_specific_visual_identity": True, "status": "TEST_ONLY"})
    _qa_sheet(output_dir / "mask-qa-sheet-v0202.png", [(f"{record['terrain_family_id']} {record['class_id']} mask {record['mask']:03d}", output_dir / record["artifact_path"]) for record in binding_records])
    _qa_sheet(output_dir / "seam-qa-sheet-v0202.png", [(f"{pair['left_tile_id']} -> {pair['right_tile_id']}", output_dir / next(tile for tile in all_tiles if tile["tile_id"] == pair["left_tile_id"])["binding"]["artifact_path"]) for pair in seam_pairs])
