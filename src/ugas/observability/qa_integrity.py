...
ACTIVE_VERSION = "0.21.1"
ACTIVE_REVIEW = "REVIEW-v0.21.1.md"
...
class ActiveEvidenceCache:
    ...
    def __init__(self, repo_root: Path, *, state_path: Path | None = None, state_schema_path: Path | None = None,
        ...
        if state_schema_path is not None:
            ...
            try:
                ...
                active_version = None
            self.state_schema_path = self.repo_root / ("schemas/current-state-v0211.json" if active_version == "0.21.1" else ("schemas/current-state-v0210.json" if active_version == "0.21.0" else "schemas/current-state-v0203.json"))
        self.checkpoint_path = checkpoint_path or self.repo_root / "CHECKPOINT.md"
        self.review_path = review_path or self.repo_root / ("REVIEW-v0.21.1.md" if self.state_schema_path.name == "current-state-v0211.json" else ("REVIEW-v0.21.0.md" if self.state_schema_path.name == "current-state-v0210.json" else "REVIEW-v0.20.3.md"))
    ...
    def validate(self) -> dict[str, Any]:
        ...
        try:
            ...
            consistency_validator = validate_state_consistency_v0211 if state.get("version") == "0.21.1" else (validate_state_consistency_v0210 if state.get("version") == "0.21.0" else validate_state_consistency_v0203)
