"""Fail-closed active-state contract for the v0.20.2 QA-integrity correction."""
...
CURRENT_GATE = "ENVIRONMENT_TILESETS_PRODUCTION_MASK_ORIGIN_VARIANT_INTEGRITY_TECHNICALLY_QUALIFIED"
NEXT_ACTION = "external_review_environment_tilesets_v0202"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str = "", review_text: str = "", roadmap_text: str = "") -> dict[str, Any]:
    ...
    v0191 = _mapping(history.get("v0.19.1"))
    v0200 = _mapping(history.get("v0.20.0"))
    v0201 = _mapping(history.get("v0.20.1"))
    if v0191.get("status") != "APPROVED_FOUNDATION" or v0191.get("merge_commit") != BASELINE_HEAD:
        failures.append("v0191_approval_history_invalid")
    if v0200.get("status") != "CORRECTION_REQUIRED" or v0200.get("rejected_reviewed_head") != "ae335ba198bb7f23210873e30948e8a19bc71cbd":
        failures.append("v0200_rejection_history_invalid")
    if v0201.get("status") != "CORRECTION_REQUIRED" or v0201.get("rejected_reviewed_head") != "0353e6785017c08db6e55c9448d9fa60e5908802":
        failures.append("v0201_rejection_history_invalid")
    ...
    literals = ("0.20.2", "0.20.1", "0.20.0", "0.19.1", CURRENT_PHASE, CURRENT_GATE, NEXT_ACTION, "items_props=APPROVED_FOUNDATION", "environment_tilesets_runtime_external_review=REQUIRED", "real_environment_asset_coverage=NONE", "synthetic_environment_fixture=TEST_ONLY", "production_approved=false", "production_routing=BLOCKED", "new_generation=0", "TEST_ONLY_TILE_QA_BOARD_V0202", "external_review_environment_tilesets_v0202", "v0201_external_review=CORRECTION_REQUIRED")
