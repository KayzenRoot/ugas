"""Fail-closed active-state checks for the v0.15.0 death-animation pilot."""
...
CURRENT_GATE = "CUTOUT_ANIMATION_RUNTIME_V1_DEATH_ANIMATION_FRONT_TECHNICALLY_QUALIFIED"
NEXT_ACTION = "external_visual_review_death_animation_front"
...
def validate_state_consistency(
    ...
    """Validate the active v0.15.0 state and its forward-only governance."""
    failures: list[str] = []
    required = {
        "schema_version",
        "version",
    ...
    if state.get("schema_version") != CURRENT_VERSION or state.get("version") != CURRENT_VERSION:
        failures.append("version_must_be_0.15.0")
