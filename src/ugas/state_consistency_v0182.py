"""Fail-closed active-state checks for the v0.18.2 correction."""
...
CURRENT_GATE = "CREATURES_MONSTERS_DERIVED_VARIANT_AND_STATE_CONTRACT_TECHNICALLY_QUALIFIED"
NEXT_ACTION = "external_review_creatures_monsters_v0182"
BASELINE_HEAD = "39e148bef50c8f04db194048dbe9fbb15d8ff3d4"
FEATURE_BRANCH = "codex/v0.18.0-creatures-monsters-runtime-foundation"
REJECTED_V0181_HEAD = "b52dc26d935fd84b25a991ae81c5d508b0a39807"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str = "", review_text: str = "", roadmap_text: str = "") -> dict[str, Any]:
    ...
    expected = {
        ...
        "current_gate": CURRENT_GATE, "equipment_outfits": "APPROVED_FOUNDATION",
        "creatures_monsters_runtime": "DERIVED_VARIANT_AND_STATE_CONTRACT_TECHNICALLY_QUALIFIED",
        "creatures_monsters_runtime_external_review": "REQUIRED", "v0181_external_review": "CORRECTION_REQUIRED",
        "v0180_external_review": "CORRECTION_REQUIRED", "real_creature_asset_coverage": "NONE",
        "synthetic_creature_fixture": "TEST_ONLY", "production_routing": "BLOCKED",
    ...
    if state.get("production_approved") is not False: failures.append("production_must_remain_unapproved")
    ...
    if not isinstance(forbidden, list) or not required_forbidden.issubset(forbidden): failures.append("forbidden_boundaries_incomplete")
    previous = _mapping(state.get("previous_release"))
    if previous.get("version") != "0.17.1" or previous.get("merge_commit") != BASELINE_HEAD: failures.append("previous_release_must_bind_v0171")
    history = _mapping(state.get("correction_history")); v0181 = _mapping(history.get("v0.18.1")); v0180 = _mapping(history.get("v0.18.0"))
    if v0181.get("status") != "CORRECTION_REQUIRED" or v0181.get("rejected_reviewed_head") != REJECTED_V0181_HEAD: failures.append("v0181_rejection_history_invalid")
    if v0180.get("status") != "CORRECTION_REQUIRED": failures.append("v0180_rejection_history_invalid")
    external = _mapping(state.get("external_visual_review"))
    if external.get("creatures_monsters_runtime") != "REQUIRED" or external.get("v0181") != "CORRECTION_REQUIRED": failures.append("external_review_boundary_invalid")
    ...
    if review.get("pr_number") != 8 or review.get("real_pr_checks_green") not in {True, False}: failures.append("review:pr_binding")
    nested = _mapping(state.get("state_consistency")); nested_expected = {"status": CURRENT_GATE, "version": CURRENT_VERSION, "phase": CURRENT_PHASE, "current_gate": CURRENT_GATE, "feature_branch": FEATURE_BRANCH, "branch_base_commit": BASELINE_HEAD, "production_routing": "BLOCKED", "production_approved": False, "new_generation": 0, "allowed_next_actions": [NEXT_ACTION], "next_capability_started": False}
    for key, value in nested_expected.items():
        if nested.get(key) != value: failures.append(f"state_consistency:{key}")
    active = "\n".join((checkpoint_text, review_text, roadmap_text))
    required_literals = ("0.18.2", CURRENT_PHASE, CURRENT_GATE, NEXT_ACTION, "equipment_outfits=APPROVED_FOUNDATION", "v0181_external_review=CORRECTION_REQUIRED", "real_creature_asset_coverage=NONE", "synthetic_creature_fixture=TEST_ONLY", "production_approved=false", "production_routing=BLOCKED", "new_generation=0", "items_props")
...
__all__ = ["BASELINE_HEAD", "CURRENT_GATE", "CURRENT_PHASE", "CURRENT_VERSION", "FEATURE_BRANCH", "NEXT_ACTION", "REJECTED_V0181_HEAD", "validate_state_consistency"]
