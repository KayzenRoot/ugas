"""Fail-closed consistency checks for the active v0.11.2 QA-integrity slice."""
...
CURRENT_GATE = "CUTOUT_ANIMATION_RUNTIME_V2_ATTACK_FRONT_TECHNICALLY_QUALIFIED"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str, review_text: str) -> dict[str, Any]:
    ...
    required = {
        ...
        "generation_provider_change_authorized", "walk_authorized", "production_walk_authorized",
        "provider_smoke_status", "historical_pose_lane_status", "pose_lane_status",
        "previous_review_snapshot_status", "state_consistency",
    }
    failures.extend(f"missing:{key}" for key in sorted(required - set(state)))
    if state.get("schema_version") != CURRENT_SCHEMA_VERSION: failures.append("state_schema_version_must_be_0.11.2")
    if state.get("version") != CURRENT_SCHEMA_VERSION: failures.append("state_version_must_be_0.11.2")
    if state.get("phase") != CURRENT_PHASE: failures.append("state_phase_invalid_for_v0112")
    ...
    if previous.get("version") != "0.11.1": failures.append("previous_release_must_be_0.11.1")
    if state.get("allowed_next_actions") != ["external_review_attack_front_v2_v0112"]: failures.append("allowed_next_actions_must_stop_at_attack_v2_v0112_external_review")
    ...
    if nested.get("new_generation_started") is not False or nested.get("new_generation_jobs") != 0 or nested.get("sam2_runs") != 0 or nested.get("comfyui_generation_jobs") != 0 or nested.get("diffusion_runs") != 0: failures.append("v0112_must_record_no_new_generation")
    ...
    if nested.get("attack_front_v1_external_visual") != "APPROVED_PILOT": failures.append("attack_v1_external_decision_missing")
    if nested.get("attack_front_v2_external_visual") != "REQUIRED": failures.append("attack_v2_external_review_must_be_required")
    if nested.get("production_routing") != "BLOCKED": failures.append("production_routing_must_be_blocked")
    combined = f"{checkpoint_text}\n{review_text}"
    required_texts = (
        "0.11.2", "0.11.1", "0.11.0", CURRENT_PHASE, "attack-front-v2", "motion_tracks", "key_pose_bindings",
        "QA integrity", "scope recovery", "rejected history", "APPROVED_PILOT",
        "SDXL_OPENPOSE_CONTROL_GAP_CONFIRMED_AT_MODEL_CARD_SETTINGS", "LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED",
        "REVIEW_INDEX_VERIFIED", CURRENT_GATE, "external_review_attack_front_v2_v0112", "production_routing=BLOCKED",
        "sam2_runs=0", "comfyui_generation_jobs=0", "diffusion_runs=0", "new_generation=0", "historical replay",
        "decision=QUALIFIED",
    )
    combined_lower = combined.casefold()
    failures.extend(f"active_documents_missing:{item}" for item in required_texts if item.casefold() not in combined_lower)
    ...
    return {
        ...
        "failures": failures,
        "checked": {
            "phase": state.get("phase"), "current_gate": state.get("current_gate"),
            "walk_front_v081_external_visual": nested.get("walk_front_v081_external_visual"),
            "idle_front_v1_external_visual": nested.get("idle_front_v1_external_visual"),
            "attack_front_v1_external_visual": nested.get("attack_front_v1_external_visual"),
            "attack_front_v2_external_visual": nested.get("attack_front_v2_external_visual"),
            "production_routing": nested.get("production_routing"),
            "new_generation_started": nested.get("new_generation_started"),
            "new_generation_jobs": nested.get("new_generation_jobs"),
        },
    }
