"""Fail-closed active-state contract for the v0.21.1 maps/minimap correction."""
...
CURRENT_GATE = "MAPS_MINIMAP_QA_CONTRACT_INTEGRITY_TECHNICALLY_QUALIFIED"
NEXT_ACTION = "external_review_maps_minimap_v0211"
...
def validate_state_consistency(state: Mapping[str, Any], checkpoint_text: str = "", review_text: str = "", roadmap_text: str = "") -> dict[str, Any]:
    ...
    if approved_v0203.get("status") != "APPROVED_FOUNDATION" or approved_v0203.get("merge_commit") != BASELINE_HEAD or approved_v0203.get("historical_evidence_unchanged") is not True:
        failures.append("v0203_approval_history_invalid")
    correction_v0210 = _mapping(history.get("v0.21.0"))
    if correction_v0210.get("status") != "CORRECTION_REQUIRED" or correction_v0210.get("rejected_reviewed_head") != REJECTED_V0210_HEAD or correction_v0210.get("historical_evidence_unchanged") is not True:
        failures.append("v0210_rejection_history_invalid")
    ...
    expected_review = {
        "repository": "KayzenRoot/ugas", "baseline_head": BASELINE_HEAD, "branch_base_commit": BASELINE_HEAD,
        "feature_branch": FEATURE_BRANCH, "execution_mode": "GITHUB_PR_FIRST", "merge_policy": "NO_SELF_MERGE_UNTIL_EXTERNAL_REVIEW",
        "no_self_merge_until_external_approval": True, "external_review_required": True, "merge_authorization": "NOT_AUTHORIZED", "do_not_merge": True,
    }
    ...
    nested_expected = {
        "status": CURRENT_GATE, "version": CURRENT_VERSION, "phase": CURRENT_PHASE, "current_gate": CURRENT_GATE,
        "feature_branch": FEATURE_BRANCH, "branch_base_commit": BASELINE_HEAD, "production_routing": "BLOCKED",
        "production_approved": False, "new_generation": 0, "allowed_next_actions": [NEXT_ACTION], "next_capability_started": False,
    }
    ...
    literals = ("0.21.1", "0.21.0", "0.20.3", "MAPS_MINIMAP", CURRENT_GATE, NEXT_ACTION, "environment_tilesets=APPROVED_FOUNDATION", "maps_minimap_runtime=TECHNICALLY_QUALIFIED_FOUNDATION", "real_map_asset_coverage=NONE", "real_minimap_asset_coverage=NONE", "synthetic_map_fixture=TEST_ONLY", "production_approved=false", "production_routing=BLOCKED", "new_generation=0", "v0.21.0", REJECTED_V0210_HEAD)
...
__all__ = ["BASELINE_HEAD", "CURRENT_GATE", "CURRENT_PHASE", "CURRENT_VERSION", "FEATURE_BRANCH", "NEXT_ACTION", "REJECTED_V0210_HEAD", "validate_state_consistency"]
