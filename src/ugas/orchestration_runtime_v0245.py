"""Fail-closed, provider-neutral orchestration runtime for UGAS v0.24.5.

Forward-only correction of the v0.24.5 TEST_ONLY control plane.  This module
...
TEST_ONLY_ROUTE_EXECUTOR_IDENTITY = "test-only-fake-executor-v0245"
...
def assert_provider_boundary(source: str | Path | None = None) -> dict[str, Any]:
    if source is None:
        ...
        source_name = "ugas.orchestration_runtime_v0245.OrchestrationRuntime"
...
UADS_HANDOFF_FIELDS = frozenset({
    ...
    "selected_profile_digest", "dispatch_status", "execution_mode", "project_footprint",
})
...
def sanitize_uads_handoff(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if value.get("execution_mode") != "GLOBAL_FIRST" or value.get("project_footprint") != "ZERO":
        _reject("ORCH_UADS_HANDOFF_INVALID", "UADS handoff must remain GLOBAL_FIRST with ZERO project footprint")
    for key, child in value.items():
        if not isinstance(child, str) or not child:
            _reject("ORCH_UADS_HANDOFF_INVALID", f"UADS handoff field {key} must be a non-empty string")
        if ntpath.isabs(child) or ("\\" in child and ":" in child) or child.startswith("~") or ".uads" in child.casefold():
...
__all__ = [
    ...
    "sha256_bytes", "sha256_value", "strict_boolean_observation", "strict_gate", "validate_authority_binding", "validate_checkpoint",
