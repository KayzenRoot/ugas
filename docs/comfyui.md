# ComfyUI integration notes - UGAS v0.16.2

The active v0.16.2 slice is MULTI_DIRECTION_ANIMATION_RUNTIME cache/state correction, not a ComfyUI generation lane. `DOCKER_ALWAYS_ON_LOCAL` remains read-only and always-on, `production_approved=false`, `production_routing=BLOCKED`, `new_generation=0`, and no ComfyUI/SAM2/diffusion generation is authorized. Sol recorded `APPROVED_FOUNDATION / APPROVED_TO_MERGE` for reviewed head `2864b8ca392725b6da0616916ef3a3c38ce0a0d6`; PR #6 is OPEN with `real_pr_checks_green=true`, and the only next action is `governed_merge_pr_6`. The v0.16.0 and v0.16.1 records are `CORRECTION_REQUIRED`. `next_candidate=EQUIPMENT_OUTFITS`, but equipment/outfits is not started before the governed merge. The v0.15.1, v0.15.0, v0.14.1 and older ComfyUI material below is historical.
