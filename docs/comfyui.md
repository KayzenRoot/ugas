# ComfyUI integration notes - UGAS v0.10.0

The active release is v0.10.0 and the deterministic cutout-rig lane executes without ComfyUI generation. The v0.8.0/v0.7.3/v0.9.0/v0.9.1 structural and runtime/model/custom-node records below remain historical.
...
The v0.10.0 animation runtime performs no ComfyUI generation. `comfyui_generation_jobs=0`, `sam2_runs=0`, `new_generation=0`, and `diffusion_runs=0`; the source-only cutout runtime is the qualified lane for this slice. The generic package gate is `decision=QUALIFIED`, event markers are hash-bound, and external attack-front review remains required.
