...
# ComfyUI integration notes - UGAS v0.19.0

The active v0.19.0 slice is the ITEMS_PROPS runtime foundation, not a ComfyUI generation lane. `DOCKER_ALWAYS_ON_LOCAL` remains read-only and always-on, `production_approved=false`, `production_routing=BLOCKED`, `new_generation=0`, and no ComfyUI/SAM2/diffusion generation is authorized. Item/prop fixtures are synthetic TEST_ONLY only; the production registry is empty and real item/prop coverage is `NONE`. v0.18.2 creatures/monsters remains approved foundation history. External items/props review is required; environment/tilesets and downstream asset families remain out of scope.
