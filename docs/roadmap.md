...
# v0.21.2 - MAPS_MINIMAP raster + governance integrity correction

The active phase is `MAPS_MINIMAP` and the current gate is `MAPS_MINIMAP_RASTER_GOVERNANCE_INTEGRITY_TECHNICALLY_QUALIFIED`. This is the same PR #11 correction over base `0bf04cb92e8619ea10cf82af8dbf2d9abe599e05`; rejected v0.21.1 head `cfde03cce2e31cc688f9d94a659b577042b8538a` remains immutable history. The runtime now proves orientation-safe raster cell bounds for TOP_LEFT/Y_DOWN and CENTER/Y_UP, validates historical candidates through one immutable-authority validator, and accepts hard-gate observations only when they are exactly boolean `True`. Evidence records 22 hard gates and 29 semantic negative controls, including real collapsed-raster rejection, real historical mutation rejection and non-boolean gate rejection. `real_map_asset_coverage=NONE`, `real_minimap_asset_coverage=NONE`, `production_approved=false`, `production_routing=BLOCKED`, `new_generation=0`, and the only next action is `external_review_maps_minimap_v0212`. Require exact-head success for the three required contexts and the bounded artifact, then stop OPEN and unmerged; do not start UI, VFX, orchestration or production art.
