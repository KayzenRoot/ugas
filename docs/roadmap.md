...
## v0.19.1 - ITEMS_PROPS linkage, representation and stack identity correction / external review required

The active phase is `ITEMS_PROPS` and the gate is `ITEMS_PROPS_LINKAGE_REPRESENTATION_STACK_INTEGRITY_TECHNICALLY_QUALIFIED`. The v0.18.2 creatures/monsters runtime foundation is `APPROVED_FOUNDATION` at merge `52938a04016352d50ad54621a4df981a9c36b058`; v0.19.0 was rejected at reviewed head `44937935e644202836b3e1f081b6a63201b850db` and remains immutable history. The v0.19.1 forward-only correction stays on PR #9 and branch `codex/v0.19.0-items-props-runtime-foundation`, based on that exact main head. `creatures_monsters=APPROVED_FOUNDATION`; `items_props_runtime=TECHNICALLY_QUALIFIED_FOUNDATION`; `items_props_runtime_external_review=REQUIRED`; `real_item_prop_asset_coverage=NONE`; `synthetic_item_prop_fixture=TEST_ONLY`; `production_approved=false`; `production_routing=BLOCKED`; `new_generation=0`; `allowed_next_actions=[external_review_items_props_v0191]`; `next_capability_started=false`. Equipment references are bound to frozen v0.17.1 authority, every resolved representation is backed by exact bytes and decoded pixels, full-slice two-run determinism is checked, and stack identities are item-family specific. The only next action is external review of PR #9; do not merge it and do not start environment/tilesets, maps/minimap, UI, VFX or orchestration hardening.
