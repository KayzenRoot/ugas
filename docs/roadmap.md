...
The active phase is `MAPS_MINIMAP` and the current gate is `MAPS_MINIMAP_APPROVED_FOUNDATION_MERGED`; Maps/Minimap is `APPROVED_FOUNDATION` and `MERGED_CLOSED` after protected PR #11 merge `0c9b721b43d3cc12605ce009f6e0deb232b00045`. The semantic approval head remains `812a2bed1f77df3d630425f23032c22fc72d5961`, and the final bookkeeping head was `1ba088b40583a0ac6f287cbcc8ed8511031cd7e5`. The pre-merge approval record is immutable at `docs/evidence/github-governance-v0220/v0213-external-approval.json`; the forward-only post-merge binding is `docs/evidence/github-governance-v0220/v0213-post-merge-binding.json`. v0.21.2 remains frozen `CORRECTION_REQUIRED` history at reviewed head `b80f30074374c539e008ce160846848f1c171d9a`, with its evidence unchanged. The sole next candidate/action is `start_ui_asset_family_v0220`, but a new governance-only closure PR from main must receive Sol review and merge before any UI work order starts. `real_map_asset_coverage=NONE`, `real_minimap_asset_coverage=NONE`, `synthetic_map_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. Do not start UI, VFX, orchestration or production art in this closure increment.
