...
## v0.21.3 - MAPS_MINIMAP approved foundation / governed merge

The active phase is `MAPS_MINIMAP` and the technical gate remains `MAPS_MINIMAP_RASTER_GOVERNANCE_INTEGRITY_TECHNICALLY_QUALIFIED`; Maps/Minimap is now `APPROVED_FOUNDATION` after external review of exact semantic head `812a2bed1f77df3d630425f23032c22fc72d5961`. This is the same PR #11 over base `0bf04cb92e8619ea10cf82af8dbf2d9abe599e05`; v0.21.2 is frozen `CORRECTION_REQUIRED` history at reviewed head `b80f30074374c539e008ce160846848f1c171d9a`, with its evidence unchanged. The forward-only approval record is `docs/evidence/github-governance-v0220/v0213-external-approval.json`; the only pre-merge action is `bookkeeping_reproof_and_governed_merge_pr_11`. The historical validator compares raw Git-object bytes without newline normalization, requires explicit equality of the candidate and approved authority blob SHAs, and records separate commit/blob and raw-byte SHA-256 provenance. Real CRLF-only and append mutations traverse the validator and expose the observed rejection class. Evidence is under `docs/evidence/maps-minimap-runtime-v0213/`; `real_map_asset_coverage=NONE`, `real_minimap_asset_coverage=NONE`, `synthetic_map_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. UI is only the next candidate after protected merge; do not start UI, VFX, orchestration or production art before then.
