...
## v0.21.0 - MAPS_MINIMAP runtime foundation

The active phase is `MAPS_MINIMAP` and the current gate is `MAPS_MINIMAP_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED`. The approved v0.20.3 environment/tilesets foundation is bound to protected merge `0bf04cb92e8619ea10cf82af8dbf2d9abe599e05`. Two deterministic TEST_ONLY maps cover dimensions/aspect ratios, chunks, layers, typed world props, regions/zones, POI/portal/spawn markers, minimap projection/inverse projection, visibility masks, provenance and cache identity. Evidence records 21 hard gates and 20 semantic negative controls. `real_map_asset_coverage=NONE`, `real_minimap_asset_coverage=NONE`, `production_approved=false`, `production_routing=BLOCKED`, `new_generation=0`, and the only next action is `external_review_maps_minimap_v0210`. Create one GitHub PR, require exact-head success for the three required contexts and the bounded artifact, then stop OPEN and unmerged; do not start UI, VFX, orchestration or production art.
