...
## v0.22.2 - UI_ASSET_FAMILY correction delta F-15/F-16/F-18/F-20/F-21

The active phase is `UI_ASSET_FAMILY` and the current gate is `UI_ASSET_FAMILY_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED`. This forward-only correction remains on PR #13 and branch `codex/v0.22.0-ui-asset-family-runtime-foundation`, over rejected v0.22.1 HEAD `9afcf52143f27db3bfbb88fa4394e0e77f9c404a`; v0.22.0 and v0.22.1 evidence roots remain immutable history and v0.22.2 evidence is under `docs/evidence/ui-asset-family-runtime-v0222/`. The canonical v0.21.3 closure authority is `docs/evidence/github-governance-v0220/v0213-post-merge-binding.json`; the superseded stale pointer is rejected. The runtime proves a complete normalized progress-bar contract, independent decoded 9-slice invariants at two non-native sizes, local UI style/Art-DNA authority for generic classes, and a live-only post-merge lifecycle in which VFX is exposed only after governed merge and successful post-merge main CI. `ui_asset_family_external_review=REQUIRED`, `real_ui_asset_coverage=NONE`, `synthetic_ui_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. The sole tracked action is `external_review_ui_asset_family_v0222`; PR #13 must remain OPEN/unmerged and VFX/orchestration/production remain blocked.
