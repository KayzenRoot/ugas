...
## v0.17.0 - EQUIPMENT_OUTFITS runtime foundation (active)

The active gate is `EQUIPMENT_OUTFITS_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED`, based on merged v0.16.2 commit `a8d2897211c4b72c2cd2fe7a7f5729c7009d8566`, with only `external_review_equipment_outfits_v0170` allowed next. This slice implements deterministic modular wearable composition over the approved R4 rig and front animation frames. Every record declares slot, explicit layer graph/layer group, R4 joint anchors, direction coverage, compatible animation profiles, rig revision, replacement/hide rules, explicit asset-bound occlusion masks, provenance hash, variant and TEST_ONLY/production boundary. The resolver cache includes equipment_id, slot, variant, rig_revision, canonical direction, animation capability/profile, asset revision, request mode and registry mode. Missing directions fail closed; mirror requires manifest permission and preview fallback is explicitly TEST_ONLY. Base pixels, frame hashes, timing and event markers remain immutable; two identical compositions must be byte-identical. `multi_direction_animation_runtime=APPROVED_FOUNDATION`, `real_directional_character_asset_coverage=SOUTH_ONLY`, `real_equipment_asset_coverage=NONE_OR_EXPLICITLY_APPROVED_ONLY`, `synthetic_equipment_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. The production registry is empty. Leave the PR open for external review; do not start creatures/monsters or downstream capabilities.
