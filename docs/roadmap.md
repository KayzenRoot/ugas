...
## v0.16.0 - MULTI_DIRECTION_ANIMATION_RUNTIME foundation technically qualified / external review required (active: MULTI_DIRECTION_ANIMATION_RUNTIME)

The active phase is MULTI_DIRECTION_ANIMATION_RUNTIME and the gate is MULTI_DIRECTION_ANIMATION_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED. The foundation freezes the canonical eight directions, deterministic aliases/vector sectors, direction-aware identity and truthful resolver outcomes. multi_direction_animation_runtime=TECHNICALLY_QUALIFIED_FOUNDATION; real_directional_character_asset_coverage=SOUTH_ONLY; death_animation_front=APPROVED_PILOT; hit_reaction_front=APPROVED_PILOT; run_front_v1=APPROVED_PILOT. The six approved front profiles map `front` to `south` without byte or source-hash changes. Missing directions fail closed; preview fallback and mirror are explicit non-production modes. TEST_ONLY_SYNTHETIC_DIRECTION_FIXTURE is review-only. production_routing=BLOCKED, new_generation=0, production_approved=false. GITHUB_OPERATIONS_AUTOMATION_POLICY=ENABLED, USER_MANUAL_GITHUB_OPERATIONS=FALLBACK_ONLY, GITHUB_REVIEW_MODE=PR_FIRST, NO_SELF_MERGE_UNTIL_EXTERNAL_VISUAL_APPROVAL=true, ALWAYS_ON_DASHBOARD_POLICY=ENABLED and DOCKER_ALWAYS_ON_LOCAL remain binding. The active repository is KayzenRoot/ugas and the v0.16.0 PR must remain OPEN for external review. Merge v0.16.0 PR only after external review. Do not merge v0.16.0. Do not start equipment/outfits.
