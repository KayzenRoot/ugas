...
The active phase is `ENVIRONMENT_TILESETS` and the gate is `ENVIRONMENT_TILESETS_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED`. v0.19.1 Items/Props is approved foundation at protected merge `93293d21f301e6d64232a992eb92533db9741118`; the forward-only approval record is `docs/evidence/github-governance-v0200/v0191-external-approval.json`. The v0.19.0 candidate remains rejected history at `44937935e644202836b3e1f081b6a63201b850db`. The v0.20.0 branch is `codex/v0.20.0-environment-tilesets-runtime-foundation`, based on that exact main head. `creatures_monsters=APPROVED_FOUNDATION`; `items_props=APPROVED_FOUNDATION`; `environment_tilesets_runtime=TECHNICALLY_QUALIFIED_FOUNDATION`; `environment_tilesets_runtime_external_review=REQUIRED`; `real_environment_asset_coverage=NONE`; `synthetic_environment_fixture=TEST_ONLY`; `production_approved=false`; `production_routing=BLOCKED`; `new_generation=0`; `allowed_next_actions=[external_review_environment_tilesets_v0200]`; `next_capability_started=false`. The runtime evidence contains six stable classes, metric/layer contracts, adjacency/autotile, byte-bound atlases and seams, collision/navigation, variants, cache identity, provenance, 18 negative controls and isolated determinism. The only authorized next step is external review; do not merge v0.20.0 or start Maps/Minimap, UI, VFX, orchestration or production art.
