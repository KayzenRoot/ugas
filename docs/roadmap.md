...
## v0.21.3 - MAPS_MINIMAP historical byte integrity correction

The active phase is `MAPS_MINIMAP` and the current gate remains `MAPS_MINIMAP_RASTER_GOVERNANCE_INTEGRITY_TECHNICALLY_QUALIFIED`. This is the same PR #11 correction over base `0bf04cb92e8619ea10cf82af8dbf2d9abe599e05`; v0.21.2 is frozen `CORRECTION_REQUIRED` history at reviewed head `b80f30074374c539e008ce160846848f1c171d9a`, with its evidence unchanged. The historical validator compares raw Git-object bytes without newline normalization, requires explicit equality of the candidate and approved authority blob SHAs, and records separate commit/blob and raw-byte SHA-256 provenance. Real CRLF-only and append mutations traverse the validator and expose the observed rejection class. Evidence is under `docs/evidence/maps-minimap-runtime-v0213/`; `real_map_asset_coverage=NONE`, `real_minimap_asset_coverage=NONE`, `synthetic_map_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. Require exact-head success for the three required contexts and the bounded artifact, then stop OPEN and unmerged; do not start UI, VFX, orchestration or production art.
