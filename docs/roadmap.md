...
## v0.17.1 - EQUIPMENT_OUTFITS runtime and QA integrity correction (active)

The active gate is `EQUIPMENT_OUTFITS_RUNTIME_AND_QA_INTEGRITY_TECHNICALLY_QUALIFIED`, based on merged v0.16.2 commit `a8d2897211c4b72c2cd2fe7a7f5729c7009d8566`, with only `external_review_equipment_outfits_v0171` allowed next. The v0.17.0 candidate was rejected at reviewed head `1c73e6a2ff5259226afe9ca03ef10e1822a7fdf2` and remains frozen history. This correction implements deterministic modular wearable composition over the approved R4 rig and front animation frames, including executed replacement winner/hide semantics, actual hashed asset-bound masks, strict negative controls and non-destructive determinism. Missing directions, mirror without implemented pixel operation, and declared secondary anchors fail closed. `multi_direction_animation_runtime=APPROVED_FOUNDATION`, `real_directional_character_asset_coverage=SOUTH_ONLY`, `real_equipment_asset_coverage=NONE_OR_EXPLICITLY_APPROVED_ONLY`, `synthetic_equipment_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. The production registry is empty. Leave PR #7 open for external review; do not start creatures/monsters or downstream capabilities.
