...
The v0.20.1 slice is the active `ENVIRONMENT_TILESETS` QA-integrity correction. It validates six stable tile classes, class/layer-safe autotile routing, mask-specific TEST_ONLY bytes, pixel/world metric separation, explicit layer order, cardinal/eight-neighbor masks, byte-derived edge/seam compatibility, exact standalone/atlas pixels, collision/navigation metadata, effective variants, complete cache identity, gate-specific proof, 18 historical plus 15 corrective negative controls and full-slice two-run deterministic fixture runs. `creatures_monsters=APPROVED_FOUNDATION`, `items_props=APPROVED_FOUNDATION`, `environment_tilesets_runtime=TECHNICALLY_QUALIFIED_FOUNDATION`, `real_environment_asset_coverage=NONE`, `synthetic_environment_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`; Maps/Minimap, UI, VFX, orchestration and production art remain out of scope. v0.20.0 remains immutable `CORRECTION_REQUIRED` history.
