# 2D master pipeline v0.16.2

The active slice is `MULTI_DIRECTION_ANIMATION_RUNTIME` for v0.16.2: a generic eight-direction contract, deterministic vector/alias normalization, strict invalid-vector rejection, direction-aware asset identity and fail-closed resolution. Unresolved normalization class is part of the cache identity, and request mode is separated from registry mode so test-only resolution never implies production. Existing front profiles remain byte-compatible through `front -> south`; real directional character artwork coverage is `SOUTH_ONLY`. `DOCKER_ALWAYS_ON_LOCAL` remains online, `production_approved=false`, `production_routing=BLOCKED`, `new_generation=0`. Sol recorded `APPROVED_FOUNDATION / APPROVED_TO_MERGE` for reviewed head `2864b8ca392725b6da0616916ef3a3c38ce0a0d6`; the post-bookkeeping PR #6 state is `pr_number=6`, `pr_state=OPEN`, `real_pr_checks_green=true`, and `merge_authorization=APPROVED_TO_MERGE`. The capability matrix advances `next_candidate=EQUIPMENT_OUTFITS`, but that next capability is not started until the governed merge. The v0.16.0 and v0.16.1 reviews are `CORRECTION_REQUIRED`; older materials below are preserved historical context. Merge after recorded external approval and exact post-bookkeeping reproof.
...
6. A/C/R, IP-Adapter and SDXL remain historical. The active lane runs zero new generation, SAM2, ComfyUI or diffusion jobs and does not authorize walk.
...
## v0.11.2 active boundary

The active correction is QA integrity and scope recovery only. `motion_tracks` and `key_pose_bindings` equal the v0.11.0 baseline; all pixels and package outputs are byte-identical. `decision=QUALIFIED`, `attack_front_v2_external_visual=REQUIRED`, `production_approved=false`, `production_routing=BLOCKED`, and the only allowed next action is `external_review_attack_front_v2_v0112`. The rejected v0.11.1 history and evidence remain immutable.

## v0.11.0 historical boundary

The v0.11.0 generic motion-quality and attack-front-v2 evidence is preserved as the immutable restoration baseline. v0.11.1 is rejected history and v0.11.2 does not rewrite either record.
