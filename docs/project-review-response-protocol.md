...
The response must reconcile GitHub LIVE, the current main SHA, the active PR and exact-head checks when relevant, then read `CHECKPOINT.md`, `docs/evidence/current-state.json`, and the capability matrix. A stale progress snapshot cannot override those sources. If live state is unavailable, mark the affected values as unverified instead of manufacturing precision.

For v0.21.3, Maps/Minimap is `APPROVED_FOUNDATION` and `MERGED_CLOSED`; UI Asset Family is the sole next candidate/action, but its implementation remains blocked until the governance closure review is approved and merged. Production remains `BLOCKED`, with `new_generation=0`.
