...
10. Reconcile the sole `allowed_next_actions` entry with any unresolved closure or review gate. While PR #12 is OPEN and unapproved, the only tracked action is `resolve_post_merge_closure_gate_pr_12`; never authorize UI from a stale snapshot.
...
- Resolve UI eligibility from GitHub LIVE PR #12 state and external approval; a simulated or stale tracked value cannot unblock UI.
