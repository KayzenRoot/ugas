...
Universal Game Asset Studio: pipeline local-first para assets 2D com ComfyUI nativo, evidência reproduzível, transparência e governança de revisão. O release ativo adiciona observabilidade local read-only, near-real-time, sem telemetria remota.

O v0.11.2 permanece preservado como release anterior: sua decisão visual externa é `APPROVED_PILOT` para pipeline/piloto, nunca aprovação de produção. O v0.11.1 permanece como rejected history.
...
Leia `docs/evidence/current-state.json` e siga os gates documentados em [REVIEW-v0.12.0.md](REVIEW-v0.12.0.md). O runtime SAM2, o bundle MediaPipe e os checkpoints históricos são externos; nenhum peso é distribuído com o repositório.

## v0.12.0 local observability

`python -m ugas.cli dashboard` abre o dashboard local em `127.0.0.1:8765`. Use `--port 0` para um smoke test efêmero, `--no-open` para não abrir o navegador e `--host` somente com um endereço loopback. A UI é estática, sem CDN/build chain, e expõe apenas GET read-only: status, sistema/GPU, processos/ComfyUI, jobs, assets allowlisted, QA, health, eventos e stream SSE.

O banco local fica em `.ugas/runtime/telemetry.db` (SQLite WAL) e é ignorado pelo Git. GPU ausente é reportada como `GPU_UNAVAILABLE` com motivo; não há zero fabricado. Os artefatos canônicos de governança continuam sendo `docs/evidence/current-state.json` e o review index.
...
Consulte [INSTALL.md](INSTALL.md), [CHECKPOINT.md](CHECKPOINT.md), [REVIEW-v0.12.0.md](REVIEW-v0.12.0.md) e [docs/evidence/current-state.json](docs/evidence/current-state.json). O REVIEW-v0.11.1 e suas evidências continuam disponíveis como rejected history.
