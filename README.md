...
Universal Game Asset Studio: pipeline local-first para assets 2D com evidência reproduzível, transparência e governança de revisão. O release ativo executa somente a correção CREATURES_MONSTERS v0.18.2, com validação efetiva de variantes derivadas, identidade separada de revisões, rotas de estado compatíveis com arquétipo e fixtures sintéticas TEST_ONLY; mantém o dashboard Dockerizado always-on/read-only/local e não habilita produção.
...
python scripts/validation/validate_state_consistency_v0182.py
python scripts/validation/run_creatures_monsters_runtime_v0182.py
...
Leia `docs/evidence/current-state.json` e siga os gates documentados em [REVIEW-v0.18.2.md](REVIEW-v0.18.2.md). O runtime SAM2, o bundle MediaPipe e os checkpoints históricos são externos; nenhum peso é distribuído com o repositório.
