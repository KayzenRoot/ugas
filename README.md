...
Universal Game Asset Studio: pipeline local-first para assets 2D com evidência reproduzível, transparência e governança de revisão. O release v0.20.2 corrige a integridade do runtime ENVIRONMENT_TILESETS com bloqueio público de produção, produto exato classe×máscara, semântica de origem TOP_LEFT/CENTER, identidade autoritativa de variantes e controles correntes ET-NC-01..18; mantém o dashboard Dockerizado always-on/read-only/local e não habilita produção. O v0.19.1 Items/Props permanece a release aprovada anterior no merge `93293d21f301e6d64232a992eb92533db9741118`; v0.20.1 permanece rejeitado como histórico de correção.
...
python scripts/validation/validate_state_consistency_v0202.py
python scripts/validation/run_environment_tilesets_runtime_v0202.py
...
The active gate is `CREATURES_MONSTERS_RUNTIME_AND_QA_INTEGRITY_TECHNICALLY_QUALIFIED`. The correction adds executable unique 8-way direction asset identities, explicit state route IDs with timing/phase metadata, two derived TEST_ONLY variant resolutions, strict collision geometry, canonical CR-NC-01..15 semantic rejection evidence, and isolated two-run determinism. `real_creature_asset_coverage=NONE`, `synthetic_creature_fixture=TEST_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. `v0180_external_review=CORRECTION_REQUIRED`; the only allowed next action is `external_review_creatures_monsters_v0181`; the PR remains OPEN and must not be merged or advance to Items/Props.

## Historical v0.18.0 CREATURES_MONSTERS runtime foundation
...
The active gate is `EQUIPMENT_OUTFITS_RUNTIME_AND_QA_INTEGRITY_TECHNICALLY_QUALIFIED`. Wearables declare slot, deterministic layer group, R4 anchors, SOUTH_ONLY direction coverage, compatible front animation profiles, rig revision, replacement/hide rules, hashed geometric asset-bound occlusion masks, provenance hash, variant and TEST_ONLY/production boundaries. Replacement arbitration and shared negative comparators are executed against real pixels; mirror and secondary anchors fail closed until implemented. The eight synthetic fixtures in `docs/evidence/equipment-outfits-runtime-v0171/synthetic-fixture-manifest-v0171.json` are review-only; the production registry is empty, `real_equipment_asset_coverage=NONE_OR_EXPLICITLY_APPROVED_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0`. PR-first review remains required; do not start creatures/monsters or downstream asset families.
