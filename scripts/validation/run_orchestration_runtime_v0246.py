"""Execute the complete deterministic UGAS v0.24.6 correction slice."""
...
EVIDENCE = ROOT / "docs/evidence/orchestration-runtime-v0246"
...
def _with_historical_candidate(mutator: Callable[[Path], None], *, auto_stage: bool = True) -> None:
    with tempfile.TemporaryDirectory(prefix="ugas-hist-v0246-") as directory:
        ...
        try:
            ...
            subprocess.run(["git", "-C", str(candidate), "commit", "-m", "ugas-v0246-historical-mutation", "--allow-empty"], check=True, capture_output=True)
...
def _no_git_authority_root() -> tempfile.TemporaryDirectory[str]:
    directory = tempfile.TemporaryDirectory(prefix="ugas-nogit-v0246-")
...
def _authority_mode_controls() -> list[dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-emptygit-v0246-") as directory:
    ...
    try:
        ...
        payload["ugas_v0246_semantic_probe"] = True
...
def _request(dependency_refs: list[dict[str, Any]] | None = None, *, timeout: int = 30) -> dict[str, Any]:
    ...
    return build_request(
        request_id="orchestration-v0246-fixture",
        ...
        idempotency_key="orchestration-v0246-fixture-idempotency",
...
def _node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 30, priority: int = 10, authority_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    base = {"node_id": node_id, "dependencies": dependencies or [], "operation": "EXECUTE", "dependency_hashes": {}, "timeout_seconds": timeout, "retry_policy": {"max_attempts": 2, "retryable_errors": ["TRANSIENT_EXECUTION_ERROR", "REQUEST_DEADLINE_EXCEEDED", "NODE_DEADLINE_EXCEEDED"]}, "resource_class": "cpu-test", "scheduling_key": "orchestration-v0246", "priority": priority, "asset_family": family}
    base["input_contract_hash"] = node_authority_input_contract_hash(base, payload) if payload else sha256_value({"node": node_id, "version": "0.24.6"})
    return base
...
def _negative_controls(request: dict[str, Any], dag: dict[str, Any], checkpoint: dict[str, Any], authority: dict[str, Any], *, execution_policy: dict[str, Any]) -> dict[str, Any]:
    ...
    controls.append(_expect_rejection("NC-HIST-05", "nonexistent historical root", "HISTORICAL_ROOT_EMPTY", lambda: compare_historical_evidence_tree(ROOT, V0241_AUTHORITY_HEAD, ["docs/evidence/does-not-exist-v0246"], label="v0241-missing-root")))
...
def run(output_dir: Path = EVIDENCE) -> dict[str, Any]:
    ...
    telemetry = sanitize_telemetry({"event": "orchestration-v0246-complete", "request_hash": request["request_hash"], "dag_hash": dag["dag_hash"], "state": snapshot_one["status"], "elapsed_seconds": snapshot_one["clock_elapsed_seconds"], "deadline_seconds": request["timeout_seconds"]})
    ...
    boundary = assert_provider_boundary(ROOT / "src/ugas/orchestration_runtime_v0246.py")
    ...
    observations = {
        ...
        "scheduler_dependency_first": all((__import__("ugas.orchestration_runtime_v0246", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(dep) < __import__("ugas.orchestration_runtime_v0246", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(node["node_id"])) for node in dag["nodes"] for dep in node["dependencies"]),
    ...
    _write(output_dir / "request-contract-v0246.json", {"request": request, "status": "PASS"})
    _write(output_dir / "dag-scheduler-v0246.json", {"dag": dag, "status": "PASS"})
    _write(output_dir / "execution-state-v0246.json", snapshot_one)
    _write(output_dir / "deadline-timeout-v0246.json", {"status": "PASS", "clock": snapshot_one["clock_elapsed_seconds"], "request_deadline_seconds": request["timeout_seconds"], "node_deadlines": snapshot_one["node_deadlines"], "probes": result["correction_probes"]})
    _write(output_dir / "checkpoint-resume-v0246.json", result["checkpoint"])
    _write(output_dir / "idempotency-coordinator-v0246.json", result["idempotency"])
    _write(output_dir / "circuit-breaker-runtime-v0246.json", result["circuit"])
    _write(output_dir / "dependency-authority-v0246.json", authority)
    _write(output_dir / "provider-boundary-v0246.json", result["provider"])
    _write(output_dir / "family-concurrency-v0246.json", batch)
    _write(output_dir / "historical-immutability-v0246.json", {"v0241": frozen_v0241, "v0242": frozen_v0242, "status": "PASS" if frozen_v0241.get("status") == "PASS" and frozen_v0242.get("status") == "PASS" else "FAIL"})
    _write(output_dir / "frozen-v0241-fingerprint-v0246.json", frozen_v0241)
    _write(output_dir / "hard-gates-v0246.json", gates)
    _write(output_dir / "negative-controls-v0246.json", controls)
    _write(output_dir / "full-slice-two-run-determinism-v0246.json", {"status": "PASS" if snapshot_one == snapshot_two else "FAIL", "run_1": snapshot_one, "run_2": snapshot_two})
    _write(output_dir / "production-boundary-v0246.json", production)
    _write(output_dir / "correction-history-v0246.json", {"schema_version": VERSION, "status": "CORRECTION_REQUIRED", "base_main_sha": BASE_MAIN_SHA, "reviewed_head": REJECTED_REVIEWED_HEAD, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "semantic_v0244_head": V0244_SEMANTIC_HEAD, "v0243_rejected_head": V0243_REJECTED_HEAD, "branch": BRANCH, "findings": ["F-43R", "F-44", "F-45", "F-46"], "preserves": ["F-31R", "F-32RR", "F-33", "F-34", "F-35RR", "F-35RC", "F-36", "F-38", "F-37R", "F-39", "F-40", "F-41", "F-42"], "historical_evidence_unchanged": True})
    _write(output_dir / "uads-handoff-v0246.json", uads_handoff)
    _write(output_dir / "execution-evidence-v0246.json", result)
    return result
