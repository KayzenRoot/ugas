"""Record truthful v0.17.0 unit, validation and gate totals."""
...
def main() -> int:
    ...
    for name in ("state", "equipment", "direction", "matrix", "front-compatibility", "workflow", "manifest", "security"):
    ...
    tests["schema_version"] = validation["schema_version"] = "0.17.0"
    (output / "test-results-v0170.json").write_text(json.dumps(tests, indent=2) + "\n", encoding="utf-8")
    (output / "validation-results-v0170.json").write_text(json.dumps(validation, indent=2) + "\n", encoding="utf-8")
    inputs = {
        ...
        "equipment_runtime": (None, args.equipment_exit_code),
    ...
    value = {"schema_version": "0.17.0", "overall_status": "PASS" if all(item["status"] == "PASS" for item in gates) else "FAIL", "gates": gates}
    (output / "gate-results-v0170.json").write_text(json.dumps(value, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0170_RESULTS_RECORDED", "tests": tests, "validation": validation, "gates": value}, ensure_ascii=False))
    return 0
