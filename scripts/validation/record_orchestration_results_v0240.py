"""Record bounded v0.24.0 test and official-validation totals from CI logs."""
...
def main() -> int:
    ...
    tests_text = _read(Path(args.tests_log)); validation_text = _read(Path(args.validation_log))
    ...
    (output / "test-results-v0240.json").write_text(json.dumps(test_record, indent=2) + "\n", encoding="utf-8")
    (output / "validation-results-v0240.json").write_text(json.dumps(validation_record, indent=2) + "\n", encoding="utf-8")
    statuses = [args.tests_exit, args.validation_exit, args.state_exit, args.matrix_exit, args.orchestration_exit, args.workflow_exit]
    print(json.dumps({"status": "PASS" if all(code == 0 for code in statuses) and test_record["status"] == "PASS" and validation_record["status"] == "PASS" else "FAIL", "tests": test_record, "validation": validation_record, "exit_codes": statuses}))
    return 0 if all(code == 0 for code in statuses) and test_record["status"] == "PASS" and validation_record["status"] == "PASS" else 1
