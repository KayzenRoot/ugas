"""Execute the complete deterministic UGAS v0.24.4 correction slice."""
...
EVIDENCE = ROOT / "docs/evidence/orchestration-runtime-v0244"
BASE_MAIN_SHA = "dee98f8cd89ebd83a36ead7a22a184700d6e916f"
REJECTED_REVIEWED_HEAD = "5a619c0b98ced7e4c09afd9ca117a039f0d5d068"
...
def _with_historical_candidate(mutator: Callable[[Path], None]) -> None:
    with tempfile.TemporaryDirectory(prefix="ugas-hist-v0244-") as directory:
        ...
        try:
            ...
            subprocess.run(["git", "-C", str(candidate), "commit", "-m", "ugas-v0244-historical-mutation", "--allow-empty"], check=True, capture_output=True)
...
def _no_git_authority_root() -> tempfile.TemporaryDirectory[str]:
    directory = tempfile.TemporaryDirectory(prefix="ugas-nogit-v0244-")
...
def _authority_mode_controls() -> list[dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-emptygit-v0244-") as directory:
    ...
    try:
        ...
        payload["ugas_v0244_semantic_probe"] = True
...
def _request(dependency_refs: list[dict[str, Any]] | None = None, *, timeout: int = 30) -> dict[str, Any]:
    ...
    return build_request(
        request_id="orchestration-v0244-fixture",
        ...
        idempotency_key="orchestration-v0244-fixture-idempotency",
...
def _node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 30, priority: int = 10, authority_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    base = {"node_id": node_id, "dependencies": dependencies or [], "operation": "EXECUTE", "dependency_hashes": {}, "timeout_seconds": timeout, "retry_policy": {"max_attempts": 2, "retryable_errors": ["TRANSIENT_EXECUTION_ERROR", "REQUEST_DEADLINE_EXCEEDED", "NODE_DEADLINE_EXCEEDED"]}, "resource_class": "cpu-test", "scheduling_key": "orchestration-v0244", "priority": priority, "asset_family": family}
    base["input_contract_hash"] = node_authority_input_contract_hash(base, payload) if payload else sha256_value({"node": node_id, "version": "0.24.4"})
    return base
...
def run(output_dir: Path = EVIDENCE) -> dict[str, Any]:
    ...
    telemetry = sanitize_telemetry({"event": "orchestration-v0244-complete", "request_hash": request["request_hash"], "dag_hash": dag["dag_hash"], "state": snapshot_one["status"], "elapsed_seconds": snapshot_one["clock_elapsed_seconds"], "deadline_seconds": request["timeout_seconds"]})
    ...
    boundary = assert_provider_boundary(ROOT / "src/ugas/orchestration_runtime_v0244.py")
    ...
    observations = {
        ...
        "scheduler_dependency_first": all((__import__("ugas.orchestration_runtime_v0244", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(dep) < __import__("ugas.orchestration_runtime_v0244", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(node["node_id"])) for node in dag["nodes"] for dep in node["dependencies"]),
        ...
        "historical_closure_binding": frozen_v0241["status"] == "PASS" and frozen_v0241["authority_commit"] == V0241_AUTHORITY_HEAD and frozen_v0242["status"] == "PASS" and frozen_v0242["authority_commit"] == V0242_AUTHORITY_HEAD,
    ...
    _write(output_dir / "request-contract-v0244.json", {"request": request, "status": "PASS"})
    _write(output_dir / "dag-scheduler-v0244.json", {"dag": dag, "status": "PASS"})
    _write(output_dir / "execution-state-v0244.json", snapshot_one)
    _write(output_dir / "deadline-timeout-v0244.json", {"status": "PASS", "clock": snapshot_one["clock_elapsed_seconds"], "request_deadline_seconds": request["timeout_seconds"], "node_deadlines": snapshot_one["node_deadlines"], "probes": result["correction_probes"]})
    _write(output_dir / "checkpoint-resume-v0244.json", result["checkpoint"])
    _write(output_dir / "idempotency-coordinator-v0244.json", result["idempotency"])
    _write(output_dir / "circuit-breaker-runtime-v0244.json", result["circuit"])
    _write(output_dir / "dependency-authority-v0244.json", authority)
    _write(output_dir / "provider-boundary-v0244.json", result["provider"])
    _write(output_dir / "family-concurrency-v0244.json", batch)
    _write(output_dir / "historical-immutability-v0244.json", {"v0241": frozen_v0241, "v0242": frozen_v0242, "status": "PASS" if frozen_v0241.get("status") == "PASS" and frozen_v0242.get("status") == "PASS" else "FAIL"})
    _write(output_dir / "frozen-v0241-fingerprint-v0244.json", frozen_v0241)
    _write(output_dir / "hard-gates-v0244.json", gates)
    _write(output_dir / "negative-controls-v0244.json", controls)
    _write(output_dir / "full-slice-two-run-determinism-v0244.json", {"status": "PASS" if snapshot_one == snapshot_two else "FAIL", "run_1": snapshot_one, "run_2": snapshot_two})
    _write(output_dir / "production-boundary-v0244.json", production)
    _write(output_dir / "correction-history-v0244.json", {"schema_version": VERSION, "status": "CORRECTION_REQUIRED", "base_main_sha": BASE_MAIN_SHA, "reviewed_head": REJECTED_REVIEWED_HEAD, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "branch": BRANCH, "findings": ["F-38", "F-37R", "F-39", "F-40"], "preserves": ["F-31R", "F-32RR", "F-33", "F-34", "F-35RR", "F-35RC", "F-36"], "historical_evidence_unchanged": True})
    _write(output_dir / "execution-evidence-v0244.json", result)
    return result
