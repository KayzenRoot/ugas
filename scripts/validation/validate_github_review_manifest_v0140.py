"""Fail-closed validation for the active v0.14.0 review manifest."""
...
def validate(manifest_path: Path, visual_path: Path, root: Path = ROOT) -> dict[str, Any]:
    failures: list[str] = []
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8")); schema = json.loads((root / "schemas/github-review-manifest-v0140.json").read_text(encoding="utf-8")); validate_schema_document(schema); validate_instance(manifest, schema); visual = json.loads(visual_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, ValueError, TypeError) as exc:
        return {"status": "V0140_GITHUB_REVIEW_MANIFEST_FAILED", "failures": [f"schema:{type(exc).__name__}:{exc}"]}
    ...
    if manifest.get("scope", {}).get("version") != "0.14.0" or manifest.get("scope", {}).get("phase") != "HIT_REACTION_FRONT": failures.append("active-scope-invalid")
    ...
    return {"status": "V0140_GITHUB_REVIEW_MANIFEST_PASSED" if not failures else "V0140_GITHUB_REVIEW_MANIFEST_FAILED", "failures": failures, "overall_status": manifest.get("overall_status"), "base_sha": pr.get("base_sha"), "head_sha": pr.get("head_sha"), "visual_count": visual_result.get("visual_count", 0)}
...
if __name__ == "__main__":
    ...
    print(json.dumps(result, indent=2, ensure_ascii=False)); raise SystemExit(0 if result["status"] == "V0140_GITHUB_REVIEW_MANIFEST_PASSED" else 1)
