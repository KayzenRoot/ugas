"""Validate bounded v0.23.4 artifact security boundaries."""
...
def main() -> int:
    ...
    try:
        ...
        if value.get("schema_version") != "0.23.4": failures.append("schema-version")
    ...
    result = {"schema_version": "0.23.4", "status": "PASS" if not failures else "FAIL", "failures": failures, "file_count": sum(1 for x in a.artifact_dir.rglob("*") if x.is_file())}; a.output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8"); print(json.dumps(result)); return 0 if not failures else 1
