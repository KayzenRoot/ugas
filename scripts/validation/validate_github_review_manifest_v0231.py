"""Fail-closed exact-head validator for the v0.23.1 review manifest."""
...
def main() -> int:
    ...
    try:
        ...
        if value.get("schema_version") != "0.23.1" or value.get("manifest_type") != "github-ci-vfx-asset-family-v0231-review": failures.append("manifest-identity-invalid")
        if pr.get("number") != 14 or pr.get("base_sha") != BASE_MAIN or pr.get("head_sha") != current or not pr.get("head_branch"): failures.append("exact-head-pr-binding-invalid")
        expected_scope = {"version": "0.23.1", "phase": "VFX_ASSET_FAMILY", "current_gate": "VFX_ASSET_FAMILY_RUNTIME_CORRECTION_F22_F28_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED", "baseline_main_sha": BASE_MAIN, "allowed_next_actions": ["external_review_vfx_asset_family_v0231"], "next_candidate": "ORCHESTRATION_RUNTIME_HARDENING", "new_generation": 0}; failures.extend(f"scope:{key}" for key, expected in expected_scope.items() if scope.get(key) != expected)
        ...
        if len(gates) != 32 or any(item.get("status") != "PASS" or type(item.get("observed")) is not bool or item.get("observed") is not True for item in gates.values()): failures.append("hard-gates")
        if len(controls) != 30 or any(item.get("status") != "PASS" or item.get("result") != "REJECT" or item.get("expected_rejection_class") != item.get("observed_rejection_class") for item in controls.values()): failures.append("negative-controls")
        ...
        for relative in ("docs/evidence/vfx-asset-family-runtime-v0230/", "REVIEW-v0.23.0.md", "docs/evidence/ui-asset-family-runtime-v0223/"):
    ...
    result = {"schema_version": "0.23.1", "status": "PASS" if not failures else "FAIL", "failures": failures, "checked_manifest": str(args.manifest)}; args.result_output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8"); print(json.dumps(result, ensure_ascii=False)); return 0 if not failures else 1
