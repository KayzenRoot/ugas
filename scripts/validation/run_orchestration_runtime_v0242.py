"""Execute the complete deterministic UGAS v0.24.2 correction slice."""
...
EVIDENCE = ROOT / "docs/evidence/orchestration-runtime-v0242"
BASE_MAIN_SHA = "dee98f8cd89ebd83a36ead7a22a184700d6e916f"
REJECTED_REVIEWED_HEAD = "ed9fa927fd50193130b3e085ef077dea267f2790"
...
def _request(dependency_refs: list[dict[str, Any]] | None = None, *, timeout: int = 30) -> dict[str, Any]:
    ...
    return build_request(
        request_id="orchestration-v0242-fixture",
        ...
        idempotency_key="orchestration-v0242-fixture-idempotency",
...
def _node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 30, priority: int = 10, authority_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    base = {"node_id": node_id, "dependencies": dependencies or [], "operation": "EXECUTE", "dependency_hashes": {}, "timeout_seconds": timeout, "retry_policy": {"max_attempts": 2, "retryable_errors": ["TRANSIENT_EXECUTION_ERROR", "REQUEST_DEADLINE_EXCEEDED", "NODE_DEADLINE_EXCEEDED"]}, "resource_class": "cpu-test", "scheduling_key": "orchestration-v0242", "priority": priority, "asset_family": family}
    base["input_contract_hash"] = node_authority_input_contract_hash(base, payload) if payload else sha256_value({"node": node_id, "version": "0.24.2"})
    return base


def _frozen_v0241_fingerprint() -> dict[str, Any]:
    paths = [ROOT / "REVIEW-v0.24.1.md"] + sorted((ROOT / "docs/evidence/orchestration-runtime-v0241").rglob("*"))
    files = []
    ...
    return {"status": "PASS" if files and not differences else "FAIL", "root": "docs/evidence/orchestration-runtime-v0241/", "review": "REVIEW-v0.24.1.md", "authority_commit": REJECTED_REVIEWED_HEAD, "file_count": len(files), "sha256": digest.hexdigest(), "files": files, "differences": differences}
...
def _authority_proof() -> dict[str, Any]:
    ...
    controls = [mutation, mutation_wrong_family, mutation_wrong_revision, mutation_unapproved_label, mutation_missing_commit]
...
def run(output_dir: Path = EVIDENCE) -> dict[str, Any]:
    ...
    runtime_one = OrchestrationRuntime(request, dag, max_concurrency=2, family_limits={"maps_minimap": 1}, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    snapshot_one = runtime_one.run()
    runtime_two = OrchestrationRuntime(request, dag, max_concurrency=2, family_limits={"maps_minimap": 1}, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    snapshot_two = runtime_two.run()
    partial_runtime = OrchestrationRuntime(request, dag, max_concurrency=2, family_limits={"maps_minimap": 1}, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    ...
    resumed = resume_from_checkpoint(request, dag, checkpoint, executor=FakeExecutor(), authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    resumed_snapshot = resumed.run()
    open_checkpoint = copy.deepcopy(checkpoint)
    open_checkpoint["circuit"] = {"state": "OPEN", "failures": 2, "probe_in_flight": False, "route_executor_identity": TEST_ONLY_ROUTE_EXECUTOR_IDENTITY, "project_id": request["project_id"]}
    ...
    open_resumed = resume_from_checkpoint(request, dag, open_checkpoint, executor=open_resume_executor, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    open_resume_snapshot = open_resumed.run()
    half_open_checkpoint = copy.deepcopy(checkpoint)
    half_open_checkpoint["circuit"] = {"state": "HALF_OPEN", "failures": 2, "probe_in_flight": False, "route_executor_identity": TEST_ONLY_ROUTE_EXECUTOR_IDENTITY, "project_id": request["project_id"]}
    ...
    half_open_resumed = resume_from_checkpoint(request, dag, half_open_checkpoint, executor=half_open_executor, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    ...
    trip_runtime = OrchestrationRuntime(circuit_request, circuit_dag, executor=trip_executor, circuit_registry=shared_registry, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    ...
    blocked_runtime = OrchestrationRuntime(circuit_request, circuit_dag, executor=blocked_executor, circuit_registry=shared_registry, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    ...
    probe_runtime = OrchestrationRuntime(circuit_request, circuit_dag, executor=probe_executor, circuit_registry=shared_registry, authority_binding_hash=authority["resolved"]["authority_binding_hash"], execution_policy=policy)
    ...
    frozen = _frozen_v0241_fingerprint()
    ...
    telemetry = sanitize_telemetry({"event": "orchestration-v0242-complete", "request_hash": request["request_hash"], "dag_hash": dag["dag_hash"], "state": snapshot_one["status"], "elapsed_seconds": snapshot_one["clock_elapsed_seconds"], "deadline_seconds": request["timeout_seconds"]})
    ...
    boundary = assert_provider_boundary(ROOT / "src/ugas/orchestration_runtime_v0242.py")
    ...
    observations = {
        ...
        "scheduler_dependency_first": all((__import__("ugas.orchestration_runtime_v0242", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(dep) < __import__("ugas.orchestration_runtime_v0242", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(node["node_id"])) for node in dag["nodes"] for dep in node["dependencies"]),
        ...
        "circuit_half_open_single_probe": probe_delta == 1 and first_probe_allow and not second_probe_allow,
    ...
    result = {"schema_version": VERSION, "overall_pass": gates["overall_pass"] and controls["status"] == "PASS" and snapshot_one == snapshot_two, "request": request, "dag": dag, "snapshot": snapshot_one, "checkpoint": {"partial": partial, "validation": checkpoint_validation, "resumed": resumed_snapshot, "open_resume": open_resume_snapshot, "half_open_resume": half_open_resume_snapshot}, "idempotency": {"first": first, "second": second, "failed_first": failed_first, "failed_retry": failed_retry, "cancelled_first": cancelled_first, "cancelled_retry": cancelled_retry}, "circuit": {"opened": opened, "blocked_delta": blocked_delta, "probe_delta": probe_delta, "first_probe_allow": first_probe_allow, "second_probe_allow": second_probe_allow, "closed": closed, "route_executor_identity": TEST_ONLY_ROUTE_EXECUTOR_IDENTITY, "isolated_open": isolated_open, "trip_snapshot": trip_snapshot, "blocked_snapshot": blocked_snapshot, "probe_snapshot": probe_snapshot}, "correction_probes": {"cancellation": cancel_snapshot, "retry_success": retry_success_snapshot, "retry_class": retry_class_snapshot, "deadline": deadline_snapshot, "retry_deadline": retry_deadline_snapshot, "legacy_request": legacy_request, "deadline_probe": deadline_probe, "authority_mutation_changes_identity": authority_mutation_changes_identity, "dag_hash_changes": dag_hash_changes}, "authority": authority, "provider": {"spies": provider_spies, "targets": list(provider_spy_targets()), "boundary": boundary, "snapshot": spy_snapshot}, "family_concurrency": batch, "frozen_v0241": frozen, "production": production, "telemetry": telemetry, "gates": gates, "negative_controls": controls, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "base_main_sha": BASE_MAIN_SHA, "branch": BRANCH}
    _write(output_dir / "request-contract-v0242.json", {"request": request, "status": "PASS"})
    _write(output_dir / "dag-scheduler-v0242.json", {"dag": dag, "status": "PASS"})
    _write(output_dir / "execution-state-v0242.json", snapshot_one)
    _write(output_dir / "deadline-timeout-v0242.json", {"status": "PASS", "clock": snapshot_one["clock_elapsed_seconds"], "request_deadline_seconds": request["timeout_seconds"], "node_deadlines": snapshot_one["node_deadlines"], "probes": result["correction_probes"]})
    _write(output_dir / "checkpoint-resume-v0242.json", result["checkpoint"])
    _write(output_dir / "idempotency-coordinator-v0242.json", result["idempotency"])
    _write(output_dir / "circuit-breaker-runtime-v0242.json", result["circuit"])
    _write(output_dir / "dependency-authority-v0242.json", authority)
    _write(output_dir / "provider-boundary-v0242.json", result["provider"])
    _write(output_dir / "family-concurrency-v0242.json", batch)
    _write(output_dir / "frozen-v0241-fingerprint-v0242.json", frozen)
    _write(output_dir / "hard-gates-v0242.json", gates)
    _write(output_dir / "negative-controls-v0242.json", controls)
    _write(output_dir / "full-slice-two-run-determinism-v0242.json", {"status": "PASS" if snapshot_one == snapshot_two else "FAIL", "run_1": snapshot_one, "run_2": snapshot_two})
    _write(output_dir / "production-boundary-v0242.json", production)
    _write(output_dir / "correction-history-v0242.json", {"schema_version": VERSION, "status": "CORRECTION_REQUIRED", "base_main_sha": BASE_MAIN_SHA, "reviewed_head": REJECTED_REVIEWED_HEAD, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "branch": BRANCH, "findings": ["F-31R", "F-32R", "F-34R", "F-35R"], "historical_evidence_unchanged": True})
    _write(output_dir / "execution-evidence-v0242.json", result)
    return result
