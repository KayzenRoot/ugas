...
def main() -> int:
    ...
    if value["version"] not in {"0.20.3", "0.21.0", "0.21.1", "0.21.2", "0.21.3", "0.22.0", "0.22.1", "0.22.2", "0.22.3", "0.23.0", "0.23.1", "0.23.2", "0.23.3", "0.23.4", "0.24.0", "0.24.1"}:
        failures.append("active-matrix-version-must-be-v0203-through-v0241")
    ...
    if value["version"] in {"0.24.0", "0.24.1"}:
        vfx = next(item for item in capabilities if item["id"] == "vfx_asset_family")
        orchestration = next(item for item in capabilities if item["id"] == "orchestration_runtime_hardening")
