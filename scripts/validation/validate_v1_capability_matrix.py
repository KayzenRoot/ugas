...
def main() -> int:
    ...
    if value["version"] not in {"0.20.3", "0.21.0", "0.21.1", "0.21.2", "0.21.3", "0.22.0", "0.22.1", "0.22.2", "0.22.3", "0.23.0", "0.23.1", "0.23.2", "0.23.3", "0.23.4"}:
        failures.append("active-matrix-version-must-be-v0203-through-v0234")
    ui = next(item for item in capabilities if item["id"] == "ui_asset_family")
    if death["status"] != "APPROVED_PILOT" or direction["status"] != "APPROVED_FOUNDATION" or equipment["status"] != "APPROVED_FOUNDATION" or creatures["status"] != "APPROVED_FOUNDATION" or items["status"] != "APPROVED_FOUNDATION" or environment["status"] not in {"APPROVED_FOUNDATION", "TECHNICALLY_QUALIFIED_FOUNDATION; QA GOVERNANCE INTEGRITY CORRECTION; EXTERNAL REVIEW REQUIRED"} or value["next_candidate"] not in {"ENVIRONMENT_TILESETS", "MAPS_MINIMAP", "UI_ASSET_FAMILY", "ORCHESTRATION_RUNTIME_HARDENING"}:
    ...
    if value["version"] in {"0.23.0", "0.23.1", "0.23.2", "0.23.3", "0.23.4"}:
        ...
        if maps["status"] != "APPROVED_FOUNDATION" or ui["status"] != "APPROVED_FOUNDATION; MERGED_CLOSED" or not vfx["status"].startswith("TECHNICALLY_QUALIFIED_FOUNDATION") or orchestration["status"] != "Blocked until VFX merged closed and post-merge main CI succeeds" or value["next_candidate"] != "ORCHESTRATION_RUNTIME_HARDENING":
