"""Enforce the final fail-closed v0.21.0 review result after upload."""
...
def main() -> int:
    ...
    if manifest.get("overall_status") != "PASS" or manifest.get("scope", {}).get("version") != "0.21.0": failures.append("manifest-not-pass-or-scope-invalid")
    ...
    print(json.dumps({"status": "PASS" if not failures else "FAIL", "failures": failures, "stop": "PR_OPEN" if not failures else "REVIEW_BLOCKED"})); return 0 if not failures else 1
