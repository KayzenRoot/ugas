"""Build the bounded v0.16.1 multi-direction runtime correction manifest."""
...
EVIDENCE = "docs/evidence/multi-direction-runtime-v0161"
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "correction_history": {"version": "0.16.0", "status": "CORRECTION_REQUIRED", "rejected_head": REJECTED_HEAD, "record": f"{EVIDENCE}/v0160-correction-record-v0161.json"},
        "visual_manifest": "visual-manifest.json",
        "direction_runtime_evidence": {"contract": f"{EVIDENCE}/direction-contract-v0161.json", "coverage_manifest": f"{EVIDENCE}/coverage-manifest-v0161.json", "validation": f"{EVIDENCE}/validation-evidence-v0161.json", "negative_controls": f"{EVIDENCE}/negative-controls-v0161.json", "invalid_vector_qa": f"{EVIDENCE}/invalid-vector-qa-v0161.json", "test_only_production_safety_qa": f"{EVIDENCE}/test-only-production-safety-qa-v0161.json", "state_consistency": f"{EVIDENCE}/state-consistency-v0161.json", "fixture_manifest": f"{EVIDENCE}/synthetic-fixture-manifest-v0161.json", "contact_sheet": f"{EVIDENCE}/synthetic-direction-contact-sheet-v0160.png"},
...
def main() -> int:
    ...
    manifest = build(args)
    (output / "github-review-manifest-v0161.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (output / "visual-manifest.json").write_text(json.dumps(manifest["visual_transport"], indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0161_GITHUB_REVIEW_MANIFEST_BUILT", "base_sha": manifest["pull_request"]["base_sha"], "head_sha": manifest["pull_request"]["head_sha"]}, ensure_ascii=False))
    return 0
