"""Run the real, fail-closed v0.16.1 direction-runtime correction gates."""
...
OUT = ROOT / "docs/evidence/multi-direction-runtime-v0161"
...
from ugas.state_consistency_v0161 import validate_state_consistency  # noqa: E402
...
def main() -> int:
    ...
    coverage_path = OUT / "coverage-manifest-v0161.json"
    coverage = json.loads(coverage_path.read_text(encoding="utf-8"))
    schema = json.loads((ROOT / "schemas/direction-runtime-v0161.json").read_text(encoding="utf-8"))
    contract = json.loads((OUT / "direction-contract-v0161.json").read_text(encoding="utf-8"))
    gates: dict[str, bool] = {}
    ...
    gate("zero_vector_does_not_guess", normalize_direction((0, 0)) is None and retained.direction == "west" and retained.outcome == "ZERO_VECTOR_RETAINED_FACING", "only a finite numeric zero vector may use explicit retained facing")
    invalid_cases = [(math.nan, 0), (math.inf, 0), (-math.inf, 0), ("1", 0), {"dx": 1}, (1, 0, 2)]
    gate("invalid_vectors_never_retain", all(normalize_direction_result(value, retained_facing="west").outcome == "INVALID_VECTOR_UNRESOLVED" for value in invalid_cases), "non-finite, nonnumeric, missing and malformed vectors remain unresolved even with retained facing")
    ...
    fixture = json.loads((OUT / "synthetic-fixture-manifest-v0161.json").read_text(encoding="utf-8"))
    ...
    _write("direction-quantization-v0161.json", {"status": "DIRECTION_QUANTIZATION_PASSED" if gates["vector_quantization_deterministic"] else "DIRECTION_QUANTIZATION_FAILED", "contract": direction_contract(), "vectors": [{"dx": vector[0], "dy": vector[1], "direction": result} for vector, result in zip(vectors, first)]})
    _write("alias-mapping-v0161.json", {"status": "DIRECTION_ALIAS_MAPPING_PASSED" if gates["alias_mapping_unambiguous"] else "DIRECTION_ALIAS_MAPPING_FAILED", "aliases": aliases})
    _write("fallback-qa-v0161.json", {"status": "DIRECTION_FALLBACK_QA_PASSED" if gates["fallback_is_explicit_and_nonproduction"] and gates["missing_direction_fails_closed"] else "DIRECTION_FALLBACK_QA_FAILED", "missing": missing.to_dict(), "preview": preview.to_dict()})
    _write("mirror-qa-v0161.json", {"status": "DIRECTION_MIRROR_QA_PASSED" if gates["mirror_requires_explicit_permission"] and gates["mirror_unsafe_asymmetry_rejected"] else "DIRECTION_MIRROR_QA_FAILED", "implicit": implicit_mirror.to_dict(), "unsafe_explicit": unsafe_mirror.to_dict(), "safe_explicit": safe_mirror.to_dict()})
    _write("cache-key-qa-v0161.json", {"status": "DIRECTION_CACHE_KEY_QA_PASSED" if gates["direction_in_cache_key"] else "DIRECTION_CACHE_KEY_QA_FAILED", "keys": list(resolver.cache_keys())})
    _write("provenance-qa-v0161.json", {"status": "DIRECTION_PROVENANCE_QA_PASSED" if gates["asset_hash_matches_manifest"] and gates["capability_and_direction_match_asset_metadata"] else "DIRECTION_PROVENANCE_QA_FAILED", "coverage": coverage_result})
    _write("fixture-qa-v0161.json", {"status": "TEST_ONLY_SYNTHETIC_DIRECTION_FIXTURE_QA_PASSED" if gates["synthetic_fixture_not_in_production_registry"] else "TEST_ONLY_SYNTHETIC_DIRECTION_FIXTURE_QA_FAILED", "manifest": fixture})
    _write("invalid-vector-qa-v0161.json", {"status": "INVALID_VECTOR_QA_PASSED" if gates["invalid_vectors_never_retain"] else "INVALID_VECTOR_QA_FAILED", "retained_facing": "west", "cases": [{"input": value, "result": normalize_direction_result(value, retained_facing="west").to_dict()} for value in invalid_cases], "zero_vector": retained.to_dict(), "policy": "ZERO_VECTOR_RETAINED_FACING is reserved for finite numeric zero; all invalid vectors use INVALID_VECTOR_UNRESOLVED"})
    ...
    gate("test_only_exact_resolution_not_production_safe", test_only_result.asset_id == "qa-south" and test_only_result.production_safe is False, "test-only exact matches remain non-production-safe in non-production resolver")
    test_only_production_error: str | None = None
    try:
        DirectionResolver([test_only_asset], production_registry=True)
    except DirectionManifestError as exc:
        test_only_production_error = str(exc)
    gate("test_only_production_registry_rejected", test_only_production_error == "test_only_fixture_cannot_enter_production_registry", "production registry rejects test-only fixtures")
    _write("test-only-production-safety-qa-v0161.json", {"status": "TEST_ONLY_PRODUCTION_SAFETY_QA_PASSED" if gates["test_only_exact_resolution_not_production_safe"] and gates["test_only_production_registry_rejected"] else "TEST_ONLY_PRODUCTION_SAFETY_QA_FAILED", "non_production_exact": test_only_result.to_dict(), "production_registry_error": test_only_production_error})
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-v0161-nc-") as directory:
        mutated_manifest = Path(directory) / "coverage-manifest-v0161.json"
    ...
    blocked_result = validate_state_consistency(mutated_state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.16.1.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"))
    ...
    _write("negative-controls-v0161.json", {"status": negative_status, "control_count": len(negative), "controls": negative, "method": "each control constructs or mutates runtime input and records the observed rejection"})
    gates["negative_controls_reject"] = negative_status == "DIR_NC_01_TO_12_PASSED"
    _write("resolution-evidence-v0161.json", {"status": "DIRECTION_RESOLUTION_EVIDENCE_PASSED" if gates["front_to_south_backward_compatibility"] else "DIRECTION_RESOLUTION_EVIDENCE_FAILED", "all_canonical_directions": {direction: resolver.resolve("death_animation_front", direction).to_dict() for direction in CANONICAL_DIRECTIONS}, "front_alias": front.to_dict()})
    result = {"schema_version": "0.16.1", "status": "MULTI_DIRECTION_ANIMATION_RUNTIME_INTEGRITY_TECHNICALLY_QUALIFIED" if all(gates.values()) else "MULTI_DIRECTION_ANIMATION_RUNTIME_INTEGRITY_FAILED", "gates": {name: {"passed": passed, "detail": details.get(name, "")} for name, passed in gates.items()}, "passed": sum(gates.values()), "failed": sum(not value for value in gates.values()), "production_coverage": ["south"], "production_approved": False, "production_routing": "BLOCKED", "new_generation": 0, "negative_controls": {"status": negative_status, "count": len(negative)}}
    _write("validation-evidence-v0161.json", result)
