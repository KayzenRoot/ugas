...
def _v0201_checks() -> None:
    ...
    try:
        ...
        if state.get("version") == "0.20.1":
            ...
            check("v0201:state-forward-advanced", state.get("version") in {"0.21.0", "0.21.1", "0.21.2", "0.21.3", "0.22.0", "0.22.1", "0.22.2", "0.22.3", "0.23.0", "0.23.1"} and state.get("environment_tilesets") == "APPROVED_FOUNDATION", "v0.20.1 active state advanced forward")
        fixture_root = evidence_root / "fixture"
...
def _v0202_checks() -> None:
    ...
    try:
        ...
        if state.get("version") == "0.20.2":
            ...
            check("v0202:state-forward-advanced", state.get("version") in {"0.21.0", "0.21.1", "0.21.2", "0.21.3", "0.22.0", "0.22.1", "0.22.2", "0.22.3", "0.23.0", "0.23.1"} and state.get("environment_tilesets") == "APPROVED_FOUNDATION", "v0.20.2 active state advanced forward")
        fixture_root = evidence_root / "fixture"
...
def _v0220_checks() -> None:
    ...
    try:
        ...
        if state.get("version") == "0.22.0":
            ...
            check("v0220:state-forward-advanced", state.get("version") in {"0.22.1", "0.22.2", "0.22.3", "0.23.0", "0.23.1"} and state.get("correction_history", {}).get("v0.22.0", {}).get("historical_evidence_unchanged") is True, "v0.22.0 state/evidence is preserved while active state advances")
...
def _v0230_checks() -> None:
    ...
    try:
        ...
        validate_schema_document(schema)
        check("v0230:state-consistency", state.get("version") == "0.23.1" and state.get("correction_history", {}).get("v0.23.0", {}).get("historical_evidence_unchanged") is True, "v0.23.0 is preserved as immutable history while active state advances")
...
def main() -> int:
    ...
    check("version:consistency", UGAS_VERSION == package_version == pyproject_version == init_version == current_state_version == "0.23.1", f"runtime={UGAS_VERSION}, package={package_version}, pyproject={pyproject_version}, current_state={current_state_version}")
    check("version:maps-schema", active_maps_schema_version == "0.21.3", f"active maps/minimap schema={active_maps_schema_version}")
    docs = ["README.md", "INSTALL.md", "CHECKPOINT.md", "REVIEW-v0.23.1.md", "docs/2d-master-pipeline.md", "docs/comfyui.md", "docs/roadmap.md"]
    check("docs:version", all(UGAS_VERSION in (ROOT / path).read_text(encoding="utf-8") for path in docs), "current operational docs identify 0.23.1")
