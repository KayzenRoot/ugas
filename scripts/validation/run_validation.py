...
def _v0190_checks() -> None:
    ...
    try:
        ...
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        check("v0190:capability-matrix", matrix.get("next_candidate") == "ITEMS_PROPS" and next(item for item in matrix["capabilities"] if item["id"] == "creatures_monsters")["status"] == "APPROVED_FOUNDATION", "matrix retains the items/props candidate while v0.19.0 remains historical")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
...
def _v0191_checks() -> None:
    ...
    try:
        ...
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        check("v0191:capability-matrix", matrix.get("version") == "0.19.1" and matrix.get("next_candidate") == "ITEMS_PROPS" and next(item for item in matrix["capabilities"] if item["id"] == "creatures_monsters")["status"] == "APPROVED_FOUNDATION" and "TECHNICALLY QUALIFIED" in next(item for item in matrix["capabilities"] if item["id"] == "items_props")["status"], "matrix records the v0.19.1 items/props correction")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
