"""Objective UGAS validation, including immutable history and active v0.13.0."""
...
def _v0130_checks() -> None:
    """Validate the active deterministic RUN_FRONT_V1 slice."""
    required = [
    ...
    for relative in required:
        path = ROOT / relative
        check(f"v0130:path:{relative}", path.is_file(), "present" if path.is_file() else "missing")
    try:
        ...
        consistency = validate_state_consistency_v0130(
            state,
        ...
        check("v0130:state-consistency", consistency["status"] == state["current_gate"] and consistency["failures"] == [], "; ".join(consistency["failures"]) or "active v0.13.0 state is consistent")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0130:state-consistency", False, str(exc))
    ...
    check("v0130:contract", contract.get("capability") == "run_front_v1" and contract.get("dependencies", {}).get("matrix_capability_count") == 16 and contract.get("review_policy", {}).get("external_visual") == "REQUIRED" and contract.get("review_policy", {}).get("production_routing") == "BLOCKED" and len(contract.get("negative_controls", [])) == 8, "RUN_FRONT_V1 contract is dependency, scope and review bound")
    spec = load_json(ROOT / "profiles/animation/run-front-v1.json")
    check("v0130:profile", spec.get("frame_count") == 8 and spec.get("fps") == 12 and spec.get("loop") is True and spec.get("direction") == "front" and len(spec.get("motion_tracks", [])) == 12 and spec.get("provenance", {}).get("source_only_pixels") is True and spec.get("provenance", {}).get("comfyui_generation_jobs") == 0, "front run profile has eight deterministic source-only frames and twelve frozen tracks")
    qa = load_json(ROOT / "docs/evidence/animation-runtime-v0130/run-front-v1/qa-result.json")
    check("v0130:qa", qa.get("decision") == "QUALIFIED" and qa.get("failures") == [] and all(qa.get("hard_gates", {}).values()) and all(qa.get("temporal", {}).get("hard_gates", {}).values()), "all RUN_FRONT_V1 frame and temporal gates pass")
    package = load_json(ROOT / "docs/evidence/animation-runtime-v0130/run-front-v1/package-manifest.json")
    check("v0130:package", package.get("qa_decision") == "QUALIFIED" and package.get("production_routing") == "BLOCKED" and package.get("format") == "RGBA" and len(package.get("event_markers", [])) == 9, "package is bound to qualified QA, RGBA frames and phase markers")
    negative = load_json(ROOT / "docs/evidence/animation-runtime-v0130/run-front-gate-negative-controls-v0130.json")
    check("v0130:negative-controls", negative.get("status") == "NC_01_TO_NC_08_PASSED" and len(negative.get("controls", {})) == 8 and all(item.get("status") == "REJECTED" for item in negative.get("controls", {}).values()), "all eight mutated fixtures reject at their intended gate")
    execution = load_json(ROOT / "docs/evidence/animation-runtime-v0130/execution-evidence-v0.13.0.json")
    check("v0130:execution-boundary", execution.get("source_only_pixels") is True and execution.get("new_generation") == 0 and execution.get("external_visual") == "REQUIRED" and execution.get("next_capability_started") is False and execution.get("approved_assets_untouched") == "APPROVED_ASSETS_UNTOUCHED", "execution evidence preserves source-only and external-review boundaries")
...
def main() -> int:
    ...
    check("version:consistency", UGAS_VERSION == package_version == pyproject_version == init_version == "0.13.0", f"runtime={UGAS_VERSION}, package={package_version}, pyproject={pyproject_version}")
    docs = ["README.md", "INSTALL.md", "CHECKPOINT.md", "REVIEW-v0.13.0.md", "docs/2d-master-pipeline.md", "docs/comfyui.md", "docs/roadmap.md"]
    check("docs:version", all(UGAS_VERSION in (ROOT / path).read_text(encoding="utf-8") for path in docs), "current operational docs identify 0.13.0")
