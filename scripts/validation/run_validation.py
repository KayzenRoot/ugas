...
from ugas.state_consistency_v0220 import validate_state_consistency as validate_state_consistency_v0220
from ugas.state_consistency_v0221 import validate_state_consistency as validate_state_consistency_v0221
from ugas.state_consistency_v0222 import validate_state_consistency as validate_state_consistency_v0222
from ugas.state_consistency_v0223 import validate_state_consistency as validate_state_consistency_v0223
from ugas.state_consistency_v0230 import validate_state_consistency as validate_state_consistency_v0230
from ugas.state_consistency_v0231 import validate_state_consistency as validate_state_consistency_v0231
from ugas.state_consistency_v0232 import validate_state_consistency as validate_state_consistency_v0232
from ugas.state_consistency_v0233 import validate_state_consistency as validate_state_consistency_v0233
from ugas.state_consistency_v0234 import validate_state_consistency as validate_state_consistency_v0234
from ugas.state_consistency_v0240 import validate_state_consistency as validate_state_consistency_v0240
from ugas.state_consistency_v0241 import validate_state_consistency as validate_state_consistency_v0241
from ugas.state_consistency_v0242 import validate_state_consistency as validate_state_consistency_v0242
...
from ugas.state_consistency_v0251 import definition_of_done_hard_gate_count as definition_of_done_hard_gate_count_v0251
...
def snapshot_check() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-v051-snapshot-") as directory:
        ...
        for name, command in (
            ...
            # exit status and cannot manufacture a green result.
...
def _v0181_checks() -> None:
    """Validate frozen rejected v0.18.1 evidence without regenerating it."""
    ...
    required = [
        ...
        "docs/evidence/current-state-v0.18.1.json", "docs/evidence/current-state-v0.18.0.json", "docs/evidence/current-state-v0171.json",
    ...
    try:
        ...
        state = load_json(ROOT / "docs/evidence/current-state-v0.18.1.json")
...
def _v0182_checks() -> None:
    ...
    try:
        ...
        def strict(controls: dict[str, Any]) -> bool:
            return bool(controls) and all(item.get("rejected") is True and item.get("passed") is True and item.get("status") == "REJECTED" and item.get("observed", {}).get("result") == "REJECTED" and item["observed"].get("error_code") == item.get("expected_error_code") and item["observed"].get("rejection_class") == item.get("expected_rejection_class") for item in controls.values())
        gates = execution.get("gates", {}); check("v0182:execution", execution.get("status") == "CREATURES_MONSTERS_DERIVED_VARIANT_AND_STATE_CONTRACT_TECHNICALLY_QUALIFIED" and execution.get("failed") == 0 and execution.get("production_routing") == "BLOCKED" and execution.get("new_generation") == 0 and all(item.get("status") == "PASS" for item in gates.values()), "all v0.18.2 hard gates pass")
        check("v0182:gate-set", {"effective_variant_validation", "effective_direction_asset_revision_consistent", "locomotion_state_matches_archetype", "derived_revision_identity", "direction_hash_preservation", "derived_negative_controls_strict", "canonical_negative_controls_strict", "isolated_two_run_determinism", "production_registry_empty", "state_consistency"}.issubset(gates), "effective variant, revision and state gates are present")
        check("v0182:canonical-negative-controls", negative.get("status") == "CR_NC_01_TO_15_PASSED" and len(negative.get("controls", {})) == 15 and strict(negative.get("controls", {})), "CR-NC-01..15 remain strict")
        check("v0182:derived-negative-controls", derived_negative.get("status") == "DERIVED_VARIANT_NEGATIVE_CONTROLS_PASSED" and len(derived_negative.get("controls", {})) == 6 and strict(derived_negative.get("controls", {})), "DV-NC-01..06 reject after real materialization")
        check("v0182:state-controls", supplemental.get("status") == "ARCHETYPE_STATE_COMPATIBILITY_PASSED" and supplemental.get("controls", {}).get("SUP-NC-07", {}).get("observed", {}).get("error_code") == "LOCOMOTION_STATE_ARCHETYPE_MISMATCH" and supplemental.get("controls", {}).get("SUP-NC-08", {}).get("observed", {}).get("error_code") == "CREATURE_STATE_UNSUPPORTED", "moving and stationary locomotion boundaries are strict")
        check("v0182:directions", len(direction.get("records", [])) == 48 and len({item.get("direction_asset_id") for item in direction.get("records", [])}) == 48 and all(item.get("sha256") == item.get("direction_content_hash") == item.get("frozen_v0181_sha256") for item in direction.get("records", [])), "48 direction pixels/hashes remain frozen-identical")
        check("v0182:state-routes", len(state_routes.get("records", [])) == 30 and all(item.get("availability") == "REQUIRED" for item in state_routes.get("records", []) if item.get("state") == "locomotion" and item.get("archetype") != "stationary_structure"), "moving archetypes carry locomotion route/timing")
        check("v0182:fixtures", fixture.get("fixture_count") == 48 and fixture.get("unique_hash_count") == 48 and fixture.get("production_registry") is False and all(item.get("test_only") is True and item.get("production_safe") is False for item in fixture.get("fixtures", [])), "synthetic fixtures remain TEST_ONLY")
        check("v0182:determinism", determinism.get("status") == "TWO_RUN_DETERMINISM_PASSED" and determinism.get("second_run_reads_first_run") is False and determinism.get("mutated_control_error_code") == "NONDETERMINISTIC_SECOND_FIXTURE_OUTPUT", "isolated determinism and mutation comparator pass")
...
def _v0201_checks() -> None:
    ...
    try:
        ...
        check("v0201:capability-matrix", (matrix.get("version") == "0.20.1" and environment.get("status", "").startswith("TECHNICALLY_QUALIFIED_FOUNDATION") and matrix.get("next_candidate") == "ENVIRONMENT_TILESETS") or (matrix.get("version") in {"0.21.0", "0.21.1", "0.21.2", "0.21.3", "0.22.0", "0.22.1", "0.22.2", "0.22.3"} and environment.get("status") == "APPROVED_FOUNDATION" and matrix.get("next_candidate") == "UI_ASSET_FAMILY") or (matrix.get("version") in {"0.23.0", "0.23.1", "0.23.2", "0.23.3", "0.23.4"} and environment.get("status") == "APPROVED_FOUNDATION" and matrix.get("next_candidate") == "ORCHESTRATION_RUNTIME_HARDENING") or (matrix.get("version") in {"0.24.0", "0.24.1", "0.24.2", "0.24.3", "0.24.4", "0.24.5", "0.24.6", "0.24.7", "0.25.0"} and environment.get("status") == "APPROVED_FOUNDATION" and matrix.get("next_candidate") == "V1_FINAL_ACCEPTANCE"), "capability matrix preserves v0.20.1 history while allowing forward advancement")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
...
def _v0212_checks() -> None:
    """Validate the active v0.21.2 Maps/Minimap raster and governance correction."""
    evidence_root = ROOT / "docs/evidence/maps-minimap-runtime-v0212"
    required = [
        "REVIEW-v0.21.2.md", "schemas/current-state-v0212.json", "schemas/maps-minimap-runtime-v0212.json",
        "src/ugas/maps_minimap_runtime_v0210.py", "src/ugas/state_consistency_v0212.py",
        "scripts/validation/run_maps_minimap_runtime_v0210.py", "scripts/validation/validate_state_consistency_v0212.py",
        "tests/test_maps_minimap_runtime_v0210.py", "docs/evidence/current-state-v0212.json",
        "docs/evidence/github-governance-v0210/v0203-external-approval.json",
        "docs/evidence/maps-minimap-runtime-v0212/v0.21.1-rejection-correction-record-v0212.json",
    ]
    required += [f"docs/evidence/maps-minimap-runtime-v0212/{name}" for name in (
        "map-contract-v0212.json", "environment-authority-bindings-v0212.json", "items-props-authority-bindings-v0212.json",
        "layer-cell-matrix-v0212.json", "chunk-partition-roundtrip-v0212.json", "regions-zones-v0212.json", "marker-contract-v0212.json",
        "minimap-projection-v0212.json", "visibility-state-qa-v0212.json", "cache-identity-v0212.json", "provenance-v0212.json",
        "raster-geometry-v0212.json", "hard-gates-v0212.json", "gate-specific-proof-v0212.json", "negative-controls-v0212.json",
        "full-slice-two-run-determinism-v0212.json", "production-registry-v0212.json", "historical-immutability-v0212.json",
        "test-only-fixture-manifest-v0212.json", "execution-evidence-v0212.json", "capability-matrix-validation-v0212.json",
    )]
    for relative in required:
        path = ROOT / relative
        check(f"v0212:path:{relative}", path.is_file(), "present" if path.is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state-v0212.json")
        state_schema = load_json(ROOT / "schemas/current-state-v0212.json")
        validate_schema_document(state_schema); validate_instance(state, state_schema)
        consistency = validate_state_consistency_v0212(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.21.2.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"))
        check("v0212:state-consistency", consistency["status"] == state["current_gate"] and not consistency["failures"], "; ".join(consistency["failures"]) or "active v0.21.2 state is consistent")
        execution = load_json(evidence_root / "execution-evidence-v0212.json")
        validate_instance(execution, load_json(ROOT / "schemas/maps-minimap-runtime-v0212.json"))
        gates = load_json(evidence_root / "hard-gates-v0212.json")
        proof = load_json(evidence_root / "gate-specific-proof-v0212.json")
        expected_gates = {"map_schema_valid", "map_dimensions_and_world_metrics_valid", "environment_authority_binding_valid", "items_props_authority_binding_valid", "layer_contract_valid", "cell_coordinates_in_bounds", "chunk_partition_exact", "chunk_roundtrip_valid", "regions_zones_in_bounds", "marker_identity_valid", "minimap_projection_valid", "raster_geometry_valid", "minimap_inverse_projection_valid", "visibility_mask_dimensions_valid", "minimap_derived_from_map_identity", "map_provenance_valid", "cache_identity_complete", "stale_cache_cross_map_chunk_projection_rejected", "test_fixture_nonproduction", "production_registry_empty", "production_routing_blocked", "isolated_full_slice_determinism"}
        gate_records = gates.get("gates", {})
        check("v0212:execution", execution.get("status") == "MAPS_MINIMAP_RASTER_GOVERNANCE_INTEGRITY_TECHNICALLY_QUALIFIED" and execution.get("hard_gate_count") == 22 and execution.get("negative_control_count") == 29 and execution.get("production_routing") == "BLOCKED" and execution.get("new_generation") == 0, "raster/governance correction is technically qualified and blocked from production")
        check("v0212:hard-gates", gates.get("status") == "PASS" and set(gate_records) == expected_gates and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gate_records.values()), "all 22 named hard gates have strict boolean observed proof")
        check("v0212:gate-specific-proof", proof.get("status") == "PASS" and proof.get("gates") == gate_records, "gate-specific proof is bound to the hard-gate record")
        negative = load_json(evidence_root / "negative-controls-v0212.json")
        controls = negative.get("controls", [])
        expected_controls = {f"MM-NC-{index:02d}" for index in range(1, 30)}
        check("v0212:negative-controls", negative.get("status") == "PASS" and {item.get("control_id") for item in controls} == expected_controls and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls), "MM-NC-01..29 inject semantic defects and observe exact rejection classes")
        raster = load_json(evidence_root / "raster-geometry-v0212.json")
        cells = [cell for record in raster.get("maps", []) for cell in record.get("cells", [])]
        check("v0212:raster-geometry", raster.get("status") == "PASS" and set(raster.get("supported_orientations", [])) == {"TOP_LEFT/Y_DOWN", "CENTER/Y_UP"} and cells and all(cell.get("x_min") < cell.get("x_max") and cell.get("y_min") < cell.get("y_max") and cell.get("pixel_width", 0) > 0 and cell.get("pixel_height", 0) > 0 for cell in cells), "runtime raster geometry has non-zero 2D bounds for both supported orientations")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0212.json")
        check("v0212:determinism", determinism.get("status") == "TWO_RUN_DETERMINISM_PASSED" and determinism.get("equal") is True and determinism.get("differences") == [] and all("\\" not in str(item) and not Path(str(item)).is_absolute() for item in determinism.get("files", [])), "independent full-slice outputs are deterministic with stable relative evidence paths")
        production = load_json(evidence_root / "production-registry-v0212.json")
        snapshot = production.get("runtime_snapshot", {})
        check("v0212:production-boundary", snapshot.get("entry_count") == 0 and snapshot.get("entries") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "actual production registry snapshot is empty and routing remains blocked")
        historical = load_json(evidence_root / "historical-immutability-v0212.json")
        check("v0212:historical-immutability", historical.get("status") == "PASS" and historical.get("authority_blob") and historical.get("authority_sha256") == historical.get("observed_sha256") and historical.get("mutation_control", {}).get("result") == "REJECT" and historical.get("mutation_control", {}).get("observed_rejection_class") == "HISTORICAL_EVIDENCE_MUTATION_REJECTED", "current historical file validates and a mutated candidate is rejected by the same validator")
        rejection = load_json(evidence_root / "v0.21.1-rejection-correction-record-v0212.json")
        check("v0212:rejected-head-preserved", rejection.get("rejected_reviewed_head") == "cfde03cce2e31cc688f9d94a659b577042b8538a" and rejection.get("historical_evidence_unchanged") is True and rejection.get("status") == "CORRECTION_REQUIRED", "v0.21.1 rejected reviewed head remains explicit history")
        matrix = load_json(evidence_root / "capability-matrix-validation-v0212.json")
        check("v0212:capability-matrix", matrix.get("version") == "0.21.2" and matrix.get("next_candidate") == "MAPS_MINIMAP" and matrix.get("environment_tilesets_status") == "APPROVED_FOUNDATION" and matrix.get("production_routing") == "BLOCKED" and matrix.get("new_generation") == 0, "historical v0.21.2 matrix validation remains preserved")
        ...
        check("v0212:source-hard-gate-integrity", "observed is not False" not in runner_text and "type(observed) is bool and observed is True" in runner_text and "MINIMAP_RASTER_CELL_GEOMETRY_INVALID" in runtime_text and "validate_historical_authority" in runtime_text, "active source uses strict gate, runtime geometry rejection and reusable history authority")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0212:evidence", False, str(exc))


def _v0213_checks() -> None:
    """Validate the active v0.21.3 historical byte-integrity correction."""
    evidence_root = ROOT / "docs/evidence/maps-minimap-runtime-v0213"
    required = [
    ...
    required += [f"docs/evidence/maps-minimap-runtime-v0213/{name}" for name in (
        "map-contract-v0213.json", "environment-authority-bindings-v0213.json", "items-props-authority-bindings-v0213.json",
        "layer-cell-matrix-v0213.json", "chunk-partition-roundtrip-v0213.json", "regions-zones-v0213.json", "marker-contract-v0213.json",
        "minimap-projection-v0213.json", "visibility-state-qa-v0213.json", "cache-identity-v0213.json", "provenance-v0213.json",
        "raster-geometry-v0213.json", "hard-gates-v0213.json", "gate-specific-proof-v0213.json", "negative-controls-v0213.json",
        "full-slice-two-run-determinism-v0213.json", "production-registry-v0213.json", "historical-immutability-v0213.json",
        "test-only-fixture-manifest-v0213.json", "execution-evidence-v0213.json", "capability-matrix-validation-v0213.json",
    )]
    for relative in required:
        path = ROOT / relative
    ...
    try:
        ...
        gates = load_json(evidence_root / "hard-gates-v0213.json")
        gate_records = gates.get("gates", {})
        expected_gates = {"map_schema_valid", "map_dimensions_and_world_metrics_valid", "environment_authority_binding_valid", "items_props_authority_binding_valid", "layer_contract_valid", "cell_coordinates_in_bounds", "chunk_partition_exact", "chunk_roundtrip_valid", "regions_zones_in_bounds", "marker_identity_valid", "minimap_projection_valid", "raster_geometry_valid", "minimap_inverse_projection_valid", "visibility_mask_dimensions_valid", "minimap_derived_from_map_identity", "map_provenance_valid", "cache_identity_complete", "stale_cache_cross_map_chunk_projection_rejected", "test_fixture_nonproduction", "production_registry_empty", "production_routing_blocked", "isolated_full_slice_determinism"}
        check("v0213:hard-gates", gates.get("status") == "PASS" and set(gate_records) == expected_gates and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gate_records.values()), "all 22 named hard gates retain strict boolean observed proof")
        negative = load_json(evidence_root / "negative-controls-v0213.json")
        controls = negative.get("controls", [])
        expected_controls = {f"MM-NC-{index:02d}" for index in range(1, 31)}
        check("v0213:negative-controls", negative.get("status") == "PASS" and {item.get("control_id") for item in controls} == expected_controls and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls), "MM-NC-01..30 observe exact rejection classes including real line-ending mutation")
        raster = load_json(evidence_root / "raster-geometry-v0213.json")
        cells = [cell for record in raster.get("maps", []) for cell in record.get("cells", [])]
        check("v0213:raster-geometry", raster.get("status") == "PASS" and set(raster.get("supported_orientations", [])) == {"TOP_LEFT/Y_DOWN", "CENTER/Y_UP"} and cells and all(cell.get("x_min") < cell.get("x_max") and cell.get("y_min") < cell.get("y_max") and cell.get("pixel_width", 0) > 0 and cell.get("pixel_height", 0) > 0 for cell in cells), "Y_DOWN/Y_UP raster geometry remains non-zero and orientation-safe")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0213.json")
        check("v0213:determinism", determinism.get("status") == "TWO_RUN_DETERMINISM_PASSED" and determinism.get("equal") is True and determinism.get("differences") == [] and all("\\" not in str(item) and not Path(str(item)).is_absolute() for item in determinism.get("files", [])), "independent full-slice outputs are deterministic")
        production = load_json(evidence_root / "production-registry-v0213.json")
        snapshot = production.get("runtime_snapshot", {})
        check("v0213:production-boundary", snapshot.get("entry_count") == 0 and snapshot.get("entries") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        historical = load_json(evidence_root / "historical-immutability-v0213.json")
        mutation_controls = historical.get("mutation_controls", [])
        check("v0213:historical-byte-integrity", historical.get("status") == "PASS" and historical.get("authority_commit_sha") == "0bf04cb92e8619ea10cf82af8dbf2d9abe599e05" and historical.get("authority_blob_sha") == "6a44fb35ab99ffc87fc852fab9d65bfb1b5dc608" and historical.get("candidate_blob_sha") == historical.get("authority_blob_sha") and historical.get("authority_sha256") == historical.get("candidate_sha256") and {item.get("mutation_type") for item in mutation_controls} == {"line-ending-only", "append-bytes"} and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("observed_rejection_class") == "HISTORICAL_EVIDENCE_MUTATION_REJECTED" for item in mutation_controls), "raw Git-object bytes and both real mutation rejection paths are provenance-bound")
        ...
        check("v0213:evidence", False, str(exc))


def _v0220_checks() -> None:
    """Validate the active v0.22.0 UI foundation and its bounded evidence."""
    evidence_root = ROOT / "docs/evidence/ui-asset-family-runtime-v0220"
    required = [
        "schemas/current-state-v0220.json", "schemas/ui-asset-family-runtime-v0220.json", "src/ugas/ui_asset_family_runtime_v0220.py", "src/ugas/state_consistency_v0220.py", "scripts/validation/run_ui_asset_family_runtime_v0220.py", "scripts/validation/validate_state_consistency_v0220.py", "tests/test_ui_asset_family_runtime_v0220.py", "REVIEW-v0.22.0.md", "docs/evidence/current-state.json", "docs/evidence/current-state-v0213.json", "docs/evidence/github-governance-v0220/v0213-closure-completion-binding.json",
    ]
    required += [f"docs/evidence/ui-asset-family-runtime-v0220/{name}" for name in ("ui-family-manifest-v0220.json", "component-class-matrix-v0220.json", "state-variant-matrix-v0220.json", "style-token-authority-v0220.json", "nine-slice-geometry-v0220.json", "safe-area-geometry-v0220.json", "scale-consistency-v0220.json", "integration-linkage-v0220.json", "cache-identity-v0220.json", "provenance-v0220.json", "hard-gates-v0220.json", "negative-controls-v0220.json", "full-slice-two-run-determinism-v0220.json", "production-registry-v0220.json", "test-only-fixture-manifest-v0220.json", "execution-evidence-v0220.json", "ui-state-contact-sheet-v0220.png", "ui-geometry-qa-sheet-v0220.png")]
    for relative in required:
        path = ROOT / relative
        check(f"v0220:path:{relative}", path.is_file(), "present" if path.is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        if state.get("version") == "0.22.0":
            binding = load_json(ROOT / "docs/evidence/github-governance-v0220/v0213-closure-completion-binding.json")
        ...
        check("v0220:hard-gates", gates.get("required_gate_count") == 22 and len(gates.get("gates", {})) == 22 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all UI hard gates have strict observed booleans")
        check("v0220:negative-controls", controls.get("required_control_count") == 22 and len(controls.get("controls", {})) == 22 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all UI negative controls observe their real rejection classes")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0220.json")
        check("v0220:determinism", determinism.get("status") == "PASS" and determinism.get("first_output_count") == determinism.get("second_output_count") == 168, "two isolated UI fixture runs are byte-identical")
        execution = load_json(evidence_root / "execution-evidence-v0220.json")
        check("v0220:execution", execution.get("status") == "UI_ASSET_FAMILY_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED" and execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "UI foundation is technically qualified but remains TEST_ONLY")
        production = load_json(evidence_root / "production-registry-v0220.json")
        check("v0220:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("real_ui_asset_coverage") == "NONE" and production.get("synthetic_ui_fixture") == "TEST_ONLY", "production registry remains empty and blocked")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0220:evidence", False, str(exc))


def _v0221_checks() -> None:
    """Validate the active v0.22.1 UI semantic-integrity correction."""
    evidence_root = ROOT / "docs/evidence/ui-asset-family-runtime-v0221"
    active_version = load_json(ROOT / "docs/evidence/current-state.json").get("version")
    if active_version != "0.22.1":
        frozen = all((evidence_root / name).exists() for name in ("hard-gates-v0221.json", "negative-controls-v0221.json", "execution-evidence-v0221.json", "full-slice-two-run-determinism-v0221.json"))
        check("v0221:frozen-history", frozen, "v0.22.1 evidence remains present as immutable correction-required history")
        return
    required = [
        "schemas/current-state-v0221.json", "src/ugas/ui_asset_family_runtime_v0221.py", "src/ugas/state_consistency_v0221.py",
        "scripts/validation/run_ui_asset_family_runtime_v0221.py", "scripts/validation/validate_state_consistency_v0221.py",
        "scripts/validation/validate_github_governance_v0221.py", "tests/test_ui_asset_family_runtime_v0221.py",
        "docs/evidence/current-state.json", "docs/evidence/github-governance-v0221/v0213-closure-completion-binding-v2.json",
        "docs/evidence/ui-asset-family-runtime-v0220/", "docs/evidence/maps-minimap-runtime-v0213/",
    ]
    required += [f"docs/evidence/ui-asset-family-runtime-v0221/{name}" for name in ("ui-family-manifest-v0221.json", "component-class-contract-v0221.json", "state-applicability-matrix-v0221.json", "stretch-policy-and-reconstruction-v0221.json", "scale-pixel-correspondence-v0221.json", "integration-authority-bindings-v0221.json", "cache-semantic-identity-v0221.json", "hard-gates-v0221.json", "negative-controls-v0221.json", "full-slice-two-run-determinism-v0221.json", "production-registry-v0221.json", "historical-authority-v0221.json", "closure-binding-validation-v0221.json", "execution-evidence-v0221.json", "ui-state-contact-sheet-v0221.png", "ui-geometry-stretch-qa-sheet-v0221.png")]
    for relative in required: check(f"v0221:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); schema = load_json(ROOT / "schemas/current-state-v0221.json"); binding = load_json(ROOT / "docs/evidence/github-governance-v0221/v0213-closure-completion-binding-v2.json")
        validate_schema_document(schema); validate_instance(state, schema)
        consistency = validate_state_consistency_v0221(state, binding, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/project-review-response-protocol.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"))
        check("v0221:state-consistency", consistency["status"] == "UI_ASSET_FAMILY_SEMANTIC_INTEGRITY_TECHNICALLY_QUALIFIED" and not consistency["failures"], "; ".join(consistency["failures"]) or "active UI v0.22.1 state is consistent")
        check("v0221:governance-binding", validate_governance_binding_v0221(binding).get("status") == "PASS", "PR #12 closure provenance and exact main context set are validated")
        gates = load_json(evidence_root / "hard-gates-v0221.json"); controls = load_json(evidence_root / "negative-controls-v0221.json"); execution = load_json(evidence_root / "execution-evidence-v0221.json")
        check("v0221:hard-gates", gates.get("required_gate_count") == 22 and len(gates.get("gates", {})) == 22 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.22.1 hard gates have observed strict booleans")
        check("v0221:negative-controls", controls.get("required_control_count") == 22 and len(controls.get("controls", {})) == 22 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.22.1 negative controls observe real rejection classes")
        check("v0221:execution", execution.get("status") == "UI_ASSET_FAMILY_SEMANTIC_INTEGRITY_TECHNICALLY_QUALIFIED" and execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "v0.22.1 semantic correction is qualified but TEST_ONLY")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0221.json"); check("v0221:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256"), "two isolated v0.22.1 fixture runs are byte-identical")
        production = load_json(evidence_root / "production-registry-v0221.json"); check("v0221:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        check("v0221:source-integrity", "type(observed) is bool and observed is True" in (ROOT / "scripts/validation/run_ui_asset_family_runtime_v0221.py").read_text(encoding="utf-8") and "render_component_to_size" in (ROOT / "src/ugas/ui_asset_family_runtime_v0221.py").read_text(encoding="utf-8") and "validate_scale_pixel_relation" in (ROOT / "src/ugas/ui_asset_family_runtime_v0221.py").read_text(encoding="utf-8"), "runtime and runner contain direct semantic proofs")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0221:evidence", False, str(exc))


def _v0222_checks() -> None:
    """Validate the active v0.22.2 F-15/F-16/F-18/F-20/F-21 correction."""
    evidence_root = ROOT / "docs/evidence/ui-asset-family-runtime-v0222"
    active_version = load_json(ROOT / "docs/evidence/current-state.json").get("version")
    if active_version != "0.22.2":
        _v0222_history_checks()
        return
    required = [
        "schemas/current-state-v0222.json", "src/ugas/ui_asset_family_runtime_v0222.py", "src/ugas/state_consistency_v0222.py",
        "scripts/validation/run_ui_asset_family_runtime_v0222.py", "scripts/validation/validate_state_consistency_v0222.py",
        "scripts/validation/validate_github_governance_v0222.py", "tests/test_ui_asset_family_runtime_v0222.py", "REVIEW-v0.22.2.md",
        "docs/evidence/current-state.json", "docs/evidence/github-governance-v0222/v0222-ui-closure-binding.json",
        "docs/evidence/ui-asset-family-runtime-v0220/", "docs/evidence/ui-asset-family-runtime-v0221/", "docs/evidence/maps-minimap-runtime-v0213/",
    ]
    required += [f"docs/evidence/ui-asset-family-runtime-v0222/{name}" for name in ("ui-family-manifest-v0222.json", "ui-style-authority-v0222.json", "component-class-contract-v0222.json", "progress-bar-contract-v0222.json", "nine-slice-independent-verification-v0222.json", "integration-authority-bindings-v0222.json", "cache-semantic-identity-v0222.json", "hard-gates-v0222.json", "negative-controls-v0222.json", "full-slice-two-run-determinism-v0222.json", "historical-authority-v0222.json", "governance-binding-validation-v0222.json", "production-registry-v0222.json", "capability-matrix-validation-v0222.json", "execution-evidence-v0222.json", "v0.22.1-rejection-correction-record-v0222.json")]
    for relative in required:
        check(f"v0222:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); schema = load_json(ROOT / "schemas/current-state-v0222.json"); binding = load_json(ROOT / "docs/evidence/github-governance-v0222/v0222-ui-closure-binding.json")
        validate_schema_document(schema); validate_instance(state, schema)
        consistency = validate_state_consistency_v0222(state, binding, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.22.2.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"))
        check("v0222:state-consistency", consistency["status"] == "UI_ASSET_FAMILY_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and not consistency["failures"], "; ".join(consistency["failures"]) or "active UI v0.22.2 state is consistent")
        check("v0222:governance-binding", validate_governance_binding_v0222(binding).get("status") == "PASS", "canonical v0.21.3 authority and PR #13 boundary are validated")
        gates = load_json(evidence_root / "hard-gates-v0222.json"); controls = load_json(evidence_root / "negative-controls-v0222.json"); execution = load_json(evidence_root / "execution-evidence-v0222.json")
        check("v0222:hard-gates", gates.get("required_gate_count") >= 22 and len(gates.get("gates", {})) == gates.get("required_gate_count") and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.22.2 hard gates have strict observed booleans")
        check("v0222:negative-controls", controls.get("required_control_count") >= 22 and len(controls.get("controls", {})) == controls.get("required_control_count") and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.22.2 negative controls observe real rejection classes")
        check("v0222:execution", execution.get("status") == "UI_ASSET_FAMILY_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "v0.22.2 correction is technically qualified but TEST_ONLY")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0222.json"); check("v0222:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256"), "two isolated v0.22.2 fixture runs are byte-identical")
        production = load_json(evidence_root / "production-registry-v0222.json"); check("v0222:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        check("v0222:source-integrity", "verify_nine_slice_invariants" in (ROOT / "src/ugas/ui_asset_family_runtime_v0222.py").read_text(encoding="utf-8") and "type(observed) is bool and observed is True" in (ROOT / "scripts/validation/run_ui_asset_family_runtime_v0222.py").read_text(encoding="utf-8") and "LOCAL_UI_STYLE_ART_DNA" in (ROOT / "src/ugas/ui_asset_family_runtime_v0222.py").read_text(encoding="utf-8"), "runtime and runner contain direct F-15/F-16/F-18/F-20/F-21 proofs")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0222:evidence", False, str(exc))


def _v0222_history_checks() -> None:
    """Preserve the rejected v0.22.2 evidence without treating it as active state."""
    evidence_root = ROOT / "docs/evidence/ui-asset-family-runtime-v0222"
    required = [
        "docs/evidence/ui-asset-family-runtime-v0222/",
        "schemas/current-state-v0222.json",
        "src/ugas/ui_asset_family_runtime_v0222.py",
        "src/ugas/state_consistency_v0222.py",
        "REVIEW-v0.22.2.md",
    ]
    required += [f"docs/evidence/ui-asset-family-runtime-v0222/{name}" for name in ("ui-family-manifest-v0222.json", "ui-style-authority-v0222.json", "hard-gates-v0222.json", "negative-controls-v0222.json", "full-slice-two-run-determinism-v0222.json", "execution-evidence-v0222.json", "v0.22.1-rejection-correction-record-v0222.json")]
    for relative in required:
        check(f"v0222:history:{relative}", (ROOT / relative).exists(), "immutable historical evidence present" if (ROOT / relative).exists() else "missing")
    try:
        execution = load_json(evidence_root / "execution-evidence-v0222.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0222.json")
        check("v0222:history:execution", execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("new_generation") == 0, "rejected v0.22.2 execution evidence remains successful TEST_ONLY history")
        check("v0222:history:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256"), "rejected v0.22.2 deterministic evidence remains intact")
    except (OSError, json.JSONDecodeError, KeyError, TypeError):
        check("v0222:history:evidence", False, "historical evidence is unreadable")


def _v0223_checks() -> None:
    """Validate the active v0.22.3 F-15/F-16/F-18/F-21 correction."""
    evidence_root = ROOT / "docs/evidence/ui-asset-family-runtime-v0223"
    active_version = load_json(ROOT / "docs/evidence/current-state.json").get("version")
    if active_version != "0.22.3":
        required_history = ["schemas/current-state-v0223.json", "src/ugas/ui_asset_family_runtime_v0223.py", "src/ugas/state_consistency_v0223.py", "scripts/validation/run_ui_asset_family_runtime_v0223.py", "tests/test_ui_asset_family_runtime_v0223.py", "REVIEW-v0.22.3.md", "docs/evidence/ui-asset-family-runtime-v0223/hard-gates-v0223.json", "docs/evidence/ui-asset-family-runtime-v0223/negative-controls-v0223.json", "docs/evidence/ui-asset-family-runtime-v0223/execution-evidence-v0223.json"]
        for relative in required_history:
            check(f"v0223:history:{relative}", (ROOT / relative).exists(), "immutable historical evidence present" if (ROOT / relative).exists() else "missing")
        return
    required = [
        "schemas/current-state-v0223.json", "src/ugas/ui_asset_family_runtime_v0223.py", "src/ugas/state_consistency_v0223.py",
        "scripts/validation/run_ui_asset_family_runtime_v0223.py", "scripts/validation/validate_state_consistency_v0223.py",
        "scripts/validation/validate_github_governance_v0223.py", "tests/test_ui_asset_family_runtime_v0223.py", "REVIEW-v0.22.3.md",
        "docs/evidence/current-state.json", "docs/evidence/github-governance-v0223/v0223-ui-closure-binding.json",
        "docs/evidence/github-governance-v0221/v0213-closure-completion-binding-v2.json", "docs/evidence/ui-asset-family-runtime-v0222/",
    ]
    required += [f"docs/evidence/ui-asset-family-runtime-v0223/{name}" for name in ("ui-family-manifest-v0223.json", "ui-style-authority-v0223.json", "component-class-contract-v0223.json", "progress-bar-contract-v0223.json", "nine-slice-independent-verification-v0223.json", "integration-authority-bindings-v0223.json", "cache-semantic-identity-v0223.json", "hard-gates-v0223.json", "negative-controls-v0223.json", "full-slice-two-run-determinism-v0223.json", "historical-authority-v0223.json", "governance-binding-validation-v0223.json", "production-registry-v0223.json", "capability-matrix-validation-v0223.json", "execution-evidence-v0223.json", "v0.22.2-rejection-correction-record-v0223.json")]
    for relative in required:
        check(f"v0223:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); schema = load_json(ROOT / "schemas/current-state-v0223.json"); binding = load_json(ROOT / "docs/evidence/github-governance-v0223/v0223-ui-closure-binding.json")
        validate_schema_document(schema); validate_instance(state, schema)
        consistency = validate_state_consistency_v0223(state, binding, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.22.3.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"))
        check("v0223:state-consistency", consistency["status"] == "UI_ASSET_FAMILY_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and not consistency["failures"], "; ".join(consistency["failures"]) or "active UI v0.22.3 state is consistent")
        check("v0223:governance-binding", validate_governance_binding_v0223(binding).get("status") == "PASS", "final v0.21.3 authority and PR #13 boundary are validated")
        gates = load_json(evidence_root / "hard-gates-v0223.json"); controls = load_json(evidence_root / "negative-controls-v0223.json"); execution = load_json(evidence_root / "execution-evidence-v0223.json")
        check("v0223:hard-gates", gates.get("required_gate_count") >= 30 and len(gates.get("gates", {})) == gates.get("required_gate_count") and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.22.3 hard gates have strict observed booleans")
        check("v0223:negative-controls", controls.get("required_control_count") >= 38 and len(controls.get("controls", {})) == controls.get("required_control_count") and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.22.3 negative controls observe real rejection classes")
        check("v0223:execution", execution.get("status") == "UI_ASSET_FAMILY_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "v0.22.3 correction is technically qualified but TEST_ONLY")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0223.json"); check("v0223:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256"), "two isolated v0.22.3 fixture runs are byte-identical")
        production = load_json(evidence_root / "production-registry-v0223.json"); check("v0223:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        record = load_json(evidence_root / "v0.22.2-rejection-correction-record-v0223.json"); check("v0223:correction-record", record.get("rejected_reviewed_head") == "97dde2064bd7b381362e3d79f2a27d2bb78ddc36" and set(record.get("findings", [])) == {"F-15", "F-16", "F-18", "F-21"} and record.get("preserved_findings") == ["F-20"] and record.get("historical_evidence_unchanged") is True, "forward-only rejection record preserves F-20 and v0.22.2 history")
        runner_text = (ROOT / "scripts/validation/run_ui_asset_family_runtime_v0223.py").read_text(encoding="utf-8"); runtime_text = (ROOT / "src/ugas/ui_asset_family_runtime_v0223.py").read_text(encoding="utf-8")
        check("v0223:source-integrity", "validate_historical_authority" in runtime_text and "HISTORICAL_EVIDENCE_MUTATION_REJECTED" in runtime_text and "clamp\"] is False" in runner_text and "merge_commit_sha" in (ROOT / "src/ugas/state_consistency_v0223.py").read_text(encoding="utf-8"), "runtime and runner contain direct F-15/F-16/F-18/F-21 proofs")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0223:evidence", False, str(exc))


def _v0230_checks() -> None:
    """Validate the active v0.23.0 VFX foundation and its bounded evidence."""
    evidence_root = ROOT / "docs/evidence/vfx-asset-family-runtime-v0230"
    required = [
        "schemas/current-state-v0230.json", "schemas/vfx-asset-family-runtime-v0230.json", "src/ugas/vfx_asset_family_runtime_v0230.py", "src/ugas/state_consistency_v0230.py", "scripts/validation/run_vfx_asset_family_runtime_v0230.py", "scripts/validation/validate_state_consistency_v0230.py", "tests/test_vfx_asset_family_runtime_v0230.py", "REVIEW-v0.23.0.md", "docs/evidence/current-state.json", "docs/ugas-v1-capability-matrix.json", "docs/roadmap.md", "CHECKPOINT.md",
    ]
    ...
    required += [f"docs/evidence/vfx-asset-family-runtime-v0230/{name}" for name in evidence_names]
    for relative in required:
        check(f"v0230:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        ...
        check("v0230:execution", execution.get("status") == "VFX_ASSET_FAMILY_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "VFX foundation is technically qualified but remains TEST_ONLY")
        check("v0230:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256") and determinism.get("equal") is True, "two isolated VFX fixture runs are byte-identical")
        check("v0230:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("real_vfx_asset_coverage") == "NONE" and production.get("synthetic_vfx_fixture") == "TEST_ONLY" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        transition = load_json(evidence_root / "v0.22.3-approval-transition-v0230.json"); check("v0230:ui-transition", transition.get("source_version") == "0.22.3" and transition.get("status") == "APPROVED_FOUNDATION_MERGED_CLOSED" and transition.get("source_evidence_unchanged") is True and transition.get("forward_only") is True, "approved UI history is preserved while VFX advances")
        runtime_text = (ROOT / "src/ugas/vfx_asset_family_runtime_v0230.py").read_text(encoding="utf-8"); runner_text = (ROOT / "scripts/validation/run_vfx_asset_family_runtime_v0230.py").read_text(encoding="utf-8"); check("v0230:source-integrity", "type(observed) is bool" in runtime_text and "observed is True" in runtime_text and "VFX-NC-36" in runner_text and "provider_generation_jobs" in runner_text and "production_routing" in runtime_text, "runtime and runner contain direct VFX contract and boundary proofs")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0230:evidence", False, str(exc))


def _v0231_checks() -> None:
    """Validate the active v0.23.1 F-22..F-28 correction and boundaries."""
    if load_json(ROOT / "docs/evidence/current-state.json").get("version") != "0.23.1":
        check("v0231:historical-presence", (ROOT / "REVIEW-v0.23.1.md").is_file() and (ROOT / "docs/evidence/vfx-asset-family-runtime-v0231").is_dir(), "v0.23.1 remains preserved historical evidence")
        return
    evidence_root = ROOT / "docs/evidence/vfx-asset-family-runtime-v0231"
    required = [
        "schemas/current-state-v0231.json", "schemas/vfx-asset-family-runtime-v0231.json", "src/ugas/vfx_asset_family_runtime_v0231.py", "src/ugas/state_consistency_v0231.py", "scripts/validation/run_vfx_asset_family_runtime_v0231.py", "scripts/validation/validate_state_consistency_v0231.py", "tests/test_vfx_asset_family_runtime_v0231.py", "REVIEW-v0.23.1.md", "docs/evidence/current-state.json", "docs/ugas-v1-capability-matrix.json", "docs/roadmap.md", "CHECKPOINT.md",
    ]
    names = ("vfx-family-manifest-v0231.json", "effect-class-contract-v0231.json", "lifecycle-timing-v0231.json", "blend-alpha-contract-v0231.json", "alpha-decoded-v0231.json", "spatial-anchor-contract-v0231.json", "budget-authority-v0231.json", "fallback-degradation-v0231.json", "representation-import-v0231.json", "integration-linkage-v0231.json", "cache-provenance-v0231.json", "hard-gates-v0231.json", "negative-controls-v0231.json", "full-slice-two-run-determinism-v0231.json", "production-registry-v0231.json", "test-only-fixture-manifest-v0231.json", "execution-evidence-v0231.json", "historical-immutability-v0231.json", "capability-matrix-validation-v0231.json", "state-consistency-v0231.json", "schema-validation-v0231.json", "v0.23.0-rejection-correction-v0231.json", "exact-sha-lifecycle-v0231.json", "qa-contact-timing-v0231.json", "vfx-effect-contact-sheet-v0231.png", "vfx-timing-qa-sheet-v0231.png", "vfx-budget-fallback-qa-sheet-v0231.png")
    required += [f"docs/evidence/vfx-asset-family-runtime-v0231/{name}" for name in names]
    for relative in required:
        check(f"v0231:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); schema = load_json(ROOT / "schemas/current-state-v0231.json"); manifest = load_json(evidence_root / "vfx-family-manifest-v0231.json"); gates = load_json(evidence_root / "hard-gates-v0231.json"); controls = load_json(evidence_root / "negative-controls-v0231.json"); execution = load_json(evidence_root / "execution-evidence-v0231.json"); determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0231.json"); production = load_json(evidence_root / "production-registry-v0231.json"); matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        validate_schema_document(schema); validate_instance(state, schema)
        consistency = validate_state_consistency_v0231(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"), matrix)
        check("v0231:state-consistency", consistency["status"] == state.get("current_gate") and not consistency["failures"], "; ".join(consistency["failures"]) or "active v0.23.1 state is consistent")
        check("v0231:manifest", manifest.get("schema_version") == "0.23.1" and manifest.get("family_id") == "vfx_asset_family" and manifest.get("registry_mode") == "TEST_ONLY" and len(manifest.get("effects", [])) == 10, "ten-class TEST_ONLY VFX manifest is present")
        check("v0231:hard-gates", gates.get("status") == "PASS" and gates.get("required_gate_count") == 32 and len(gates.get("gates", {})) == 32 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.23.1 hard gates have strict observed booleans")
        check("v0231:negative-controls", controls.get("status") == "PASS" and controls.get("required_control_count") == 30 and len(controls.get("controls", {})) == 30 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.23.1 negative controls observe real rejection classes")
        check("v0231:execution", execution.get("status") == state.get("current_gate") and execution.get("overall_pass") is True and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0 and set(execution.get("corrections", [])) == {"F-22", "F-23", "F-24", "F-25", "F-26", "F-27", "F-28"}, "v0.23.1 correction is qualified but remains TEST_ONLY")
        check("v0231:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256") and determinism.get("equal") is True, "two isolated v0.23.1 fixture runs are byte-identical")
        check("v0231:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        rejection = load_json(evidence_root / "v0.23.0-rejection-correction-v0231.json"); check("v0231:rejection-record", rejection.get("reviewed_head") == "c8967f5f7b4d3f66b402d10ae8fe5f29cb9fbb72" and set(rejection.get("findings", [])) == {"F-22", "F-23", "F-24", "F-25", "F-26", "F-27", "F-28"} and rejection.get("historical_evidence_unchanged") is True, "forward-only F-22..F-28 rejection record preserves v0.23.0")
        runtime_text = (ROOT / "src/ugas/vfx_asset_family_runtime_v0231.py").read_text(encoding="utf-8"); state_text = (ROOT / "src/ugas/state_consistency_v0231.py").read_text(encoding="utf-8"); check("v0231:source-integrity", "VFX_GAMEPLAY_AUTHORITY_FORBIDDEN" in runtime_text and "semantic_contract" in runtime_text and "type(observed) is bool" in runtime_text and "merge_commit_sha" in state_text and "head_sha" in state_text, "F-22..F-28 runtime and live governance proofs are present")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0231:evidence", False, str(exc))


def _v0232_checks() -> None:
    """Validate the active v0.23.2 F-23R/F-26R/F-27R correction."""
    if load_json(ROOT / "docs/evidence/current-state.json").get("version") != "0.23.2":
        check("v0232:historical-root", (ROOT / "docs/evidence/vfx-asset-family-runtime-v0232").is_dir(), "v0.23.2 evidence root remains present as historical correction")
        check("v0232:historical-review", (ROOT / "REVIEW-v0.23.2.md").is_file(), "v0.23.2 review handoff remains present as historical correction")
        return
    evidence_root = ROOT / "docs/evidence/vfx-asset-family-runtime-v0232"
    names = ("vfx-family-manifest-v0232.json", "effect-class-contract-v0232.json", "lifecycle-timing-v0232.json", "blend-alpha-contract-v0232.json", "alpha-decoded-v0232.json", "spatial-anchor-contract-v0232.json", "budget-authority-v0232.json", "fallback-degradation-v0232.json", "representation-import-v0232.json", "integration-linkage-v0232.json", "cache-provenance-v0232.json", "fallback-output-proof-v0232.json", "hard-gates-v0232.json", "negative-controls-v0232.json", "full-slice-two-run-determinism-v0232.json", "degraded-output-determinism-v0232.json", "production-registry-v0232.json", "test-only-fixture-manifest-v0232.json", "execution-evidence-v0232.json", "historical-immutability-v0232.json", "capability-matrix-validation-v0232.json", "state-consistency-v0232.json", "schema-validation-v0232.json", "v0.23.1-rejection-correction-v0232.json", "exact-sha-lifecycle-v0232.json", "qa-contact-timing-v0232.json", "vfx-effect-contact-sheet-v0232.png", "vfx-timing-qa-sheet-v0232.png", "vfx-budget-fallback-qa-sheet-v0232.png")
    required = ["schemas/current-state-v0232.json", "schemas/vfx-asset-family-runtime-v0232.json", "src/ugas/vfx_asset_family_runtime_v0232.py", "src/ugas/state_consistency_v0232.py", "scripts/validation/run_vfx_asset_family_runtime_v0232.py", "scripts/validation/validate_state_consistency_v0232.py", "tests/test_vfx_asset_family_runtime_v0232.py", "REVIEW-v0.23.2.md", "docs/evidence/current-state.json", "docs/ugas-v1-capability-matrix.json", "docs/roadmap.md", "CHECKPOINT.md"] + [f"docs/evidence/vfx-asset-family-runtime-v0232/{name}" for name in names]
    for relative in required:
        check(f"v0232:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); schema = load_json(ROOT / "schemas/current-state-v0232.json"); manifest = load_json(evidence_root / "vfx-family-manifest-v0232.json"); gates = load_json(evidence_root / "hard-gates-v0232.json"); controls = load_json(evidence_root / "negative-controls-v0232.json"); execution = load_json(evidence_root / "execution-evidence-v0232.json"); determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0232.json"); production = load_json(evidence_root / "production-registry-v0232.json"); matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        validate_schema_document(schema); validate_instance(state, schema)
        consistency = validate_state_consistency_v0232(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"), matrix)
        check("v0232:state-consistency", consistency["status"] == state.get("current_gate") and not consistency["failures"], "; ".join(consistency["failures"]) or "active v0.23.2 state is consistent")
        check("v0232:manifest", manifest.get("schema_version") == "0.23.2" and manifest.get("family_id") == "vfx_asset_family" and manifest.get("registry_mode") == "TEST_ONLY" and len(manifest.get("effects", [])) == 10, "ten-class TEST_ONLY VFX manifest is present")
        check("v0232:hard-gates", gates.get("status") == "PASS" and gates.get("required_gate_count") == 32 and len(gates.get("gates", {})) == 32 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.23.2 hard gates have strict observed booleans")
        check("v0232:negative-controls", controls.get("status") == "PASS" and controls.get("required_control_count") == 18 and len(controls.get("controls", {})) == 18 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.23.2 negative controls observe real rejection classes")
        check("v0232:execution", execution.get("status") == state.get("current_gate") and execution.get("overall_pass") is True and set(execution.get("corrections", [])) == {"F-23R", "F-26R", "F-27R"} and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "v0.23.2 correction is qualified but remains TEST_ONLY")
        check("v0232:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256") and determinism.get("equal") is True, "two isolated v0.23.2 fixture runs are byte-identical")
        check("v0232:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        rejection = load_json(evidence_root / "v0.23.1-rejection-correction-v0232.json"); check("v0232:rejection-record", rejection.get("reviewed_head") == "136079540f674c7467d93bd28e9834addbfce5c3" and set(rejection.get("findings", [])) == {"F-23R", "F-26R", "F-27R"} and rejection.get("historical_evidence_unchanged") is True, "forward-only correction record preserves v0.23.1")
        runtime_text = (ROOT / "src/ugas/vfx_asset_family_runtime_v0232.py").read_text(encoding="utf-8"); check("v0232:source-integrity", "VFX_SEMANTIC_UNKNOWN_FIELD" in runtime_text and "raw_semantic_input_hash" in runtime_text and "render_fallback_output" in runtime_text and "SKIPPED_VISUAL" in runtime_text, "F-23R/F-26R/F-27R runtime proofs are present")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0232:evidence", False, str(exc))


def _v0233_checks() -> None:
    """Validate the active v0.23.3 F-26S/F-29/F-30 correction."""
    if load_json(ROOT / "docs/evidence/current-state.json").get("version") != "0.23.3":
        check("v0233:historical-root", (ROOT / "docs/evidence/vfx-asset-family-runtime-v0233").is_dir(), "v0.23.3 evidence root remains present as historical correction")
        check("v0233:historical-review", (ROOT / "REVIEW-v0.23.3.md").is_file(), "v0.23.3 review handoff remains present as historical correction")
        return
    evidence_root = ROOT / "docs/evidence/vfx-asset-family-runtime-v0233"
    names = ("vfx-family-manifest-v0233.json", "effect-class-contract-v0233.json", "lifecycle-timing-v0233.json", "blend-alpha-contract-v0233.json", "alpha-decoded-v0233.json", "spatial-anchor-contract-v0233.json", "budget-authority-v0233.json", "fallback-degradation-v0233.json", "representation-import-v0233.json", "integration-linkage-v0233.json", "cache-provenance-v0233.json", "fallback-output-proof-v0233.json", "hard-gates-v0233.json", "negative-controls-v0233.json", "full-slice-two-run-determinism-v0233.json", "degraded-output-determinism-v0233.json", "production-registry-v0233.json", "test-only-fixture-manifest-v0233.json", "execution-evidence-v0233.json", "historical-immutability-v0233.json", "capability-matrix-validation-v0233.json", "state-consistency-v0233.json", "schema-validation-v0233.json", "v0.23.2-rejection-correction-v0233.json", "exact-sha-lifecycle-v0233.json", "vfx-effect-contact-sheet-v0233.png", "vfx-timing-qa-sheet-v0233.png", "vfx-budget-fallback-qa-sheet-v0233.png")
    required = ["schemas/current-state-v0233.json", "schemas/vfx-asset-family-runtime-v0233.json", "src/ugas/vfx_asset_family_runtime_v0233.py", "src/ugas/historical_immutability_v0233.py", "src/ugas/state_consistency_v0233.py", "scripts/validation/run_vfx_asset_family_runtime_v0233.py", "scripts/validation/validate_state_consistency_v0233.py", "tests/test_vfx_asset_family_runtime_v0233.py", "REVIEW-v0.23.3.md", "docs/evidence/current-state.json", "docs/ugas-v1-capability-matrix.json", "docs/roadmap.md", "CHECKPOINT.md"] + [f"docs/evidence/vfx-asset-family-runtime-v0233/{name}" for name in names]
    for relative in required:
        check(f"v0233:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); schema = load_json(ROOT / "schemas/current-state-v0233.json"); manifest = load_json(evidence_root / "vfx-family-manifest-v0233.json"); gates = load_json(evidence_root / "hard-gates-v0233.json"); controls = load_json(evidence_root / "negative-controls-v0233.json"); execution = load_json(evidence_root / "execution-evidence-v0233.json"); determinism = load_json(evidence_root / "degraded-output-determinism-v0233.json"); historical = load_json(evidence_root / "historical-immutability-v0233.json"); production = load_json(evidence_root / "production-registry-v0233.json"); matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        validate_schema_document(schema); validate_instance(state, schema)
        consistency = validate_state_consistency_v0233(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"), matrix)
        check("v0233:state-consistency", consistency["status"] == state.get("current_gate") and not consistency["failures"], "; ".join(consistency["failures"]) or "active v0.23.3 state is consistent")
        check("v0233:manifest", manifest.get("schema_version") == "0.23.3" and manifest.get("family_id") == "vfx_asset_family" and manifest.get("registry_mode") == "TEST_ONLY" and len(manifest.get("effects", [])) == 10, "ten-class TEST_ONLY VFX manifest is present")
        check("v0233:hard-gates", gates.get("status") == "PASS" and gates.get("required_gate_count") == 32 and len(gates.get("gates", {})) == 32 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.23.3 hard gates have strict observed booleans")
        check("v0233:negative-controls", controls.get("status") == "PASS" and controls.get("required_control_count") == 18 and len(controls.get("controls", {})) == 18 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.23.3 negative controls observe real rejection classes")
        check("v0233:execution", execution.get("status") == state.get("current_gate") and execution.get("overall_pass") is True and set(execution.get("corrections", [])) == {"F-26S", "F-29", "F-30"} and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "v0.23.3 correction is qualified but remains TEST_ONLY")
        check("v0233:degraded-determinism", determinism.get("status") == "PASS" and determinism.get("first_output_hash") == determinism.get("repeat_output_hash") and determinism.get("equal") is True and determinism.get("first_root") != determinism.get("repeat_root"), "independent degraded outputs are byte-equivalent")
        check("v0233:historical-immutability", historical.get("status") == "PASS" and historical.get("historical_evidence_unchanged") is True and historical.get("mutation_controls_pass") is True, "historical refs and injected mutation controls are valid")
        check("v0233:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        rejection = load_json(evidence_root / "v0.23.2-rejection-correction-v0233.json"); check("v0233:rejection-record", rejection.get("reviewed_head") == "d7501177a9f4480921226b9832ffc237544a6433" and set(rejection.get("findings", [])) == {"F-26S", "F-29", "F-30"} and rejection.get("historical_evidence_unchanged") is True, "forward-only correction record preserves v0.23.2")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0233:evidence", False, str(exc))


def _v0234_checks() -> None:
    """Validate the active v0.23.4 F-29R exact historical-tree correction."""
    evidence_root = ROOT / "docs/evidence/vfx-asset-family-runtime-v0234"
    names = ("vfx-family-manifest-v0234.json", "effect-class-contract-v0234.json", "lifecycle-timing-v0234.json", "blend-alpha-contract-v0234.json", "alpha-decoded-v0234.json", "spatial-anchor-contract-v0234.json", "budget-authority-v0234.json", "fallback-degradation-v0234.json", "representation-import-v0234.json", "integration-linkage-v0234.json", "cache-provenance-v0234.json", "fallback-output-proof-v0234.json", "hard-gates-v0234.json", "negative-controls-v0234.json", "full-slice-two-run-determinism-v0234.json", "degraded-output-determinism-v0234.json", "production-registry-v0234.json", "test-only-fixture-manifest-v0234.json", "execution-evidence-v0234.json", "historical-immutability-v0234.json", "capability-matrix-validation-v0234.json", "state-consistency-v0234.json", "schema-validation-v0234.json", "v0.23.3-rejection-correction-v0234.json", "exact-sha-lifecycle-v0234.json", "vfx-effect-contact-sheet-v0234.png", "vfx-timing-qa-sheet-v0234.png", "vfx-budget-fallback-qa-sheet-v0234.png")
    required = ["schemas/current-state-v0234.json", "schemas/vfx-asset-family-runtime-v0234.json", "src/ugas/vfx_asset_family_runtime_v0234.py", "src/ugas/historical_immutability_v0234.py", "src/ugas/state_consistency_v0234.py", "scripts/validation/run_vfx_asset_family_runtime_v0234.py", "scripts/validation/validate_state_consistency_v0234.py", "tests/test_vfx_asset_family_runtime_v0234.py", "REVIEW-v0.23.4.md", "docs/evidence/current-state.json", "docs/evidence/github-governance-v0230/v0234-external-approval.json", "docs/ugas-v1-capability-matrix.json", "docs/roadmap.md", "CHECKPOINT.md"] + [f"docs/evidence/vfx-asset-family-runtime-v0234/{name}" for name in names]
    for relative in required:
        check(f"v0234:path:{relative}", (ROOT / relative).exists(), "present" if (ROOT / relative).exists() else "missing")
    try:
        schema = load_json(ROOT / "schemas/current-state-v0234.json"); runtime_schema = load_json(ROOT / "schemas/vfx-asset-family-runtime-v0234.json"); manifest = load_json(evidence_root / "vfx-family-manifest-v0234.json"); gates = load_json(evidence_root / "hard-gates-v0234.json"); controls = load_json(evidence_root / "negative-controls-v0234.json"); execution = load_json(evidence_root / "execution-evidence-v0234.json"); determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0234.json"); historical = load_json(evidence_root / "historical-immutability-v0234.json"); production = load_json(evidence_root / "production-registry-v0234.json"); matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json"); consistency = load_json(evidence_root / "state-consistency-v0234.json")
        validate_schema_document(schema); validate_schema_document(runtime_schema); validate_instance(manifest, runtime_schema)
        check("v0234:state-consistency", consistency.get("status") == "VFX_ASSET_FAMILY_RUNTIME_CORRECTION_F29R_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and not consistency.get("failures"), "frozen v0.23.4 state-consistency evidence remains valid after the forward v0.24.0 state advance")
        check("v0234:manifest", manifest.get("schema_version") == "0.23.4" and manifest.get("family_id") == "vfx_asset_family" and manifest.get("registry_mode") == "TEST_ONLY" and len(manifest.get("effects", [])) == 10, "ten-class TEST_ONLY VFX manifest is present")
        check("v0234:hard-gates", gates.get("status") == "PASS" and gates.get("required_gate_count") == 33 and len(gates.get("gates", {})) == 33 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates["gates"].values()), "all v0.23.4 hard gates have strict observed booleans")
        check("v0234:negative-controls", controls.get("status") == "PASS" and controls.get("required_control_count") == 18 and len(controls.get("controls", {})) == 18 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls["controls"].values()), "all v0.23.4 runtime negative controls observe real rejection classes")
        check("v0234:execution", execution.get("status") == "VFX_ASSET_FAMILY_RUNTIME_CORRECTION_F29R_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED" and execution.get("overall_pass") is True and set(execution.get("corrections", [])) == {"F-29R"} and execution.get("production_routing") == "BLOCKED" and execution.get("production_approved") is False and execution.get("new_generation") == 0, "v0.23.4 F-29R correction is qualified but remains TEST_ONLY")
        check("v0234:determinism", determinism.get("status") == "PASS" and determinism.get("first_run_sha256") == determinism.get("second_run_sha256") and determinism.get("equal") is True, "two isolated v0.23.4 fixture runs are byte-identical")
        check("v0234:historical-immutability", historical.get("status") == "PASS" and historical.get("historical_evidence_unchanged") is True and historical.get("mutation_controls_pass") is True and historical.get("extra_file_controls_pass") is True, "historical refs, extra-file and byte-mutation controls are valid")
        check("v0234:production-boundary", production.get("registry") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry remains empty and blocked")
        rejection = load_json(evidence_root / "v0.23.3-rejection-correction-v0234.json"); check("v0234:rejection-record", rejection.get("reviewed_head") == "9eda08b25a674a5423a99d35a42711102df91b8b" and rejection.get("findings") == ["F-29R"] and rejection.get("historical_evidence_unchanged") is True, "forward-only F-29R correction record preserves v0.23.3")
        approval = load_json(ROOT / "docs/evidence/github-governance-v0230/v0234-external-approval.json"); check("v0234:approval-record", approval.get("status") == "APPROVED_FOUNDATION" and approval.get("approved_semantic_head") == "9f8030d2c0ee0e00d10d64a9331899e21ebd5842" and approval.get("approval_comment_id") == 5593830005 and approval.get("merge_verified_via_protected_path") is False, "forward-only approval remains merge-unverified until governed closure")
        runtime_text = (ROOT / "src/ugas/historical_immutability_v0234.py").read_text(encoding="utf-8"); runner_text = (ROOT / "scripts/validation/run_vfx_asset_family_runtime_v0234.py").read_text(encoding="utf-8"); check("v0234:source-integrity", "_tracked_entries" in runtime_text and "_filesystem_entries" in runtime_text and "observed_ref" in runtime_text and "actual_paths = expected_relative" not in runtime_text and "extra_file_controls_pass" in runner_text, "F-29R derives observed Git/file sets and exercises real extra-file controls")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0234:evidence", False, str(exc))


def _v0240_checks() -> None:
    """Validate the active v0.24.0 orchestration foundation."""
    evidence_root = ROOT / "docs/evidence/orchestration-runtime-v0240"
    required = [
        "schemas/current-state-v0240.json", "schemas/orchestration-runtime-v0240.json", "src/ugas/orchestration_runtime_v0240.py",
        "src/ugas/state_consistency_v0240.py", "scripts/validation/run_orchestration_runtime_v0240.py", "scripts/validation/validate_state_consistency_v0240.py",
        "tests/test_orchestration_runtime_v0240.py", "CHECKPOINT.md", "docs/roadmap.md", "docs/ugas-v1-capability-matrix.json", "docs/evidence/current-state.json",
    ]
    required += [f"docs/evidence/orchestration-runtime-v0240/{name}" for name in ("request-contract-v0240.json", "dag-scheduler-v0240.json", "execution-state-v0240.json", "checkpoint-resume-v0240.json", "cache-provenance-v0240.json", "hard-gates-v0240.json", "negative-controls-v0240.json", "full-slice-two-run-determinism-v0240.json", "production-boundary-v0240.json", "v0234-post-merge-closure-binding-v0240.json", "execution-evidence-v0240.json")]
    for relative in required:
        check(f"v0240:path:{relative}", (ROOT / relative).is_file(), "present" if (ROOT / relative).is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json"); state_schema = load_json(ROOT / "schemas/current-state-v0240.json"); runtime_schema = load_json(ROOT / "schemas/orchestration-runtime-v0240.json"); execution = load_json(evidence_root / "execution-evidence-v0240.json"); gates = load_json(evidence_root / "hard-gates-v0240.json"); controls = load_json(evidence_root / "negative-controls-v0240.json"); determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0240.json"); production = load_json(evidence_root / "production-boundary-v0240.json"); binding = load_json(evidence_root / "v0234-post-merge-closure-binding-v0240.json"); matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        validate_schema_document(state_schema); validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        historical_state = state.get("correction_history", {}).get("v0.24.0", {})
        check("v0240:state-history", historical_state.get("status") == "CORRECTION_REQUIRED" and historical_state.get("rejected_reviewed_head") == "36064a215bf3e7bff06ac22e750a242c6556f83e" and historical_state.get("historical_evidence_unchanged") is True, "v0.24.0 remains immutable correction-required history while v0.24.1 is active")
        check("v0240:execution", execution.get("schema_version") == "0.24.0" and execution.get("overall_pass") is True and execution.get("production", {}).get("production_routing") == "BLOCKED" and execution.get("production", {}).get("production_approved") is False and execution.get("production", {}).get("new_generation") == 0, "v0.24.0 orchestration execution is technically qualified and TEST_ONLY")
        check("v0240:hard-gates", gates.get("overall_pass") is True and len(gates.get("gates", {})) >= 30 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True for item in gates.get("gates", {}).values()), "v0.24.0 hard gates use strict observed booleans")
        check("v0240:negative-controls", controls.get("status") == "PASS" and len(controls.get("controls", {})) >= 30 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls.get("controls", {}).values()), "NC-01 onward observe actual rejection classes")
        check("v0240:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.0 executions are deterministic")
        check("v0240:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0240:v0234-closure", binding.get("evidence_status") == "MERGED_CLOSED" and binding.get("merge_main_sha") == "dee98f8cd89ebd83a36ead7a22a184700d6e916f" and binding.get("historical_evidence_immutable") is True, "v0.23.4 closure is bound forward-only and immutable")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0240:evidence", False, str(exc))


def _v0241_checks() -> None:
    """Validate the active v0.24.1 F31-F36 orchestration correction."""
    evidence_root = ROOT / "docs/evidence/orchestration-runtime-v0241"
    required = [
        "schemas/current-state-v0241.json", "schemas/orchestration-runtime-v0241.json", "src/ugas/orchestration_runtime_v0241.py", "src/ugas/state_consistency_v0241.py",
        "scripts/validation/run_orchestration_runtime_v0241.py", "scripts/validation/validate_state_consistency_v0241.py", "scripts/validation/validate_orchestration_schema_v0241.py",
        "scripts/validation/build_github_review_manifest_v0241.py", "scripts/validation/validate_github_review_manifest_v0241.py", "scripts/validation/validate_github_review_security_v0241.py", "scripts/validation/enforce_github_review_v0241.py", "scripts/validation/record_orchestration_results_v0241.py",
        "tests/test_orchestration_runtime_v0241.py", "CHECKPOINT.md", "docs/roadmap.md", "docs/ugas-v1-capability-matrix.json", "docs/evidence/current-state.json", "REVIEW-v0.24.1.md",
    ]
    names = ("request-contract-v0241.json", "dag-scheduler-v0241.json", "execution-state-v0241.json", "deadline-timeout-v0241.json", "checkpoint-resume-v0241.json", "idempotency-coordinator-v0241.json", "circuit-breaker-runtime-v0241.json", "dependency-authority-v0241.json", "provider-boundary-v0241.json", "family-concurrency-v0241.json", "frozen-v0240-fingerprint-v0241.json", "hard-gates-v0241.json", "negative-controls-v0241.json", "full-slice-two-run-determinism-v0241.json", "production-boundary-v0241.json", "correction-history-v0241.json", "execution-evidence-v0241.json", "schema-validation-v0241.json", "capability-matrix-validation-v0241.json")
    required += [f"docs/evidence/orchestration-runtime-v0241/{name}" for name in names]
    for relative in required:
        check(f"v0241:path:{relative}", (ROOT / relative).is_file(), "present" if (ROOT / relative).is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        runtime_schema = load_json(ROOT / "schemas/orchestration-runtime-v0241.json")
        execution = load_json(evidence_root / "execution-evidence-v0241.json")
        gates = load_json(evidence_root / "hard-gates-v0241.json")
        controls = load_json(evidence_root / "negative-controls-v0241.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0241.json")
        production = load_json(evidence_root / "production-boundary-v0241.json")
        provider = load_json(evidence_root / "provider-boundary-v0241.json")
        family = load_json(evidence_root / "family-concurrency-v0241.json")
        frozen = load_json(evidence_root / "frozen-v0240-fingerprint-v0241.json")
        correction = load_json(evidence_root / "correction-history-v0241.json")
        schema_validation = load_json(evidence_root / "schema-validation-v0241.json")
        validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        history = state.get("correction_history", {}).get("v0.24.1", {})
        check("v0241:history", history.get("status") == "CORRECTION_REQUIRED" and history.get("rejected_reviewed_head") == "ed9fa927fd50193130b3e085ef077dea267f2790" and history.get("historical_evidence_unchanged") is True, "v0.24.1 remains immutable correction-required history")
        check("v0241:execution", execution.get("schema_version") == "0.24.1" and execution.get("overall_pass") is True and execution.get("production", {}).get("production_routing") == "BLOCKED" and execution.get("production", {}).get("production_approved") is False and execution.get("production", {}).get("new_generation") == 0, "v0.24.1 F31-F36 correction is technically qualified and TEST_ONLY")
        check("v0241:hard-gates", gates.get("overall_pass") is True and len(gates.get("gates", {})) >= 40 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gates.get("gates", {}).values()), "all v0.24.1 hard gates use strict observed booleans")
        controls_ok = controls.get("status") == "PASS" and len(controls.get("controls", {})) >= 8 and all(
            item.get("status") == "PASS"
            and item.get("result") == "REJECT"
            and (item.get("observed_rejection_class") is None or item.get("expected_rejection_class") == item.get("observed_rejection_class"))
            for item in controls.get("controls", {}).values()
        )
        gate_control = controls.get("controls", {}).get("NC-GATE-01", {})
        check("v0241:negative-controls", controls_ok and gate_control.get("observed_type") == "str" and gate_control.get("observed_value") == "true" and gate_control.get("observed_gate_status") == "FAIL", "F31-F36 negative controls traverse real rejections and preserve non-bool gate observations")
        check("v0241:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.1 executions are deterministic")
        check("v0241:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0241:provider-boundary", provider.get("boundary", {}).get("status") == "PASS" and provider.get("snapshot", {}).get("provider_submit_calls") == 0 and all(value == 0 for value in provider.get("spies", {}).values()), "actual provider entrypoint spies observe zero submissions and the source boundary is clean")
        check("v0241:family-concurrency", family.get("peak_global", 0) > 1 and family.get("peak_global", 0) <= 3 and family.get("dispatch_count") == 4 and family.get("family_peaks", {}).get("maps_minimap") == 1, "independent family work is admitted while global and family limits remain bounded")
        check("v0241:authority", len(execution.get("authority", {}).get("refs", [])) == 4 and execution.get("authority", {}).get("resolved", {}).get("status") == "PASS", "read-only dependency authorities are resolved from repository bytes and hashes")
        check("v0241:frozen-v0240", frozen.get("status") == "PASS" and frozen.get("authority_commit") == "36064a215bf3e7bff06ac22e750a242c6556f83e" and frozen.get("differences") == [], "v0.24.0 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0241:correction-record", correction.get("status") == "CORRECTION_REQUIRED" and correction.get("base_main_sha") == "dee98f8cd89ebd83a36ead7a22a184700d6e916f" and correction.get("reviewed_head") == "36064a215bf3e7bff06ac22e750a242c6556f83e" and correction.get("findings") == ["F-31", "F-32", "F-33", "F-34", "F-35", "F-36"] and correction.get("historical_evidence_unchanged") is True, "forward-only F31-F36 correction record binds the rejected v0.24.0 head")
        check("v0241:schema-validation", schema_validation.get("status") == "PASS" and all(item.get("status") == "PASS" for item in schema_validation.get("checks", [])), "active state and runtime evidence validate against v0.24.1 schemas")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0241:evidence", False, str(exc))


def _v0242_checks() -> None:
    """Validate immutable v0.24.2 F31R-F35R orchestration correction history."""
    evidence_root = ROOT / "docs/evidence/orchestration-runtime-v0242"
    required = [
        "schemas/current-state-v0242.json", "schemas/orchestration-runtime-v0242.json", "src/ugas/orchestration_runtime_v0242.py", "src/ugas/state_consistency_v0242.py",
    ...
    try:
        ...
        gates = load_json(evidence_root / "hard-gates-v0242.json")
        controls = load_json(evidence_root / "negative-controls-v0242.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0242.json")
        production = load_json(evidence_root / "production-boundary-v0242.json")
        provider = load_json(evidence_root / "provider-boundary-v0242.json")
        family = load_json(evidence_root / "family-concurrency-v0242.json")
        frozen = load_json(evidence_root / "frozen-v0241-fingerprint-v0242.json")
        correction = load_json(evidence_root / "correction-history-v0242.json")
        schema_validation = load_json(evidence_root / "schema-validation-v0242.json")
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        ...
        history = state.get("correction_history", {}).get("v0.24.2", {})
        check("v0242:history", history.get("status") == "CORRECTION_REQUIRED" and history.get("rejected_reviewed_head") == "cd3345db7c0e587915e56eb9314879c4a9cf98a3" and history.get("historical_evidence_unchanged") is True, "v0.24.2 remains immutable correction-required history")
        check("v0242:execution", execution.get("schema_version") == "0.24.2" and execution.get("overall_pass") is True and execution.get("production", {}).get("production_routing") == "BLOCKED" and execution.get("production", {}).get("production_approved") is False and execution.get("production", {}).get("new_generation") == 0, "v0.24.2 F31R-F35R correction is technically qualified and TEST_ONLY")
        check("v0242:hard-gates", gates.get("overall_pass") is True and len(gates.get("gates", {})) >= 40 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gates.get("gates", {}).values()), "all v0.24.2 hard gates use strict observed booleans")
        controls_ok = controls.get("status") == "PASS" and len(controls.get("controls", {})) >= 8 and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and (item.get("observed_rejection_class") is None or item.get("expected_rejection_class") == item.get("observed_rejection_class")) for item in controls.get("controls", {}).values())
        gate_control = controls.get("controls", {}).get("NC-GATE-01", {})
        check("v0242:negative-controls", controls_ok and gate_control.get("observed_type") == "str" and gate_control.get("observed_value") == "true" and gate_control.get("observed_gate_status") == "FAIL", "F31R-F35R negative controls traverse real rejections and preserve non-bool gate observations")
        check("v0242:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.2 executions are deterministic")
        check("v0242:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0242:provider-boundary", provider.get("boundary", {}).get("status") == "PASS" and provider.get("snapshot", {}).get("provider_submit_calls") == 0 and all(value == 0 for value in provider.get("spies", {}).values()), "actual provider entrypoint spies observe zero submissions and the source boundary is clean")
        check("v0242:family-concurrency", family.get("peak_global", 0) > 1 and family.get("peak_global", 0) <= 3 and family.get("dispatch_count") == 4 and family.get("family_peaks", {}).get("maps_minimap") == 1, "independent family work is admitted while global and family limits remain bounded")
        check("v0242:authority", len(execution.get("request", {}).get("dependency_refs", [])) >= 3 and execution.get("authority", {}).get("resolved", {}).get("status") == "PASS", "read-only approved authorities are resolved from repository bytes and hashes")
        check("v0242:frozen-v0241", frozen.get("status") == "PASS" and frozen.get("authority_commit") == "ed9fa927fd50193130b3e085ef077dea267f2790" and frozen.get("differences") == [], "v0.24.1 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0242:correction-record", correction.get("status") == "CORRECTION_REQUIRED" and correction.get("base_main_sha") == "dee98f8cd89ebd83a36ead7a22a184700d6e916f" and correction.get("reviewed_head") == "ed9fa927fd50193130b3e085ef077dea267f2790" and correction.get("findings") == ["F-31R", "F-32R", "F-34R", "F-35R"] and correction.get("historical_evidence_unchanged") is True, "forward-only F31R-F35R correction record binds the rejected v0.24.1 head")
        check("v0242:schema-validation", schema_validation.get("status") == "PASS" and all(item.get("status") == "PASS" for item in schema_validation.get("checks", [])), "v0.24.2 runtime evidence validates against v0.24.2 schemas")
        check("v0242:deadline-probe", execution.get("correction_probes", {}).get("deadline_probe", {}).get("before_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("exact_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("after_boundary") is True, "request deadline hard gate sources the deterministic boundary probe")
        check("v0242:circuit-runtime", execution.get("circuit", {}).get("opened") is True and execution.get("circuit", {}).get("blocked_delta") == 0 and execution.get("circuit", {}).get("probe_delta") == 1, "actual runtime circuit breaker opens and blocks dispatch before executor increment")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0242:evidence", False, str(exc))


def _v0243_checks() -> None:
    """Validate immutable v0.24.3 F32RR-F35RR-F35RC-F37 orchestration correction history."""
    evidence_root = ROOT / "docs/evidence/orchestration-runtime-v0243"
    required = [
        "schemas/current-state-v0243.json", "schemas/orchestration-runtime-v0243.json", "src/ugas/orchestration_runtime_v0243.py", "src/ugas/state_consistency_v0243.py",
        "scripts/validation/run_orchestration_runtime_v0243.py", "scripts/validation/validate_state_consistency_v0243.py", "scripts/validation/validate_orchestration_schema_v0243.py",
        "scripts/validation/build_github_review_manifest_v0243.py", "scripts/validation/validate_github_review_manifest_v0243.py", "scripts/validation/validate_github_review_security_v0243.py", "scripts/validation/enforce_github_review_v0243.py", "scripts/validation/record_orchestration_results_v0243.py",
        "tests/test_orchestration_runtime_v0243.py", "CHECKPOINT.md", "docs/roadmap.md", "docs/ugas-v1-capability-matrix.json", "docs/evidence/current-state.json", "REVIEW-v0.24.3.md",
    ]
    names = ("request-contract-v0243.json", "dag-scheduler-v0243.json", "execution-state-v0243.json", "deadline-timeout-v0243.json", "checkpoint-resume-v0243.json", "idempotency-coordinator-v0243.json", "circuit-breaker-runtime-v0243.json", "dependency-authority-v0243.json", "provider-boundary-v0243.json", "family-concurrency-v0243.json", "frozen-v0241-fingerprint-v0243.json", "historical-immutability-v0243.json", "hard-gates-v0243.json", "negative-controls-v0243.json", "full-slice-two-run-determinism-v0243.json", "production-boundary-v0243.json", "correction-history-v0243.json", "execution-evidence-v0243.json", "schema-validation-v0243.json", "capability-matrix-validation-v0243.json")
    required += [f"docs/evidence/orchestration-runtime-v0243/{name}" for name in names]
    for relative in required:
        check(f"v0243:path:{relative}", (ROOT / relative).is_file(), "present" if (ROOT / relative).is_file() else "missing")
    try:
        execution = load_json(evidence_root / "execution-evidence-v0243.json")
        gates = load_json(evidence_root / "hard-gates-v0243.json")
        controls = load_json(evidence_root / "negative-controls-v0243.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0243.json")
        production = load_json(evidence_root / "production-boundary-v0243.json")
        provider = load_json(evidence_root / "provider-boundary-v0243.json")
        family = load_json(evidence_root / "family-concurrency-v0243.json")
        frozen_v0241 = load_json(evidence_root / "frozen-v0241-fingerprint-v0243.json")
        historical = load_json(evidence_root / "historical-immutability-v0243.json")
        correction = load_json(evidence_root / "correction-history-v0243.json")
        schema_validation = load_json(evidence_root / "schema-validation-v0243.json")
        runtime_schema = load_json(ROOT / "schemas/orchestration-runtime-v0243.json")
        validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        state = load_json(ROOT / "docs/evidence/current-state.json")
        history = state.get("correction_history", {}).get("v0.24.3", {})
        check("v0243:history", history.get("status") == "CORRECTION_REQUIRED" and history.get("rejected_reviewed_head") == "5a619c0b98ced7e4c09afd9ca117a039f0d5d068" and history.get("historical_evidence_unchanged") is True, "v0.24.3 remains immutable correction-required history")
        check("v0243:execution", execution.get("schema_version") == "0.24.3" and execution.get("overall_pass") is True and execution.get("production", {}).get("production_routing") == "BLOCKED" and execution.get("production", {}).get("production_approved") is False and execution.get("production", {}).get("new_generation") == 0, "v0.24.3 F32RR-F35RR-F35RC-F37 correction is technically qualified and TEST_ONLY")
        check("v0243:hard-gates", gates.get("overall_pass") is True and len(gates.get("gates", {})) >= 40 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gates.get("gates", {}).values()), "all v0.24.3 hard gates use strict observed booleans")
        controls_ok = controls.get("status") == "PASS" and len(controls.get("controls", {})) >= 20 and all(item.get("status") == "PASS" and item.get("result") in {"REJECT", None} and (item.get("observed_rejection_class") is None or item.get("expected_rejection_class") == item.get("observed_rejection_class")) for item in controls.get("controls", {}).values())
        gate_control = controls.get("controls", {}).get("NC-GATE-01", {})
        check("v0243:negative-controls", controls_ok and gate_control.get("observed_type") == "str" and gate_control.get("observed_value") == "true" and gate_control.get("observed_gate_status") == "FAIL", "F32RR-F37 negative controls traverse real rejections and preserve non-bool gate observations")
        check("v0243:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.3 executions are deterministic")
        check("v0243:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0243:provider-boundary", provider.get("boundary", {}).get("status") == "PASS" and provider.get("snapshot", {}).get("provider_submit_calls") == 0 and all(value == 0 for value in provider.get("spies", {}).values()), "actual provider entrypoint spies observe zero submissions and the source boundary is clean")
        check("v0243:family-concurrency", family.get("peak_global", 0) > 1 and family.get("peak_global", 0) <= 3 and family.get("dispatch_count") == 4 and family.get("family_peaks", {}).get("maps_minimap") == 1, "independent family work is admitted while global and family limits remain bounded")
        check("v0243:authority", len(execution.get("request", {}).get("dependency_refs", [])) >= 3 and execution.get("authority", {}).get("resolved", {}).get("status") == "PASS", "read-only approved authorities are resolved from repository bytes and hashes")
        check("v0243:frozen-v0241", frozen_v0241.get("status") == "PASS" and frozen_v0241.get("authority_commit") == "ed9fa927fd50193130b3e085ef077dea267f2790" and frozen_v0241.get("differences") == [], "v0.24.1 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0243:frozen-v0242", historical.get("status") == "PASS" and historical.get("v0242", {}).get("status") == "PASS" and historical.get("v0242", {}).get("authority_commit") == "cd3345db7c0e587915e56eb9314879c4a9cf98a3", "v0.24.2 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0243:correction-record", correction.get("status") == "CORRECTION_REQUIRED" and correction.get("base_main_sha") == "dee98f8cd89ebd83a36ead7a22a184700d6e916f" and correction.get("reviewed_head") == "cd3345db7c0e587915e56eb9314879c4a9cf98a3" and correction.get("findings") == ["F-32RR", "F-35RR", "F-35RC", "F-37"] and correction.get("historical_evidence_unchanged") is True, "forward-only F32RR-F37 correction record binds the rejected v0.24.2 head")
        check("v0243:schema-validation", schema_validation.get("status") == "PASS" and all(item.get("status") == "PASS" for item in schema_validation.get("checks", [])), "v0.24.3 runtime evidence validates against v0.24.3 schemas")
        check("v0243:deadline-probe", execution.get("correction_probes", {}).get("deadline_probe", {}).get("before_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("exact_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("after_boundary") is True, "request deadline hard gate sources the deterministic boundary probe")
        check("v0243:circuit-runtime", execution.get("circuit", {}).get("opened") is True and execution.get("circuit", {}).get("blocked_delta") == 0 and execution.get("circuit", {}).get("probe_delta") == 1 and execution.get("circuit", {}).get("half_open_second_runtime", {}).get("status") == "PASS", "actual runtime circuit breaker opens, blocks dispatch and admits only one half-open probe")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0243:evidence", False, str(exc))


def _v0244_checks() -> None:
    """Validate immutable v0.24.4 F-38/F-37R/F-39/F-40 orchestration correction history."""
    evidence_root = ROOT / "docs/evidence/orchestration-runtime-v0244"
    required = [
        "schemas/current-state-v0244.json", "schemas/orchestration-runtime-v0244.json", "src/ugas/orchestration_runtime_v0244.py", "src/ugas/state_consistency_v0244.py",
        "scripts/validation/run_orchestration_runtime_v0244.py", "scripts/validation/validate_state_consistency_v0244.py", "scripts/validation/validate_orchestration_schema_v0244.py",
        "scripts/validation/build_github_review_manifest_v0244.py", "scripts/validation/validate_github_review_manifest_v0244.py", "scripts/validation/validate_github_review_security_v0244.py", "scripts/validation/enforce_github_review_v0244.py", "scripts/validation/record_orchestration_results_v0244.py",
        "tests/test_orchestration_runtime_v0244.py", "CHECKPOINT.md", "docs/roadmap.md", "docs/ugas-v1-capability-matrix.json", "docs/evidence/current-state.json", "REVIEW-v0.24.4.md",
    ]
    names = ("request-contract-v0244.json", "dag-scheduler-v0244.json", "execution-state-v0244.json", "deadline-timeout-v0244.json", "checkpoint-resume-v0244.json", "idempotency-coordinator-v0244.json", "circuit-breaker-runtime-v0244.json", "dependency-authority-v0244.json", "provider-boundary-v0244.json", "family-concurrency-v0244.json", "frozen-v0241-fingerprint-v0244.json", "historical-immutability-v0244.json", "hard-gates-v0244.json", "negative-controls-v0244.json", "full-slice-two-run-determinism-v0244.json", "production-boundary-v0244.json", "correction-history-v0244.json", "execution-evidence-v0244.json", "schema-validation-v0244.json", "capability-matrix-validation-v0244.json", "pre-fix-reproduction-v0244.json")
    required += [f"docs/evidence/orchestration-runtime-v0244/{name}" for name in names]
    for relative in required:
        check(f"v0244:path:{relative}", (ROOT / relative).is_file(), "present" if (ROOT / relative).is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        state_schema = load_json(ROOT / "schemas/current-state-v0244.json")
        runtime_schema = load_json(ROOT / "schemas/orchestration-runtime-v0244.json")
        execution = load_json(evidence_root / "execution-evidence-v0244.json")
        gates = load_json(evidence_root / "hard-gates-v0244.json")
        controls = load_json(evidence_root / "negative-controls-v0244.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0244.json")
        production = load_json(evidence_root / "production-boundary-v0244.json")
        provider = load_json(evidence_root / "provider-boundary-v0244.json")
        family = load_json(evidence_root / "family-concurrency-v0244.json")
        frozen_v0241 = load_json(evidence_root / "frozen-v0241-fingerprint-v0244.json")
        historical = load_json(evidence_root / "historical-immutability-v0244.json")
        correction = load_json(evidence_root / "correction-history-v0244.json")
        schema_validation = load_json(evidence_root / "schema-validation-v0244.json")
        reproduction = load_json(evidence_root / "pre-fix-reproduction-v0244.json")
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        history = state.get("correction_history", {}).get("v0.24.4", {})
        check("v0244:history", history.get("status") == "CORRECTION_REQUIRED" and history.get("rejected_reviewed_head") == "55d6ee80f29d4cf6ed9f2d65e0173dbba570a124" and history.get("historical_evidence_unchanged") is True, "v0.24.4 remains immutable correction-required history")
        check("v0244:execution", execution.get("schema_version") == "0.24.4" and execution.get("overall_pass") is True and execution.get("production", {}).get("production_routing") == "BLOCKED" and execution.get("production", {}).get("production_approved") is False and execution.get("production", {}).get("new_generation") == 0, "v0.24.4 F-38/F-37R/F-39/F-40 correction is technically qualified and TEST_ONLY")
        check("v0244:hard-gates", gates.get("overall_pass") is True and len(gates.get("gates", {})) >= 40 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gates.get("gates", {}).values()), "all v0.24.4 hard gates use strict observed booleans")
        controls_ok = controls.get("status") == "PASS" and len(controls.get("controls", {})) >= 20 and all(item.get("status") == "PASS" and item.get("result") in {"REJECT", None} and (item.get("observed_rejection_class") is None or item.get("expected_rejection_class") == item.get("observed_rejection_class")) for item in controls.get("controls", {}).values())
        gate_control = controls.get("controls", {}).get("NC-GATE-01", {})
        hist_ok = all(controls.get("controls", {}).get(name, {}).get("result") == "REJECT" and controls.get("controls", {}).get(name, {}).get("actual_exception") for name in ("NC-HIST-01", "NC-HIST-02", "NC-HIST-03"))
        check("v0244:negative-controls", controls_ok and hist_ok and gate_control.get("observed_type") == "str" and gate_control.get("observed_value") == "true" and gate_control.get("observed_gate_status") == "FAIL", "F-38/F-37R negative controls traverse real public validators")
        check("v0244:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.4 executions are deterministic")
        check("v0244:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0244:provider-boundary", provider.get("boundary", {}).get("status") == "PASS" and provider.get("snapshot", {}).get("provider_submit_calls") == 0 and all(value == 0 for value in provider.get("spies", {}).values()), "actual provider entrypoint spies observe zero submissions and the source boundary is clean")
        check("v0244:family-concurrency", family.get("peak_global", 0) > 1 and family.get("peak_global", 0) <= 3 and family.get("dispatch_count") == 4 and family.get("family_peaks", {}).get("maps_minimap") == 1, "independent family work is admitted while global and family limits remain bounded")
        check("v0244:authority", len(execution.get("request", {}).get("dependency_refs", [])) >= 3 and execution.get("authority", {}).get("resolved", {}).get("status") == "PASS", "read-only approved authorities are resolved from repository bytes and hashes")
        check("v0244:frozen-v0241", frozen_v0241.get("status") == "PASS" and frozen_v0241.get("authority_commit") == "ed9fa927fd50193130b3e085ef077dea267f2790" and frozen_v0241.get("differences") == [], "v0.24.1 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0244:frozen-v0242", historical.get("status") == "PASS" and historical.get("v0242", {}).get("status") == "PASS" and historical.get("v0242", {}).get("authority_commit") == "cd3345db7c0e587915e56eb9314879c4a9cf98a3", "v0.24.2 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0244:correction-record", correction.get("status") == "CORRECTION_REQUIRED" and correction.get("base_main_sha") == "dee98f8cd89ebd83a36ead7a22a184700d6e916f" and correction.get("reviewed_head") == "5a619c0b98ced7e4c09afd9ca117a039f0d5d068" and correction.get("findings") == ["F-38", "F-37R", "F-39", "F-40"] and correction.get("historical_evidence_unchanged") is True, "forward-only F-38/F-37R/F-39/F-40 correction record binds the rejected v0.24.3 head")
        check("v0244:schema-validation", schema_validation.get("status") == "PASS" and all(item.get("status") == "PASS" for item in schema_validation.get("checks", [])), "v0.24.4 runtime evidence validates against v0.24.4 schemas")
        check("v0244:deadline-probe", execution.get("correction_probes", {}).get("deadline_probe", {}).get("before_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("exact_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("after_boundary") is True, "request deadline hard gate sources the deterministic boundary probe")
        check("v0244:circuit-runtime", execution.get("circuit", {}).get("opened") is True and execution.get("circuit", {}).get("blocked_delta") == 0 and execution.get("circuit", {}).get("probe_delta") == 1 and execution.get("circuit", {}).get("half_open_second_runtime", {}).get("status") == "PASS", "actual runtime circuit breaker opens, blocks dispatch and admits only one half-open probe")
        check("v0244:pre-fix-reproduction", reproduction.get("status") == "REPRODUCED" and reproduction.get("observed_rejection_class") == "ORCH_DEPENDENCY_AUTHORITY_COMMIT_MISSING" and reproduction.get("rejected_head") == "5a619c0b98ced7e4c09afd9ca117a039f0d5d068", "v0.24.3 snapshot/no-git failure was reproduced before the correction")
        check("v0244:git-witness", execution.get("authority", {}).get("resolved", {}).get("verification_mode") == "GIT_COMMIT_WITNESS", "git-backed executions record GIT_COMMIT_WITNESS")
        check("v0244:binding-equal", controls.get("controls", {}).get("NC-AUTH-BIND-EQ", {}).get("status") == "PASS", "git and no-git modes produce the same authority_binding_hash")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0244:evidence", False, str(exc))


def _v0245_checks() -> None:
    """Validate immutable v0.24.5 F-41/F-42/F-43 orchestration correction history."""
    evidence_root = ROOT / "docs/evidence/orchestration-runtime-v0245"
    required = [
        "schemas/current-state-v0245.json", "schemas/orchestration-runtime-v0245.json", "src/ugas/orchestration_runtime_v0245.py", "src/ugas/state_consistency_v0245.py",
        "scripts/validation/run_orchestration_runtime_v0245.py", "scripts/validation/validate_state_consistency_v0245.py", "scripts/validation/validate_orchestration_schema_v0245.py",
        "scripts/validation/build_github_review_manifest_v0245.py", "scripts/validation/validate_github_review_manifest_v0245.py", "scripts/validation/validate_github_review_security_v0245.py", "scripts/validation/enforce_github_review_v0245.py", "scripts/validation/record_orchestration_results_v0245.py",
        "tests/test_orchestration_runtime_v0245.py", "CHECKPOINT.md", "docs/roadmap.md", "docs/ugas-v1-capability-matrix.json", "docs/evidence/current-state.json", "REVIEW-v0.24.5.md",
    ]
    names = ("request-contract-v0245.json", "dag-scheduler-v0245.json", "execution-state-v0245.json", "deadline-timeout-v0245.json", "checkpoint-resume-v0245.json", "idempotency-coordinator-v0245.json", "circuit-breaker-runtime-v0245.json", "dependency-authority-v0245.json", "provider-boundary-v0245.json", "family-concurrency-v0245.json", "frozen-v0241-fingerprint-v0245.json", "historical-immutability-v0245.json", "hard-gates-v0245.json", "negative-controls-v0245.json", "full-slice-two-run-determinism-v0245.json", "production-boundary-v0245.json", "correction-history-v0245.json", "execution-evidence-v0245.json", "schema-validation-v0245.json", "capability-matrix-validation-v0245.json", "uads-handoff-v0245.json")
    required += [f"docs/evidence/orchestration-runtime-v0245/{name}" for name in names]
    for relative in required:
        check(f"v0245:path:{relative}", (ROOT / relative).is_file(), "present" if (ROOT / relative).is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        runtime_schema = load_json(ROOT / "schemas/orchestration-runtime-v0245.json")
        execution = load_json(evidence_root / "execution-evidence-v0245.json")
        gates = load_json(evidence_root / "hard-gates-v0245.json")
        controls = load_json(evidence_root / "negative-controls-v0245.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0245.json")
        production = load_json(evidence_root / "production-boundary-v0245.json")
        provider = load_json(evidence_root / "provider-boundary-v0245.json")
        family = load_json(evidence_root / "family-concurrency-v0245.json")
        frozen_v0241 = load_json(evidence_root / "frozen-v0241-fingerprint-v0245.json")
        historical = load_json(evidence_root / "historical-immutability-v0245.json")
        correction = load_json(evidence_root / "correction-history-v0245.json")
        schema_validation = load_json(evidence_root / "schema-validation-v0245.json")
        uads_handoff = load_json(evidence_root / "uads-handoff-v0245.json")
        validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        history = state.get("correction_history", {}).get("v0.24.5", {})
        check("v0245:history", history.get("status") == "CORRECTION_REQUIRED" and history.get("rejected_reviewed_head") == "2a5c1d6d88c1348c1490cb9c89f16e2cc6b64362" and history.get("historical_evidence_unchanged") is True, "v0.24.5 remains immutable correction-required history")
        check("v0245:execution", execution.get("schema_version") == "0.24.5" and execution.get("overall_pass") is True and execution.get("production", {}).get("production_routing") == "BLOCKED" and execution.get("production", {}).get("production_approved") is False and execution.get("production", {}).get("new_generation") == 0, "v0.24.5 F-41/F-42/F-43 correction is technically qualified and TEST_ONLY")
        check("v0245:hard-gates", gates.get("overall_pass") is True and len(gates.get("gates", {})) >= 56 and all(item.get("status") == "PASS" and type(item.get("observed")) is bool and item.get("observed") is True and item.get("observed_type") == "bool" for item in gates.get("gates", {}).values()), "all v0.24.5 hard gates use strict observed booleans")
        controls_ok = controls.get("status") == "PASS" and len(controls.get("controls", {})) >= 33 and all(item.get("status") == "PASS" and item.get("result") in {"REJECT", None} and (item.get("observed_rejection_class") is None or item.get("expected_rejection_class") == item.get("observed_rejection_class")) for item in controls.get("controls", {}).values())
        gate_control = controls.get("controls", {}).get("NC-GATE-01", {})
        ...
        check("v0245:negative-controls", controls_ok and hist_ok and status_ok and gate_control.get("observed_type") == "str" and gate_control.get("observed_value") == "true" and gate_control.get("observed_gate_status") == "FAIL", "F-41/F-42 negative controls traverse real public validators")
        check("v0245:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.5 executions are deterministic")
        check("v0245:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0245:provider-boundary", provider.get("boundary", {}).get("status") == "PASS" and provider.get("snapshot", {}).get("provider_submit_calls") == 0 and all(value == 0 for value in provider.get("spies", {}).values()), "actual provider entrypoint spies observe zero submissions and the source boundary is clean")
        check("v0245:family-concurrency", family.get("peak_global", 0) > 1 and family.get("peak_global", 0) <= 3 and family.get("dispatch_count") == 4 and family.get("family_peaks", {}).get("maps_minimap") == 1, "independent family work is admitted while global and family limits remain bounded")
        check("v0245:authority", len(execution.get("request", {}).get("dependency_refs", [])) >= 3 and execution.get("authority", {}).get("resolved", {}).get("status") == "PASS", "read-only approved authorities are resolved from repository bytes and hashes")
        check("v0245:frozen-v0241", frozen_v0241.get("status") == "PASS" and frozen_v0241.get("authority_commit") == "ed9fa927fd50193130b3e085ef077dea267f2790" and frozen_v0241.get("differences") == [] and frozen_v0241.get("equality") is True and frozen_v0241.get("authority_fingerprint") == frozen_v0241.get("observed_fingerprint"), "v0.24.1 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0245:frozen-v0242", historical.get("status") == "PASS" and historical.get("v0242", {}).get("status") == "PASS" and historical.get("v0242", {}).get("authority_commit") == "cd3345db7c0e587915e56eb9314879c4a9cf98a3" and historical.get("v0242", {}).get("equality") is True, "v0.24.2 review/evidence remains unchanged against its rejected reviewed HEAD")
        check("v0245:correction-record", correction.get("status") == "CORRECTION_REQUIRED" and correction.get("base_main_sha") == "dee98f8cd89ebd83a36ead7a22a184700d6e916f" and correction.get("reviewed_head") == "55d6ee80f29d4cf6ed9f2d65e0173dbba570a124" and correction.get("findings") == ["F-41", "F-42", "F-43"] and correction.get("historical_evidence_unchanged") is True, "forward-only F-41/F-42/F-43 correction record binds the rejected v0.24.4 head")
        check("v0245:schema-validation", schema_validation.get("status") == "PASS" and all(item.get("status") == "PASS" for item in schema_validation.get("checks", [])), "v0.24.5 runtime evidence validates against v0.24.5 schemas")
        check("v0245:deadline-probe", execution.get("correction_probes", {}).get("deadline_probe", {}).get("before_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("exact_boundary") is True and execution.get("correction_probes", {}).get("deadline_probe", {}).get("after_boundary") is True, "request deadline hard gate sources the deterministic boundary probe")
        check("v0245:circuit-runtime", execution.get("circuit", {}).get("opened") is True and execution.get("circuit", {}).get("blocked_delta") == 0 and execution.get("circuit", {}).get("probe_delta") == 1 and execution.get("circuit", {}).get("half_open_second_runtime", {}).get("status") == "PASS", "actual runtime circuit breaker opens, blocks dispatch and admits only one half-open probe")
        check("v0245:git-witness", execution.get("authority", {}).get("resolved", {}).get("verification_mode") == "GIT_COMMIT_WITNESS", "git-backed executions record GIT_COMMIT_WITNESS")
        check("v0245:binding-equal", controls.get("controls", {}).get("NC-AUTH-BIND-EQ", {}).get("status") == "PASS", "git and no-git modes produce the same authority_binding_hash")
        check("v0245:uads-handoff", uads_handoff.get("execution_mode") == "GLOBAL_FIRST" and uads_handoff.get("project_footprint") == "ZERO" and uads_handoff.get("work_order_id") == "wo_e3f4f80bd105a820" and uads_handoff.get("run_or_dispatch_id") == "er_9287a86fc6478cfa" and ".uads" not in json.dumps(uads_handoff), "sanitized UADS identifiers persist without project-local runtime material")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0245:evidence", False, str(exc))
...
def _v0246_checks() -> None:
    ...
    required = [
        ...
        "scripts/validation/run_orchestration_runtime_v0246.py", "scripts/validation/validate_state_consistency_v0246.py", "scripts/validation/validate_orchestration_schema_v0246.py",
        "scripts/validation/build_github_review_manifest_v0246.py", "scripts/validation/validate_github_review_manifest_v0246.py", "scripts/validation/validate_github_review_security_v0246.py", "scripts/validation/enforce_github_review_v0246.py", "scripts/validation/record_orchestration_results_v0246.py",
        "tests/test_orchestration_runtime_v0246.py", "CHECKPOINT.md", "docs/roadmap.md", "docs/ugas-v1-capability-matrix.json", "docs/evidence/current-state.json", "REVIEW-v0.24.6.md",
    ]
    names = ("request-contract-v0246.json", "dag-scheduler-v0246.json", "execution-state-v0246.json", "deadline-timeout-v0246.json", "checkpoint-resume-v0246.json", "idempotency-coordinator-v0246.json", "circuit-breaker-runtime-v0246.json", "dependency-authority-v0246.json", "provider-boundary-v0246.json", "family-concurrency-v0246.json", "frozen-v0241-fingerprint-v0246.json", "historical-immutability-v0246.json", "hard-gates-v0246.json", "negative-controls-v0246.json", "full-slice-two-run-determinism-v0246.json", "production-boundary-v0246.json", "correction-history-v0246.json", "execution-evidence-v0246.json", "schema-validation-v0246.json", "capability-matrix-validation-v0246.json", "uads-handoff-v0246.json")
    required += [f"docs/evidence/orchestration-runtime-v0246/{name}" for name in names]
    for relative in required:
    ...
    try:
        ...
        controls = load_json(evidence_root / "negative-controls-v0246.json")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0246.json")
        production = load_json(evidence_root / "production-boundary-v0246.json")
        provider = load_json(evidence_root / "provider-boundary-v0246.json")
        family = load_json(evidence_root / "family-concurrency-v0246.json")
        frozen_v0241 = load_json(evidence_root / "frozen-v0241-fingerprint-v0246.json")
        historical = load_json(evidence_root / "historical-immutability-v0246.json")
        ...
        sum_ok = all(controls.get("controls", {}).get(name, {}).get("status") == "PASS" for name in ("NC-SUM-01", "NC-SUM-02", "NC-SUM-03", "NC-SUM-04", "NC-SUM-05"))
        ...
        check("v0246:negative-controls", controls_ok and hist_ok and status_ok and uads_ok and sum_ok and chist_ok and gate_control.get("observed_type") == "str" and gate_control.get("observed_value") == "true" and gate_control.get("observed_gate_status") == "FAIL", "F-43R/F-44/F-45 negative controls traverse real public validators")
        check("v0246:determinism", determinism.get("status") == "PASS" and determinism.get("run_1") == determinism.get("run_2"), "two independent v0.24.6 executions are deterministic")
        check("v0246:production-boundary", production == {"production_approved": False, "production_routing": "BLOCKED", "real_asset_generation": "NONE", "new_generation": 0, "provider_submit_calls": 0, "synthetic_fixture": "TEST_ONLY"}, "production remains blocked with no real generation or provider submission")
        check("v0246:provider-boundary", provider.get("boundary", {}).get("status") == "PASS" and provider.get("snapshot", {}).get("provider_submit_calls") == 0 and all(value == 0 for value in provider.get("spies", {}).values()), "actual provider entrypoint spies observe zero submissions and the source boundary is clean")
        check("v0246:family-concurrency", family.get("peak_global", 0) > 1 and family.get("peak_global", 0) <= 3 and family.get("dispatch_count") == 4 and family.get("family_peaks", {}).get("maps_minimap") == 1, "independent family work is admitted while global and family limits remain bounded")
        check("v0246:authority", len(execution.get("request", {}).get("dependency_refs", [])) >= 3 and execution.get("authority", {}).get("resolved", {}).get("status") == "PASS", "read-only approved authorities are resolved from repository bytes and hashes")
        ...
        check("v0246:binding-equal", controls.get("controls", {}).get("NC-AUTH-BIND-EQ", {}).get("status") == "PASS", "git and no-git modes produce the same authority_binding_hash")
...
def _v0247_checks() -> None:
    ...
    required = [
        ...
        "docs/evidence/github-governance-v0247/v0247-external-approval.json",
        "docs/evidence/orchestration-runtime-v0247/state-snapshot-v0247.json", "docs/evidence/orchestration-runtime-v0247/matrix-snapshot-v0247.json", "docs/evidence/orchestration-runtime-v0247/tracked-docs-snapshot-v0247.json",
        "scripts/validation/build_v0247_tracked_docs_snapshot.py",
    ]
    ...
    try:
        state = load_json(ROOT / "docs/evidence/orchestration-runtime-v0247/state-snapshot-v0247.json")
        ...
        approval_record = load_json(ROOT / "docs/evidence/github-governance-v0247/v0247-external-approval.json")
        tracked_docs = load_json(ROOT / "docs/evidence/orchestration-runtime-v0247/tracked-docs-snapshot-v0247.json")
        matrix = load_json(ROOT / "docs/evidence/orchestration-runtime-v0247/matrix-snapshot-v0247.json")
        validate_schema_document(state_schema); validate_instance(state, state_schema); validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        consistency = validate_state_consistency_v0247(state, str(tracked_docs.get("checkpoint_text", "")), str(tracked_docs.get("roadmap_text", "")), matrix, correction, state.get("evidence", {}))
        ...
        check("v0247:approval-record", approval_record.get("status") == "APPROVED_FOUNDATION" and approval_record.get("approved_semantic_head") == "6b1af57ec5f488d71bafafa17a892467adf1d1c1" and approval_record.get("approval_comment_id") == 5621522467 and approval_record.get("merge_authorization") == "GOVERNED_MERGE_AUTHORIZED_AFTER_EXACT_HEAD_REPROOF" and approval_record.get("approval_boundary", {}).get("production_approved") is False and approval_record.get("approval_boundary", {}).get("production_routing") == "BLOCKED" and approval_record.get("approval_boundary", {}).get("new_generation") == 0 and approval_record.get("bookkeeping_authorization", {}).get("post_bookkeeping_reproof_required") is True, "canonical approval record binds the approval comment, approved semantic head and production block")
...
def _v0250_checks() -> None:
    """Validate the active v0.25.0 V1 final acceptance candidate slice."""
    evidence_root = ROOT / "docs/evidence/v1-final-acceptance"
    evidence_names = (
        "capability-matrix-audit.json", "architecture-audit.json", "security-audit.json", "reproducibility-audit.json",
        "state-consistency-audit.json", "test-summary.json", "hard-gates.json", "negative-controls.json",
        "production-boundary.json", "uads-handoff.json", "final-acceptance-summary.json",
    )
    required = [
        "docs/definition-of-done.md", "REVIEW-UGAS-V1-FINAL-ACCEPTANCE.md", "schemas/current-state-v0250.json",
        "src/ugas/acceptance_v0250.py", "src/ugas/state_consistency_v0250.py",
        "scripts/validation/run_v1_final_acceptance_v0250.py", "scripts/validation/validate_state_consistency_v0250.py",
        "scripts/validation/validate_v1_capability_matrix.py", "scripts/validation/validate_github_review_security_v0250.py",
        "scripts/validation/build_github_review_manifest_v0250.py", "scripts/validation/validate_github_review_manifest_v0250.py",
    ...
    for relative in required:
        ...
        check(f"v0250:path:{relative}", present, "present" if present else "missing")
    state = matrix = None
    try:
        state = load_json(ROOT / V1_STATE_SNAPSHOT)
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        state_schema = load_json(ROOT / "schemas/current-state-v0250.json")
        validate_schema_document(state_schema)
        validate_instance(state, state_schema)
        check("v0250:state-schema", True, "the preserved v0.25.0 state snapshot validates against the v0.25.0 schema")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0250:state-schema", False, str(exc))
    try:
        summary = load_json(evidence_root / "final-acceptance-summary.json")
        capability_audit = load_json(evidence_root / "capability-matrix-audit.json")
        architecture = load_json(evidence_root / "architecture-audit.json")
        security = load_json(evidence_root / "security-audit.json")
        reproducibility = load_json(evidence_root / "reproducibility-audit.json")
        state_audit = load_json(evidence_root / "state-consistency-audit.json")
        gates = load_json(evidence_root / "hard-gates.json")
        controls = load_json(evidence_root / "negative-controls.json")
        production = load_json(evidence_root / "production-boundary.json")
        uads_evidence = load_json(evidence_root / "uads-handoff.json")
        capability_validation = load_json(evidence_root / "capability-matrix-validation-v0250.json")
        ...
        check("v0250:observability", acceptance.get("observability_visual_review") == "PASS" and acceptance.get("critical") == 0 and acceptance.get("high") == 0 and acceptance.get("unresolved_high_critical") == 0 and acceptance.get("medium_blocking_acceptance") == 0, "observability visual gate is resolved against the bound approval and no blocking finding remains open")
        audit_input = {
            ...
            "observability": {"visual_review_status": acceptance.get("observability_visual_review")},
        }
        consistency = validate_state_consistency_v0250(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"), matrix, audit_input)
        check("v0250:state-consistency", not consistency["failures"] and consistency["status"] != "V1_FINAL_ACCEPTANCE_STATE_FAILED" and state_audit.get("status") == consistency["status"] and not state_audit.get("failures"), "; ".join(consistency["failures"]) or "checkpoint, roadmap, state, schema and capability matrix stay consistent")
        snapshot = reproducibility.get("snapshot_binding", {}) if isinstance(reproducibility.get("snapshot_binding"), dict) else {}
        snapshot_entries = snapshot.get("entries", []) if isinstance(snapshot.get("entries"), list) else []
        check("v0250:snapshot-binding", reproducibility.get("status") == "PASS" and snapshot.get("status") == "PASS" and len(snapshot_entries) == 2 and all(isinstance(entry, dict) and entry.get("status") == "MATCH" and entry.get("observed_sha256") == entry.get("expected_sha256") for entry in snapshot_entries), "the frozen v0.24.7 state and matrix snapshots still match their bound digests")
        determinism = summary.get("determinism", {}) if isinstance(summary.get("determinism"), dict) else {}
        digests = [determinism.get("run_1"), determinism.get("run_2"), determinism.get("run_3")]
        digests_ok = all(isinstance(value, str) and re.fullmatch(r"[0-9a-f]{64}", value) is not None for value in digests)
        check("v0250:two-run-determinism", determinism.get("status") == "PASS" and digests_ok and len(set(digests)) == 1, "independent acceptance executions produced one canonical deterministic digest")
        gate_records = gates.get("gates", {}) if isinstance(gates.get("gates"), dict) else {}
        gates_ok = all(isinstance(gate_records.get(gate_id), dict) and type(gate_records[gate_id].get("observed")) is bool and (gate_records[gate_id].get("status") == "PASS") is gate_records[gate_id].get("observed") for gate_id in HARD_GATE_IDS)
        check("v0250:hard-gates-strict", gates_ok and len(gate_records) >= len(HARD_GATE_IDS), f"{len(HARD_GATE_IDS)} hard gates recorded as strict boolean observations with matching status")
        environment = summary.get("environment", {}) if isinstance(summary.get("environment"), dict) else {}
        environment_gates = environment.get("gates", {}) if isinstance(environment.get("gates"), dict) else {}
        environment_issues = []
        for gate_id in ENVIRONMENT_GATE_IDS:
            record = gate_records.get(gate_id) if isinstance(gate_records.get(gate_id), dict) else {}
            observed = record.get("observed")
            if type(observed) is not bool or observed is not (environment_gates.get(gate_id) == "PASS"):
                environment_issues.append(gate_id)
        check("v0250:environment-cross-binding", not environment_issues and environment.get("status") in {"MISSING", "PENDING", "PASS"}, "; ".join(environment_issues) or f"environment gate observations cross-bind the recorded status ({environment.get('status')})")
        review = summary.get("review", {}) if isinstance(summary.get("review"), dict) else {}
        pr_number = review.get("pr_number")
        pr_head_sha = review.get("pr_head_sha")
        pr_bound = type(pr_number) is int and pr_number >= 1 and isinstance(pr_head_sha, str) and re.fullmatch(r"[0-9a-f]{40}", pr_head_sha) is not None and review.get("pr_state") == "OPEN"
        ...
        check("v0250:production-boundary", production.get("status") == "BLOCKED" and production.get("acceptance_claim_allowed") is False and boundary.get("production_approved") is False and boundary.get("production_routing") == "BLOCKED" and boundary.get("new_generation") == 0 and boundary.get("provider_submit_calls") == 0 and boundary.get("real_asset_generation") == "NONE" and boundary.get("v1_technical_acceptance_is_not_production_approval") is True, "technical acceptance stays inside the blocked production boundary")
        handoff = uads_evidence.get("handoff", {}) if isinstance(uads_evidence.get("handoff"), dict) else {}
        work_order_id = str(handoff.get("work_order_id", ""))
        dispatch_status = handoff.get("dispatch_status")
        dispatch_ok = dispatch_status == "DISPATCHED" and re.fullmatch(r"er_[0-9a-f]{16}", str(handoff.get("run_or_dispatch_id"))) is not None
        pending_ok = dispatch_status == "PENDING_DISPATCH" and handoff.get("run_or_dispatch_id") is None
        check("v0250:uads-handoff", handoff.get("execution_mode") == "GLOBAL_FIRST" and handoff.get("project_footprint") == "ZERO" and handoff.get("route_status") == "SELECTED" and handoff.get("repo_local_material") == "ABSENT" and re.fullmatch(r"wo_[0-9a-f]{16}", work_order_id) is not None and (dispatch_ok or pending_ok) and uads_evidence.get("validation", {}).get("status") == "PASS" and ".uads" not in json.dumps(uads_evidence), "the sanitized UADS handoff exposes no repository-local runtime material")
        check("v0250:capability-matrix-validation", capability_validation.get("status") == "V1_CAPABILITY_MATRIX_PASSED" and not capability_validation.get("failures") and capability_validation.get("capability_count") == 16 and capability_validation.get("next_candidate") == "V1_FINAL_ACCEPTANCE" and capability_validation.get("production_routing") == "BLOCKED" and capability_validation.get("new_generation") == 0, "the capability order validator passes on the active matrix")
        inventory = summary.get("evidence_inventory", []) if isinstance(summary.get("evidence_inventory"), list) else []
        inventory_names = sorted(entry.get("name") for entry in inventory if isinstance(entry, dict))
        inventory_issues = []
        for entry in inventory:
            ...
            if entry.get("status") != expected:
                inventory_issues.append(f"{entry.get('name')}:{entry.get('status')}")
        check("v0250:evidence-inventory", len(inventory) == len(evidence_names) and inventory_names == sorted(evidence_names) and not inventory_issues, "; ".join(inventory_issues) or "all 11 acceptance evidence files are present")
        definition = " ".join((ROOT / "docs/definition-of-done.md").read_text(encoding="utf-8").split())
        definition_literals = ("V1_FINAL_ACCEPTANCE", "production_approved=false", "production_routing=BLOCKED", "real_asset_generation=NONE", "new_generation=0", "production readiness workstream", "not production approval")
        check("v0250:definition-of-done", all(literal in definition for literal in definition_literals), "; ".join(literal for literal in definition_literals if literal not in definition) or "Definition of Done separates technical V1 acceptance from production readiness")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0250:evidence", False, str(exc))


def _v0251_checks() -> None:
    """Validate the active v0.25.1 canonical-state reconciliation and merged closure evidence."""
    evidence_root = ROOT / "docs/evidence/canonical-state-reconciliation-v0251"
    required = [
        "schemas/current-state-v0251.json",
        "src/ugas/state_consistency_v0251.py",
        "scripts/validation/run_canonical_state_reconciliation_v0251.py",
        "scripts/validation/validate_state_consistency_v0251.py",
        "scripts/validation/validate_v1_closure_v0251.py",
        "tests/test_canonical_state_reconciliation_v0251.py",
        V1_STATE_SNAPSHOT,
        "docs/definition-of-done.md",
        "docs/evidence/v1-final-acceptance/hard-gates.json",
    ]
    required += [f"docs/evidence/canonical-state-reconciliation-v0251/{name}" for name in V0251_CLOSURE_EVIDENCE_FILES]
    required += ["docs/evidence/canonical-state-reconciliation-v0251/proposed-checkpoint-delta-v0251.md"]
    for relative in required:
        present = (ROOT / relative).is_file()
        check(f"v0251:path:{relative}", present, "present" if present else "missing")
    state = None
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        schema = load_json(ROOT / "schemas/current-state-v0251.json")
        validate_schema_document(schema)
        validate_instance(state, schema)
        check("v0251:state-schema", state.get("version") == V0251_VERSION and state.get("schema_version") == V0251_VERSION, "the active current-state validates against the v0.25.1 schema")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
        check("v0251:state-schema", False, str(exc))
    try:
        if state is None:
            raise ValueError("active canonical state unavailable")
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        capability_audit = load_json(ROOT / "docs/evidence/v1-final-acceptance/capability-matrix-audit.json")
        summary = load_json(ROOT / "docs/evidence/v1-final-acceptance/final-acceptance-summary.json")
        acceptance = summary.get("acceptance", {}) if isinstance(summary.get("acceptance"), dict) else {}
        audit_input = {
            "capability_count": capability_audit.get("capability_count"),
            "production_routing": state.get("production_routing"),
        ...
        consistency = validate_state_consistency_v0251(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"), matrix, audit_input)
        check("v0251:state-consistency", consistency["status"] == CURRENT_GATE_V0251 and not consistency["failures"], "; ".join(consistency["failures"]) or "the merged-closed v0.25.1 canonical state is consistent across checkpoint, roadmap, matrix and schema")
        check("v0251:production-boundary", state.get("production_routing") == "BLOCKED" and state.get("production_approved") is False and state.get("real_asset_generation") == "NONE" and state.get("new_generation") == 0 and state.get("provider_submit_calls") == 0 and state.get("production_readiness_workstream") == "NOT_STARTED_REQUIRED_SEPARATELY", "the merged V1 technical baseline stays inside the blocked production boundary")
        allowed = state.get("allowed_next_actions", []) if isinstance(state.get("allowed_next_actions"), list) else []
        forbidden = state.get("forbidden_actions", []) if isinstance(state.get("forbidden_actions"), list) else []
        check("v0251:allowed-forbidden", allowed == ["define_and_review_production_readiness_work_order"] and not any(action in forbidden for action in allowed), "the sole allowed next action defines the production-readiness work order and is not forbidden")
        binding_evidence = load_json(evidence_root / "closure-binding-v0251.json")
        binding_failures = validate_closure_binding_v0251(binding_evidence)
        closure = state.get("v1_closure", {}) if isinstance(state.get("v1_closure"), dict) else {}
        record = binding_evidence.get("binding", {}) if isinstance(binding_evidence.get("binding"), dict) else {}
        cross_binding = [key for key in ("semantic_head", "bookkeeping_head", "merge_main_sha", "post_merge_ci_run", "unit_job", "docker_job", "closure_comment_id", "audit_comment_id", "audit_verdict") if closure.get(key) != record.get(key)]
        check("v0251:closure-binding", not binding_failures and not cross_binding, "; ".join([*binding_failures, *cross_binding]) or "the append-only closure evidence binds the merged V1 identities and matches the tracked v1 closure block")
        snapshot = binding_evidence.get("v0250_state_snapshot", {}) if isinstance(binding_evidence.get("v0250_state_snapshot"), dict) else {}
        snapshot_sha256 = hashlib.sha256((ROOT / V1_STATE_SNAPSHOT).read_bytes().replace(b"\r\n", b"\n")).hexdigest()
        ...
        roots_guarded = bool(frozen_roots) and all(isinstance(record, dict) and record.get("status") == "UNCHANGED" and record.get("git_diff_empty") is True and record.get("git_status_clean") is True for record in frozen_roots.values())
        check("v0251:immutability-proof", not immutability_problems and immutability_evidence.get("status") == "PASS" and immutability_evidence.get("git_guarded") is True and roots_guarded, "; ".join(immutability_problems) or "both frozen evidence roots are recorded byte-identical, git-guarded and diff-free at the merged baseline")
        handoff_evidence = load_json(evidence_root / "uads-handoff-v0251.json")
        handoff = handoff_evidence.get("handoff", {}) if isinstance(handoff_evidence.get("handoff"), dict) else {}
        handoff_validation = handoff_evidence.get("validation") if isinstance(handoff_evidence.get("validation"), dict) else {}
        handoff_ok = handoff_evidence.get("status") == "PASS" and handoff_evidence.get("version") == V0251_VERSION and handoff_validation.get("status") == "PASS" and ".uads" not in json.dumps(handoff_evidence) and handoff.get("execution_mode") == "GLOBAL_FIRST" and handoff.get("project_footprint") == "ZERO" and handoff.get("route_status") == "SELECTED" and handoff.get("repo_local_material") == "ABSENT" and re.fullmatch(r"wo_[0-9a-f]{16}", str(handoff.get("work_order_id"))) is not None and re.fullmatch(r"er_[0-9a-f]{16}", str(handoff.get("run_or_dispatch_id"))) is not None and handoff.get("dispatch_status") in {"PREPARED", "DISPATCHED"}
        check("v0251:uads-handoff", handoff_ok, "the sanitized v0.25.1 UADS handoff is global-first with zero repository-local footprint")
        ...
        declared = definition_of_done_hard_gate_count_v0251(definition)
        check("v0251:definition-of-done", declared == V0251_HARD_GATE_COUNT and len(gate_records) == V0251_HARD_GATE_COUNT and hard_gates.get("overall_pass") is True, f"Definition of Done declares {declared} hard gates and the frozen acceptance evidence passes {len(gate_records)}")
    except (OSError, json.JSONDecodeError, KeyError, SchemaValidationError, ValueError, TypeError) as exc:
...
def _v0210_checks() -> None:
    ...
    required = [
        ...
        "src/ugas/maps_minimap_runtime_v0210.py", "src/ugas/state_consistency_v0210.py",
        "scripts/validation/run_maps_minimap_runtime_v0210.py", "scripts/validation/validate_state_consistency_v0210.py",
        "tests/test_maps_minimap_runtime_v0210.py", "docs/evidence/current-state.json", "docs/ugas-v1-capability-matrix.json",
        "docs/evidence/maps-minimap-runtime-v0210/map-contract-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/environment-authority-bindings-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/items-props-authority-bindings-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/layer-cell-matrix-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/chunk-partition-roundtrip-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/regions-zones-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/marker-contract-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/minimap-projection-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/visibility-state-qa-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/cache-identity-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/provenance-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/hard-gates-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/gate-specific-proof-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/negative-controls-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/full-slice-two-run-determinism-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/production-registry-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/test-only-fixture-manifest-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/execution-evidence-v0210.json",
        "docs/evidence/maps-minimap-runtime-v0210/capability-matrix-validation-v0210.json",
        "docs/evidence/github-governance-v0210/v0203-external-approval.json",
    ]
    for relative in required:
        path = ROOT / relative
        check(f"v0210:path:{relative}", path.is_file(), "present" if path.is_file() else "missing")
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        state_schema = load_json(ROOT / "schemas/current-state-v0210.json")
        validate_schema_document(state_schema); validate_instance(state, state_schema)
        consistency = validate_state_consistency_v0210(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.21.0.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"))
        check("v0210:state-consistency", consistency["status"] == state["current_gate"] and not consistency["failures"], "; ".join(consistency["failures"]) or "active v0.21.0 state is consistent")
        approval = load_json(ROOT / "docs/evidence/github-governance-v0210/v0203-external-approval.json")
        check("v0210:v0203-approved-baseline", approval.get("status") == "APPROVED_FOUNDATION" and approval.get("merged_main_sha") == "0bf04cb92e8619ea10cf82af8dbf2d9abe599e05", "v0.21.0 binds the exact protected v0.20.3 merge")

        execution = load_json(evidence_root / "execution-evidence-v0210.json")
        validate_instance(execution, load_json(ROOT / "schemas/maps-minimap-runtime-v0210.json"))
        gates = load_json(evidence_root / "hard-gates-v0210.json")
        proof = load_json(evidence_root / "gate-specific-proof-v0210.json")
        expected_gates = {"map_schema_valid", "map_dimensions_and_world_metrics_valid", "environment_authority_binding_valid", "items_props_authority_binding_valid", "layer_contract_valid", "cell_coordinates_in_bounds", "chunk_partition_exact", "chunk_roundtrip_valid", "regions_zones_in_bounds", "marker_identity_valid", "minimap_projection_valid", "minimap_inverse_projection_valid", "visibility_mask_dimensions_valid", "minimap_derived_from_map_identity", "map_provenance_valid", "cache_identity_complete", "stale_cache_cross_map_chunk_projection_rejected", "test_fixture_nonproduction", "production_registry_empty", "production_routing_blocked", "isolated_full_slice_determinism"}
        gate_records = gates.get("gates", {})
        check("v0210:execution", execution.get("status") == "MAPS_MINIMAP_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED" and execution.get("hard_gate_count") == 21 and execution.get("negative_control_count") == 20 and execution.get("production_routing") == "BLOCKED" and execution.get("new_generation") == 0, "maps/minimap execution is technically qualified and blocked from production")
        check("v0210:hard-gates", gates.get("status") == "PASS" and set(gate_records) == expected_gates and all(item.get("status") == "PASS" and item.get("observed") is True and item.get("checker") and item.get("assertion") for item in gate_records.values()), "all 21 named hard gates have observed boolean proof")
        check("v0210:gate-specific-proof", proof.get("status") == "PASS" and proof.get("gates") == gate_records, "gate-specific proof is bound to the hard-gate record")

        negative = load_json(evidence_root / "negative-controls-v0210.json")
        controls = negative.get("controls", [])
        expected_controls = {f"MM-NC-{index:02d}" for index in range(1, 21)}
        check("v0210:negative-controls", negative.get("status") == "PASS" and {item.get("control_id") for item in controls} == expected_controls and all(item.get("status") == "PASS" and item.get("result") == "REJECT" and item.get("expected_rejection_class") == item.get("observed_rejection_class") for item in controls), "MM-NC-01..20 inject semantic defects and observe exact rejection classes")

        fixture = load_json(evidence_root / "test-only-fixture-manifest-v0210.json")
        dimensions = {(tuple(item.get("dimensions", [])), item.get("aspect_ratio")) for item in fixture.get("maps", [])}
        check("v0210:fixture", fixture.get("status") == "TEST_ONLY" and fixture.get("map_count") == 2 and len(dimensions) == 2 and all(item.get("multiple_chunks") is True and item.get("all_primary_layers_present") is True and item.get("typed_world_prop") is True and item.get("region_zone") is True and item.get("poi_portal_spawn") is True for item in fixture.get("maps", [])) and fixture.get("real_map_asset_coverage") == "NONE" and fixture.get("real_minimap_asset_coverage") == "NONE", "two differing multi-chunk TEST_ONLY maps cover the required semantic fixtures")
        determinism = load_json(evidence_root / "full-slice-two-run-determinism-v0210.json")
        check("v0210:determinism", determinism.get("status") == "TWO_RUN_DETERMINISM_PASSED" and determinism.get("equal") is True and determinism.get("differences") == [] and all("\\" not in str(item) and not Path(str(item)).is_absolute() for item in determinism.get("files", [])), "independent full-slice outputs are deterministic with stable relative evidence paths")
        production = load_json(evidence_root / "production-registry-v0210.json")
        check("v0210:production-boundary", production.get("entries") == [] and production.get("production_approved") is False and production.get("production_routing") == "BLOCKED" and production.get("new_generation") == 0, "production registry is empty and routing remains blocked")
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
...
def main() -> int:
    ...
    test_run = _run([sys.executable, "-m", "unittest", "discover", "-s", "tests", "-q"], ROOT, timeout=900); test_text = test_run.stdout + test_run.stderr; match = re.search(r"Ran (\d+) tests", test_text)
    if test_run.returncode != 0:
        print("RETRY tests:unit", flush=True)
        test_run = _run([sys.executable, "-m", "unittest", "discover", "-s", "tests", "-q"], ROOT, timeout=900); test_text = test_run.stdout + test_run.stderr; match = re.search(r"Ran (\d+) tests", test_text); check("tests:unit-retry", test_run.returncode == 0 and match is not None and int(match.group(1)) >= 300, test_text.strip()[-800:])
    check("tests:unit", test_run.returncode == 0 and match is not None and int(match.group(1)) >= 300, test_text.strip()[-800:])
