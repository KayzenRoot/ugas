...
from ugas.state_consistency_v0245 import validate_state_consistency as validate_state_consistency_v0245
...
def _v0220_checks() -> None:
    ...
    try:
        ...
        if state.get("version") == "0.22.0":
            ...
            check("v0220:state-forward-advanced", state.get("version") in {"0.22.1", "0.22.2", "0.22.3", "0.23.0", "0.23.1", "0.23.2", "0.23.3", "0.23.4", "0.24.0", "0.24.1", "0.24.2", "0.24.3", "0.24.4", "0.24.5"} and state.get("correction_history", {}).get("v0.22.0", {}).get("historical_evidence_unchanged") is True, "v0.22.0 state/evidence is preserved while active state advances")
...
def _v0230_checks() -> None:
    ...
    try:
        ...
        validate_schema_document(schema)
        check("v0230:state-consistency", state.get("version") in {"0.23.3", "0.23.4", "0.24.0", "0.24.1", "0.24.2", "0.24.3", "0.24.4", "0.24.5"} and state.get("correction_history", {}).get("v0.23.0", {}).get("historical_evidence_unchanged") is True, "v0.23.0 is preserved as immutable history while active state advances")
...
def _v0245_checks() -> None:
    """Validate the active v0.24.5 F-41/F-42/F-43 orchestration correction."""
    ...
    try:
        state = load_json(ROOT / "docs/evidence/current-state.json")
        state_schema = load_json(ROOT / "schemas/current-state-v0245.json")
        ...
        matrix = load_json(ROOT / "docs/ugas-v1-capability-matrix.json")
        validate_schema_document(state_schema); validate_instance(state, state_schema); validate_schema_document(runtime_schema); validate_instance(execution, runtime_schema)
        consistency = validate_state_consistency_v0245(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "docs/roadmap.md").read_text(encoding="utf-8"), matrix, correction, state.get("evidence", {}))
        check("v0245:state-consistency", consistency["status"] == state.get("current_gate") and not consistency["failures"], "; ".join(consistency["failures"]) or "active v0.24.5 state is consistent")
        ...
        check("v0245:schema-validation", schema_validation.get("status") == "PASS" and all(item.get("status") == "PASS" for item in schema_validation.get("checks", [])), "active state and runtime evidence validate against v0.24.5 schemas")
        check("v0245:matrix", matrix.get("version") == "0.24.5" and matrix.get("next_candidate") == "V1_FINAL_ACCEPTANCE" and matrix.get("production_routing") == "BLOCKED" and matrix.get("new_generation") == 0, "capability matrix records the forward v0.24.5 correction boundary")
...
def _v0210_checks() -> None:
    ...
    _v0231_checks()
    _v0232_checks()
    _v0233_checks(); _v0234_checks(); _v0240_checks(); _v0241_checks(); _v0242_checks(); _v0243_checks(); _v0244_checks(); _v0245_checks()
    return
...
def main() -> int:
    ...
    check("version:consistency", UGAS_VERSION == package_version == pyproject_version == init_version == "0.24.5" and current_state_version in {"0.24.5"}, f"runtime={UGAS_VERSION}, package={package_version}, pyproject={pyproject_version}, current_state={current_state_version}")
