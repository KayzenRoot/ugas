"""Validate bounded v0.23.3 artifact security boundaries."""
...
def main() -> int:
    ...
    try:
        value = json.loads(a.manifest.read_text(encoding="utf-8")); boundary = value.get("security_boundary", {})
        if value.get("schema_version") != "0.23.3": failures.append("schema-version")
    ...
    result = {"schema_version": "0.23.3", "status": "PASS" if not failures else "FAIL", "failures": failures, "file_count": sum(1 for x in a.artifact_dir.rglob("*") if x.is_file())}; a.output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8"); print(json.dumps(result)); return 0 if not failures else 1
