...
def build(args: argparse.Namespace) -> dict:
    ...
    post_merge = state.get("current_gate") == "MAPS_MINIMAP_APPROVED_FOUNDATION_MERGED"
    review_boundary = {
        "external_review_required": closure.get("external_review_required", True) if post_merge else True,
        "do_not_merge": closure.get("do_not_merge", True) if post_merge else True,
        "merge_authorization": closure.get("merge_authorization", "NOT_AUTHORIZED_UNTIL_SOL_APPROVAL") if post_merge else "APPROVED_TO_MERGE_AFTER_BOOKKEEPING_REPROOF",
        "production_map_art_started": False,
        "production_minimap_art_started": False,
    }
    return {
        "schema_version": "0.21.3", "manifest_type": "github-ci-maps-minimap-v0213-post-merge-closure" if post_merge else "github-ci-maps-minimap-v0213-review",
        ...
        "scope": {"version": state["version"], "phase": state["phase"], "current_gate": state["current_gate"], "baseline_main_sha": state["baseline_main_sha"], "allowed_next_actions": state["allowed_next_actions"], "next_candidate": state.get("next_candidate"), "new_generation": state["new_generation"]},
        "changed_files": changed, "change_statistics": {"files": len(changed)},
        "current_state": {"path": "docs/evidence/current-state.json", "version": state["version"], "phase": state["phase"], "baseline_main_sha": state["baseline_main_sha"], "current_gate": state["current_gate"], "production_approved": state["production_approved"], "production_routing": state["production_routing"], "allowed_next_actions": state["allowed_next_actions"], "next_candidate": state.get("next_candidate")},
        ...
        "post_merge_binding": "docs/evidence/github-governance-v0220/v0213-post-merge-binding.json" if post_merge else None,
        "governance_correction_record": "docs/evidence/github-governance-v0220/v0213-state-truth-correction-v1.json" if post_merge else None,
