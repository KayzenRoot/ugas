"""Build the active v0.13.0 RUN_FRONT_V1 GitHub review manifest."""
...
BRANCH = "codex/v0.13.0-run-front-v1"
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tests = _load_result(Path(args.tests_json)) if getattr(args, "tests_json", None) else {"schema_version": "0.13.0", "status": "not_run", "exit_code": 99, "count": None, "passed": None, "failed": None, "parse_status": "not_run"}
    validation = _load_result(Path(args.validation_json), validation=True) if getattr(args, "validation_json", None) else {"schema_version": "0.13.0", "status": "not_run", "exit_code": 99, "checks": None, "passed": None, "failed": None, "parse_status": "not_run"}
    ...
    return {
        ...
        "review_index": {"historical_path": "docs/evidence/review-index-v0.12.2.json", "historical_version": "0.12.2", "status": "PRESERVED_BASELINE_VALIDATED", "active_manifest_path": "github-review-manifest-v0130.json"},
        "visual_manifest": "visual-manifest.json",
        "run_front_evidence": {"contract": "run-front-v0130/run-front-contract-v0130.json", "execution": "run-front-v0130/execution-evidence-v0.13.0.json", "visual_manifest": "run-front-v0130/run-front-visual-manifest-v0130.json", "negative_controls": "run-front-v0130/run-front-gate-negative-controls-v0130.json", "preview_gif": "run-front-v0130/run-front-v1/run-front-preview-v0130.gif"},
...
def main() -> int:
    ...
    (output / "github-review-manifest-v0130.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    print(json.dumps({"status": "V0130_GITHUB_REVIEW_MANIFEST_BUILT", "head_sha": manifest["pull_request"]["head_sha"], "base_sha": manifest["pull_request"]["base_sha"]}, ensure_ascii=False)); return 0
