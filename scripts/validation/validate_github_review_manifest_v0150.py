"""Fail-closed validation for the v0.15.0 review manifest and artifact bindings."""
...
DEATH_EVIDENCE_ROOT = "docs/evidence/animation-runtime-v0150"
...
def validate(manifest_path: Path, visual_path: Path, root: Path = ROOT) -> dict[str, Any]:
    ...
    try:
        ...
        schema = json.loads((root / "schemas/github-review-manifest-v0150.json").read_text(encoding="utf-8"))
    ...
    if scope.get("version") != "0.15.0" or scope.get("phase") != "DEATH_ANIMATION_FRONT":
        failures.append("active-scope-invalid")
    if scope.get("current_gate") != "CUTOUT_ANIMATION_RUNTIME_V1_DEATH_ANIMATION_FRONT_TECHNICALLY_QUALIFIED":
        failures.append("active-gate-invalid")
    if current_state.get("version") != "0.15.0" or current_state.get("phase") != "DEATH_ANIMATION_FRONT":
