"""Execute the v0.13.1 RUN_FRONT_V1 flight/QA correction without rewriting v0.13.0 evidence."""
...
OUT = ROOT / "docs/evidence/animation-runtime-v0131"
...
REVIEWED_HEAD = "c658a2984bbee7c0b257b7537e840c07a80eee1e"
PROTECTED_ASSETS = [
...
def _approved_assets_untouched(base_commit: str | None = IMMUTABLE_BASE) -> dict[str, Any]:
    if not base_commit or len(str(base_commit)) != 40:
        return {"schema_version": "0.13.1", "status": "APPROVED_ASSET_BASELINE_UNAVAILABLE", "base_commit": base_commit, "checks": [], "failures": ["immutable_baseline_missing_or_invalid"]}
    rev = subprocess.run(["git", "rev-parse", "--verify", f"{base_commit}^{{commit}}"], cwd=ROOT, capture_output=True, text=True, check=False)
    if rev.returncode != 0:
        return {"schema_version": "0.13.1", "status": "APPROVED_ASSET_BASELINE_UNAVAILABLE", "base_commit": base_commit, "checks": [], "failures": ["immutable_baseline_cannot_be_resolved"]}
    checks = []
    failures = []
    ...
    return {"schema_version": "0.13.1", "status": status, "base_commit": base_commit, "comparison": f"git show {base_commit}:path", "head_fallback_used": False, "checks": checks, "failures": failures}


def _negative_controls(spec: dict[str, Any], context: dict[str, Any], prepared: dict[str, Any], manifest_path: Path, qa: dict[str, Any]) -> dict[str, Any]:
    ...
    controls["NC-01_break_loop"] = {"gate": "loop_root_close", "status": "REJECTED" if not broken_loop["hard_gates"]["loop_root_close"] else "ACCEPTED", "hard_gates": broken_loop["hard_gates"]}
    ...
    controls["NC-02_zero_root_and_torso"] = {"gate": "body_root_participation", "status": "REJECTED" if not zero_result["hard_gates"]["body_root_participation"] else "ACCEPTED", "hard_gates": zero_result["hard_gates"]}

    planted_flight = copy.deepcopy(qa["foot_ground"]["frames"])
    planted_flight[3]["support_side"] = "right"
    planted_flight[3]["feet"]["right"]["role"] = "planted"
    planted_flight[3]["feet"]["right"]["visible_clearance_px"] = 0.0
    flight_injected = run_adapter._flight_semantic_qa(spec, planted_flight)
    controls["NC-03_flight_support_or_clearance"] = {"gate": "flight_both_feet_clear", "status": "REJECTED" if flight_injected["status"] != "RUN_FLIGHT_SEMANTIC_QA_PASSED" else "ACCEPTED", "hard_gates": flight_injected["hard_gates"]}
    ...
    controls["NC-04_foot_slide"] = {"gate": "foot_contact_windows", "status": "REJECTED" if not slide_result["hard_gates"]["foot_contact_windows"] else "ACCEPTED", "hard_gates": slide_result["hard_gates"]}
    ...
    controls["NC-05_duplicate_leg_phase"] = {"gate": "cadence_phase_alternates", "status": "REJECTED" if not duplicate_result["hard_gates"]["cadence_phase_alternates"] else "ACCEPTED", "hard_gates": duplicate_result["hard_gates"]}
    ...
    controls["NC-06_invert_arm_opposition"] = {"gate": "arm_leg_opposition", "status": "REJECTED" if not inverted_result["hard_gates"]["arm_leg_opposition"] else "ACCEPTED", "hard_gates": inverted_result["hard_gates"]}

    jump_result = _semantic_fixture(spec, context, prepared, lambda fixture: fixture["targets"][4]["joints"]["knee_right"].__setitem__("x", fixture["targets"][4]["joints"]["knee_right"]["x"] + 80.0))
    ...
    try:
        ...
        controls["NC-08_remove_dependency_hash"] = {"gate": "source_dependency_hash", "status": "REJECTED", "error": type(exc).__name__}
    else:
        controls["NC-08_remove_dependency_hash"] = {"gate": "source_dependency_hash", "status": "ACCEPTED"}

    with tempfile.TemporaryDirectory(prefix="ugas-v0131-package-nc-", dir=ROOT / "tmp") as directory:
        temp = Path(directory)
        ...
        try:
            ...
            controls["NC-09_false_gate_in_package"] = {"gate": "package_qualified_qa", "status": "REJECTED", "error": type(exc).__name__}
        else:
            controls["NC-09_false_gate_in_package"] = {"gate": "package_qualified_qa", "status": "ACCEPTED"}

    with tempfile.TemporaryDirectory(prefix="ugas-v0131-asset-nc-", dir=ROOT / "tmp") as directory:
        isolated = Path(directory) / "walk-front-v1.json"
        isolated.parent.mkdir(parents=True, exist_ok=True)
        source = ROOT / PROTECTED_ASSETS[0]
        isolated.write_bytes(source.read_bytes() + b"\nmutated-approved-asset\n")
        mutated_check = isolated.read_bytes() != subprocess.run(["git", "show", f"{IMMUTABLE_BASE}:{PROTECTED_ASSETS[0]}"], cwd=ROOT, capture_output=True, check=False).stdout
        controls["NC-10_mutate_approved_historical_asset"] = {"gate": "approved_asset_integrity", "status": "REJECTED" if mutated_check else "ACCEPTED"}

    unavailable = _approved_assets_untouched("0" * 40)
    controls["NC-11_unavailable_immutable_baseline"] = {"gate": "approved_asset_baseline_fail_closed", "status": "REJECTED" if unavailable["status"] == "APPROVED_ASSET_BASELINE_UNAVAILABLE" else "ACCEPTED"}

    outside = gif_timing_within_tolerance(spec, {"frame_count": 8, "loop": 0, "durations_ms": [200] * 8, "total_cycle_ms": 1600, "effective_fps": 5.0})
    controls["NC-12_gif_duration_outside_tolerance"] = {"gate": "gif_timing_package_integrity", "status": "REJECTED" if outside["status"] != "GIF_TIMING_PASSED" else "ACCEPTED", "hard_gates": outside["hard_gates"]}

    passed = all(item["status"] == "REJECTED" for item in controls.values())
    return {"schema_version": "0.13.1", "status": "NC_01_TO_NC_12_PASSED" if passed else "NC_01_TO_NC_12_GAP", "controls": controls, "source": "scripts/validation/run_animation_runtime_v0131.py independent fixture mutations"}
...
def _marker_sheet(manifest: dict[str, Any], qa: dict[str, Any]) -> tuple[Path, list[dict[str, Any]]]:
    ...
    sheet_path = OUT / "run-front-phase-markers-v0131.png"
...
def run() -> dict[str, Any]:
    ...
    negative = _negative_controls(spec, context, prepared, manifest_path, qa)
    write_json(OUT / "run-front-targets-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "phase_order": list(run_adapter.PHASES), "targets": prepared["targets"], "target_hashes": [target["target_joint_sha256"] for target in prepared["targets"]], "key_pose_bindings": spec["key_pose_bindings"], "motion_tracks_sha256": prepared["track_hash"], "parameters_frozen_before_render": True, "source_only_pixels": True})
    write_json(OUT / "run-front-frame-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "status": qa["status"], "decision": qa["decision"], "frames": qa["frames"]})
    write_json(OUT / "run-front-temporal-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "status": qa["temporal"]["status"], "metrics": qa["temporal"]["metrics"], "hard_gates": qa["temporal"]["hard_gates"], "cadence_phase": qa["temporal"]["cadence_phase"], "arm_opposition": qa["temporal"]["arm_opposition"], "flight": qa["temporal"].get("flight")})
    write_json(OUT / "run-front-loop-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "status": "RUN_LOOP_CLOSURE_PASSED" if all(value for key, value in qa["temporal"]["hard_gates"].items() if key.startswith("loop_")) else "RUN_LOOP_CLOSURE_GAP", "loop_metrics": {key: value for key, value in qa["temporal"]["metrics"].items() if "loop" in key}, "gates": {key: value for key, value in qa["temporal"]["hard_gates"].items() if key.startswith("loop_")}, "edge": [7, 0]})
    write_json(OUT / "run-front-cadence-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "status": "RUN_CADENCE_PHASE_PASSED" if qa["temporal"]["hard_gates"]["cadence_phase_alternates"] else "RUN_CADENCE_PHASE_GAP", "phase_order": list(run_adapter.PHASES), "contact_frames": spec["adapter_parameters"]["contact_frames"], "passing_frames": spec["adapter_parameters"]["passing_frames"], "flight_frames": spec["adapter_parameters"]["flight_frames"], "cadence_phase": qa["temporal"]["cadence_phase"]})
    write_json(OUT / "run-front-foot-ground-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "status": qa["foot_ground"]["status"], "frames": qa["foot_ground"]["frames"], "contact": qa["foot_ground"]["contact"], "flight": qa["foot_ground"].get("flight"), "ground_reference_y": qa["foot_ground"].get("ground_reference_y")})
    write_json(OUT / "run-front-body-mechanics-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], **qa["body_mechanics"]})
    write_json(OUT / "run-front-continuity-qa-v0131.json", {"schema_version": "0.13.1", "animation_id": spec["animation_id"], "status": "RUN_CONTINUITY_INTERPOLATION_PASSED" if qa["temporal"]["hard_gates"]["angular_continuity"] and qa["temporal"]["hard_gates"]["angular_acceleration_continuity"] and qa["temporal"]["hard_gates"]["nonfinite_and_gap_free"] else "RUN_CONTINUITY_INTERPOLATION_GAP", "gates": {key: value for key, value in qa["temporal"]["hard_gates"].items() if key in {"angular_continuity", "angular_acceleration_continuity", "nonfinite_and_gap_free", "foreground_height_stability"}}, "metrics": {key: value for key, value in qa["temporal"]["metrics"].items() if "angle" in key or "height" in key}})
    write_json(OUT / "run-front-gate-negative-controls-v0131.json", negative)
    assets = _approved_assets_untouched(IMMUTABLE_BASE)
    write_json(OUT / "approved-assets-untouched-v0131.json", assets)
    write_json(OUT / "run-front-gif-timing-v0131.json", {"schema_version": "0.13.1", **gif_check, "package_metadata": {"fps": package.get("fps"), "per_frame_duration_ms": package.get("per_frame_duration_ms"), "gif_encoded_frame_durations_ms": package.get("gif_encoded_frame_durations_ms"), "gif_total_cycle_ms": package.get("gif_total_cycle_ms"), "gif_effective_fps": package.get("gif_effective_fps")}})
    ...
    visual_manifest = {"schema_version": "0.13.1", "review_state": "run-front-v1-technically-qualified", "review_subject": {"animation_id": spec["animation_id"], "direction": spec["direction"], "frame_count": spec["frame_count"], "fps": spec["fps"], "source_r4_sha256": spec["provenance"]["source_sha256"]}, "event_markers": spec["event_markers"], "event_markers_sha256": manifest["event_markers_sha256"], "motion_tracks_sha256": manifest["motion_tracks_sha256"], "gif_timing": decoded_gif, "images": visual_images, "marker_frames": marker_records, "source_only_pixels": True, "external_visual": "REQUIRED", "production_routing": "BLOCKED", "package_manifest": {"path": relative(package_path), "sha256": digest(package_path)}}
    write_json(OUT / "run-front-visual-manifest-v0131.json", visual_manifest)
    execution = {
        "schema_version": "0.13.1",
        "prompt": "PROMPT-CORRETIVO-UGAS-v0.13.1-RUN-FRONT-FLIGHT-QA-VISUAL-INTEGRITY",
        ...
        "review_artifacts": {"visual_manifest": relative(OUT / "run-front-visual-manifest-v0131.json"), "phase_marker_sheet": relative(marker_sheet), "negative_controls": relative(OUT / "run-front-gate-negative-controls-v0131.json")},
        "historical_v0130_preserved": True,
        "next_capability_started": False,
        "executor_does_not_claim_visual_approval": True,
    }
    write_json(OUT / "execution-evidence-v0.13.1.json", execution)
    if negative["status"] != "NC_01_TO_NC_12_PASSED" or assets["status"] != "APPROVED_ASSETS_UNTOUCHED":
        raise RuntimeError(f"V0131_GATES_FAILED:{negative['status']}:{assets['status']}")
    return {"status": "ANIMATION_RUNTIME_V0131_PASSED", "decision": qa["decision"], "animation_id": spec["animation_id"], "frames": len(manifest["frames"]), "package": relative(package_path), "preview_gif": package["preview_gif"]["path"], "negative_controls": negative["status"], "approved_assets": assets["status"], "gif_effective_fps": decoded_gif["effective_fps"], "gif_total_cycle_ms": decoded_gif["total_cycle_ms"], "external_visual": "REQUIRED", "production_routing": "BLOCKED"}
...
if __name__ == "__main__":
    try:
        ...
        print(json.dumps({"status": "ANIMATION_RUNTIME_V0131_FAILED", "error": f"{type(exc).__name__}: {exc}"}, indent=2, ensure_ascii=False))
        raise SystemExit(2)
