"""Validate the bounded v0.24.2 exact-head review manifest fail-closed."""
...
def main() -> int:
    ...
    failures: list[str] = []
    if value.get("schema_version") != VERSION or value.get("manifest_type") != "github-ci-orchestration-runtime-v0242-review":
        failures.append("identity")
    scope = value.get("scope", {})
    expected_scope = {
        "version": VERSION,
        "phase": "ORCHESTRATION_RUNTIME_HARDENING",
        "current_gate": "ORCHESTRATION_RUNTIME_HARDENING_F31R_F36_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED",
        "baseline_main_sha": BASE_MAIN,
        "rejected_reviewed_head": REJECTED_HEAD,
        "allowed_next_actions": ["external_review_orchestration_runtime_v0242"],
    ...
    for item in controls_map.values():
        ...
        elif not item.get("expected_rejection_class") or item.get("observed_rejection_class") != item.get("expected_rejection_class") or not item.get("actual_exception"):
    ...
    if state_validation.get("status") != "ORCHESTRATION_RUNTIME_HARDENING_F31R_F36_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED":
