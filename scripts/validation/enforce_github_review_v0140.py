"""Enforce active v0.14.0 review gates after artifact upload."""
...
def main() -> int:
    ...
    if validation.get("status") != "V0140_GITHUB_REVIEW_MANIFEST_PASSED": failures.append("manifest-validation-not-pass")
    if security.get("status") != "PASS": failures.append("security-not-pass")
    try:
        if int(Path(args.runtime_exit).read_text(encoding="utf-8").strip()) != 0: failures.append("hit-front-runtime-not-pass")
    except (OSError, ValueError): failures.append("hit-front-runtime-exit-unavailable")
    result = {"schema_version": "0.14.0", "status": "PASS" if not failures else "FAIL", "failures": failures}; print(json.dumps(result, indent=2, ensure_ascii=False)); return 0 if not failures else 1
