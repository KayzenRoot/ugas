"""Record bounded PR-run results for the v0.20.1 review artifact."""
...
def _test_result(log: str, exit_code: int) -> dict:
    ...
    return {"schema_version": "0.20.1", "status": "passed" if exit_code == 0 and count > 0 else "failed", "count": count, "passed": count if exit_code == 0 else 0, "failed": 0 if exit_code == 0 else 1, "exit_code": exit_code}
...
def _validation_result(log: str, exit_code: int) -> dict:
    ...
    return {"schema_version": "0.20.1", "status": "passed" if exit_code == 0 and failed == 0 else "failed", "checks": checks, "passed": passed, "failed": failed, "exit_code": exit_code}
...
def main() -> int:
    ...
    gates = {
        ...
        "environment_tilesets_runtime": {"status": "PASS" if environment else "FAIL", "detail": "run_environment_tilesets_runtime_v0201.py"},
        "state_consistency": {"status": "PASS" if state else "FAIL", "detail": "validate_state_consistency_v0201.py"},
    ...
    (args.output_dir / "test-results-v0201.json").write_text(json.dumps(tests, indent=2) + "\n", encoding="utf-8")
    (args.output_dir / "validation-results-v0201.json").write_text(json.dumps(validation, indent=2) + "\n", encoding="utf-8")
    (args.output_dir / "gate-results-v0201.json").write_text(json.dumps({"schema_version": "0.20.1", "gates": gates, "overall_status": "PASS" if all(item["status"] == "PASS" for item in gates.values()) else "FAIL"}, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0201_RESULTS_RECORDED", "overall_status": "PASS" if all(item["status"] == "PASS" for item in gates.values()) else "FAIL"}))
    return 0
