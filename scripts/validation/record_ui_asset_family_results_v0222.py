"""Record bounded v0.22.2 test and official-validation totals."""
...
def main() -> int:
    ...
    validation = {"schema_version": "0.22.2", "status": "failed", "checks": 0, "passed": 0, "failed": 1, "exit_code": _int(args.validation_exit)}
    if checks:
    ...
    tests = {"schema_version": "0.22.2", "status": "passed" if _int(args.tests_exit) == 0 and test_count > 0 else "failed", "count": test_count, "passed": test_count if _int(args.tests_exit) == 0 else 0, "failed": 0 if _int(args.tests_exit) == 0 else 1, "exit_code": _int(args.tests_exit)}
    ...
    args.output_dir.mkdir(parents=True, exist_ok=True); (args.output_dir / "test-results-v0222.json").write_text(json.dumps(tests, indent=2) + "\n", encoding="utf-8"); (args.output_dir / "validation-results-v0222.json").write_text(json.dumps(validation, indent=2) + "\n", encoding="utf-8"); (args.output_dir / "gate-results-v0222.json").write_text(json.dumps({"schema_version": "0.22.2", "gates": {key: {"status": "PASS" if value else "FAIL"} for key, value in gates.items()}, "overall_status": "PASS" if overall else "FAIL"}, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0222_RESULTS_RECORDED", "overall_status": "PASS" if overall else "FAIL", "test_count": test_count, "validation_checks": validation["checks"]})); return 0 if overall else 1
