"""Execute the complete deterministic UGAS v0.24.7 correction slice."""
...
EVIDENCE = ROOT / "docs/evidence/orchestration-runtime-v0247"
...
def _with_historical_candidate(mutator: Callable[[Path], None], *, auto_stage: bool = True) -> None:
    with tempfile.TemporaryDirectory(prefix="ugas-hist-v0247-") as directory:
        ...
        try:
            ...
            subprocess.run(["git", "-C", str(candidate), "commit", "-m", "ugas-v0247-historical-mutation", "--allow-empty"], check=True, capture_output=True)
...
def _no_git_authority_root() -> tempfile.TemporaryDirectory[str]:
    directory = tempfile.TemporaryDirectory(prefix="ugas-nogit-v0247-")
...
def _authority_mode_controls() -> list[dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-emptygit-v0247-") as directory:
    ...
    try:
        ...
        payload["ugas_v0247_semantic_probe"] = True
...
def _request(dependency_refs: list[dict[str, Any]] | None = None, *, timeout: int = 30) -> dict[str, Any]:
    ...
    return build_request(
        request_id="orchestration-v0247-fixture",
        ...
        idempotency_key="orchestration-v0247-fixture-idempotency",
...
def _node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 30, priority: int = 10, authority_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    base = {"node_id": node_id, "dependencies": dependencies or [], "operation": "EXECUTE", "dependency_hashes": {}, "timeout_seconds": timeout, "retry_policy": {"max_attempts": 2, "retryable_errors": ["TRANSIENT_EXECUTION_ERROR", "REQUEST_DEADLINE_EXCEEDED", "NODE_DEADLINE_EXCEEDED"]}, "resource_class": "cpu-test", "scheduling_key": "orchestration-v0247", "priority": priority, "asset_family": family}
    base["input_contract_hash"] = node_authority_input_contract_hash(base, payload) if payload else sha256_value({"node": node_id, "version": "0.24.7"})
    return base
...
def _handoff_with(**overrides: Any) -> dict[str, Any]:
    value = dict(SANITIZED_UADS_HANDOFF)
...
def _uads_controls() -> list[dict[str, Any]]:
    controls = [
        _expect_rejection("NC-UADS-01", "route_status=BLOCKED", "ORCH_UADS_ROUTE_STATUS_REJECTED", lambda: sanitize_uads_handoff(_handoff_with(route_status="BLOCKED"))),
        _expect_rejection("NC-UADS-02", "route_status=UNKNOWN", "ORCH_UADS_ROUTE_STATUS_REJECTED", lambda: sanitize_uads_handoff(_handoff_with(route_status="UNKNOWN"))),
        _expect_rejection("NC-UADS-03", "dispatch_status=NOT_DISPATCHED", "ORCH_UADS_DISPATCH_STATUS_REJECTED", lambda: sanitize_uads_handoff(_handoff_with(dispatch_status="NOT_DISPATCHED"))),
        _expect_rejection("NC-UADS-04", "dispatch_status=FAILED", "ORCH_UADS_DISPATCH_STATUS_REJECTED", lambda: sanitize_uads_handoff(_handoff_with(dispatch_status="FAILED"))),
        _expect_rejection("NC-UADS-05", "missing selected_profile_id", "ORCH_UADS_HANDOFF_INVALID", lambda: sanitize_uads_handoff(_handoff_with(selected_profile_id=None))),
        _expect_rejection("NC-UADS-06", "absolute or tilde path in ID", "ORCH_UADS_HANDOFF_PATH_REJECTED", lambda: sanitize_uads_handoff(_handoff_with(work_order_id="~/wo_secret"))),
        _expect_rejection("NC-UADS-07", "secret-like extra field", "ORCH_SECRET_LIKE_DATA_REJECTED", lambda: sanitize_uads_handoff({**SANITIZED_UADS_HANDOFF, "api_key": "injected"})),
    ]
    with tempfile.TemporaryDirectory(prefix="ugas-uads-local-") as directory:
        local = Path(directory)
        (local / ".uads").mkdir()
        controls.append(_expect_rejection("NC-UADS-08", "project-local .uads directory", "ORCH_REPO_LOCAL_UADS_FOOTPRINT", lambda: validate_no_repo_local_uads(local)))
    tracked_dir = Path(tempfile.mkdtemp(prefix="ugas-uads-tracked-"))
    try:
        subprocess.run(["git", "init"], cwd=tracked_dir, check=True, capture_output=True)
        nested = tracked_dir / ".uads"
        nested.mkdir()
        (nested / "state.json").write_text("{}\n", encoding="utf-8")
        subprocess.run(["git", "add", "-f", ".uads"], cwd=tracked_dir, check=True, capture_output=True)
        subprocess.run(["git", "-c", "user.email=ugas@example.invalid", "-c", "user.name=UGAS", "-c", "commit.gpgsign=false", "commit", "-m", "track uads"], cwd=tracked_dir, check=True, capture_output=True)
        shutil.rmtree(nested)
        controls.append(_expect_rejection("NC-UADS-09", "tracked .uads path", "ORCH_REPO_LOCAL_UADS_FOOTPRINT", lambda: validate_no_repo_local_uads(tracked_dir)))
    finally:
        shutil.rmtree(tracked_dir, ignore_errors=True)
    positive = sanitize_uads_handoff(SANITIZED_UADS_HANDOFF)
    controls.append({"control_id": "NC-UADS-PASS", "injected_defect": "none", "expected_rejection_class": None, "observed_rejection_class": None, "status": "PASS" if positive.get("route_status") == "SELECTED" and positive.get("dispatch_status") == "DISPATCHED" else "FAIL", "result": None, "actual_exception": None, "detail": "positive actual v0.24.7 UADS handoff"})
    return controls


def _summary_controls() -> list[dict[str, Any]]:
    import importlib.util
    path = ROOT / "scripts/validation/record_orchestration_results_v0247.py"
    spec = importlib.util.spec_from_file_location("record_orchestration_results_v0247", path)
    module = importlib.util.module_from_spec(spec)
    assert spec is not None and spec.loader is not None
    spec.loader.exec_module(module)
    select_final_validation_summary = module.select_final_validation_summary

    def _control(control_id: str, defect: str, text: str, predicate: Callable[[dict[str, Any]], bool]) -> dict[str, Any]:
        observed = select_final_validation_summary(text)
        ok = predicate(observed)
        return {"control_id": control_id, "injected_defect": defect, "expected_rejection_class": None, "observed_rejection_class": None, "status": "PASS" if ok else "FAIL", "result": None if observed.get("status") == "PASS" and ok else "REJECT", "actual_exception": None, "detail": json.dumps({"checks": observed.get("checks"), "selected_summary_index": observed.get("selected_summary_index"), "selected_terminal_line": observed.get("selected_terminal_line"), "summary_prefixed_count": observed.get("summary_prefixed_count"), "canonical_summary_count": observed.get("canonical_summary_count"), "reason": observed.get("reason")}, sort_keys=True)}

    return [
        _control("NC-SUM-01", "early 100 then final 120", "SUMMARY checks=100 passed=100 failed=0\nSUMMARY checks=120 passed=120 failed=0\n", lambda item: item.get("status") == "PASS" and item.get("checks") == 120 and item.get("selected_summary_index") == 1 and item.get("summary_prefixed_count") == 2 and item.get("canonical_summary_count") == 2),
        _control("NC-SUM-02", "early PASS plus final FAIL", "SUMMARY checks=100 passed=100 failed=0\nSUMMARY checks=120 passed=119 failed=1\n", lambda item: item.get("status") == "FAIL" and item.get("failed") == 1 and item.get("selected_summary_index") == 1 and item.get("reason") == "FINAL_SUMMARY_NOT_CLEAN"),
        _control("NC-SUM-03", "early FAIL plus final PASS", "SUMMARY checks=100 passed=90 failed=10\nSUMMARY checks=120 passed=120 failed=0\n", lambda item: item.get("status") == "PASS" and item.get("checks") == 120 and item.get("selected_summary_index") == 1),
        _control("NC-SUM-04", "snapshot summaries plus final aggregate", "PASS snapshot:validation - SUMMARY checks=3044 passed=3044 failed=0\nPASS snapshot:no-git - SUMMARY checks=3044 passed=3044 failed=0\nSUMMARY checks=3050 passed=3050 failed=0\n", lambda item: item.get("status") == "PASS" and item.get("checks") == 3050 and item.get("selected_summary_index") == 0 and item.get("summary_prefixed_count") == 1 and item.get("canonical_summary_count") == 1 and item.get("selected_terminal_line") == 2),
        _control("NC-SUM-05", "no SUMMARY-prefixed aggregate", "official validation started\n", lambda item: item.get("status") == "FAIL" and item.get("reason") == "NO_FINAL_SUMMARY" and item.get("summary_prefixed_count") == 0),
        _control("NC-SUM-06", "early valid plus terminal missing failed", "SUMMARY checks=100 passed=100 failed=0\nSUMMARY checks=120 passed=120\n", lambda item: item.get("status") == "FAIL" and item.get("reason") == "MALFORMED_TERMINAL_SUMMARY" and item.get("selected_summary_index") == 1),
        _control("NC-SUM-07", "early valid plus terminal failed=x", "SUMMARY checks=100 passed=100 failed=0\nSUMMARY checks=120 passed=120 failed=x\n", lambda item: item.get("status") == "FAIL" and item.get("reason") == "MALFORMED_TERMINAL_SUMMARY"),
        _control("NC-SUM-08", "early valid plus trailing junk", "SUMMARY checks=100 passed=100 failed=0\nSUMMARY checks=120 passed=120 failed=0 trailing\n", lambda item: item.get("status") == "FAIL" and item.get("reason") == "MALFORMED_TERMINAL_SUMMARY"),
        _control("NC-SUM-09", "embedded summary token without standalone aggregate", "nested text SUMMARY checks=120 passed=120 failed=0 inside a line\n", lambda item: item.get("status") == "FAIL" and item.get("reason") == "NO_FINAL_SUMMARY" and item.get("summary_prefixed_count") == 0),
    ]


def _correction_history_controls() -> list[dict[str, Any]]:
    canonical = dict(CANONICAL_CORRECTION_HISTORY)
    stale = dict(canonical); stale["findings"] = ["F-43R", "F-44", "F-45", "F-46"]
    wrong_head = dict(canonical); wrong_head["rejected_reviewed_head"] = "0" * 40
    missing = dict(canonical); missing["findings"] = ["F-44R"]
    extra = dict(canonical); extra["findings"] = ["F-44R", "F-46R", "F-99"]
    unchanged = dict(canonical); unchanged["historical_evidence_unchanged"] = False
    controls = [
        _expect_rejection("NC-CHIST-01", "stale prior-cycle findings", "ORCH_CORRECTION_HISTORY_MISMATCH", lambda: validate_canonical_correction_history(stale, canonical)),
        _expect_rejection("NC-CHIST-02", "wrong rejected_reviewed_head", "ORCH_CORRECTION_HISTORY_MISMATCH", lambda: validate_canonical_correction_history(wrong_head, canonical)),
        _expect_rejection("NC-CHIST-03", "missing finding", "ORCH_CORRECTION_HISTORY_MISMATCH", lambda: validate_canonical_correction_history(missing, canonical)),
        _expect_rejection("NC-CHIST-04", "extra fabricated finding", "ORCH_CORRECTION_HISTORY_MISMATCH", lambda: validate_canonical_correction_history(extra, canonical)),
        _expect_rejection("NC-CHIST-05", "historical_evidence_unchanged false", "ORCH_CORRECTION_HISTORY_MISMATCH", lambda: validate_canonical_correction_history(unchanged, canonical)),
    ]
    positive = validate_canonical_correction_history(canonical, canonical)
    controls.append({"control_id": "NC-CHIST-PASS", "injected_defect": "none", "expected_rejection_class": None, "observed_rejection_class": None, "status": "PASS" if positive.get("status") == "PASS" else "FAIL", "result": None, "actual_exception": None, "detail": "positive manifest binds exact canonical correction-history object"})
    return controls


def _artifact_inventory_controls() -> list[dict[str, Any]]:
    import importlib.util
    path = ROOT / "scripts/validation/validate_github_review_security_v0247.py"
    spec = importlib.util.spec_from_file_location("validate_github_review_security_v0247", path)
    module = importlib.util.module_from_spec(spec)
    assert spec is not None and spec.loader is not None
    spec.loader.exec_module(module)
    self_name = module.SELF_ATTESTATION

    def _seed(root: Path) -> None:
        (root / "logs").mkdir(parents=True, exist_ok=True)
        (root / "docs/evidence/orchestration-runtime-v0247").mkdir(parents=True, exist_ok=True)
        (root / "REVIEW-v0.24.7.md").write_text("# review\n", encoding="utf-8")
        (root / "docs/evidence/current-state.json").write_text("{}\n", encoding="utf-8")
        (root / "logs/ugas-tests.log").write_text("ok\n", encoding="utf-8")
        (root / "pre-upload-enforcement-v0247.json").write_text("{}\n", encoding="utf-8")
        (root / "manifest-validation-results-v0247.json").write_text("{}\n", encoding="utf-8")

    def _control(control_id: str, defect: str, mutator: Callable[[Path, dict[str, Any]], tuple[dict[str, Any], tuple[str, ...] | list[str]]], expected_reason: str | None) -> dict[str, Any]:
        with tempfile.TemporaryDirectory(prefix="ugas-art-v0247-") as directory:
            root = Path(directory)
            _seed(root)
            inventory = module.build_artifact_inventory(root, excluded_paths=(self_name,))
            mutated_inventory, excluded = mutator(root, inventory)
            observed = module.verify_artifact_inventory(root, mutated_inventory, excluded_paths=excluded)
        ok = observed.get("status") == ("PASS" if expected_reason is None else "FAIL") and observed.get("reason") == expected_reason
        return {"control_id": control_id, "injected_defect": defect, "expected_rejection_class": None, "observed_rejection_class": None, "status": "PASS" if ok else "FAIL", "result": None if expected_reason is None and ok else "REJECT", "actual_exception": None, "detail": json.dumps({"reason": observed.get("reason"), "failures": observed.get("failures"), "final_staged_file_count": observed.get("final_staged_file_count"), "scanned_file_count": observed.get("scanned_file_count")}, sort_keys=True)}

    def add_file(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        (root / "logs/injected.txt").write_text("extra\n", encoding="utf-8")
        return inventory, (self_name,)

    def modify_bytes(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        path = root / "REVIEW-v0.24.7.md"
        path.write_bytes(path.read_bytes() + b"mutated\n")
        return inventory, (self_name,)

    def remove_file(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        (root / "logs/ugas-tests.log").unlink()
        return inventory, (self_name,)

    def rename_file(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        (root / "logs/ugas-tests.log").rename(root / "logs/ugas-tests-renamed.log")
        return inventory, (self_name,)

    def unexpected_extra(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        (root / "injected-root.txt").write_text("unexpected\n", encoding="utf-8")
        return inventory, (self_name,)

    def tamper_size(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        mutated = copy.deepcopy(inventory)
        mutated["entries"][0]["size"] = int(mutated["entries"][0]["size"]) + 9
        return mutated, (self_name,)

    def tamper_sha(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        mutated = copy.deepcopy(inventory)
        mutated["entries"][0]["sha256"] = "0" * 64
        return mutated, (self_name,)

    def stale_digest(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        mutated = copy.deepcopy(inventory)
        mutated["inventory_digest"] = "0" * 64
        return mutated, (self_name,)

    def multi_exclusion(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        return inventory, (self_name, "pre-upload-enforcement-v0247.json")

    def positive(root: Path, inventory: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
        (root / self_name).write_text("{}\n", encoding="utf-8")
        return inventory, (self_name,)

    controls = [
        _control("NC-ART-01", "add file after inventory", add_file, "ARTIFACT_FILE_ADDED"),
        _control("NC-ART-02", "modify bytes after inventory", modify_bytes, "ARTIFACT_FILE_MODIFIED"),
        _control("NC-ART-03", "remove scanned file", remove_file, "ARTIFACT_FILE_REMOVED"),
        _control("NC-ART-04", "rename scanned file", rename_file, "ARTIFACT_FILE_RENAMED"),
        _control("NC-ART-05", "inject unexpected extra file", unexpected_extra, "ARTIFACT_UNEXPECTED_EXTRA"),
        _control("NC-ART-06", "tamper recorded size", tamper_size, "ARTIFACT_SIZE_TAMPER"),
        _control("NC-ART-07", "tamper recorded SHA-256", tamper_sha, "ARTIFACT_SHA_TAMPER"),
        _control("NC-ART-08", "stale inventory_digest", stale_digest, "ARTIFACT_STALE_DIGEST"),
        _control("NC-ART-09", "more than one self-exclusion", multi_exclusion, "ARTIFACT_MULTI_SELF_EXCLUSION"),
        _control("NC-ART-PASS", "complete staged artifact plus trusted self-attestation", positive, None),
    ]
    return controls
...
def _negative_controls(request: dict[str, Any], dag: dict[str, Any], checkpoint: dict[str, Any], authority: dict[str, Any], *, execution_policy: dict[str, Any]) -> dict[str, Any]:
    ...
    controls.append(_expect_rejection("NC-HIST-05", "nonexistent historical root", "HISTORICAL_ROOT_EMPTY", lambda: compare_historical_evidence_tree(ROOT, V0241_AUTHORITY_HEAD, ["docs/evidence/does-not-exist-v0247"], label="v0241-missing-root")))
    ...
    controls.extend(_uads_controls())
    controls.extend(_summary_controls())
    controls.extend(_correction_history_controls())
    controls.extend(_artifact_inventory_controls())
...
def run(output_dir: Path = EVIDENCE) -> dict[str, Any]:
    ...
    telemetry = sanitize_telemetry({"event": "orchestration-v0247-complete", "request_hash": request["request_hash"], "dag_hash": dag["dag_hash"], "state": snapshot_one["status"], "elapsed_seconds": snapshot_one["clock_elapsed_seconds"], "deadline_seconds": request["timeout_seconds"]})
    ...
    boundary = assert_provider_boundary(ROOT / "src/ugas/orchestration_runtime_v0247.py")
    ...
    observations = {
        ...
        "scheduler_dependency_first": all((__import__("ugas.orchestration_runtime_v0247", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(dep) < __import__("ugas.orchestration_runtime_v0247", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(node["node_id"])) for node in dag["nodes"] for dep in node["dependencies"]),
        ...
        "uads_global_first_mode": uads_handoff["execution_mode"] == "GLOBAL_FIRST",
        "uads_zero_project_footprint": uads_handoff["project_footprint"] == "ZERO",
        "uads_route_selected": uads_handoff["route_status"] == "SELECTED",
        "uads_dispatch_successful": uads_handoff["dispatch_status"] == "DISPATCHED",
        "uads_selected_profile_present": bool(uads_handoff.get("selected_profile_id")),
        "uads_repo_local_absent": True,
        "artifact_inventory_consistent": controls["controls"].get("NC-ART-PASS", {}).get("status") == "PASS",
        "artifact_single_self_attestation": controls["controls"].get("NC-ART-09", {}).get("status") == "PASS" and controls["controls"].get("NC-ART-PASS", {}).get("status") == "PASS",
    }
    ...
    _write(output_dir / "request-contract-v0247.json", {"request": request, "status": "PASS"})
    _write(output_dir / "dag-scheduler-v0247.json", {"dag": dag, "status": "PASS"})
    _write(output_dir / "execution-state-v0247.json", snapshot_one)
    _write(output_dir / "deadline-timeout-v0247.json", {"status": "PASS", "clock": snapshot_one["clock_elapsed_seconds"], "request_deadline_seconds": request["timeout_seconds"], "node_deadlines": snapshot_one["node_deadlines"], "probes": result["correction_probes"]})
    _write(output_dir / "checkpoint-resume-v0247.json", result["checkpoint"])
    _write(output_dir / "idempotency-coordinator-v0247.json", result["idempotency"])
    _write(output_dir / "circuit-breaker-runtime-v0247.json", result["circuit"])
    _write(output_dir / "dependency-authority-v0247.json", authority)
    _write(output_dir / "provider-boundary-v0247.json", result["provider"])
    _write(output_dir / "family-concurrency-v0247.json", batch)
    _write(output_dir / "historical-immutability-v0247.json", {"v0241": frozen_v0241, "v0242": frozen_v0242, "status": "PASS" if frozen_v0241.get("status") == "PASS" and frozen_v0242.get("status") == "PASS" else "FAIL"})
    _write(output_dir / "frozen-v0241-fingerprint-v0247.json", frozen_v0241)
    _write(output_dir / "hard-gates-v0247.json", gates)
    _write(output_dir / "negative-controls-v0247.json", controls)
    _write(output_dir / "full-slice-two-run-determinism-v0247.json", {"status": "PASS" if snapshot_one == snapshot_two else "FAIL", "run_1": snapshot_one, "run_2": snapshot_two})
    _write(output_dir / "production-boundary-v0247.json", production)
    _write(output_dir / "correction-history-v0247.json", {"schema_version": VERSION, "status": "CORRECTION_REQUIRED", "base_main_sha": BASE_MAIN_SHA, "reviewed_head": REJECTED_REVIEWED_HEAD, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "semantic_v0244_head": V0244_SEMANTIC_HEAD, "v0243_rejected_head": V0243_REJECTED_HEAD, "branch": BRANCH, "findings": ["F-44R", "F-46R"], "preserves": ["F-31R", "F-32RR", "F-33", "F-34", "F-35RR", "F-35RC", "F-36", "F-38", "F-37R", "F-39", "F-40", "F-41", "F-42", "F-43R", "F-45"], "historical_evidence_unchanged": True})
    _write(output_dir / "uads-handoff-v0247.json", uads_handoff)
    _write(output_dir / "execution-evidence-v0247.json", result)
    return result
