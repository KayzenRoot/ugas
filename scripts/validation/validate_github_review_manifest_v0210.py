"""Fail-closed validation of the bounded v0.21.0 review manifest."""
...
def _artifact_path(artifact: Path, relative: str) -> Path:
    ...
    if direct.is_file():
        return direct
    prefix = "docs/evidence/maps-minimap-runtime-v0210/"
...
def main() -> int:
    ...
    try:
        ...
        if value.get("schema_version") != "0.21.0" or value.get("manifest_type") != "github-ci-maps-minimap-v0210-review": failures.append("manifest-identity-invalid")
        ...
        if scope.get("version") != "0.21.0" or scope.get("phase") != "MAPS_MINIMAP" or scope.get("current_gate") != "MAPS_MINIMAP_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED" or scope.get("allowed_next_actions") != ["external_review_maps_minimap_v0210"]: failures.append("active-scope-invalid")
    ...
    result = {"schema_version": "0.21.0", "status": "PASS" if not failures else "FAIL", "failures": failures, "checked_manifest": str(args.manifest)}
