"""Validate the bounded v0.24.6 exact-head review manifest fail-closed."""
...
from ugas.state_consistency_v0246 import CURRENT_GATE, NEXT_ACTION, REJECTED_REVIEWED_HEAD, VERSION as STATE_VERSION
...
def main() -> int:
    ...
    failures: list[str] = []
    if value.get("schema_version") != VERSION or value.get("manifest_type") != "github-ci-orchestration-runtime-v0246-review":
    ...
    if not isinstance(gates, dict) or len(gates) < 62 or any(not isinstance(item, dict) or item.get("status") != "PASS" or not strict_true(item.get("observed")) or item.get("observed_type") != "bool" for item in gates.values()):
    ...
    if controls.get("status") != "PASS" or len(controls_map) < 47:
    ...
    if value.get("uads_handoff_status") != "PASS":
        failures.append("uads-handoff")
    uads = value.get("uads_handoff", {})
    if uads.get("execution_mode") != "GLOBAL_FIRST" or uads.get("project_footprint") != "ZERO" or uads.get("route_status") != "SELECTED" or uads.get("dispatch_status") != "DISPATCHED" or not uads.get("selected_profile_id"):
        failures.append("uads-handoff-semantics")
    canonical_path = ROOT / "docs/evidence/orchestration-runtime-v0246/correction-history-v0246.json"
    try:
        canonical = json.loads(canonical_path.read_text(encoding="utf-8"))
        if not isinstance(canonical, dict) or not canonical:
            raise ValueError("empty")
    except (OSError, json.JSONDecodeError, ValueError):
        failures.append("correction-history-missing")
        canonical = {}
    manifest_history = value.get("correction_history", {})
