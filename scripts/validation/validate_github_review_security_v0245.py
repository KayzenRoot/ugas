"""Validate the bounded v0.24.5 review artifact security and path boundary."""
...
def main() -> int:
    ...
    failures: list[str] = []
    allowed_prefixes = (
        "docs/evidence/orchestration-runtime-v0245/",
        "docs/evidence/current-state.json",
        "schemas/current-state-v0245.json",
        "schemas/orchestration-runtime-v0245.json",
        "REVIEW-v0.24.5.md",
        "github-review-manifest-v0245.json",
        "test-results-v0245.json",
        "validation-results-v0245.json",
        "state-validation-v0245.json",
        "schema-validation-v0245.json",
        "security-results-v0245.json",
        "manifest-validation-results-v0245.json",
        "logs/",
    )
    ...
    result = {"schema_version": "0.24.5", "status": "PASS" if not failures else "FAIL", "failures": failures, "file_count": len(files), "secrets_included": False, "model_weights_included": False, "telemetry_db_included": False, "local_credentials_included": False}
    Path(args.output).write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps(result, ensure_ascii=False))
    for failure in failures:
        print(f"::error title=UGAS v0.24.5 artifact security::{failure}")
