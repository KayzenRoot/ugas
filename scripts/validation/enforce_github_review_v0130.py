"""Enforce active v0.13.0 review gates after artifact upload."""
...
def main() -> int:
    ...
    if validation.get("status") != "V0130_GITHUB_REVIEW_MANIFEST_PASSED": failures.append("manifest-validation-not-pass")
    ...
    result = {"schema_version": "0.13.0", "status": "PASS" if not failures else "FAIL", "failures": failures}; print(json.dumps(result, indent=2, ensure_ascii=False)); return 0 if not failures else 1
