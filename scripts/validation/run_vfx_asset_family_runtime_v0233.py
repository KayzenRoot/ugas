"""Execute the complete deterministic VFX v0.23.3 F-26S/F-29/F-30 correction."""
...
from ugas.historical_immutability_v0233 import (
...
from ugas.state_consistency_v0233 import (
    BASELINE_MAIN_SHA,
    CURRENT_GATE,
    FEATURE_BRANCH,
    NEXT_ACTION,
...
EVIDENCE = ROOT / "docs/evidence/vfx-asset-family-runtime-v0233"
...
def _controls(
    ...
    controls.append(_expect_rejection("VFX233-NC-01", "REDUCE_OPACITY with unchanged full alpha pixels", "VFX_DEGRADED_PIXELS_INVALID", lambda: validate_degraded_output(opacity_record, bad, root, root)))

    bad = deepcopy(opacity_output)
    bad["frames"] = bad["frames"] + [deepcopy(bad["frames"][-1])]
    bad["frame_count"] = len(bad["frames"])
    bad["output_hash"] = _expected_output_hash(opacity_record, bad, bad["frames"])
    controls.append(_expect_rejection("VFX233-NC-02", "REDUCE_FRAME_COUNT while all full frames remain", "VFX_FRAME_COUNT_INVALID", lambda: validate_degraded_output(opacity_record, bad, root, root)))

    for control_id, label, record, output in (
        ("VFX233-NC-03", "REDUCE_RADIUS with unchanged decoded bounds", radius_record, radius_output),
        ("VFX233-NC-04", "REDUCE_VISUAL_AREA with unchanged decoded bounds", radius_record, radius_output),
    ):
        bad = deepcopy(output)
    ...
    controls.append(_expect_rejection("VFX233-NC-05", "copy full frame bytes and recompute hashes", "VFX_DEGRADED_PIXELS_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))
    ...
    bad["output_hash"] = _expected_output_hash(loop, bad, bad["frames"])
    controls.append(_expect_rejection("VFX233-NC-06", "replace with unrelated valid PNG and recompute hashes", "VFX_DEGRADED_PIXELS_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))

    bad_result = deepcopy(loop_result)
    bad_result["applied_steps"] = list(reversed(bad_result["applied_steps"]))
    controls.append(_expect_rejection("VFX233-NC-07", "reorder applied fallback steps", "VFX_FALLBACK_ORDER_INVALID", lambda: validate_fallback_result(loop, bad_result, profile)))

    bad_result = deepcopy(loop_result)
    bad_result["applied_steps"] = bad_result["applied_steps"][1:]
    controls.append(_expect_rejection("VFX233-NC-08", "skip an earlier required fallback step", "VFX_FALLBACK_ORDER_INVALID", lambda: validate_fallback_result(loop, bad_result, profile)))

    bad = deepcopy(degraded)
    bad["degradation_mechanisms"] = [*bad["degradation_mechanisms"], "REDUCE_OPACITY"]
    controls.append(_expect_rejection("VFX233-NC-09", "add mechanism absent from validated steps", "VFX_FALLBACK_MECHANISM_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))

    bad = deepcopy(degraded)
    bad["semantic_identity"]["gameplay_authority"] = "COMBAT"
    controls.append(_expect_rejection("VFX233-NC-10", "mutate gameplay authority during fallback", "VFX_DEGRADED_OUTPUT_SEMANTIC_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))

    bad = deepcopy(skip_output)
    bad["frames"] = [deepcopy(degraded["frames"][0])]
    controls.append(_expect_rejection("VFX233-NC-11", "terminal SKIP_VISUAL with non-empty frames", "VFX_FALLBACK_SKIP_RECORD_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))

    bad = deepcopy(repeat_output)
    bad["output_hash"] = "0" * 64
    controls.append(_expect_rejection("VFX233-NC-12", "mismatched independent repeat hash", "VFX_DEGRADED_DETERMINISM_MISMATCH", lambda: validate_degraded_determinism(degraded, bad)))

    bad_result = deepcopy(loop_result)
    bad_result["after"] = deepcopy(bad_result["after"])
    bad_result["after"]["layers"] = 999
    controls.append(_expect_rejection("VFX233-NC-13", "mutate fallback after-state", "VFX_FALLBACK_STATE_INVALID", lambda: validate_fallback_result(loop, bad_result, profile)))

    bad = deepcopy(degraded)
    bad["frames"][0]["source_index"] = 1
    controls.append(_expect_rejection("VFX233-NC-14", "mutate selected source frame index", "VFX_FRAME_SELECTION_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))

    bad = deepcopy(opacity_output)
    bad["degradation_mechanisms"] = [step for step in bad["degradation_mechanisms"] if step != "REDUCE_OPACITY"]
    controls.append(_expect_rejection("VFX233-NC-15", "omit a validated degradation mechanism", "VFX_FALLBACK_MECHANISM_INVALID", lambda: validate_degraded_output(opacity_record, bad, root, root)))

    bad = deepcopy(skip_output)
    bad["skipped_output"] = {}
    controls.append(_expect_rejection("VFX233-NC-16", "remove explicit skipped-output record", "VFX_FALLBACK_SKIP_RECORD_INVALID", lambda: validate_degraded_output(loop, bad, root, root)))

    controls.append(_expect_rejection("VFX233-NC-17", "non-boolean hard gate", "VFX_HARD_GATE_NON_BOOLEAN_REJECTED", lambda: _raise_if_not(strict_boolean_observation("PASS"), "VFX_HARD_GATE_NON_BOOLEAN_REJECTED", "PASS")))
    controls.append(_expect_rejection("VFX233-NC-18", "production registry entry", "VFX_PRODUCTION_BOUNDARY_REJECTED", lambda: validate_production_registry([{"effect_id": base_record["effect_id"]}])))
    return controls
...
def _historical_proof() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-vfx-v0233-history-") as temp:
        ...
        for relative_root, authority_ref in (
            ...
            candidate_root = candidate / relative_root
            ...
            mutations.append(_expect_rejection(f"VFX233-F29-{authority_ref[:4]}", f"mutated {relative_root} historical file", HISTORICAL_REJECTION_CLASS, lambda relative_root=relative_root, authority_ref=authority_ref, candidate_root=candidate_root: validate_historical_root(ROOT, authority_ref, relative_root, candidate_root)))
        for relative_path, authority_ref in (("REVIEW-v0.23.0.md", "c8967f5f7b4d3f66b402d10ae8fe5f29cb9fbb72"), ("REVIEW-v0.23.1.md", "136079540f674c7467d93bd28e9834addbfce5c3")):
            ...
            mutations.append(_expect_rejection(f"VFX233-F29-{relative_path}", f"mutated {relative_path}", HISTORICAL_REJECTION_CLASS, lambda relative_path=relative_path, authority_ref=authority_ref, candidate_file=candidate_file: validate_historical_file(ROOT, authority_ref, relative_path, candidate_file)))
    ...
    proof["status"] = "PASS" if proof["mutation_controls_pass"] else "FAIL"
    return proof
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ugas-vfx-v0233-a-") as first_dir, tempfile.TemporaryDirectory(prefix="ugas-vfx-v0233-b-") as second_dir:
    ...
    contact.replace(EVIDENCE / "vfx-effect-contact-sheet-v0233.png")
    timing.replace(EVIDENCE / "vfx-timing-qa-sheet-v0233.png")
    ...
    build_budget_fallback_sheet(EVIDENCE, first_root, loop, degraded, EVIDENCE / "vfx-budget-fallback-qa-sheet-v0233.png")
    ...
    state_schema = _load(ROOT / "schemas/current-state-v0233.json")
    runtime_schema = _load(ROOT / "schemas/vfx-asset-family-runtime-v0233.json")
    ...
    checks = [
        ("VFX233-HG-01", lambda: len(EFFECT_CLASSES) == 10),
        ("VFX233-HG-02", lambda: validate_vfx_manifest(manifest, EVIDENCE)["status"] == "VFX_ASSET_FAMILY_MANIFEST_VALID"),
        ("VFX233-HG-03", lambda: all(validate_effect_record(item, EVIDENCE) is None for item in records)),
        ("VFX233-HG-04", lambda: validate_fallback_result(loop, loop_result, profile) is None),
        ("VFX233-HG-05", lambda: validate_degraded_output(loop, degraded, EVIDENCE, first_root) is None),
        ("VFX233-HG-06", lambda: validate_degraded_output(loop, repeat, EVIDENCE, repeat_root) is None),
        ("VFX233-HG-07", lambda: validate_degraded_determinism(degraded, repeat) is None),
        ("VFX233-HG-08", lambda: degraded["output_hash"] != loop["content_hash"]),
        ("VFX233-HG-09", lambda: degraded["frames"][0]["source_index"] == 0),
        ("VFX233-HG-10", lambda: opacity_output["frames"][0]["alpha_range"] != opacity_record["frames"][0]["alpha_range"]),
        ("VFX233-HG-11", lambda: opacity_output["frame_count"] < opacity_record["lifecycle"]["frame_count"]),
        ("VFX233-HG-12", lambda: radius_output["frames"][0]["alpha_bounds"] != radius_record["frames"][0].get("alpha_bounds")),
        ("VFX233-HG-13", lambda: skip_output["status"] == "SKIPPED" and skip_output["frames"] == []),
        ("VFX233-HG-14", lambda: len(set(degraded["degradation_mechanisms"])) >= 3),
        ("VFX233-HG-15", lambda: degraded["semantic_preserved"] is True and degraded["changes_gameplay"] is False),
        ("VFX233-HG-16", lambda: (EVIDENCE / "vfx-budget-fallback-qa-sheet-v0233.png").is_file()),
        ("VFX233-HG-17", lambda: historical["status"] == "PASS"),
        ("VFX233-HG-18", lambda: historical["mutation_controls_pass"] is True),
        ("VFX233-HG-19", lambda: first_hash == second_hash),
        ("VFX233-HG-20", lambda: state_result["status"] == CURRENT_GATE and not state_result["failures"]),
        ("VFX233-HG-21", lambda: validate_schema_document(state_schema) is None and validate_instance(state, state_schema) is None),
        ("VFX233-HG-22", lambda: validate_schema_document(runtime_schema) is None and validate_instance(manifest, runtime_schema) is None),
        ("VFX233-HG-23", lambda: strict_boolean_observation(True) is True),
        ("VFX233-HG-24", lambda: strict_boolean_observation(False) is False),
        ("VFX233-HG-25", lambda: strict_boolean_observation(1) is False),
        ("VFX233-HG-26", lambda: strict_boolean_observation("PASS") is False),
        ("VFX233-HG-27", lambda: resolve_next_actions(state, {"source": "GITHUB_LIVE", "repository": "KayzenRoot/ugas", "pr_number": PR_NUMBER, "branch": FEATURE_BRANCH, "base_sha": BASELINE_MAIN_SHA, "head_sha": "a" * 40, "pr_state": "OPEN", "merged": False, "external_approval": False})["allowed_next_actions"] == [NEXT_ACTION]),
        ("VFX233-HG-28", lambda: validate_production_registry([])["registry"] == []),
        ("VFX233-HG-29", lambda: manifest["production_routing"] == "BLOCKED" and manifest["production_approved"] is False and manifest["new_generation"] == 0),
        ("VFX233-HG-30", lambda: all(item["provenance"]["provider"] is None for item in records)),
        ("VFX233-HG-31", lambda: all(item["fallback_result"]["semantic_preserved"] is True for item in (degraded, repeat, opacity_output, radius_output))),
        ("VFX233-HG-32", lambda: all(item["semantic_identity"]["gameplay_authority"] == "NONE" for item in (degraded, repeat, opacity_output, radius_output))),
    ]
    ...
    write_json(EVIDENCE / "hard-gates-v0233.json", {"status": "PASS" if all_gates_pass else "FAIL", "required_gate_count": len(gates), "gates": {item["gate_id"]: item for item in gates}})
    write_json(EVIDENCE / "negative-controls-v0233.json", {"status": "PASS" if all_controls_pass else "FAIL", "required_control_count": len(controls), "controls": {item["control_id"]: item for item in controls}})
    write_json(EVIDENCE / "full-slice-two-run-determinism-v0233.json", {"status": "PASS" if first_hash == second_hash else "FAIL", "first_run_sha256": first_hash, "second_run_sha256": second_hash, "equal": first_hash == second_hash})
    write_json(EVIDENCE / "degraded-output-determinism-v0233.json", {"status": "PASS" if degraded["output_hash"] == repeat["output_hash"] else "FAIL", "first_output_hash": degraded["output_hash"], "repeat_output_hash": repeat["output_hash"], "first_root": str(first_root.relative_to(ROOT)).replace("\\", "/"), "repeat_root": str(repeat_root.relative_to(ROOT)).replace("\\", "/"), "first_frame_hashes": [item["sha256"] for item in degraded["frames"]], "repeat_frame_hashes": [item["sha256"] for item in repeat["frames"]], "equal": degraded["output_hash"] == repeat["output_hash"]})
    write_json(EVIDENCE / "historical-immutability-v0233.json", historical)
    write_json(EVIDENCE / "production-registry-v0233.json", {**validate_production_registry([]), "real_vfx_asset_coverage": "NONE", "synthetic_vfx_fixture": "TEST_ONLY"})
    write_json(EVIDENCE / "state-consistency-v0233.json", state_result)
    write_json(EVIDENCE / "schema-validation-v0233.json", {"status": "PASS", "schema": "schemas/current-state-v0233.json", "instance": "docs/evidence/current-state.json"})
    write_json(EVIDENCE / "capability-matrix-validation-v0233.json", {"status": "PASS" if matrix.get("version") == VERSION and matrix.get("next_candidate") == NEXT_CANDIDATE else "FAIL", "version": matrix.get("version"), "next_candidate": matrix.get("next_candidate"), "production_routing": matrix.get("production_routing"), "new_generation": matrix.get("new_generation")})
    write_json(EVIDENCE / "v0.23.2-rejection-correction-v0233.json", {"status": "CORRECTION_REQUIRED", "reviewed_head": "d7501177a9f4480921226b9832ffc237544a6433", "findings": ["F-26S", "F-29", "F-30"], "forward_only": True, "historical_evidence_unchanged": historical["historical_evidence_unchanged"]})
    write_json(EVIDENCE / "exact-sha-lifecycle-v0233.json", {"source": "GitHub LIVE exact-head metadata", "pr_number": PR_NUMBER, "pr_state": "OPEN", "merged": False, "tracked_head_sha": None, "base_main_sha": BASELINE_MAIN_SHA, "current_head_resolution": "external exact-head artifact / GitHub LIVE"})
    write_json(EVIDENCE / "execution-evidence-v0233.json", {"status": CURRENT_GATE if overall else "VFX_ASSET_FAMILY_RUNTIME_CORRECTION_FAILED", "overall_pass": overall, "schema_version": VERSION, "family_id": FAMILY_ID, "corrections": ["F-26S", "F-29", "F-30"], "hard_gate_count": len(gates), "negative_control_count": len(controls), "effect_class_count": len(records), "production_routing": "BLOCKED", "production_approved": False, "real_vfx_asset_coverage": "NONE", "synthetic_vfx_fixture": "TEST_ONLY", "new_generation": 0, "provider_generation_jobs": 0, "diffusion_runs": 0, "base_main_sha": BASELINE_MAIN_SHA, "rejected_reviewed_head": "d7501177a9f4480921226b9832ffc237544a6433"})
