"""Build the bounded exact-head v0.24.1 review manifest."""
...
def main() -> int:
    ...
    execution = load(root / f"{EVIDENCE}/execution-evidence-v0241.json")
    authority = load(root / f"{EVIDENCE}/dependency-authority-v0241.json")
    ...
    overall = (
        ...
        and state_validation.get("status") == "ORCHESTRATION_RUNTIME_HARDENING_F31_F36_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED"
    ...
    value = {
        ...
        "manifest_type": "github-ci-orchestration-runtime-v0241-review",
        ...
        "scope": {"version": VERSION, "phase": "ORCHESTRATION_RUNTIME_HARDENING", "current_gate": "ORCHESTRATION_RUNTIME_HARDENING_F31_F36_CORRECTION_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED", "baseline_main_sha": BASE_MAIN, "rejected_reviewed_head": REJECTED_HEAD, "allowed_next_actions": ["external_review_orchestration_runtime_v0241"], "next_candidate": "V1_FINAL_ACCEPTANCE", "new_generation": 0, "test_only": True, "provider_neutral": True},
        ...
        "correction_history": {"status": "CORRECTION_REQUIRED", "rejected_reviewed_head": REJECTED_HEAD, "findings": ["F-31", "F-32", "F-33", "F-34", "F-35", "F-36"], "historical_evidence_unchanged": True},
    ...
    (output / "github-review-manifest-v0241.json").write_text(json.dumps(value, indent=2, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0241_GITHUB_REVIEW_MANIFEST_BUILT", "overall_status": value["overall_status"], "head_sha": args.head_ref, "pr_number": args.pr_number}, ensure_ascii=False))
    return 0
