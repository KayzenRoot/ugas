"""Record bounded v0.23.3 test and official-validation totals."""
...
def main() -> int:
    ...
    validation = {"schema_version": "0.23.3", "status": "failed", "checks": 0, "passed": 0, "failed": 1, "exit_code": int(a.validation_exit)}
    if checks:
    ...
    tests = {"schema_version": "0.23.3", "status": "passed" if a.tests_exit == "0" and count > 0 else "failed", "count": count, "passed": count if a.tests_exit == "0" else 0, "failed": 0 if a.tests_exit == "0" else 1, "exit_code": int(a.tests_exit)}
    ...
    (a.output_dir / "test-results-v0233.json").write_text(json.dumps(tests, indent=2) + "\n", encoding="utf-8")
    (a.output_dir / "validation-results-v0233.json").write_text(json.dumps(validation, indent=2) + "\n", encoding="utf-8")
    (a.output_dir / "gate-results-v0233.json").write_text(json.dumps({"schema_version": "0.23.3", "gates": {k: {"status": "PASS" if v else "FAIL"} for k, v in gates.items()}, "overall_status": "PASS" if overall else "FAIL"}, indent=2) + "\n", encoding="utf-8")
