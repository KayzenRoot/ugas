"""Fail-closed exact-head validator for the v0.22.3 review manifest."""
...
def main() -> int:
    ...
    try:
        ...
        if value.get("schema_version") != "0.22.3" or value.get("manifest_type") != "github-ci-ui-asset-family-v0223-review": failures.append("manifest-identity-invalid")
        ...
        expected_scope = {"version": "0.22.3", "phase": "UI_ASSET_FAMILY", "current_gate": "UI_ASSET_FAMILY_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED", "baseline_main_sha": "1fb298885c56ccbf3df7dfdcb1be37fe2dc23af3", "allowed_next_actions": ["external_review_ui_asset_family_v0223"], "next_candidate": "UI_ASSET_FAMILY", "new_generation": 0}; failures.extend(f"scope:{key}" for key, expected in expected_scope.items() if scope.get(key) != expected)
        ...
        for relative in ("docs/evidence/ui-asset-family-runtime-v0220/", "docs/evidence/ui-asset-family-runtime-v0221/", "docs/evidence/ui-asset-family-runtime-v0222/", "docs/evidence/maps-minimap-runtime-v0213/", "docs/evidence/github-governance-v0221/v0213-closure-completion-binding-v2.json", "docs/evidence/github-governance-v0220/v0213-post-merge-binding.json"):
    ...
    result = {"schema_version": "0.22.3", "status": "PASS" if not failures else "FAIL", "failures": failures, "checked_manifest": str(args.manifest)}; args.result_output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8"); print(json.dumps(result, ensure_ascii=False)); return 0 if not failures else 1
