"""Fail-closed validation of the bounded v0.20.1 review manifest."""
...
def main() -> int:
    ...
    try:
        ...
        if value.get("schema_version") != "0.20.1" or value.get("manifest_type") != "github-ci-environment-tilesets-v0201-review": failures.append("manifest-identity-invalid")
        if value.get("scope", {}).get("version") != "0.20.1" or value.get("scope", {}).get("phase") != "ENVIRONMENT_TILESETS" or value.get("scope", {}).get("current_gate") != "ENVIRONMENT_TILESETS_AUTOTILE_WORLD_METRIC_VARIANT_INTEGRITY_TECHNICALLY_QUALIFIED": failures.append("active-scope-invalid")
    ...
    result = {"schema_version": "0.20.1", "status": "PASS" if not failures else "FAIL", "failures": failures, "checked_manifest": str(args.manifest)}
