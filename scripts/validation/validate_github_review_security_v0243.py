"""Validate the bounded v0.24.3 review artifact security and path boundary."""
...
def main() -> int:
    ...
    failures: list[str] = []
    allowed_prefixes = (
        "docs/evidence/orchestration-runtime-v0243/",
        "docs/evidence/current-state.json",
        "schemas/current-state-v0243.json",
        "schemas/orchestration-runtime-v0243.json",
        "REVIEW-v0.24.3.md",
        "github-review-manifest-v0243.json",
        "test-results-v0243.json",
        "validation-results-v0243.json",
        "state-validation-v0243.json",
        "schema-validation-v0243.json",
        "security-results-v0243.json",
        "manifest-validation-results-v0243.json",
        "logs/",
    )
    ...
    result = {"schema_version": "0.24.3", "status": "PASS" if not failures else "FAIL", "failures": failures, "file_count": len(files), "secrets_included": False, "model_weights_included": False, "telemetry_db_included": False, "local_credentials_included": False}
    Path(args.output).write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps(result, ensure_ascii=False))
    for failure in failures:
        print(f"::error title=UGAS v0.24.3 artifact security::{failure}")
