"""Record truthful v0.15.0 test, validation and review-gate results."""
...
def main() -> int:
    ...
    (output / "test-results-v0150.json").write_text(json.dumps(tests, indent=2) + "\n", encoding="utf-8")
    (output / "validation-results-v0150.json").write_text(json.dumps(validation, indent=2) + "\n", encoding="utf-8")

    runtime = _file_result(args.runtime_log, args.runtime_exit_code)
    recovery = _file_result(None, args.recovery_exit_code)
    (output / "runtime-results-v0150.json").write_text(json.dumps(runtime, indent=2) + "\n", encoding="utf-8")
    (output / "recovery-results-v0150.json").write_text(json.dumps(recovery, indent=2) + "\n", encoding="utf-8")
    ...
    (output / "gate-results-v0150.json").write_text(json.dumps(gate_result, indent=2) + "\n", encoding="utf-8")
