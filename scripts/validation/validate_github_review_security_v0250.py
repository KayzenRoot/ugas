"""Validate the bounded v0.25.0 V1 final acceptance artifact security, inventory and path boundary."""
...
SELF_ATTESTATION = "security-results-v0250.json"
REQUIRED_BASE_SHA = "6c6d53dab5a95226bf9578a6099d755d51327d8e"
REQUIRED_BRANCH = "codex/v1-final-acceptance"
...
ALLOWED_PREFIXES = (
    ...
    "docs/evidence/github-governance-v0124/dashboard-external-visual-approval.json",
    ...
    "github-review-manifest-v0250.json",
    "test-results-v0250.json",
    "validation-results-v0250.json",
    "state-validation-v0250.json",
    "environment-results-v0250.json",
    "security-results-v0250.json",
    "manifest-validation-results-v0250.json",
    "pre-upload-enforcement-v0250.json",
    "logs/",
)
...
def verify_artifact_inventory(root: Path, inventory: dict[str, Any], *, excluded_paths: tuple[str, ...] | list[str] = (SELF_ATTESTATION,)) -> dict[str, Any]:
    unique_excluded = sorted(set(list(excluded_paths)))
    if len(unique_excluded) > 1:
        return {"status": "FAIL", "reason": "ARTIFACT_MULTI_SELF_EXCLUSION", "failures": ["multi-self-exclusion"], "inventory_consistency": "FAIL", "excluded_paths": unique_excluded, "scanned_file_count": inventory.get("scanned_file_count"), "final_staged_file_count": None, "inventory_digest": inventory.get("inventory_digest")}
...
def main() -> int:
    ...
    for failure in result.get("failures", []):
        print(f"::error title=UGAS v0.25.0 artifact security::{failure}")
