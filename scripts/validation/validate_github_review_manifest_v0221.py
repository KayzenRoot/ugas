"""Fail-closed validator for the bounded v0.22.1 review manifest."""
...
def main() -> int:
    ...
    try:
        value = json.loads(args.manifest.read_text(encoding="utf-8")); required = ("schema_version", "manifest_type", "repository", "pull_request", "scope", "tests", "validation", "gates", "negative_controls", "determinism", "ui_evidence", "governance", "production_boundary", "review_boundary", "security_boundary"); failures.extend(f"missing:{item}" for item in required if item not in value)
        if value.get("schema_version") != "0.22.1" or value.get("manifest_type") != "github-ci-ui-asset-family-v0221-review": failures.append("manifest-identity-invalid")
        ...
        scope = value.get("scope", {}); expected_scope = {"version": "0.22.1", "phase": "UI_ASSET_FAMILY", "current_gate": "UI_ASSET_FAMILY_SEMANTIC_INTEGRITY_TECHNICALLY_QUALIFIED", "baseline_main_sha": "1fb298885c56ccbf3df7dfdcb1be37fe2dc23af3", "allowed_next_actions": ["external_review_ui_asset_family_v0221"], "next_candidate": "UI_ASSET_FAMILY", "new_generation": 0}
        ...
        if value.get("governance", {}).get("validation", {}).get("status") != "PASS": failures.append("governance")
        ...
        for relative in ("docs/evidence/github-governance-v0221/v0213-closure-completion-binding-v2.json", "docs/evidence/ui-asset-family-runtime-v0220/", "docs/evidence/maps-minimap-runtime-v0213/"):
    ...
    result = {"schema_version": "0.22.1", "status": "PASS" if not failures else "FAIL", "failures": failures, "checked_manifest": str(args.manifest)}; args.result_output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8"); print(json.dumps(result, ensure_ascii=False)); return 0 if not failures else 1
