"""Record truthful v0.14.1 test, validation and gate totals."""
...
def _test_result(text: str, exit_code: int) -> dict[str, Any]:
    value = _old_test_result(text, exit_code); value["schema_version"] = "0.14.1"; return value


def _validation_result(text: str, exit_code: int) -> dict[str, Any]:
    value = _old_validation_result(text, exit_code); value["schema_version"] = "0.14.1"; return value
...
def main() -> int:
    ...
    (output / "test-results-v0141.json").write_text(json.dumps(tests, indent=2) + "\n", encoding="utf-8")
    (output / "validation-results-v0141.json").write_text(json.dumps(validation, indent=2) + "\n", encoding="utf-8")
    ...
    gate_result = {"schema_version": "0.14.1", "overall_status": "PASS" if all(item["status"] == "PASS" for item in gates) else "FAIL", "gates": gates}
    (output / "gate-results-v0141.json").write_text(json.dumps(gate_result, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0141_RESULTS_RECORDED", "tests": tests, "validation": validation, "gates": gate_result}, ensure_ascii=False))
    return 0
