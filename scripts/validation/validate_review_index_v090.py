...
def validate(path: Path) -> dict[str, object]:
    ...
    publication = value.get("publication", {}); build_head = str(publication.get("index_build_git_head", "")); final_head = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT, capture_output=True, text=True, check=False).stdout.strip()
    ...
    seen: set[str] = set(); visual_manifest = json.loads((ROOT / "docs/evidence/review-visuals-v0.9.0.json").read_text(encoding="utf-8")); listed = {item.get("path") for item in artifacts if isinstance(item, dict)}
    for item in artifacts:
        ...
        if not local.is_file(): failures.append(f"artifact_missing:{artifact_path}")
        elif digest(local) != item.get("sha256"): failures.append(f"artifact_hash_mismatch:{artifact_path}")
