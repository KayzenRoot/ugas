"""Build the bounded exact-head v0.24.2 review manifest."""
...
REJECTED_HEAD = "ed9fa927fd50193130b3e085ef077dea267f2790"
EVIDENCE = "docs/evidence/orchestration-runtime-v0242"
...
def main() -> int:
    ...
    execution = load(root / f"{EVIDENCE}/execution-evidence-v0242.json")
    authority = load(root / f"{EVIDENCE}/dependency-authority-v0242.json")
    ...
    value = {
        ...
        "manifest_type": "github-ci-orchestration-runtime-v0242-review",
    ...
    (output / "github-review-manifest-v0242.json").write_text(json.dumps(value, indent=2, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"status": "V0242_GITHUB_REVIEW_MANIFEST_BUILT", "overall_status": value["overall_status"], "head_sha": args.head_ref, "pr_number": args.pr_number}, ensure_ascii=False))
    return 0
