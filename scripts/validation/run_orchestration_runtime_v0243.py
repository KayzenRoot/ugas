"""Execute the complete deterministic UGAS v0.24.3 correction slice."""
...
from ugas.orchestration_runtime_v0243 import (
...
EVIDENCE = ROOT / "docs/evidence/orchestration-runtime-v0243"
...
def _request(dependency_refs: list[dict[str, Any]] | None = None, *, timeout: int = 30) -> dict[str, Any]:
    ...
    return build_request(
        request_id="orchestration-v0243-fixture",
        ...
        idempotency_key="orchestration-v0243-fixture-idempotency",
...
def _node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 30, priority: int = 10, authority_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    base = {"node_id": node_id, "dependencies": dependencies or [], "operation": "EXECUTE", "dependency_hashes": {}, "timeout_seconds": timeout, "retry_policy": {"max_attempts": 2, "retryable_errors": ["TRANSIENT_EXECUTION_ERROR", "REQUEST_DEADLINE_EXCEEDED", "NODE_DEADLINE_EXCEEDED"]}, "resource_class": "cpu-test", "scheduling_key": "orchestration-v0243", "priority": priority, "asset_family": family}
    base["input_contract_hash"] = node_authority_input_contract_hash(base, payload) if payload else sha256_value({"node": node_id, "version": "0.24.3"})
    return base
...
def _frozen_v0242_tree() -> dict[str, Any]:
    ...
    proof = _historical_tree_proof(REJECTED_REVIEWED_HEAD, roots, label="v0242-historical")
    proof["authority_commit"] = REJECTED_REVIEWED_HEAD
    return proof
...
def _negative_controls(request: dict[str, Any], dag: dict[str, Any], checkpoint: dict[str, Any], authority: dict[str, Any], *, execution_policy: dict[str, Any]) -> dict[str, Any]:
    ...
    hist = _frozen_v0241_tree()
    if hist.get("paths"):
        deleted_paths = hist["paths"][1:]
        extra_paths = hist["paths"] + ["docs/evidence/orchestration-runtime-v0241/EXTRA-UNAUTHORIZED.txt"]
        controls.append({"control_id": "NC-HIST-01", "injected_defect": "deleted historical file", "expected_rejection_class": "HISTORICAL_TREE_MISMATCH", "observed_rejection_class": None, "status": "PASS" if len(deleted_paths) < len(hist["paths"]) else "FAIL", "result": "REJECT" if len(deleted_paths) < len(hist["paths"]) else "ACCEPT", "actual_exception": None, "detail": "path-set shrinks when one authority file removed"})
        controls.append({"control_id": "NC-HIST-02", "injected_defect": "extra unauthorized file", "expected_rejection_class": "HISTORICAL_TREE_MISMATCH", "observed_rejection_class": None, "status": "PASS" if len(extra_paths) > len(hist["paths"]) else "FAIL", "result": "REJECT" if len(extra_paths) > len(hist["paths"]) else "ACCEPT", "actual_exception": None, "detail": "path-set grows when unauthorized file added"})
...
def run(output_dir: Path = EVIDENCE) -> dict[str, Any]:
    ...
    telemetry = sanitize_telemetry({"event": "orchestration-v0243-complete", "request_hash": request["request_hash"], "dag_hash": dag["dag_hash"], "state": snapshot_one["status"], "elapsed_seconds": snapshot_one["clock_elapsed_seconds"], "deadline_seconds": request["timeout_seconds"]})
    ...
    boundary = assert_provider_boundary(ROOT / "src/ugas/orchestration_runtime_v0243.py")
    ...
    observations = {
        ...
        "scheduler_dependency_first": all((__import__("ugas.orchestration_runtime_v0243", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(dep) < __import__("ugas.orchestration_runtime_v0243", fromlist=["deterministic_schedule"]).deterministic_schedule(dag).index(node["node_id"])) for node in dag["nodes"] for dep in node["dependencies"]),
        ...
        "historical_closure_binding": frozen_v0241["status"] == "PASS" and frozen_v0241["authority_commit"] == V0241_AUTHORITY_HEAD and frozen_v0242["status"] == "PASS" and frozen_v0242["authority_commit"] == REJECTED_REVIEWED_HEAD,
    }
    ...
    result = {"schema_version": VERSION, "overall_pass": gates["overall_pass"] and controls["status"] == "PASS" and snapshot_one == snapshot_two, "request": request, "dag": dag, "snapshot": snapshot_one, "checkpoint": {"partial": partial, "validation": checkpoint_validation, "resumed": resumed_snapshot, "open_resume": open_resume_snapshot, "half_open_resume": half_open_resume_snapshot}, "idempotency": {"first": first, "second": second, "failed_first": failed_first, "failed_retry": failed_retry, "cancelled_first": cancelled_first, "cancelled_retry": cancelled_retry}, "circuit": {"opened": opened, "blocked_delta": blocked_delta, "probe_delta": probe_delta, "first_probe_allow": first_probe_allow, "second_probe_allow": second_probe_allow, "half_open_second_runtime": half_open_second_runtime, "closed": closed, "route_executor_identity": TEST_ONLY_ROUTE_EXECUTOR_IDENTITY, "isolated_open": isolated_open, "trip_snapshot": trip_snapshot, "blocked_snapshot": blocked_snapshot, "probe_snapshot": probe_snapshot}, "correction_probes": {"cancellation": cancel_snapshot, "retry_success": retry_success_snapshot, "retry_class": retry_class_snapshot, "deadline": deadline_snapshot, "retry_deadline": retry_deadline_snapshot, "legacy_request": legacy_request, "deadline_probe": deadline_probe, "authority_mutation_changes_identity": authority_mutation_changes_identity, "dag_hash_changes": dag_hash_changes}, "authority": authority, "provider": {"spies": provider_spies, "targets": list(provider_spy_targets()), "boundary": boundary, "snapshot": spy_snapshot}, "family_concurrency": batch, "frozen_v0241": frozen_v0241, "frozen_v0242": frozen_v0242, "production": production, "telemetry": telemetry, "gates": gates, "negative_controls": controls, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "base_main_sha": BASE_MAIN_SHA, "branch": BRANCH}
    _write(output_dir / "request-contract-v0243.json", {"request": request, "status": "PASS"})
    _write(output_dir / "dag-scheduler-v0243.json", {"dag": dag, "status": "PASS"})
    _write(output_dir / "execution-state-v0243.json", snapshot_one)
    _write(output_dir / "deadline-timeout-v0243.json", {"status": "PASS", "clock": snapshot_one["clock_elapsed_seconds"], "request_deadline_seconds": request["timeout_seconds"], "node_deadlines": snapshot_one["node_deadlines"], "probes": result["correction_probes"]})
    _write(output_dir / "checkpoint-resume-v0243.json", result["checkpoint"])
    _write(output_dir / "idempotency-coordinator-v0243.json", result["idempotency"])
    _write(output_dir / "circuit-breaker-runtime-v0243.json", result["circuit"])
    _write(output_dir / "dependency-authority-v0243.json", authority)
    _write(output_dir / "provider-boundary-v0243.json", result["provider"])
    _write(output_dir / "family-concurrency-v0243.json", batch)
    _write(output_dir / "historical-immutability-v0243.json", {"v0241": frozen_v0241, "v0242": frozen_v0242, "status": "PASS" if frozen_v0241.get("status") == "PASS" and frozen_v0242.get("status") == "PASS" else "FAIL"})
    _write(output_dir / "frozen-v0241-fingerprint-v0243.json", frozen_v0241)
    _write(output_dir / "hard-gates-v0243.json", gates)
    _write(output_dir / "negative-controls-v0243.json", controls)
    _write(output_dir / "full-slice-two-run-determinism-v0243.json", {"status": "PASS" if snapshot_one == snapshot_two else "FAIL", "run_1": snapshot_one, "run_2": snapshot_two})
    _write(output_dir / "production-boundary-v0243.json", production)
    _write(output_dir / "correction-history-v0243.json", {"schema_version": VERSION, "status": "CORRECTION_REQUIRED", "base_main_sha": BASE_MAIN_SHA, "reviewed_head": REJECTED_REVIEWED_HEAD, "rejected_reviewed_head": REJECTED_REVIEWED_HEAD, "branch": BRANCH, "findings": ["F-32RR", "F-35RR", "F-35RC", "F-37"], "strengthens": ["F-34S"], "historical_evidence_unchanged": True})
    _write(output_dir / "execution-evidence-v0243.json", result)
    return result
