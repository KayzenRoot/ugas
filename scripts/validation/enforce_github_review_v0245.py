"""Fail closed after the bounded v0.24.5 artifact has been assembled."""
...
def main() -> int:
    ...
    if not passed:
        for name in result["failed_checks"]:
            print(f"::error title=UGAS v0.24.5 final enforcement::{name} failed")
        for failure in security.get("failures", []):
            print(f"::error title=UGAS v0.24.5 final enforcement::security:{failure}")
