...
## v0.19.1 ITEMS_PROPS — linkage, representation bytes and stack identity correction / external review required

`version=0.19.1`; `phase=ITEMS_PROPS`; `current_gate=ITEMS_PROPS_LINKAGE_REPRESENTATION_STACK_INTEGRITY_TECHNICALLY_QUALIFIED`; `creatures_monsters=APPROVED_FOUNDATION`; `items_props_runtime=TECHNICALLY_QUALIFIED_FOUNDATION`; `real_item_prop_asset_coverage=NONE`; `synthetic_item_prop_fixture=TEST_ONLY`; `production_approved=false`; `production_routing=BLOCKED`; `new_generation=0`; `allowed_next_actions=[external_review_items_props_v0191]`; `next_capability_started=false`. The v0.18.2 creatures/monsters foundation was merged at `52938a04016352d50ad54621a4df981a9c36b058`. The v0.19.0 candidate at `44937935e644202836b3e1f081b6a63201b850db` is preserved as `CORRECTION_REQUIRED` history; this forward-only correction remains on the same PR #9 and branch `codex/v0.19.0-items-props-runtime-foundation`. It must stop after exact-head 3/3 checks and artifact evidence; do not merge it. Do not start environment/tilesets, maps/minimap, UI, VFX, orchestration hardening, production routing, or generation.

Evidence is under `docs/evidence/items-props-runtime-v0191/`; the complete execution contract is [REVIEW-v0.19.1.md](REVIEW-v0.19.1.md). The corrected resolver binds equipment to v0.17.1 authority, requires exact representation bytes, compares the full two-run set, and enforces item-family stack identity. `v0.19.0_external_review=CORRECTION_REQUIRED` remains immutable history.
