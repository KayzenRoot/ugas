...
## Current v0.24.0 ORCHESTRATION_RUNTIME_HARDENING handoff

`version=0.24.0`; `phase=ORCHESTRATION_RUNTIME_HARDENING`; `current_gate=ORCHESTRATION_RUNTIME_HARDENING_TECHNICALLY_QUALIFIED_EXTERNAL_REVIEW_REQUIRED`; `stop_reason=ORCHESTRATION_RUNTIME_HARDENING_EXTERNAL_REVIEW_REQUIRED`; `baseline_main_sha=dee98f8cd89ebd83a36ead7a22a184700d6e916f`; the baseline is historical branch context and current main/PR head must be resolved from GitHub LIVE. v0.23.4 VFX is `MERGED_CLOSED` at main `dee98f8cd89ebd83a36ead7a22a184700d6e916f`, with post-merge main CI run `34300109485`, unit job `102305017275`, docker job `102305017485` and closure comment `5594761528`. The active orchestration control plane is TEST_ONLY/provider-neutral: `orchestration_runtime=TECHNICALLY_QUALIFIED_FOUNDATION`, `orchestration_runtime_external_review=REQUIRED`, `orchestration_lifecycle=ACTIVE_FOUNDATION`, `production_routing=BLOCKED`, `production_approved=false`, `real_asset_generation=NONE`, `new_generation=0`. The sole allowed next action is `external_review_orchestration_runtime_v0240`; UI/VFX follow-on work and V1 final acceptance remain forbidden until external review. Evidence is under `docs/evidence/orchestration-runtime-v0240/`. The new branch is `codex/v0.24.0-orchestration-runtime-hardening-foundation`; do not merge and stop after the exact-head PR contexts and bounded artifact are green.
