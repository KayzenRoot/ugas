# UGAS checkpoint - v0.17.1

## v0.17.1 EQUIPMENT_OUTFITS — runtime and QA integrity technically qualified / external review required

The active phase is `EQUIPMENT_OUTFITS` on branch `codex/v0.17.0-equipment-outfits-runtime-foundation`, based on merged v0.16.2 at `a8d2897211c4b72c2cd2fe7a7f5729c7009d8566`. The v0.17.0 reviewed head `1c73e6a2ff5259226afe9ca03ef10e1822a7fdf2` is preserved as `CORRECTION_REQUIRED`; the active gate is `EQUIPMENT_OUTFITS_RUNTIME_AND_QA_INTEGRITY_TECHNICALLY_QUALIFIED`; the only allowed next action is `external_review_equipment_outfits_v0171`. The eight wearable records are deterministic synthetic fixtures and `synthetic_equipment_fixture=TEST_ONLY`; production registry is separate and empty and `real_equipment_asset_coverage=NONE_OR_EXPLICITLY_APPROVED_ONLY`. `multi_direction_animation_runtime=APPROVED_FOUNDATION`, `real_directional_character_asset_coverage=SOUTH_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0` are authoritative. Replacement arbitration, base-part hiding, asset-bound pixel masks, strict negative controls, cache poisoning checks and byte determinism are evidenced under `docs/evidence/equipment-outfits-runtime-v0171/`. Mirror and secondary anchors fail closed because their pixel/runtime effects are not implemented. Do not enable production or start creatures/monsters/downstream asset families; leave PR #7 open for external review.
