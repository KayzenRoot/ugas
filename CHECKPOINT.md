# UGAS checkpoint - v0.17.0

## v0.17.0 EQUIPMENT_OUTFITS — runtime foundation technically qualified / external review required

The active phase is `EQUIPMENT_OUTFITS` on branch `codex/v0.17.0-equipment-outfits-runtime-foundation`, based on merged v0.16.2 at `a8d2897211c4b72c2cd2fe7a7f5729c7009d8566`. The gate is `EQUIPMENT_OUTFITS_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED`; the only allowed next action is `external_review_equipment_outfits_v0170`. The six wearable records are deterministic synthetic fixtures and `synthetic_equipment_fixture=TEST_ONLY`; production registry is empty and `real_equipment_asset_coverage=NONE_OR_EXPLICITLY_APPROVED_ONLY`. `multi_direction_animation_runtime=APPROVED_FOUNDATION`, `real_directional_character_asset_coverage=SOUTH_ONLY`, `production_approved=false`, `production_routing=BLOCKED`, and `new_generation=0` are authoritative. Wearable composition binds slots, explicit layer order, R4 anchors, replacement/hide rules, asset-bound occlusion masks, direction, animation profile, rig revision, variant and provenance. It copies the base frame without mutating pixels, timing or event markers, and must be byte-deterministic across two runs. Do not enable production or start creatures/monsters/downstream asset families; leave the v0.17.0 PR open for external review.
