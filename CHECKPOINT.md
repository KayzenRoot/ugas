...
# v0.19.0 active checkpoint

`version=0.19.0`; `phase=ITEMS_PROPS`; `current_gate=ITEMS_PROPS_RUNTIME_FOUNDATION_TECHNICALLY_QUALIFIED`; `creatures_monsters=APPROVED_FOUNDATION`; `items_props_runtime=TECHNICALLY_QUALIFIED_FOUNDATION`; `real_item_prop_asset_coverage=NONE`; `synthetic_item_prop_fixture=TEST_ONLY`; `production_approved=false`; `production_routing=BLOCKED`; `new_generation=0`; `allowed_next_actions=[external_review_items_props_v0190]`; `next_capability_started=false`. The v0.18.2 creatures/monsters foundation was merged at `52938a04016352d50ad54621a4df981a9c36b058`. The v0.19.0 branch is `codex/v0.19.0-items-props-runtime-foundation` and must stop at its open PR after exact-head 3/3 checks and artifact evidence. Do not start environment/tilesets, maps/minimap, UI, VFX, orchestration hardening, production routing, or generation.
