...
class StateConsistencyV052Tests(unittest.TestCase):
    def test_current_state_and_documents_are_consistent(self):
        ...
        review = (ROOT / "REVIEW-v0.6.0.md").read_text(encoding="utf-8")
    ...
    def test_modified_state_fails_canonical_hash(self):
        ...
        result = validate_state_consistency(state, "0.6.0 SDXL_CONTROL_POSE_PROVIDER_GAP LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED REVIEW_ARCHIVE_VERIFIED v054-provider-qualification.json review-visuals-v0.5.5.json", "0.6.0 SDXL_CONTROL_POSE_PROVIDER_GAP LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED REVIEW_ARCHIVE_VERIFIED v054-provider-qualification.json review-visuals-v0.5.5.json")
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
    ...
    def test_nested_status_must_follow_active_stop(self):
        ...
        result = validate_state_consistency(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.6.0.md").read_text(encoding="utf-8"))
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
    ...
    def test_stale_refcontrol_pending_action_is_fatal(self):
        ...
        result = validate_state_consistency(state, checkpoint, (ROOT / "REVIEW-v0.6.0.md").read_text(encoding="utf-8"))
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
