"""Regression tests for historical-state separation under the v0.5.4 gate."""
...
class StateConsistencyV052Tests(unittest.TestCase):
    def test_current_state_and_documents_are_consistent(self):
        ...
        review = (ROOT / "REVIEW-v0.5.4.md").read_text(encoding="utf-8")
    ...
    def test_previous_release_flags_are_not_reclassified(self):
        state = json.loads((ROOT / "docs/evidence/current-state.json").read_text(encoding="utf-8"))
        self.assertEqual("0.5.3", state["previous_release"]["version"])
        self.assertEqual("POSE_QA_MODEL_LICENSE_GAP", state["previous_release"]["reported_status"])
    ...
    def test_modified_state_fails_canonical_hash(self):
        ...
        result = validate_state_consistency(state, "0.5.4 POSE_METRIC_GATE_DESIGN_GAP POSE_QA_MODEL_LICENSE_GAP POSE_QA_LOCAL_USE_LICENSE_RESOLVED", "0.5.4 POSE_METRIC_GATE_DESIGN_GAP POSE_QA_MODEL_LICENSE_GAP POSE_QA_LOCAL_USE_LICENSE_RESOLVED")
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
    ...
    def test_stale_refcontrol_pending_action_is_fatal(self):
        ...
        result = validate_state_consistency(state, checkpoint, (ROOT / "REVIEW-v0.5.4.md").read_text(encoding="utf-8"))
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
