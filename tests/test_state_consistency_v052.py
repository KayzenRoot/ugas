...
class StateConsistencyV052Tests(unittest.TestCase):
    def test_current_state_and_documents_are_consistent(self):
        ...
        review = (ROOT / "REVIEW-v0.5.5.md").read_text(encoding="utf-8")
    ...
    def test_previous_release_flags_are_not_reclassified(self):
        state = json.loads((ROOT / "docs/evidence/current-state.json").read_text(encoding="utf-8"))
        self.assertEqual("0.5.4", state["previous_release"]["version"])
        self.assertEqual("LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED", state["previous_release"]["reported_status"])

    def test_modified_state_fails_canonical_hash(self):
        state = json.loads((ROOT / "docs/evidence/current-state.json").read_text(encoding="utf-8"))
        state["canonical_anchor"]["sha256"] = "0" * 64
        result = validate_state_consistency(state, "0.5.5 REVIEW_SNAPSHOT_INTEGRITY_FIXED LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED POSE_QA_LOCAL_USE_LICENSE_RESOLVED v054-provider-qualification.json", "0.5.5 REVIEW_SNAPSHOT_INTEGRITY_FIXED LOCAL_POSE_CONTROL_PROVIDER_GAP_CONFIRMED POSE_QA_LOCAL_USE_LICENSE_RESOLVED v054-provider-qualification.json")
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
    ...
    def test_stale_refcontrol_pending_action_is_fatal(self):
        ...
        result = validate_state_consistency(state, checkpoint, (ROOT / "REVIEW-v0.5.5.md").read_text(encoding="utf-8"))
        self.assertEqual("STATE_CONSISTENCY_FAILED", result["status"])
