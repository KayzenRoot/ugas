"""Focused v0.24.1 correction tests with real negative paths."""
...
def make_request(*, request_id: str = "request-0241", project_id: str = "project-0241", timeout: int = 30, idempotency_key: str = "idem-0241") -> dict:
...
def make_node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 20, priority: int = 10) -> dict:
    return {
        ...
        "scheduling_key": "orchestration-v0241",
        "priority": priority,
        "asset_family": family,
    }


class OrchestrationRuntimeV0241Tests(unittest.TestCase):
    ...
    def test_idempotency_store_capacity_fails_closed(self) -> None:
        ...
        second_request = make_request(request_id="request-0241-second", idempotency_key="idem-0241-second")
    ...
    def test_dependency_ref_is_byte_and_revision_bound(self) -> None:
        ...
        ref = build_dependency_ref(root, project_id="project-0241", family="maps_minimap", revision="v0.21.3", path=path, status="MERGED_CLOSED")
        resolved = resolve_dependency_ref(ref, root, expected_project_id="project-0241")
        self.assertEqual(resolved["resolved_content_hash"], ref["content_hash"])
        wrong = copy.deepcopy(ref); wrong["content_hash"] = "0" * 64
        self.reject("ORCH_DEPENDENCY_AUTHORITY_HASH_MISMATCH", lambda: resolve_dependency_ref(wrong, root, expected_project_id="project-0241"))
    ...
    def test_provider_boundary_and_zero_spy_are_real_contracts(self) -> None:
        proof = assert_provider_boundary(Path(__file__).resolve().parents[1] / "src/ugas/orchestration_runtime_v0241.py")
