...
class PoseQAV054Tests(unittest.TestCase):
    ...
    def test_state_schema_and_active_documents_match(self):
        ...
        result = validate_state_consistency(state, (ROOT / "CHECKPOINT.md").read_text(encoding="utf-8"), (ROOT / "REVIEW-v0.7.1.md").read_text(encoding="utf-8"))
        self.assertEqual("STATE_CONSISTENCY_PASSED", result["status"], result)
        self.assertEqual("0.7.1", state["version"])
        self.assertEqual(state["current_gate"], state["state_consistency"]["status"])
        self.assertEqual("CUTOUT_RIG_SEAM_GAP", state["pose_lane_status"])
