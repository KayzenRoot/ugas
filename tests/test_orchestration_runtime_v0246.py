...
class OrchestrationRuntimev0246Tests(unittest.TestCase):
    ...
    def test_f39_stale_gate_fails_state_consistency(self) -> None:
        ...
        if state.get("version") != "0.24.6":
            if not (root / ".git").exists():
                self.skipTest("official no-git snapshot has no frozen v0.24.6 tree; F-39 live proof is v0.24.7")
            frozen_ref = "cdc49dd96e7c683c0426d209e0bef162442a3bbb"
