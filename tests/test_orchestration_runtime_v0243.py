"""Focused v0.24.3 correction tests with real negative paths."""
...
def make_node(node_id: str, dependencies: list[str] | None = None, *, family: str = "maps_minimap", timeout: int = 20, priority: int = 10) -> dict:
    return {
        ...
        "scheduling_key": "orchestration-v0243",
        "priority": priority,
        "asset_family": family,
    }


class OrchestrationRuntimev0243Tests(unittest.TestCase):
    ...
    def test_dependency_ref_is_byte_and_revision_bound(self) -> None:
        ...
        if not (root / ".git").exists():
            self.skipTest("historical v0.24.3 git-show authority path is not available in official no-git snapshots")
    ...
    def test_provider_boundary_and_zero_spy_are_real_contracts(self) -> None:
        proof = assert_provider_boundary(Path(__file__).resolve().parents[1] / "src/ugas/orchestration_runtime_v0243.py")
