...
from ugas.state_consistency_v0223 import resolve_next_actions, validate_state_consistency
from scripts.validation.run_ui_asset_family_runtime_v0223 import _strict_gate
from ugas.ui_asset_family_runtime_v0223 import (
...
class UIAssetFamilyRuntimeV0223Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_complete_progress_contract_and_three_values(self):
        ...
        self.assertFalse(progress["class_metadata"]["normalized_fill"]["clamp"])
        self.assertEqual(progress["class_metadata"]["normalized_fill"]["out_of_range"], "REJECT")
        ...
        for value in (-0.1, 1.1):
            with self.assertRaisesRegex(UIAssetFamilyContractError, "UI_PROGRESS_NORMALIZED_RANGE_INVALID"):
                progress_fill_rect(progress, value)
    ...
    def test_progress_invalid_range_origin_direction_and_geometry_reject(self):
        ...
        with self.assertRaisesRegex(UIAssetFamilyContractError, "UI_PROGRESS_GEOMETRY_INVALID"):
            validate_progress_bar_contract(mutated)
        for field, value in (("clamp", True), ("out_of_range", "CLAMP")):
            mutated = deepcopy(progress); mutated["class_metadata"]["normalized_fill"][field] = value
            with self.assertRaisesRegex(UIAssetFamilyContractError, "UI_PROGRESS_POLICY_INVALID"):
                validate_progress_bar_contract(mutated)
    ...
    def test_independent_nine_slice_proof_at_two_non_native_sizes_and_mutations(self):
        ...
        output = render_component_to_size(component, "normal", 1, target_size)
        output.putpixel((1, component["stretch_geometry"]["slice_margins"]["top"] + 1), (4, 5, 6, 255))
        with self.assertRaisesRegex(UIAssetFamilyContractError, "UI_NINE_SLICE_INVARIANT_INVALID"):
            verify_nine_slice_invariants(source, output, component["stretch_geometry"]["slice_margins"])
    ...
    def test_lifecycle_four_live_scenarios(self):
        ...
        self.assertEqual(unresolved["allowed_next_actions"], ["external_review_ui_asset_family_v0223"])
        ...
        merge_sha = "2" * 40
        pending = resolve_next_actions(self.state, {"source_mode": "GITHUB_LIVE", "pr_number": 13, "external_approval": True, "merged": True, "merge_commit_sha": merge_sha, "current_main_sha": merge_sha, "post_merge_main_ci": {"commit_sha": merge_sha, "supported_contexts": ["UGAS CI / unit-and-validation", "UGAS CI / docker-smoke"], "contexts": [{"name": "UGAS CI / unit-and-validation", "status": "in_progress", "conclusion": None, "head_sha": merge_sha}, {"name": "UGAS CI / docker-smoke", "status": "in_progress", "conclusion": None, "head_sha": merge_sha}]}})
        self.assertFalse(pending["vfx_allowed"])
        closed = resolve_next_actions(self.state, {"source_mode": "GITHUB_LIVE", "pr_number": 13, "external_approval": True, "merged": True, "merge_commit_sha": merge_sha, "current_main_sha": merge_sha, "post_merge_main_ci": {"commit_sha": merge_sha, "supported_contexts": ["UGAS CI / unit-and-validation", "UGAS CI / docker-smoke"], "contexts": [{"name": "UGAS CI / unit-and-validation", "status": "completed", "conclusion": "success", "head_sha": merge_sha}, {"name": "UGAS CI / docker-smoke", "status": "completed", "conclusion": "success", "head_sha": merge_sha}]}})
        self.assertEqual(closed["allowed_next_actions"], ["start_vfx_asset_family_v0230"])

    def test_vfx_gate_rejects_stale_mixed_and_wrong_sha_facts(self):
        sha = "2" * 40
        base = {"source_mode": "GITHUB_LIVE", "pr_number": 13, "external_approval": True, "merged": True, "merge_commit_sha": sha, "current_main_sha": sha, "post_merge_main_ci": {"commit_sha": sha, "supported_contexts": ["UGAS CI / unit-and-validation", "UGAS CI / docker-smoke"], "contexts": [{"name": "UGAS CI / unit-and-validation", "status": "completed", "conclusion": "success", "head_sha": sha}, {"name": "UGAS CI / docker-smoke", "status": "completed", "conclusion": "success", "head_sha": sha}]}}
        for mutate in (
            lambda value: value.update(merge_commit_sha="3" * 40),
            lambda value: value["post_merge_main_ci"]["contexts"].__getitem__(0).update(head_sha="3" * 40),
            lambda value: value["post_merge_main_ci"]["contexts"].__getitem__(1).update(status="in_progress", conclusion=None),
        ):
            candidate = deepcopy(base); mutate(candidate)
            self.assertFalse(resolve_next_actions(self.state, candidate)["vfx_allowed"])

    def test_hard_gate_rejects_every_non_boolean_observation(self):
        for observed in (False, None, 0, 1, "", "PASS", [], {}, [True], {"pass": True}):
            result = _strict_gate("TEST-NON-BOOLEAN", lambda observed=observed: observed)
